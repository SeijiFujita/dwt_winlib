...
bool isImplicitly(in TypeInfo test, in TypeInfo target) {
	...
	if ((isSignedInteger(test) && isSignedInteger(target)) || (isUnsignedInteger(test)
			&& isUnsignedInteger(target)) || (isFloat(test) && isFloat(target))
...
bool isDynamicArray(in TypeInfo type) {
	...
	return ((type2.classinfo.name[9] == 'A') && (type2.classinfo.name.length == 11))
...
bool isUnsignedInteger(in TypeInfo type) {
	auto type2 = realType(type);
	return (type2 is typeid(uint) || type2 is typeid(ulong)
			|| type2 is typeid(ushort) || type2 is typeid(ubyte));
}

///
bool isSignedInteger(in TypeInfo type) {
	auto type2 = realType(type);
	return (type2 is typeid(int) || type2 is typeid(long)
			|| type2 is typeid(short) || type2 is typeid(byte));
}
...
bool isPrimitive(in TypeInfo type) {
	auto type2 = realType(type);
	return (isArray(type2) || isAssociativeArray(type2) || isCharacter(type2)
			|| isFloat(type2) || isInteger(type2));
}
...
bool isValueType(in TypeInfo type) {
	return !(isDynamicArray(type) || isAssociativeArray(type)
			|| isPointer(type) || isClass(type) || isFunction(type));
}
...
debug (UnitTest) {
	...
	unittest {
		...
		assert(baseTypes(type) == [B.classinfo, A.classinfo, Object.classinfo,
				I2.classinfo, I3.classinfo, I1.classinfo]);
	}
	...
	unittest {
		...
		assert(isDynamicArray(dynamicString),
				dynamicString.toString() ~ dynamicString.classinfo.name);
