...
unittest {
	auto s = "abаб回家\U00064321\U00064321d";
	assert(s.length == 1 + 1 + 2 + 2 + 3 + 3 + 4 + 4 + 1);
	auto ws = "abаб回家\U00064321\U00064321d"w;
	assert(ws.length == 1 + 1 + 1 + 1 + 1 + 1 + 2 + 2 + 1);
	...
	auto shifts0 = [0, 1, 1 + 1, 1 + 1 + 2, 1 + 1 + 2 + 2, 1 + 1 + 2 + 2 + 3,
		1 + 1 + 2 + 2 + 3 + 3, 1 + 1 + 2 + 2 + 3 + 3 + 4, 1 + 1 + 2 + 2 + 3 + 3 + 4 + 4];
	...
	foreach (UTF8index i, char ch; s) {
		...
		if (starts[i]) {
			...
			if (i)
				assert(s.offsetBefore(i) == prevStart);
			...
			assert(di == strides[n]);
			if (i)
				assert(s.dcharBefore(i) == s.dcharAt(prevStart));
			if (i)
				assert(s.dcharAfter(prevStart) == s.dcharAt(i));
