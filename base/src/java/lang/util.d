...
void implMissing(String file, uint line) {
	...
	getDwtLogger().fatal(file, line,
			"Please create a bug report at http://www.dsource.org/projects/dwt");
...
version (Tango) {
	public alias Format = tango.text.convert.Format.Format;
}
else { // Phobos
	...
	unittest {
		...
		assert(Formatter("{:b}", cast(int)-1) == "11111111111111111111111111111111");
		assert(Formatter("{:b}",
				cast(long)-1) == "1111111111111111111111111111111111111111111111111111111111111111");
