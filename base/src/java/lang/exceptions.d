...
version (Tango) {
	...
	public alias IllegalArgumentException = tango.core.Exception.IllegalArgumentException;
	public alias IOException = tango.core.Exception.IOException;
	public alias PlatformException = tango.core.Exception.PlatformException;
	public alias ArrayIndexOutOfBoundsException = tango.core.Exception.ArrayBoundsException;
	public alias NoSuchElementException = tango.core.Exception.NoSuchElementException;
	public alias UnicodeException = tango.core.Exception.UnicodeException;
	...
	public alias ArrayIndexOutOfBoundsException = core.exception.RangeError;
...
version (Tango) {
	...
	/// Extension to the D Exception
	void ExceptionPrintStackTrace(Throwable e, void delegate(String file,
			ulong line, String fmt, ...) dg) {
		Throwable exception = e;
		while (exception !is null) {
			dg(exception.file, exception.line, "Exception in {}({}): {}",
					exception.file, exception.line, exception.msg);
	...
	void ExceptionPrintStackTrace(Exception e) {
		Throwable exception = e;
		while (exception !is null) {
			getDwtLogger().error(exception.file, exception.line,
					"Exception in {}({}): {}", exception.file, exception.line, exception.msg);
			if (exception.info !is null) {
	...
	version (Tango) {
		...
		foreach (frame; e.info) {
			if (idx >= start && idx < start + deepth) {
				getDwtLogger().trace(__FILE__, __LINE__, "{} {}: {}", prefix,
