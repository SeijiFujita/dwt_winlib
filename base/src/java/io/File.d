...
public class File {
	...
	public String getAbsolutePath() {
		version (Tango) {
			return (new tango.io.FilePath.FilePath(mFilePath)).absolute(
					tango.sys.Environment.Environment.cwd).toString;
		}
		else { // Phobos
	...
	public bool canWrite() {
		version (Windows) { //Windows's ACL isn't supported
			...
			version (Tango) {
				return tango.stdc.posix.unistd.access(tango.stdc.stringz.toStringz(mFilePath),
						2) is 0; //W_OK = 2
			}
			else { // Phobos
