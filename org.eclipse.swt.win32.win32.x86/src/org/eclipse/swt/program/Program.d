 ...
 */
	public static Program findProgram(String extension) {
		...
		if (OS.IsWinCE) {
			void*[1] phkResult;
			if (OS.RegOpenKeyEx(cast(void*) OS.HKEY_CLASSES_ROOT, key.ptr, 0,
					OS.KEY_READ, phkResult.ptr) !is 0) {
				return null;
			}
			uint[1] lpcbData;
			...
			if (result is 0) {
				...
				result = OS.RegQueryValueEx(phkResult[0], null, null, null,
						cast(ubyte*) lpData.ptr, lpcbData.ptr);
				if (result is 0)
					program = getProgram(TCHARzToStr(lpData.ptr), extension);
			}
 ...
 */
	public static String[] getExtensions() {
		...
		while (OS.RegEnumKeyEx(cast(void*) OS.HKEY_CLASSES_ROOT, dwIndex,
				lpName.ptr, lpcName.ptr, null, null, null, &ft) !is OS.ERROR_NO_MORE_ITEMS) {
	...
	static String getKeyValue(String string, bool expand) {
		...
		if (OS.RegQueryValueEx(phkResult[0], null, null, null, null, lpcbData.ptr) is 0) {
			...
			if (length_ !is 0) {
				...
				if (OS.RegQueryValueEx(phkResult[0], null, null, null,
						cast(ubyte*) lpData.ptr, lpcbData.ptr) is 0) {
					if (!OS.IsWinCE && expand) {
	...
	static Program getProgram(String key, String extension) {
		...
		if (defaultCommand is null || defaultCommand.length is 0)
		...
		if (command is null || command.length is 0)
			return null;
	...
	public bool execute(String fileName) {
		...
		while (index < ARGUMENTS.length) {
			...
			if (i !is -1) {
				...
				suffix = command.substring(i + cast(int) /*64bit*/ ARGUMENTS[index].length,
						cast(int) /*64bit*/ command.length);
				break;
			}
			index++;
		}
		...
		bool success = cast(bool) OS.CreateProcess(null, lpCommandLine, null,
				null, false, 0, null, null, &lpStartupInfo, &lpProcessInformation);
 ...
 */
	public ImageData getImageData() {
		if (extension !is null) {
			...
			OS.SHGetFileInfo(pszPath.ptr, OS.FILE_ATTRIBUTE_NORMAL, &shfi,
