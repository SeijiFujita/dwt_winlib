 ...
 */
	...
	Control[] getControls(bool onlyVisible) {
		...
		for (int i = 0; i < children.length; i++) {
			if (null !is cast(Sash) children[i])
				continue;
	...
	void onDragSash(Event event) {
		...
		if (getOrientation() is SWT.HORIZONTAL) {
			...
			(cast(SashFormData) data1).weight = (
					(cast(long) b1.width << 16) + area.width - 1) / area.width;
			(cast(SashFormData) data2).weight = (
					(cast(long) b2.width << 16) + area.width - 1) / area.width;
		}
		else {
			...
			(cast(SashFormData) data1).weight = (
					(cast(long) b1.height << 16) + area.height - 1) / area.height;
			(cast(SashFormData) data2).weight = (
					(cast(long) b2.height << 16) + area.height - 1) / area.height;
		}
 ...
 */
	public void setOrientation(int orientation) {
		checkWidget();
		if (getOrientation() is orientation)
			return;
