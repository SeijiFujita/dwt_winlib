...
package:
     ...
     */
		void cacheLineData(StyledText styledText) {
			...
			if (styledText.isListening(LineGetBackground) || (styledText.isBidi()
					&& styledText.isListening(LineGetSegments))
     ...
     */
		...
		void init_() {
			...
			printerFont = new Font(cast(Device) printer, fontData.getName(),
     ...
     */
		void print() {
			...
			if (printOptions.printLineNumbers || printOptions.header !is null
					|| printOptions.footer !is null) {
			...
			if (printOptions.printLineNumbers) {
				...
				if (lineLabels !is null) {
					for (int i = startLine; i < Math.min(count, lineLabels.length);
							i++) {
			...
			for (int i = startLine; i <= endLine && page <= endPage; i++) {
				...
				TextLayout layout = printerRenderer.getTextLayout(i,
						orientation, width, lineSpacing);
				...
				if (paragraphBottom <= pageBottom) {
					...
					while (paragraphBottom > pageBottom && lineCount > 0) {
						lineCount--;
						paragraphBottom -= layout.getLineBounds(lineCount)
							.height + layout.getSpacing();
					}
					if (lineCount is 0) {
		...
		void printDecoration(int page, bool header, TextLayout layout) {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (segmentIndex is -1) {
					...
					printDecorationSegment(segment, i, page, header, layout);
					lastSegmentIndex = segmentIndex + cast(
							int) /*64bit*/ StyledTextPrintOptions.SEPARATOR.length;
				}
			}
		}
		/**
     ...
     * @param header true = print the header, false = print the footer
     */
		void printDecorationSegment(String segment, int alignment, int page,
				bool header, TextLayout layout) {
     ...
     */
		void write(String string, int start, int end) {
			ptrdiff_t incr;
			for (auto index = start; index < end; index += incr) {
     ...
     */
		public override void writeLine(String line, int lineOffset) {
			...
			writeStyledLine(line, lineOffset, ranges, styles, lineBackground,
					lineIndent, lineAlignment, lineJustify);
		}
		/**
     ...
     */
		void writeStyledLine(String line, int lineOffset, int[] ranges, StyleRange[] styles,
				Color lineBackground, int indent, int alignment, bool justify) {
	...
	public this(Composite parent, int style) {
		...
		if (isBidiCaret()) {
			...
			Runnable runnable = new class Runnable {
				public void run() {
					int direction = BidiUtil.getKeyboardLanguage() is BidiUtil.KEYBOARD_BIDI
						? SWT.RIGHT : SWT.LEFT;
					if (direction is caretDirection)
						return;
					if (getCaret() !is defaultCaret)
						return;
 ...
 */
	public void addExtendedModifyListener(ExtendedModifyListener extendedModifyListener) {
		checkWidget();
		if (extendedModifyListener is null)
		...
		if (isFixedLineHeight()) {
			...
			if (delta >= 0) {
				...
				if (lineIndex < lineCount
						&& -delta + renderer.getLineHeight(
							lineIndex) <= clientAreaHeight - topMargin - bottomMargin) {
					topIndex = lineIndex;
					topIndexY = -delta;
				}
				else {
 ...
 */
	void claimBottomFreeSpace() {
		...
		if (isFixedLineHeight()) {
			...
			int newVerticalOffset = Math.max(0,
					content.getLineCount() * lineHeight - clientAreaHeight);
 ...
 */
	void claimRightFreeSpace() {
		int newHorizontalOffset = Math.max(0,
				renderer.getWidth() - (clientAreaWidth - leftMargin - rightMargin));
	...
	public override Point computeSize(int wHint, int hHint, bool changed) {
		...
		if (wHint is SWT.DEFAULT || hHint is SWT.DEFAULT) {
			...
			for (int lineIndex = 0; lineIndex < lineCount; lineIndex++) {
				...
				if (wordWrap)
					layout.setWidth(wHint is 0 ? 1 : wHint);
				Rectangle rect = layout.getBounds();
				height += rect.height;
 ...
 */
	public void copy(int clipboardType) {
		checkWidget();
		if (clipboardType !is DND.CLIPBOARD && clipboardType !is DND.SELECTION_CLIPBOARD)
			return;
	...
	void doBackspace() {
		...
		if (selection.x !is selection.y) {
			...
			if (caretOffset is lineOffset) {
				...
				event.start = lineOffset + cast(int) /*64bit*/ content.getLine(lineIndex - 1)
					.length;
 ...
 */
	void doDelete() {
		...
		if (selection.x !is selection.y) {
			...
			int lineLength = cast(int) /*64bit*/ content.getLine(line).length;
 ...
 */
	void doLineEnd() {
		...
		if (wordWrap) {
			...
			int lineLength = cast(int) /*64bit*/ content.getLine(caretLine).length;
 ...
 */
	void doMouseLocationChange(int x, int y, bool select) {
		...
		if (0 <= y && y < clientAreaHeight && (0 <= x && x < clientAreaWidth
				|| wordWrap || newCaretLine !is content.getLineAtOffset(caretOffset))) {
 ...
 */
	void doMouseSelection() {
		if (caretOffset <= selection.x || (caretOffset > selection.x
		...
		if (isSingleLine()) {
			...
			if (wordWrap) {
				...
				if (index is -1 && lineIndex > 0) {
					bottomOffset = content.getOffsetAtLine(lineIndex - 1) + cast(
							int) /*64bit*/ content.getLine(lineIndex - 1).length;
				}
				else {
					bottomOffset = content.getOffsetAtLine(lineIndex) + Math.max(0,
							getPreviousCharOffset(lineIndex, layout.getLineOffsets()[index + 1]));
				}
				renderer.disposeTextLayout(layout);
			}
			else {
				int lineIndex = getBottomIndex();
				bottomOffset = content.getOffsetAtLine(lineIndex) + cast(int) /*64bit*/ content.getLine(lineIndex)
					.length;
			}
 ...
 */
	public int getBaseline(int offset) {
		...
		int lineOffset = content.getOffsetAtLine(lineIndex);
		TextLayout layout = renderer.getTextLayout(lineIndex);
		int lineInParagraph = layout.getLineIndex(Math.min(offset - lineOffset,
				cast(int) /*64bit*/ layout.getText().length));
 ...
 */
	...
	Rectangle getBoundsAtOffset(int offset) {
		...
		if (offset is caretOffset) {
			int lineEnd = lineOffset + cast(int) /*64bit*/ line.length;
 ...
 */
	int[] getBidiSegments(int lineOffset, String line) {
		...
		if (event is null || event.segments is null || event.segments.length is 0) {
			...
			int segmentCount = cast(int) /*64bit*/ event.segments.length;
 ...
 */
	int[] getBidiSegmentsCompatibility(String line, int lineOffset) {
		...
		for (int i = k; i < styles.length; i++) {
			...
			styleLineEnd = Math.min(styleLineEnd, cast(int) /*64bit*/ line.length);
			if (i > 0 && count > 1 && ((styleLineStart >= offsets[count - 2]
					&& styleLineStart <= offsets[count - 1])
 ...
 */
	...
	int getCaretDirection() {
		if (!isBidiCaret())
			return SWT.DEFAULT;
		if (ime.getCompositionOffset() !is -1)
			return SWT.DEFAULT;
		if (!updateCaretDirection && caretDirection !is SWT.NULL)
	...
	int getWordPrevious(int offset, int movement) {
		...
		if (offset <= 0) {
			...
			if (offset is lineOffset) {
				...
				newOffset = nextLineOffset + cast(int) /*64bit*/ nextLineText.length;
			}
			else {
				TextLayout layout = renderer.getTextLayout(lineIndex);
		...
		return sendWordBoundaryEvent(WordPrevious, movement, offset, newOffset,
				lineText, lineOffset);
	}
	/**
			...
			if (offsetInLine is lineLength) {
				...
				default:
					if (offsetInLine is 0) {
						...
						point = layout.getLocation(getPreviousCharOffset(lineIndex,
								offsetInLine), true);
					}
					break;
				}
		...
		ime.addListener(SWT.ImeComposition, new class Listener {
			public void handleEvent(Event event) {
				...
				case SWT.COMPOSITION_SELECTION:
					handleCompositionSelection(event); break; case SWT.COMPOSITION_CHANGED:
					handleCompositionChanged(event); break; case SWT.COMPOSITION_OFFSET:
					handleCompositionOffset(event); break; default:
				}
			}
		}
);
		if (verticalBar !is null) {
			...
			void internalRedrawRange(int start, int length) {
				...
				int lineX = leftMargin - horizontalScrollOffset,
					startLineY = getLinePixel(startLine);
				int[] offsets = layout.getLineOffsets();
				int startIndex = layout.getLineIndex(Math.min(start,
						cast(int) /*64bit*/ layout.getText().length));
				...
				if (startLine is endLine) {
					int endIndex = layout.getLineIndex(Math.min(end,
							cast(int) /*64bit*/ layout.getText().length));
					if (startIndex is endIndex) {
						/* Redraw rect between start and end offset if start and end offsets are in same wrapped line */
						Rectangle rect = layout.getBounds(start,
								getPreviousCharOffset(startLine, end));
						rect.x += lineX;
						rect.y += startLineY;
						super.redraw(rect.x, rect.y, rect.width, rect.height, false);
						renderer.disposeTextLayout(layout);
				...
				Rectangle startRect = layout.getBounds(start,
						getPreviousCharOffset(startLine, offsets[startIndex + 1]));
				...
				int endIndex = layout.getLineIndex(Math.min(end, cast(int) /*64bit*/ layout.getText()
						.length));
				Rectangle endRect = layout.getBounds(offsets[endIndex],
						getPreviousCharOffset(endLine, end));
				...
				if (endRect.y > y) {
					super.redraw(leftMargin, y,
							clientAreaWidth - rightMargin - leftMargin, endRect.y - y, false);
				}
			}
			...
			void handleCompositionChanged(Event event) {
				...
				if (length is ime.getCommitCount()) {
					...
					if (ime.getWideCaret()) {
						...
						int lineOffset = content.getOffsetAtLine(lineIndex);
						TextLayout layout = renderer.getTextLayout(lineIndex);
						caretWidth = layout.getBounds(start - lineOffset,
								getPreviousCharOffset(lineIndex, start + length - lineOffset))
							.width;
						renderer.disposeTextLayout(layout);
					}
				}
			...
			void handleKey(Event event) {
				...
				if (event.keyCode !is 0) {
					...
					if (action is SWT.NULL) {
						...
						if ((event.stateMask & SWT.CTRL) !is 0
								&& (event.character >= 0) && event.character <= 31) {
				...
				if (action is SWT.NULL) {
					...
					if (IS_CARBON) {
						...
						ignore = (event.stateMask ^ SWT.ALT) is 0 || (event.stateMask ^ SWT.CTRL) is 0
					...
					if (!ignore && event.character > 31 && event.character !is SWT.DEL
							|| event.character is SWT.CR
							|| event.character is SWT.LF || event.character is TAB) {
 ...
 */
			void handleKeyUp(Event event) {
				if (clipboardSelection !is null) {
					if (clipboardSelection.x !is selection.x || clipboardSelection.y !is selection
							.y) {
						try {
							if (selection.y - selection.x > 0) {
								setClipboardContent(selection.x,
										selection.y - selection.x, DND.SELECTION_CLIPBOARD);
							}
						}
						catch (SWTError error) {
							// Copy to clipboard failed. This happens when another application
 ...
 */
			void handleMouseDown(Event event) {
				...
				//drag detect
				if (dragDetect_ && checkDragDetect(event))
					return;
				...
				if (clickCount is 1) {
					...
					if (doubleClickEnabled) {
						...
						if ((clickCount & 1) is 0) {
							...
							end = Math.min(content.getCharCount(),
									getWordNext(start, SWT.MOVEMENT_WORD_END));
						}
						else {
							start = lineOffset;
							end = lineEnd;
 ...
 */
			void handlePaint(Event event) {
				if (event.width is 0 || event.height is 0)
					return;
				if (clientAreaWidth is 0 || clientAreaHeight is 0)
					return;
 ...
 */
			void handleResize(Event event) {
				...
				if (oldWidth !is clientAreaWidth) {
					if (rightMargin > 0) {
						int x = (oldWidth < clientAreaWidth ? oldWidth : clientAreaWidth)
							- rightMargin;
				...
				if (oldHeight !is clientAreaHeight) {
					if (bottomMargin > 0) {
						int y = (oldHeight < clientAreaHeight ? oldHeight : clientAreaHeight)
							- bottomMargin;
			...
			void handleTextChanged(TextChangedEvent event) {
				...
				if (!isFixedLineHeight() && topIndex > firstLine) {
					...
					if (lastLineBottom !is newLastLineBottom) {
						...
						super.redraw(0, firstLineTop, clientAreaWidth,
								newLastLineBottom - firstLineTop, false);
				...
				updateSelection(lastTextChangeStart,
						lastTextChangeReplaceCharCount, lastTextChangeNewCharCount);
 ...
 */
			void handleTextChanging(TextChangingEvent event) {
				...
				// caretOffset.
				int newEndOfText = content.getCharCount()
					- event.replaceCharCount + event.newCharCount;
			...
			void initializeAccessible() {
				...
				accessible.addAccessibleListener(new class AccessibleAdapter {
					override public void getName(AccessibleEvent e) {
						String name = null; Label label = getAssociatedLabel(); if (label !is null) {
							...
							override public void getHelp(AccessibleEvent e) {
								...
								override public void getKeyboardShortcut(AccessibleEvent e) {
									String shortcut = null; Label label = getAssociatedLabel();
										if (label !is null) {
											String text = label.getText(); if (text !is null) {
								...
								accessible.addAccessibleTextListener(new class AccessibleTextAdapter {
									override public void getCaretOffset(AccessibleTextEvent e) {
										...
										override public void getSelectionRange(AccessibleTextEvent e) {
											Point selection = this.outer.getSelectionRange();
											e.offset = selection.x; e.length = selection.y;
										}
									}
);
									...
									new class AccessibleControlAdapter {
										override public void getRole(AccessibleControlEvent e) {
											...
											override public void getState(AccessibleControlEvent e) {
												int state = 0; if (isEnabled())
													state |= ACC.STATE_FOCUSABLE; if (
														isFocusControl())
														state |= ACC.STATE_FOCUSED;
														if (!isVisible()) state |= ACC
														.STATE_INVISIBLE; if (!getEditable()) state |= ACC
														.STATE_READONLY; e.detail = state;
											}
											...
											addListener(SWT.FocusIn, new class(accessible) Listener {
												Accessible acc; this(Accessible acc) {
													this.acc = acc;}
											...
											Label getAssociatedLabel() {
												...
												for (int i = 0; i < siblings.length;
													...
													if (siblings[i] is this) {
														if (i > 0
															&& (null !is cast(Label) siblings[i - 1])) {
											...
											String stripMnemonic(String string) {
												...
												do {
													...
													if (string[index]!is '&') {
														...
														index - 1) ~ string.substring(index,
														length_);
													}
													index++;
												}
 ...
 */
											dchar _findMnemonic(String string) {
												...
												int length_ = cast(int) /*64bit*/ string.length;
												do {
													while (index < length_ && string[index]!is '&')
														index++;
													if (++index >= length_)
														return '\0';
													if (string[index]!is '&')
														return Character.toLowerCase(
														string.dcharAt(index));
													index++;
												}
												while (index < length_);
												return '\0';
 ...
 */
											bool isBidi() {
												return IS_GTK || IS_CARBON
												|| BidiUtil.isBidiPlatform() || isMirrored_;
											}
											...
											void modifyContent(Event event, bool updateCaret) {
												...
												if (event.doit) {
													...
													if (isListening(ExtendedModify)) {
														styledTextEvent = new StyledTextEvent(
														content);
														styledTextEvent.start = event.start;
														styledTextEvent.end = event.start + cast(
														int) /*64bit*/ event.text.length;
														styledTextEvent.text = content.getTextRange(event.start,
														replacedLength);
													}
													if (updateCaret) {
														//Fix advancing flag for delete/backspace key on direction boundary
														if (event.text.length is 0) {
															int lineIndex = content.getLineAtOffset(
															event.start);
															int lineOffset = content.getOffsetAtLine(
															lineIndex);
															TextLayout layout = renderer.getTextLayout(
															lineIndex);
															int levelStart = layout.getLevel(
															event.start - lineOffset);
															int lineIndexEnd = content.getLineAtOffset(
															event.end);
															if (lineIndex !is lineIndexEnd) {
																renderer.disposeTextLayout(layout);
																lineOffset = content.getOffsetAtLine(
																lineIndexEnd);
																layout = renderer.getTextLayout(
																lineIndexEnd);
															}
															int levelEnd = layout.getLevel(
															event.end - lineOffset);
															renderer.disposeTextLayout(layout);
													...
													content.replaceTextRange(event.start,
													replacedLength, event.text);
													// set the caret position prior to sending the modify event.
													// fixes 1GBB8NJ
													if (updateCaret) {
														// always update the caret location. fixes 1G8FODP
														setSelection(event.start + cast(int) /*64bit*/ event.text.length,
														0, true);
														showCaret();
													}
													sendModifyEvent(event);
													if (isListening(ExtendedModify)) {
														notifyListeners(ExtendedModify,
														styledTextEvent);
													}
												}
											}
 ...
 */
											public void paste() {
												checkWidget();
												String text = stringcast(
												getClipboardContent(DND.CLIPBOARD));
												if (text !is null && text.length > 0) {
													Event event = new Event();
													event.start = selection.x;
													event.end = selection.y;
 ...
 */
											public override void redraw() {
												super.redraw();
												int itemCount = getPartialBottomIndex()
												- topIndex + 1;
 ...
 * @see Control#update()
 */
											public override void redraw(int x,
											int y, int width, int height, bool all) {
											...
											void redrawLines(int startLine, int lineCount) {
												...
												int redrawBottom = getLinePixel(
												startLine + lineCount);
												int redrawWidth = clientAreaWidth
												- leftMargin - rightMargin;
												super.redraw(leftMargin, redrawTop, redrawWidth,
												redrawBottom - redrawTop, true);
											}
											...
											void redrawLinesBullet(int[] redrawLines) {
												...
												int bottomIndex = getPartialBottomIndex();
												for (int i = 0; i < redrawLines.length;
													i++) {
													int lineIndex = redrawLines[i];
													if (!(topIndex <= lineIndex
														&& lineIndex <= bottomIndex))
														continue;
													int width = -1;
													Bullet bullet = renderer.getLineBullet(lineIndex,
													null);
													if (bullet !is null) {
 ...
 */
											...
											int length, bool clearBackground) {
												checkWidget();
												int end = start + length;
 ...
 * @since 2.0
 */
											public void removeBidiSegmentListener(
											BidiSegmentListener listener) {
												checkWidget();
												if (listener is null)
													SWT.error(SWT.ERROR_NULL_ARGUMENT);
 ...
 * </ul>
 */
											public void removeLineBackgroundListener(
 ...
 * @since 3.2
 */
											public void removePaintObjectListener(
											PaintObjectListener listener) {
												checkWidget();
												if (listener is null)
													SWT.error(SWT.ERROR_NULL_ARGUMENT);
 ...
 */
											...
											public void removeWordMovementListener(
 ...
 */
											...
											void resetCache(int firstLine, int count) {
												...
												renderer.calculateClientArea();
												if (0 <= maxLineIndex
													&& maxLineIndex < content.getLineCount()) {
 ...
 */
											...
											public override void scroll(int destX, int destY, int x,
											int y, int width, int height, bool all) {
												super.scroll(destX, destY, x,
												...
												if (all) {
													...
													for (int i = 0; i < children.length;
														...
														child.setLocation(rect.x + deltaX,
														rect.y + deltaY);
													}
												}
											}
 ...
 */
											bool scrollHorizontal(int pixels, bool adjustScrollBar) {
												...
												int scrollHeight = clientAreaHeight
												- topMargin - bottomMargin;
												if (pixels > 0) {
													int sourceX = leftMargin + pixels;
													int scrollWidth = clientAreaWidth
													- sourceX - rightMargin;
													if (scrollWidth > 0) {
														scroll(leftMargin, topMargin, sourceX, topMargin,
														scrollWidth, scrollHeight, true);
													}
													if (sourceX > scrollWidth) {
														super.redraw(leftMargin + scrollWidth, topMargin,
 ...
 */
											bool scrollVertical(int pixels, bool adjustScrollBar) {
												...
												if (verticalScrollOffset !is -1) {
													...
													if (pixels > 0) {
														...
														int scrollHeight = clientAreaHeight
														- sourceY - bottomMargin;
														if (scrollHeight > 0) {
															scroll(leftMargin, topMargin, leftMargin, sourceY,
															scrollWidth, scrollHeight, true);
														}
														if (sourceY > scrollHeight) {
															int redrawY = Math.max(0,
															topMargin + scrollHeight);
															int redrawHeight = Math.min(clientAreaHeight,
															pixels - scrollHeight);
															super.redraw(leftMargin, redrawY, scrollWidth,
															redrawHeight, true);
														}
													}
													else {
											...
											void scrollText(int srcY, int destY) {
												...
												scroll(leftMargin, destY, leftMargin, srcY,
												scrollWidth, scrollHeight, true);
												if ((0 < srcY + scrollHeight) && (topMargin > srcY)) {
													super.redraw(leftMargin, deltaY,
													scrollWidth, topMargin, false);
												}
												if ((0 < destY + scrollHeight) && (topMargin > destY)) {
													super.redraw(leftMargin, 0,
													scrollWidth, topMargin, false);
												}
 ...
 */
											...
											void sendModifyEvent(Event event) {
												Accessible accessible = getAccessible();
												if (event.text.length is 0) {
													accessible.textChanged(ACC.TEXT_DELETE,
													event.start, event.end - event.start);
												}
												else {
													if (event.start is event.end) {
														accessible.textChanged(ACC.TEXT_INSERT, event.start,
														cast(int) /*64bit*/ event.text.length);
													}
													else {
														accessible.textChanged(ACC.TEXT_DELETE, event.start,
 ...
 */
											...
											int sendWordBoundaryEvent(int eventType, int movement, int offset,
											int newOffset, String lineText, int lineOffset) {
												if (isListening(eventType)) {
													StyledTextEvent event = new StyledTextEvent(
													content);
													event.detail = lineOffset;
 ...
 */
											public void setAlignment(int alignment) {
												...
												if (alignment is 0 || this.alignment is alignment)
													return;
											...
											void setCaretLocation(Point location, int direction) {
												...
												if (caret !is null) {
													...
													if (!isFixedLineHeight() && isDefaultCaret) {
														caretHeight = getBoundsAtOffset(caretOffset)
														.height;
													...
													if (isDefaultCaret && imageDirection is SWT
														.RIGHT) {
														location.x -= (caret.getSize().x - 1);
													}
													if (isDefaultCaret) {
														caret.setBounds(location.x, location.y,
														caretWidth, caretHeight);
													}
													else {
														caret.setLocation(location);
													}
													getAccessible().textCaretMoved(
													getCaretOffset());
													if (direction !is caretDirection) {
														...
														if (caretDirection is SWT.LEFT) {
															...
															BidiUtil.KEYBOARD_NON_BIDI);
														}
														else if (caretDirection is SWT.RIGHT) {
															BidiUtil.setKeyboardLanguage(
															BidiUtil.KEYBOARD_BIDI);
														}
													}
												}
												columnX = location.x;
 ...
 */
											...
											int length, int clipboardType) {
												if (clipboardType is DND.SELECTION_CLIPBOARD
													&& !(IS_MOTIF || IS_GTK))
													return;
												TextTransfer plainTextTransfer = TextTransfer.getInstance();
												TextWriter plainTextWriter = new TextWriter(start,
												length);
												String plainText = getPlatformDelimitedText(
												plainTextWriter);
												Object[] data;
												Transfer[] types;
												if (clipboardType is DND.SELECTION_CLIPBOARD) {
													...
													RTFWriter rtfWriter = new RTFWriter(start,
													length);
													String rtfText = getPlatformDelimitedText(
													rtfWriter);
													data = [cast(Object) new ArrayWrapperString(rtfText),
													new ArrayWrapperString(plainText)];
													types = [cast(Transfer) rtfTransfer,
													plainTextTransfer];
												}
 ...
 */
											public override void setCursor(Cursor cursor) {
												if (cursor is null) {
													Display display = getDisplay();
													super.setCursor(display.getSystemCursor(
													SWT.CURSOR_IBEAM));
												}
												else {
											...
											public override void setFont(Font font) {
												...
												if (isFixedLineHeight()) {
													...
													if (lineHeight !is oldLineHeight) {
														int vscroll = (getVerticalScrollOffset()
														* lineHeight / oldLineHeight) - getVerticalScrollOffset();
 ...
 */
											public void setHorizontalIndex(int offset) {
												...
												scrollHorizontal(offset - horizontalScrollOffset,
												true);
											}
											/**
 ...
 */
											public void setHorizontalPixel(int pixel) {
												...
												scrollHorizontal(pixel - horizontalScrollOffset,
												true);
											}
											/**
 ...
 */
											public void setIndent(int indent) {
												checkWidget();
												if (this.indent is indent || indent < 0)
													return;
												this.indent = indent;
												resetCache(0, content.getLineCount());
												setCaretLocation();
 ...
 * @since 3.2
 */
											public void setLineAlignment(int startLine,
											int lineCount, int alignment) {
												checkWidget();
												if (isListening(LineGetStyle))
													return;
												if (startLine < 0
													|| startLine + lineCount > content.getLineCount()) {
													SWT.error(SWT.ERROR_INVALID_ARGUMENT);
												}

												renderer.setLineAlignment(startLine,
												lineCount, alignment);
												resetCache(startLine, lineCount);
												redrawLines(startLine, lineCount);
												int caretLine = getCaretLine();
												if (startLine <= caretLine
													&& caretLine < startLine + lineCount) {
													setCaretLocation();
												}
											}
											/**
 ...
 */
											...
											int lineCount, Color background) {
												checkWidget();
												if (isListening(LineGetBackground))
													return;
												if (startLine < 0
												...
												if (background !is null) {
													renderer.setLineBackground(startLine,
													lineCount, background);
												}
												else {
													renderer.clearLineBackground(startLine,
													lineCount);
												}
												redrawLines(startLine, lineCount);
											}
 ...
 * @since 3.2
 */
											public void setLineBullet(int startLine,
											int lineCount, Bullet bullet) {
												checkWidget();
												if (isListening(LineGetStyle))
													return;
												if (startLine < 0
												...
												renderer.setLineBullet(startLine,
												lineCount, bullet);
												resetCache(startLine, lineCount);
												redrawLines(startLine, lineCount);
												int caretLine = getCaretLine();
 ...
 * @since 3.2
 */
											public void setLineIndent(int startLine,
											int lineCount, int indent) {
												checkWidget();
												if (isListening(LineGetStyle))
													return;
												if (startLine < 0
												...
												renderer.setLineIndent(startLine,
												lineCount, indent);
												resetCache(startLine, lineCount);
												redrawLines(startLine, lineCount);
												int caretLine = getCaretLine();
 ...
 * @since 3.2
 */
											public void setLineJustify(int startLine,
											int lineCount, bool justify) {
												checkWidget();
												if (isListening(LineGetStyle))
													return;
												if (startLine < 0
												...
												renderer.setLineJustify(startLine,
												lineCount, justify);
												resetCache(startLine, lineCount);
												redrawLines(startLine, lineCount);
												int caretLine = getCaretLine();
 ...
 */
											public void setLineSpacing(int lineSpacing) {
												checkWidget();
												if (this.lineSpacing is lineSpacing
													|| lineSpacing < 0)
													return;
 ...
 */
											public void setOrientation(int orientation) {
												if ((orientation & (
													SWT.RIGHT_TO_LEFT | SWT.LEFT_TO_RIGHT)) is 0) {
													return;
												}
												if ((orientation & SWT.RIGHT_TO_LEFT) !is 0
													&& (orientation & SWT.LEFT_TO_RIGHT) !is 0) {
													return;
												}
												if ((orientation & SWT.RIGHT_TO_LEFT) !is 0
													&& isMirrored()) {
											...
											void setScrollBars(bool vertical) {
												...
												if (vertical || !isFixedLineHeight()) {
													...
													if (verticalBar !is null) {
														...
														else if (verticalBar.getThumb() !is inactive
															|| verticalBar.getMaximum() !is inactive) {
												...
												if (horizontalBar !is null
													...
													if (clientAreaWidth < maximum) {
														...
														clientAreaWidth - leftMargin - rightMargin);
														horizontalBar.setPageIncrement(
														clientAreaWidth - leftMargin - rightMargin);
													}
													else if (horizontalBar.getThumb() !is inactive
														|| horizontalBar.getMaximum() !is inactive) {
 ...
 */
											void setSelection(int start, int length, bool sendEvent) {
												...
												// different?
												if (selection.x !is start || selection.y !is end || (length > 0
													&& selectionAnchor !is selection.x) || (length < 0
													&& selectionAnchor !is selection.y)) {
													...
													caretAlignment = PREVIOUS_OFFSET_TRAILING;
													internalRedrawRange(selection.x,
 ...
 */
											public void setSelectionRange(int start, int length) {
												...
												if (isLineDelimiter(start)
													|| isLineDelimiter(start + length)) {
 ...
 */
											public void setStyleRange(StyleRange range) {
												...
												if (range !is null) {
													if (range.isUnstyled()) {
														setStyleRanges(range.start,
														range.length, null, null, false);
													}
													else {
														setStyleRanges(range.start, 0,
														null, [range], false);
													}
												}
												else {
 ...
 * @since 3.2
 */
											public void setStyleRanges(int start,
											int length, int[] ranges, StyleRange[] styles) {
												checkWidget();
												if (isListening(LineGetStyle))
													return;
 ...
 */
											...
											void setStyleRanges(int start, int length,
											int[] ranges, StyleRange[] styles, bool reset) {
												...
												if (styles !is null) {
													...
													if (ranges !is null) {
														if (ranges.length !is styles.length << 1)
															SWT.error(SWT.ERROR_INVALID_ARGUMENT);
													}
													int lastOffset = 0;
													...
													for (int i = 0; i < styles.length;
														...
														if (rangeLength < 0)
															SWT.error(SWT.ERROR_INVALID_ARGUMENT);
														if (!(0 <= rangeStart
															&& rangeStart + rangeLength <= charCount))
															SWT.error(SWT.ERROR_INVALID_ARGUMENT);
												...
												if (!isFixedLineHeight() && !reset) {
													int lineEnd = content.getLineAtOffset(Math.max(end,
													rangeEnd));
													int partialTopIndex = getPartialTopIndex();
													int partialBottomIndex = getPartialBottomIndex();
													if (partialTopIndex <= lineEnd
														&& lineEnd <= partialBottomIndex) {
												...
												if (reset) {
													...
													int lineStart = content.getLineAtOffset(Math.min(start,
													rangeStart));
													int lineEnd = content.getLineAtOffset(Math.max(end,
													rangeEnd));
													...
													int partialTopIndex = getPartialTopIndex();
													int partialBottomIndex = getPartialBottomIndex();
													if (!(lineStart > partialBottomIndex
														|| lineEnd < partialTopIndex)) {
														int y = 0;
														int height = clientAreaHeight;
														if (partialTopIndex <= lineStart
															&& lineStart <= partialBottomIndex) {
															int lineTop = Math.max(y,
															getLinePixel(lineStart));
															y = lineTop;
															height -= lineTop;
														}
														if (partialTopIndex <= lineEnd
															&& lineEnd <= partialBottomIndex) {
															int newLastLineBottom = getLinePixel(
															lineEnd + 1);
															if (!isFixedLineHeight()) {
																scrollText(lastLineBottom,
																newLastLineBottom);
															}
															height = newLastLineBottom - y;
														}
														super.redraw(0, y,
														clientAreaWidth, height, false);
													}
												}
												setCaretLocation();
											}
 ...
 */
											public void setTopIndex(int topIndex) {
												...
												if (isFixedLineHeight()) {
													...
													Math.min(lineCount, getLineCountWhole()));
													if (topIndex < 0) {
														topIndex = 0;
													}
 ...
 */
											public void setTopPixel(int pixel) {
												...
												if (isFixedLineHeight()) {
													int maxTopPixel = Math.max(0,
													lineCount * getVerticalIncrement() - height);
 ...
 */
											...
											bool showLocation(Rectangle rect, bool scrollPage) {
												...
												- leftMargin - rightMargin;
												int clientAreaHeight = this.clientAreaHeight
												- topMargin - bottomMargin;
												bool scrolled = false;
												if (rect.y <= topMargin) {
													scrolled = scrollVertical(rect.y - topMargin,
													true);
												}
													...
													if (clientAreaHeight is 0) {
														...
														scrolled = scrollVertical(
														rect.y + rect.height - clientAreaHeight,
														true);
													}
												}
												if (clientAreaWidth > 0) {
													int minScroll = scrollPage ? clientAreaWidth / 4
													: 0;
													if (rect.x < leftMargin) {
														int scrollWidth = Math.max(leftMargin - rect.x,
														minScroll);
														int maxScroll = horizontalScrollOffset;
														scrolled = scrollHorizontal(-Math.min(maxScroll,
														scrollWidth), true);
													}
													else if (rect.x + rect.width > clientAreaWidth) {
														int scrollWidth = Math.max(rect.x + rect.width - clientAreaWidth,
														minScroll);
 ...
 */
											public void showSelection() {
												...
												Rectangle startBounds = getBoundsAtOffset(
												startOffset);
												...
												bool selectionFits = rightToLeft ? startBounds.x - endBounds.x <= w
												: endBounds.x - startBounds.x <= w;
												if (selectionFits) {
 ...
 */
											...
											int replacedLength, int newLength) {
												...
												if (selection.y > startOffset + replacedLength
													...
													internalRedrawRange(redrawStart,
													selection.y + netNewLength - redrawStart);
												}
												...
												if (selection.y > startOffset
													...
													setSelection(selection.x + newLength - replacedLength,
													selection.y - selection.x, true);
												}
												setCaretLocation();
											}

											// DWT: to use instead of "offsetInLine - 1"
											int getPreviousCharOffset(String F = __FILE__,
											uint L = __LINE__)(int lineIndex, int offsetInLine) {
												String line = content.getLine(lineIndex);
												if (offsetInLine < 0 || offsetInLine > line.length) {
													getDwtLogger().warn(F, L, Format("Clamped UTF-8 offset:\noffsetInLine = {}, line.length = {}, line = {}",
													offsetInLine, line.length, line));
													return offsetInLine - 1;
												}
												return cast(int) /*64bit*/ line.offsetBefore(
												offsetInLine);
											}
										}
