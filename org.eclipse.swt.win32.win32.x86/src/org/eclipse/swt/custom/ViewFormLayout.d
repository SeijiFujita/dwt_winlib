...
class ViewFormLayout : Layout {
	...
	protected override Point computeSize(Composite composite, int wHint, int hHint, bool flushCache) {
		...
		if (form.separateTopCenter || (wHint !is SWT.DEFAULT
				&& leftSize.x + centerSize.x + rightSize.x > wHint)) {
	...
	protected override bool flushCache(Control control) {
		Object data = control.getLayoutData();
		if (auto ld = cast(CLayoutData) data)
			ld.flushCache();
		return true;
	}
	...
	protected override void layout(Composite composite, bool flushCache) {
		...
		if (form.separateTopCenter || minTopWidth > rect.width) {
			...
			if (center !is null && !center.isDisposed()) {
				...
				center.setBounds(rect.x + rect.width - form.marginWidth - form.highlight - centerSize.x,
						y, centerSize.x, centerSize.y);
			...
			if (left !is null && !left.isDisposed()) {
				top = true;
				Rectangle trim = (null !is cast(Composite) left) ? (cast(Composite) left).computeTrim(0,
						0, 0, 0) : new Rectangle(0, 0, 0, 0);
		...
		if (content !is null && !content.isDisposed()) {
			...
			content.setBounds(rect.x + form.marginWidth + form.highlight, y,
					rect.width - 2 * form.marginWidth - 2 * form.highlight,
					rect.y + rect.height - y - form.marginHeight - form.highlight);
		}
		if (oldSeperator !is -1 && form.separator !is -1) {
			...
			form.redraw(form.borderLeft, t, form.getSize()
					.x - form.borderLeft - form.borderRight, b - t, false);
		}
	}
}
