...
class CBannerLayout : Layout {
	...
	protected override Point computeSize(Composite composite, int wHint, int hHint, bool flushCache) {
		...
		if (left !is null) {
			if (right is null) {
				...
				height += Math.max(leftSize.y, banner.rightMinHeight is SWT.DEFAULT
						? rightSize.y : banner.rightMinHeight);
			}
		}
		else {
			height += rightSize.y;
	...
	protected override void layout(Composite composite, bool flushCache) {
		...
		if (right !is null) {
			if (left !is null) {
				rightSize.y = Math.max(leftSize.y, banner.rightMinHeight is SWT.DEFAULT
						? rightSize.y : banner.rightMinHeight);
			}
			rightRect = new Rectangle(x, y, rightSize.x, rightSize.y);
		}
		if (banner.curveStart < oldStart) {
			banner.redraw(banner.curveStart - CBanner.CURVE_TAIL, 0,
					oldStart + banner.curve_width - banner.curveStart + CBanner.CURVE_TAIL + 5,
					size.y, false);
		}
		if (banner.curveStart > oldStart) {
			banner.redraw(oldStart - CBanner.CURVE_TAIL, 0,
					banner.curveStart + banner.curve_width - oldStart + CBanner.CURVE_TAIL + 5,
					size.y, false);
		}
		/*
