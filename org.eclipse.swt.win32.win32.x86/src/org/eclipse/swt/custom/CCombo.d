 ...
 */
	public this(Composite parent, int style) {
		...
		listener = new class Listener {
			public void handleEvent(Event event) {
				...
				if (getShell() is event.widget) {
					getDisplay().asyncExec(new class Runnable {
						public void run() {
							if (isDisposed())
								return; handleFocus(SWT.FocusOut);}
						}
);
					}
				}
			...
			int[] textEvents = [
				SWT.DefaultSelection, SWT.KeyDown, SWT.KeyUp, SWT.MenuDetect,
				SWT.Modify, SWT.MouseDown, SWT.MouseUp, SWT.MouseDoubleClick,
				SWT.MouseWheel, SWT.Traverse, SWT.FocusIn, SWT.Verify
			];
		...
		static int checkStyle(int style) {
			int mask = SWT.BORDER | SWT.READ_ONLY | SWT.FLAT | SWT.LEFT_TO_RIGHT | SWT
				.RIGHT_TO_LEFT;
 ...
 */
		...
		void createPopup(String[] items, int selectionIndex) {
			...
			int[] listEvents = [
				SWT.MouseUp, SWT.Selection, SWT.Traverse, SWT.KeyDown,
 ...
 */
		public void deselect(int index) {
			...
			if (0 <= index && index < list.getItemCount()
					&& index is list.getSelectionIndex() && text.getText()
					.equals(list.getItem(index))) {
 ...
 */
		...
		void initAccessible() {
			...
			arrow.getAccessible().addAccessibleListener(new class AccessibleAdapter {
				override public void getName(AccessibleEvent e) {
					e.result = isDropped() ? SWT.getMessage("SWT_Close")
				...
				getAccessible().addAccessibleTextListener(new class AccessibleTextAdapter {
					override public void getCaretOffset(AccessibleTextEvent e) {
						...
						override public void getSelectionRange(AccessibleTextEvent e) {
							Point sel = text.getSelection(); e.offset = sel.x; e.length
							= sel.y - sel.x;}
						}
);
						...
						new class AccessibleControlAdapter {
							override public void getChildAtPoint(AccessibleControlEvent e) {
								Point testPoint = toControl(e.x, e.y); if (getBounds()
									.contains(testPoint)) {
									e.childID = ACC.CHILDID_SELF;}
								}

								override public void getLocation(AccessibleControlEvent e) {
									Rectangle location = getBounds(); Point pt = getParent().toDisplay(location.x,
									location.y); e.x = pt.x; e.y = pt.y; e.width = location.width;
									...
									override public void getChildCount(AccessibleControlEvent e) {
										...
										override public void getRole(AccessibleControlEvent e) {
											...
											void internalLayout(bool changed) {
												...
												Point arrowSize = arrow.computeSize(SWT.DEFAULT,
												height, changed);
												text.setBounds(0, 0, width - arrowSize.x, height);
												arrow.setBounds(width - arrowSize.x,
												0, arrowSize.x, arrowSize.y);
											}
											...
											void listEvent(Event event) {
												...
												case SWT.KeyDown: {
														...
														if ((event.stateMask & SWT.ALT) !is 0
															&& (event.keyCode is SWT.ARROW_UP
															|| event.keyCode is SWT.ARROW_DOWN)) {
															dropDown(false);
														}
 ...
 */
											...
											void popupEvent(Event event) {
												...
												case SWT.Paint:
													...
													Rectangle listRect = list.getBounds();
													Color black = getDisplay().getSystemColor(
													SWT.COLOR_BLACK);
													event.gc.setForeground(black);
													event.gc.drawRectangle(0, 0,
													listRect.width + 1, listRect.height + 1);
													break;
												case SWT.Close:
													event.doit = false;
													dropDown(false);
             ...
             */
													if ("carbon" != (SWT.getPlatform())) {
														Point point = arrow.toControl(getDisplay()
														.getCursorLocation());
 ...
 */
											...
											void textEvent(Event event) {
												...
												case SWT.MouseWheel: {
														Event keyEvent = new Event();
														keyEvent.time = event.time;
														keyEvent.keyCode = event.count > 0
														? SWT.ARROW_UP : SWT.ARROW_DOWN;
														keyEvent.stateMask = event.stateMask;
														notifyListeners(SWT.KeyDown, keyEvent);
														if (isDisposed())
															break;
