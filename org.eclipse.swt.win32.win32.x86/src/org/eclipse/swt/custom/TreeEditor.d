...
*/
		...
		treeListener = new class TreeListener {
			...
			this() {
				runnable = new class Runnable {
					public void run() {
						if (this.outer.outer.editor is null || this.outer.outer.editor.isDisposed())
							return;
						if (this.outer.outer.tree.isDisposed())
							return;
						layout();
			...
			public void treeCollapsed(TreeEvent e) {
				if (this.outer.editor is null || this.outer.editor.isDisposed())
					return;
	...
	override Rectangle computeBounds() {
		if (item is null || column is -1 || item.isDisposed())
...
*/
	...
	public override void layout() {
		...
		if (item is null || item.isDisposed())
			return;
		int columnCount = tree.getColumnCount();
		if (columnCount is 0 && column !is 0)
			return;
