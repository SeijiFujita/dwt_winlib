...
class StyledTextRenderer {
	...
	int addMerge(int[] mergeRanges, StyleRange[] mergeStyles, int mergeCount,
			int modifyStart, int modifyEnd) {
		...
		if (rangeCount + grow >= ranges.length) {
			...
			if (rangeCount > modifyEnd) {
				System.arraycopy(ranges, modifyEnd, tmpRanges,
						modifyStart + mergeCount, rangeCount - modifyEnd);
				System.arraycopy(styles, modifyEnd >> 1, tmpStyles,
						(modifyStart + mergeCount) >> 1, styleCount - (modifyEnd >> 1));
			}
			ranges = tmpRanges;
			styles = tmpStyles;
		}
			...
			if (rangeCount > modifyEnd) {
				System.arraycopy(ranges, modifyEnd, ranges,
						modifyStart + mergeCount, rangeCount - modifyEnd);
				System.arraycopy(styles, modifyEnd >> 1, styles,
						(modifyStart + mergeCount) >> 1, styleCount - (modifyEnd >> 1));
			}
		}
		if (MERGE_STYLES) {
			...
			for (int i = 0; i < mergeCount; i += 2) {
				if (j > 0 && ranges[j - 2] + ranges[j - 1] is mergeRanges[i]
			...
			if (endStyle !is null && ranges[j - 2] + ranges[j - 1] is endStart
			...
			if (rangeCount > modifyEnd) {
				System.arraycopy(ranges, modifyStart + mergeCount, ranges, j,
						rangeCount - modifyEnd);
				System.arraycopy(styles, (modifyStart + mergeCount) >> 1, styles,
						j >> 1, styleCount - (modifyEnd >> 1));
			}
	...
	int addMerge(StyleRange[] mergeStyles, int mergeCount, int modifyStart, int modifyEnd) {
		...
		if (styleCount + grow >= styles.length) {
			...
			if (styleCount > modifyEnd) {
				System.arraycopy(styles, modifyEnd, tmpStyles,
						modifyStart + mergeCount, styleCount - modifyEnd);
			}
			styles = tmpStyles;
		}
		else {
			if (styleCount > modifyEnd) {
				System.arraycopy(styles, modifyEnd, styles,
						modifyStart + mergeCount, styleCount - modifyEnd);
			}
		}
		if (MERGE_STYLES) {
			...
			for (int i = 0; i < mergeCount; i++) {
				...
				if (j > 0 && (style = styles[j - 1])
						.start + style.length is newStyle.start && newStyle.similarTo(style)) {
			...
			if (endStyle !is null && style.start + style.length is endStyle.start
			...
			if (styleCount > modifyEnd) {
				System.arraycopy(styles, modifyStart + mergeCount, styles, j,
	...
	void drawBullet(Bullet bullet, GC gc, int paintX, int paintY, int index,
		...
		int type = bullet.type & (
				ST.BULLET_DOT | ST.BULLET_NUMBER | ST.BULLET_LETTER_LOWER | ST.BULLET_LETTER_UPPER);
		switch (type) {
		case ST.BULLET_DOT:
			string = "\u2022";
			break;
		case ST.BULLET_NUMBER:
			string = String_valueOf(index);
			break;
		case ST.BULLET_LETTER_LOWER:
			string = [cast(char)(index % 26 + 97)];
			break;
		case ST.BULLET_LETTER_UPPER:
			string = [cast(char)(index % 26 + 65)];
			break;
		default:
		}
		if ((bullet.type & ST.BULLET_TEXT) !is 0)
			string ~= bullet.text;
		Display display = styledText.getDisplay();
	...
	int drawLine(int lineIndex, int paintX, int paintY, GC gc,
		...
		if (selectionStart is selectionEnd || (selectionEnd <= 0 && selectionStart >= lineLength)) {
			...
			layout.draw(gc, paintX, paintY, start, end > 0
					? cast(int) /*64bit*/ line.offsetBefore(end) : end - 1,
					selectionFg, selectionBg, flags);
		}
		...
		if (bulletIndex !is -1 && bullet !is null) {
			...
			if (bullet.type is ST.BULLET_CUSTOM) {
				...
				styledText.paintObject(gc, paintX, paintY, lineAscent,
						metrics.getDescent(), bullet.style, bullet, bulletIndex);
			}
			else {
	...
	StyleRange[] getStyleRanges(int start, int length, bool includeRanges) {
		...
		if (start > style.start) {
			if (!includeRanges || ranges is null)
	...
	StyleRange getStyleRange(StyleRange style) {
		if (style.start is 0 && style.length is 0 && style.fontStyle is SWT.NORMAL)
			return style;
		StyleRange clone = cast(StyleRange) style.clone();
		clone.start = clone.length = 0;
		clone.fontStyle = SWT.NORMAL;
		if (clone.font is null)
			clone.font = getFont(style.fontStyle);
		return clone;
	}

	TextLayout getTextLayout(int lineIndex) {
		return getTextLayout(lineIndex, styledText.getOrientation(),
				styledText.getWrapWidth(), styledText.lineSpacing);
	}
	...
	TextLayout getTextLayout(int lineIndex, int orientation, int width, int lineSpacing) {
					...
					if (delta > 0) {
						if (delta < bullets.length) {
							...
							System.arraycopy(bulletsIndices, delta,
									bulletsIndices, 0, bulletsIndices.length - delta);
						}
		...
		if (event !is null) {
			...
			if (lines !is null) {
				...
				if (info !is null) {
					if ((info.flags & INDENT) !is 0)
						indent = info.indent;
					if ((info.flags & ALIGNMENT) !is 0)
						alignment = info.alignment;
					if ((info.flags & JUSTIFY) !is 0)
						justify = info.justify;
					if ((info.flags & SEGMENTS) !is 0)
						segments = info.segments;
				}
			}
			if (bulletsIndices !is null) {
			...
			if (ranges !is null) {
				...
				for (int i = rangeStart; i < rangeCount; i += 2) {
					...
					if (start >= length)
						break;
					if (lastOffset < start) {
						layout.setStyle(null, lastOffset, cast(int) /*64bit*/ line.offsetBefore(
								start));
					}
						...
						if (imeRanges.length > 0) {
							for (int i = 0; i < imeStyles.length; i++) {
								...
								for (int j = start; j <= end; j++) {
									...
									if (userStyle is null && j > 0)
										userStyle = layout.getStyle(j - 1);
									if (userStyle is null && j + 1 < length)
										userStyle = layout.getStyle(j + 1);
									if (userStyle is null) {
										...
										if (newStyle.font is null)
											newStyle.font = userStyle.font;
										if (newStyle.foreground is null)
											newStyle.foreground = userStyle.foreground;
										if (newStyle.background is null)
							...
							int end = cast(int) /*64bit*/ line.offsetBefore(
									start + compositionLength);
							TextStyle userStyle = layout.getStyle(start);
							if (userStyle is null) {
								if (start > 0)
									userStyle = layout.getStyle(cast(int) /*64bit*/ line.offsetBefore(
											start));
								if (userStyle is null && line.offsetAfter(end) < length)
									userStyle = layout.getStyle(cast(int) /*64bit*/ line.offsetAfter(
											end));
								if (userStyle !is null) {
		...
		if (styledText !is null && styledText.isFixedLineHeight()) {
			...
			if (index !is -1) {
				...
				if (styledText.verticalScrollOffset !is 0) {
					...
					if (topIndexY >= 0) {
						styledText.verticalScrollOffset = (
								topIndex - 1) * lineHeight + lineHeight - topIndexY;
					}
					else {
	...
	void setStyleRanges(int[] newRanges, StyleRange[] newStyles) {
		...
		if (ranges !is null) {
			...
			if (insert) {
				addMerge(newRanges, newStyles, cast(int) /*64bit*/ newRanges.length,
	...
	void textChanging(TextChangingEvent event) {
		...
		if (replaceCharCount is content.getCharCount())
			lines = null;
		if (replaceLineCount is lineCount) {
			...
			if (layouts !is null) {
				...
				if (delta > 0) {
					for (int i = cast(int) /*64bit*/ layouts.length - 1; i >= layoutEndLine;
							i--) {
						if (0 <= i && i < layouts.length) {
							endIndex = i + delta;
			...
			lineCount += delta;
			if (maxWidthLineIndex !is -1 && startLine <= maxWidthLineIndex
	...
	void updateRanges(int start, int replaceCharCount, int newCharCount) {
		if (styleCount is 0 || (replaceCharCount is 0 && newCharCount is 0))
			return;
		if (ranges !is null) {
			int rangeCount = styleCount << 1;
			int modifyStart = getRangeIndex(start, -1, rangeCount);
			if (modifyStart is rangeCount)
				return;
			...
			if (modifyStart is modifyEnd && ranges[modifyStart] < start
				...
				if (newCharCount is 0) {
					...
					System.arraycopy(ranges, modifyStart + 2, ranges,
							modifyStart + 4, rangeCount - (modifyStart + 2));
					System.arraycopy(styles, (modifyStart + 2) >> 1, styles,
							(modifyStart + 4) >> 1, styleCount - ((modifyStart + 2) >> 1));
			...
			if (modifyStart is modifyEnd && styles[modifyStart].start < start
				...
				if (styles[modifyStart].start < start
						&& start < styles[modifyStart].start + styles[modifyStart].length) {
					styles[modifyStart].length = start - styles[modifyStart].start;
					modifyStart++;
				}
