 ...
 */
	public override equals_t opEquals(Object object) {
		...
		if (auto style = cast(StyleRange) object) {
			if (start !is style.start)
				return false;
			if (length !is style.length)
				return false;
 ...
 */
	public bool isUnstyled() {
		...
		if (background !is null)
			return false;
		if (fontStyle !is SWT.NORMAL)
			return false;
		if (underline)
			return false;
		if (strikeout)
			return false;
		if (borderStyle !is SWT.NONE)
			return false;
		return true;
	}
 ...
 */
	public bool similarTo(StyleRange style) {
		if (!super.opEquals(style))
			return false;
		if (fontStyle !is style.fontStyle)
			return false;
		return true;
	}
