 ...
 */
	public this(Table parent, int style) {
		...
		int[] events = [
			SWT.Dispose, SWT.FocusIn, SWT.FocusOut, SWT.KeyDown, SWT.Paint, SWT.Traverse
		];
 ...
 */
	...
	void keyDown(Event event) {
		...
		case SWT.ARROW_UP:
			...
			break;
		case SWT.ARROW_DOWN:
			setRowColumn(Math.min(rowIndex + 1,
					table.getItemCount() - 1), columnIndex, true);
			break;
		case SWT.ARROW_LEFT:
		case SWT.ARROW_RIGHT: {
				...
				while (index < order.length) {
					if (order[index] is columnIndex)
						break;
					index++;
				}
				if (index is order.length)
					index = 0;
				int leadKey = (getStyle() & SWT.RIGHT_TO_LEFT) !is 0 ? SWT.ARROW_RIGHT
					: SWT.ARROW_LEFT;
	...
	void setRowColumn(int row, int column, bool notify) {
		...
		TableColumn col = column is -1 || table.getColumnCount() is 0 ? null
 ...
 */
	public void setSelection(TableItem row, int column) {
		...
		int columnCount = table.getColumnCount();
		int maxColumnIndex = columnCount is 0 ? 0 : columnCount - 1;
		if (row is null || row.isDisposed() || column < 0 || column > maxColumnIndex)
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
