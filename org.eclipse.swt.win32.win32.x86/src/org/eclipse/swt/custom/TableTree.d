 ...
 */
	public this(Composite parent, int style) {
		...
		int[] tableEvents = [SWT.MouseDown, SWT.Selection, SWT.DefaultSelection, SWT.KeyDown];
		...
		int[] events = [SWT.Dispose, SWT.Resize, SWT.FocusIn];
 ...
 */
	...
	void createImages() {
		...
		/* Plus image */
		PaletteData palette = new PaletteData([foreground.getRGB(),
	...
	Image getPlusImage() {
		if (plusImage is null)
			createImages();
		return plusImage;
	}

	Image getMinusImage() {
		if (minusImage is null)
			createImages();
		return minusImage;
	}
 ...
 */
	...
	void removeItem(TableTreeItem item) {
		int index = 0;
		while (index < items.length && items[index]!is item)
			index++;
