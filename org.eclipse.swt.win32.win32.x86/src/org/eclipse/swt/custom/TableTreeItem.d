 ...
 */
	...
	void addCheck() {
		...
		if ((table.getStyle() & SWT.CHECK) is 0)
			return;
 ...
 */
	...
	TableTreeItem getItem(TableItem tableItem) {
		if (tableItem is null)
			return null;
		if (this.tableItem is tableItem)
			return this;
	...
	void setVisible(bool show) {
		...
		if (show) {
			...
			for (int i = 0; i < columnCount; i++) {
				if (i < texts.length && texts[i]!is null)
					setText(i, texts[i]);
				if (i < images.length && images[i]!is null)
					setImage(i, images[i]);
			}
			...
			if (items.length !is 0) {
				if (expanded) {
					...
					for (int i = 0, length = cast(int) /*64bit*/ items.length; i < length;
							i++) {
