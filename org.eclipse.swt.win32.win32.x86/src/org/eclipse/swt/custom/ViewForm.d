	...
	void onPaint(GC gc) {
		...
		if (showBorder) {
			...
			if (highlight > 0) {
				...
				int y2 = size.y - 1;
				int[] shape = [
					x1, y1, x2, y1, x2, y2, x1, y2, x1, y1 + highlight,
					x1 + highlight, y1 + highlight, x1 + highlight, y2 - highlight,
					x2 - highlight, y2 - highlight, x2 - highlight,
		...
		if (separator > -1) {
			gc.setForeground(border);
			gc.drawLine(borderLeft + highlight, separator,
					size.x - borderLeft - borderRight - highlight, separator);
		}
