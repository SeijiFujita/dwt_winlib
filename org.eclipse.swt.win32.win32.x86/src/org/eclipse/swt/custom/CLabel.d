 ...
 */
	public this(Composite parent, int style) {
		...
		if ((style & (SWT.CENTER | SWT.RIGHT)) is 0)
			style |= SWT.LEFT;
 ...
 */
				dchar _findMnemonic(String str) {
					...
					do {
						while (index < length && str[index]!is '&')
							index++;
						if (++index >= length)
							return '\0';
						if (str[index]!is '&')
							return Character.toLowerCase(str.dcharAt(index));
						index++;
					}
					while (index < length);
					return '\0';
							...
							new class AccessibleControlAdapter {
								override public void getChildAtPoint(AccessibleControlEvent e) {
									...
									override public void getLocation(AccessibleControlEvent e) {
										Rectangle rect = getDisplay().map(getParent(),
										null, getBounds()); e.x = rect.x; e.y = rect.y;
										e.width = rect.width; e.height = rect.height;
									}
									...
									override public void getChildCount(AccessibleControlEvent e) {
										...
										void onMnemonic(TraverseEvent event) {
											...
											if (mnemonic is '\0')
												return;
											if (Character.toLowerCase(event.character) !is mnemonic)
												return;
										...
										void onPaint(PaintEvent event) {
											...
											if (shortenText_) {
												...
												for (int i = 0; i < lines.length;
													...
													if (e.x > availableWidth) {
														lines[i] = shortenText(gc,
														lines[i], availableWidth);
											...
											try {
												if (backgroundImage !is null) {
													...
													if (gradientColors.length is 1) {
														...
														int pos = 0;
														for (int i = 0; i < gradientPercents.length;
															++i) {
														...
														if (!gradientVertical && pos < rect.width) {
															gc.setBackground(getBackground());
															gc.fillRectangle(pos, 0,
															rect.width - pos, rect.height);
														}
													...
													if (background !is null
														|| (getStyle() & SWT.DOUBLE_BUFFERED) is 0) {
														gc.setBackground(getBackground());
														gc.fillRectangle(rect);
													}
												}
											...
											if (lines !is null) {
												...
												int textHeight = cast(int) /*64bit*/ lines.length
												* lineHeight;
												...
												for (int i = 0; i < lines.length;
													...
													if (lines.length > 1) {
														if (align_ is SWT.CENTER) {
															int lineWidth = gc.textExtent(lines[i],
															DRAW_FLAGS).x;
															lineX = x + Math.max(0,
															(extent.x - lineWidth) / 2);
														}
														if (align_ is SWT.RIGHT) {
															int lineWidth = gc.textExtent(lines[i],
															DRAW_FLAGS).x;
															lineX = Math.max(x,
															rect.x + rect.width - hIndent
															- lineWidth);
														}
													}
 ...
 */
										private void paintBorder(GC gc, Rectangle r) {
											...
											int style = getStyle();
											if ((style & SWT.SHADOW_IN) !is 0) {
												c1 = disp.getSystemColor(
												SWT.COLOR_WIDGET_NORMAL_SHADOW);
												c2 = disp.getSystemColor(
											...
											if (c1 !is null && c2 !is null) {
												gc.setLineWidth(1);
												drawBevelRect(gc, r.x, r.y,
												r.width - 1, r.height - 1, c1, c2);
											}
										}
										/**
 ...
 */
										public void setAlignment(int align_) {
											checkWidget();
											if (align_ !is SWT.LEFT
												&& align_ !is SWT.RIGHT && align_ !is SWT.CENTER) {
										...
										public override void setBackground(Color color) {
											...
											if (backgroundImage is null
												&& gradientColors is null && gradientPercents is null) {
												if (color is null) {
													if (background is null)
														return;
												}
										...
										int[] percents, bool vertical) {
											...
											if (backgroundImage is null) {
												if ((gradientColors !is null) && (colors !is null)
													...
													bool same = false;
													for (int i = 0; i < gradientColors.length;
														i++) {
														same = (gradientColors[i] is colors[i])
														|| ((gradientColors[i] is null)
														&& (colors[i] is background))
														|| ((gradientColors[i] is background)
														&& (colors[i] is null));
														if (!same)
															break;
													}
													if (same) {
														for (int i = 0; i < gradientPercents.length;
															i++) {
															same = gradientPercents[i] is percents[
															i];
															if (!same)
																break;
														}
											...
											if (colors is null) {
												...
												for (int i = 0; i < colors.length;
													++i)
													gradientColors[i] = (colors[i]!is null) ? colors[
													i] : background;
 ...
 */
										protected String shortenText(GC gc, String t, int width) {
											...
											while (min < mid && mid < max) {
												...
												String s2 = t.substring(validateOffset(layout,
												l - mid), l);
											...
											String result = mid is 0 ? t : t.substring(0,
											mid) ~ ELLIPSIS ~ t.substring(validateOffset(layout,
											l - mid), l);
											layout.dispose();
											return result;
										}
										...
										int validateOffset(TextLayout layout, int offset) {
											int nextOffset = layout.getNextOffset(offset,
											SWT.MOVEMENT_CLUSTER);
											if (nextOffset !is offset)
												return layout.getPreviousOffset(nextOffset,
												SWT.MOVEMENT_CLUSTER);
											return offset;
										}
										...
										private String[] splitString(String text) {
											...
											do {
												...
												if (pos is -1) {
													...
													lines[lines.length - 1] = text[start .. pos - (crlf
													? 1 : 0)];
													start = pos + 1;
													String[] newLines = new String[lines.length + 1];
													System.arraycopy(lines, 0,
													newLines, 0, lines.length);
													lines = newLines;
												}
											}
											while (pos !is -1);
