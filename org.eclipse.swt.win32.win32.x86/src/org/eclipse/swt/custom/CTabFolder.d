...
public class CTabFolder : Composite {
	...
	static const int[] BOTTOM_RIGHT_CORNER = [-6, 0, -5, -1, -4, -1, -1,
		-4, -1, -5, 0, -6];
	...
	static const int[] TOP_RIGHT_CORNER_BORDERLESS = [
		-7, 0, -6, 1, -5, 1, -2, 4, -2, 5, -1, 6
	];
	static const int[] BOTTOM_LEFT_CORNER_BORDERLESS = [
		0, -6, 1, -6, 1, -5, 2, -4, 4, -2, 5, -1, 6, -1, 6, 0
	];
	static const int[] BOTTOM_RIGHT_CORNER_BORDERLESS = [
	...
	static const int[] SIMPLE_BOTTOM_RIGHT_CORNER_BORDERLESS = [
		-4, 0, -3, -1, -2, -2, -1, -3
	];
 ...
 */
	public this(Composite parent, int style) {
		...
		int[] folderEvents = [
			SWT.Dispose, SWT.DragDetect, SWT.FocusIn, SWT.FocusOut, SWT.KeyDown,
     ...
     */
		if ((style & SWT.RIGHT_TO_LEFT) !is 0)
			return style;
		if ((parent.getStyle() & SWT.MIRRORED) !is 0 && (style & SWT.LEFT_TO_RIGHT) is 0)
			return style;
 ...
 */
	public void addCTabFolder2Listener(CTabFolder2Listener listener) {
		...
		SimpleType!(CTabFolder2Listener).arraycopy(folderListeners, 0,
				newListeners, 0, folderListeners.length);
 ...
 */
	public void addCTabFolderListener(CTabFolderListener listener) {
		...
		SimpleType!(CTabFolderListener).arraycopy(tabListeners, 0,
				newTabListeners, 0, tabListeners.length);
 ...
 */
	...
	void antialias(int[] shape, RGB lineRGB, RGB innerRGB, RGB outerRGB, GC gc) {
		...
		if (simple || "carbon".equals(SWT.getPlatform()) || "wpf".equals(SWT.getPlatform()))
			return; //$NON-NLS-1$
	...
	public override Rectangle computeTrim(int x, int y, int width, int height) {
		...
		if (minimized) {
			...
			int trimY = onBottom ? y - marginHeight - highlight_margin - borderTop
				: y - marginHeight - highlight_header - tabHeight - borderTop;
			int trimHeight = height + borderTop + borderBottom + 2 * marginHeight
				+ tabHeight + highlight_header + highlight_margin;
			return new Rectangle(trimX, trimY, trimWidth, trimHeight);
		}
	}
	...
	void drawBackground(GC gc, int[] shape, int x, int y, int width, int height,
			Color defaultBackground, Image image, Color[] colors, int[] percents, bool vertical) {
		Region clipping = new Region();
		gc.getClipping(clipping);
			...
			if (colors.length is 1) {
				...
				if (vertical) {
					if (onBottom) {
						...
						for (int i = cast(int) /*64bit*/ percents.length - 1; i >= 0;
								i--) {
			...
			if (highlight_margin > 0) {
				...
				if (onBottom) {
					...
					shape = [x1, y1, x2, y1, x2, y2, x2 - highlight_margin, y2,
						x2 - highlight_margin, y1 + highlight_margin, x1 + highlight_margin,
						y1 + highlight_margin, x1 + highlight_margin, y2, x1, y2];
				}
				else {
					int x1 = borderLeft;
				...
				if (selectedIndex !is -1 && selectionGradientColors !is null
						&& selectionGradientColors.length > 1 && !selectionGradientVertical) {
					drawBackground(gc, shape, true);
				}
				else if (selectedIndex is -1 && gradientColors !is null
	...
	void drawChevron(GC gc) {
		...
		int count;
		if (single) {
			count = selectedIndex is -1 ? cast(int) /*64bit*/ items.length : cast(int) /*64bit*/ items.length
				- 1;
		}
		else {
			int showCount = 0;
	...
	void drawTabArea(Event event) {
		...
		if (tabHeight is 0) {
			int style = getStyle();
			if ((style & SWT.FLAT) !is 0 && (style & SWT.BORDER) is 0)
				return;
			int x1 = borderLeft - 1;
			int x2 = size.x - borderRight;
			int y1 = onBottom ? size.y - borderBottom - highlight_header - 1
 ...
 */
	...
	public override Rectangle getClientArea() {
		...
		int height = size.y - borderTop - borderBottom - 2 * marginHeight
			- highlight_margin - highlight_header;
		height -= tabHeight;
	...
	dchar _findMnemonic(String string) {
		...
		do {
			...
			if (++index >= length_)
				return '\0';
			if (string[index]!is '&')
				return Character.toLowerCase(string.dcharAt(index));
			index++;
		}
		while (index < length_);
		return '\0';
 ...
 */
	...
	void initAccessible() {
		...
		accessible.addAccessibleListener(new class AccessibleAdapter {
			override public void getName(AccessibleEvent e) {
				String name = null; int childID = e.childID; if (childID >= 0
					...
					name = stripMnemonic(items[childID].getText());}
						...
						name = minimized ? SWT.getMessage("SWT_Restore")
							: SWT.getMessage("SWT_Minimize"); //$NON-NLS-1$ //$NON-NLS-2$
					}
					else if (childID is items.length + MAXIMIZE_CHILD_ID) {
						name = maximized ? SWT.getMessage("SWT_Restore")
							: SWT.getMessage("SWT_Maximize"); //$NON-NLS-1$ //$NON-NLS-2$
					}
					e.result = name;}

					override public void getHelp(AccessibleEvent e) {
						String help = null; int childID = e.childID; if (childID is ACC
							.CHILDID_SELF) {
							help = getToolTipText();}
								...
								accessible.addAccessibleControlListener(
									...
									override public void getChildAtPoint(AccessibleControlEvent e) {
										Point testPoint = toControl(e.x, e.y); int childID = ACC
										.CHILDID_NONE; for (int i = 0; i < items.length;
											i++) {
											...
											if (childID is ACC.CHILDID_NONE) {
												if (showChevron && chevronRect.contains(testPoint)) {
													childID = cast(int) /*64bit*/ items.length
													+ CHEVRON_CHILD_ID;}
														...
														+ MINIMIZE_CHILD_ID;}
															...
															+ MAXIMIZE_CHILD_ID;}
																...
																location.height = location.height - getClientArea()
																.height; if (
														...
														AccessibleControlEvent e) {
															...
															Point pt = null; int childID = e
															.childID; if (childID is ACC
																.CHILDID_SELF) {
																location = getBounds();
																pt = getParent().toDisplay(location.x,
																location.y);}
												else {
																	if (childID >= 0 && childID < items.length
																		&& items[childID].isShowing()) {
																		location = items[childID].getBounds();
																	}
																	else if (showChevron
																		&& childID is items.length
																		+ CHEVRON_CHILD_ID) {
															...
															override public void getChildCount(
															AccessibleControlEvent e) {
																e.detail = cast(
																int) /*64bit*/ items.length
																+ EXTRA_CHILD_ID_COUNT;
															}

															override public void getDefaultAction(
															AccessibleControlEvent e) {
																String action = null;
																int childID = e.childID;
																if (childID >= 0
																	&& childID < items.length) {
																	action = SWT.getMessage(
																	"SWT_Switch"); //$NON-NLS-1$
																}
															...
															override public void getRole(
															AccessibleControlEvent e) {
																int role = 0; int childID = e
																.childID; if (
																	childID is ACC.CHILDID_SELF) {
																	role = ACC.ROLE_TABFOLDER;
																}
												else if (childID >= 0 && childID < items.length) {
																	role = ACC.ROLE_TABITEM;
																}
																else if (childID >= items.length
																	&& childID < items.length
																	+ EXTRA_CHILD_ID_COUNT) {
																...
																override public void getSelection(
																AccessibleControlEvent e) {
																...
																AccessibleControlEvent e) {
																	...
																	if (childID is ACC.CHILDID_SELF) {
																		...
																		childID is items.length
																		+ CHEVRON_CHILD_ID) {
																		state = showChevron ? ACC.STATE_NORMAL
																		: ACC.STATE_INVISIBLE;
																	}
																	else if (
																		childID is items.length
																...
																override public void getChildren(
																AccessibleControlEvent e) {
																	int childIdCount = cast(int) /*64bit*/ items
																	.length + EXTRA_CHILD_ID_COUNT;
															...
															new class(accessible) Listener {
																Accessible acc; this(Accessible a) {
																	this.acc = a;}
															...
															void onKeyDown(Event event) {
																...
																case SWT.ARROW_RIGHT:
																	int count = cast(int) /*64bit*/ items
																	.length;
																	if (count is 0)
																		return;
																	if (selectedIndex is -1)
																		return;
																	int leadKey = (
																	getStyle() & SWT.RIGHT_TO_LEFT) !is 0 ? SWT
																	.ARROW_RIGHT : SWT.ARROW_LEFT;
																	int offset = event.keyCode is leadKey
																	? -1 : 1;
																	int index;
																	if (!mru) {
																		index = selectedIndex
																		...
																		if (current + offset >= 0
																			...
																			if (showChevron) {
																				CTabFolderEvent e = new CTabFolderEvent(
																				this);
																				e.widget = this;
																				e.time = event.time;
																				e.x = chevronRect.x;
     ...
     */
																...
																int length = cast(int) /*64bit*/ items
																.length;
															...
															void onDragDetect(Event event) {
																...
																if (chevronRect.contains(event.x, event.y)
																	|| minRect.contains(event.x,
																	event.y) || maxRect.contains(event.x,
																	event.y)) {
																	consume = true;
																}
																else {
																	for (int i = 0;
																		i < items.length;
																		i++) {
																		if (items[i].closeRect.contains(event.x,
																			event.y)) {
																			consume = true;
																			break;
																		}
															...
															void onMouseDoubleClick(Event event) {
																if (event.button !is 1
																	...
																	|| (
																	event.stateMask & SWT.BUTTON3) !is 0)
																	return;
																Event e = new Event();
																e.item = getItem(new Point(event.x,
																event.y));
																if (e.item !is null) {
															...
															void onMouse(Event event) {
																...
																case SWT.MouseExit: {
																		...
																		if (
																			chevronImageState !is NORMAL) {
																			chevronImageState = NORMAL;
																			redraw(chevronRect.x, chevronRect.y,
																			chevronRect.width,
																			chevronRect.height,
																			false);
																		}
																...
																case SWT.MouseDown: {
																		...
																		if (chevronRect.contains(x,
																			y)) {
																			if (event.button !is 1)
																				return;
																			if (
																				chevronImageState !is HOT) {
																		...
																		if (single) {
																			if (
																				selectedIndex !is -1) {
																				Rectangle bounds = items[
																				selectedIndex].getBounds();
																...
																case SWT.MouseMove: {
																		...
																		if (minRect.contains(x, y)) {
																			minimize = true;
																			if (minImageState !is SELECTED
																				&& minImageState !is HOT) {
																				minImageState = HOT;
																				redraw(minRect.x, minRect.y,
																		...
																		if (maxRect.contains(x, y)) {
																			maximize = true;
																			if (maxImageState !is SELECTED
																				&& maxImageState !is HOT) {
																				maxImageState = HOT;
																				redraw(maxRect.x, maxRect.y,
																		...
																		if (chevronRect.contains(x,
																			y)) {
																			chevron = true;
																			if (chevronImageState !is SELECTED
																				&& chevronImageState !is HOT) {
																				chevronImageState = HOT;
																				redraw(chevronRect.x, chevronRect.y,
																				chevronRect.width,
																		...
																		for (int i = 0;
																			...
																			CTabItem item = items[i];
																			close = false;
																			if (item.getBounds().contains(x,
																				y)) {
																				close = true;
																				if (item.closeRect.contains(x,
																					y)) {
																					if (item
																						.closeImageState !is SELECTED
																						&& item
																						.closeImageState !is HOT) {
																						item.closeImageState
																						= HOT;
																						redraw(item.closeRect.x,
																						item.closeRect.y,
																			...
																			if (i is selectedIndex
																				&& item.closeImageState !is NORMAL
																				&& !close) {
																				item.closeImageState
																				= NORMAL;
																				redraw(item.closeRect.x,
																		...
																		if (item !is null) {
																			if (item.closeRect.contains(x,
																				...
																				for (int j = 0;
																					...
																					.length;
																					j++) {
																					CTabFolder2Listener listener = folderListeners[
																					j];
																					listener.close(
																					e);
																				}
																				for (int j = 0;
																					...
																					.length;
																					j++) {
																					CTabFolderListener listener = tabListeners[
																					j];
																					listener.itemClosed(
																					e);
																				}
																				...
																				if (!isDisposed()
																					...
																					Point pt = display
																					.getCursorLocation();
																					pt = display.map(null, this, pt.x,
																					pt.y);
																					CTabItem nextItem = getItem(
																					pt);
																					if (
																						nextItem !is null) {
																						if (nextItem.closeRect.contains(
																							pt)) {
																							if (nextItem.closeImageState !is SELECTED && nextItem
																								.closeImageState !is HOT) {
																								nextItem
																								.closeImageState = HOT;
																								redraw(nextItem
															...
															void onResize() {
																...
																if (oldSize is null) {
																	...
																	if (onBottom
																		...
																		int y1 = Math.min(size.y,
																		oldSize.y);
																		if (size.y !is oldSize.y)
																			y1 -= borderBottom
																			+ highlight_margin;
																		int x2 = Math.max(size.x,
																		oldSize.x);
																		int y2 = Math.max(size.y,
																		oldSize.y);
																		redraw(0, y1, x2, y2 - y1,
																		false);
																		redraw(x1, 0, x2 - x1, y2,
																		false);
																	}
																}
																oldSize = size;
															...
															void redrawTabs() {
																Point size = getSize();
																if (onBottom) {
																	redraw(0, size.y - borderBottom - tabHeight
																	- highlight_header - 1, size.x,
																	borderBottom + tabHeight + highlight_header + 1,
																	false);
																}
																else {
																	redraw(0, 0, size.x,
																	borderTop + tabHeight + highlight_header + 1,
																	false);
																}
															}
															/**
 ...
 * @since 3.0
 */
															public void removeCTabFolder2Listener(
															CTabFolder2Listener listener) {
																...
																if (folderListeners.length is 0)
																	return;
																int index = -1;
																for (int i = 0; i < folderListeners
																	.length; i++) {
																	if (
																		listener is folderListeners[
																		i]) {
																		index = i;
																		break;
																	}
																...
																if (folderListeners.length is 1) {
																	folderListeners = new CTabFolder2Listener[
																	0];
																	return;
																}
																CTabFolder2Listener[] newTabListeners = new CTabFolder2Listener[
																folderListeners.length - 1];
																SimpleType!(CTabFolder2Listener).arraycopy(
																folderListeners, 0,
																newTabListeners, 0, index);
																SimpleType!(CTabFolder2Listener).arraycopy(
																folderListeners,
																index + 1, newTabListeners,
																index,
 ...
 */
															public void removeCTabFolderListener(
															CTabFolderListener listener) {
																checkWidget();
																if (listener is null)
																	SWT.error(
																	SWT.ERROR_NULL_ARGUMENT);
																...
																if (tabListeners.length is 1) {
																	tabListeners = new CTabFolderListener[
																	0];
																	return;
																}
																CTabFolderListener[] newTabListeners = new CTabFolderListener[
																tabListeners.length - 1];
																SimpleType!(CTabFolderListener).arraycopy(tabListeners,
																0, newTabListeners, 0, index);
																SimpleType!(CTabFolderListener).arraycopy(tabListeners,
																index + 1, newTabListeners,
																index,
 ...
 */
															...
															int[] percents, bool vertical) {
																...
																if (colors !is null) {
																	if (percents is null
																		|| percents.length !is colors.length
																		- 1) {
																		SWT.error(
																...
																if (bgImage is null) {
																	if ((gradientColors !is null) && (colors !is null)
																		&& (
																		gradientColors.length is colors
																		.length)) {
																		bool same = false;
																		for (int i = 0;
																			i < gradientColors
																			...
																			i++) {
																			if (
																				gradientColors[i] is null) {
																				same = colors[i] is null;
																			}
																			else {
																				same = cast(bool)(
																				gradientColors[i] == colors[
																				i]);
																			}
																			if (!same)
																				break;
																...
																if (colors is null) {
																	...
																	gradientPercents = new int[
																	percents.length];
																	for (int i = 0;
																		i < percents.length;
																		++i) {
																		gradientPercents[i] = percents[
																		i];
																	}
 ...
 */
															public void setBorderVisible(bool show) {
																...
																if ((borderLeft is 1) is show)
																	return;
																borderLeft = borderRight = show ? 1
																: 0;
																borderTop = onBottom ? borderLeft
																: 0;
																borderBottom = onBottom ? 0
																: borderLeft;
																Rectangle rectBefore = getClientArea();
																updateItems();
																Rectangle rectAfter = getClientArea();
																if (rectBefore != rectAfter) {
																	notifyListeners(SWT.Resize,
																	new Event());
																}
																redraw();
															}
															...
															void setButtonBounds() {
																...
																maxRect.x = maxRect.y = maxRect.width
																= maxRect.height = 0;
																if (showMax) {
																	maxRect.x = size.x - borderRight
																	- BUTTON_SIZE - 3;
																	if (borderRight > 0)
																		maxRect.x += 1;
																	maxRect.y = onBottom
																...
																if (oldX !is maxRect.x || oldWidth !is maxRect.width
																	|| oldY !is maxRect.y
																	|| oldHeight !is maxRect.height) {
																	int left = Math.min(oldX,
																	maxRect.x);
																	int right = Math.max(oldX + oldWidth,
																	maxRect.x + maxRect.width);
																	int top = onBottom ? size.y - borderBottom
																	- tabHeight : borderTop + 1;
																	redraw(left, top, right - left,
																	tabHeight, false);
																}
																...
																minRect.x = minRect.y = minRect.width
																= minRect.height = 0;
																if (showMin) {
																	minRect.x = size.x - borderRight - maxRect.width
																	- BUTTON_SIZE - 3;
																	if (borderRight > 0)
																		minRect.x += 1;
																	minRect.y = onBottom
																...
																if (oldX !is minRect.x || oldWidth !is minRect.width
																	|| oldY !is minRect.y
																	|| oldHeight !is minRect.height) {
																	int left = Math.min(oldX,
																	minRect.x);
																	int right = Math.max(oldX + oldWidth,
																	minRect.x + minRect.width);
																	int top = onBottom ? size.y - borderBottom
																	- tabHeight : borderTop + 1;
																	redraw(left, top, right - left,
																	tabHeight, false);
																...
																topRightRect.x = topRightRect.y = topRightRect.width
																= topRightRect.height = 0;
																if (topRight !is null) {
																	switch (topRightAlignment) {
																	case SWT.FILL: {
																			int rightEdge = size.x - borderRight - 3
																			- maxRect.width
																			- minRect.width;
																			if (!simple && borderRight > 0 && !showMax
																				&& !showMin)
																				rightEdge -= 2;
																			if (single) {
																				if (items.length is 0
																					|| selectedIndex is -1) {
																					topRightRect.x = borderLeft
																					+ 3;
																					topRightRect.width = rightEdge
																					- topRightRect
																					.x;
																				}
																				else {
																					...
																					CTabItem item = items[
																					selectedIndex];
																					if (item.x + item.width + 7 + 3
																						* BUTTON_SIZE / 2 >= rightEdge)
																						break;
																				...
																				if (items.length is 0) {
																					...
																					CTabItem item = items[items.length
																					- 1];
																					topRightRect.x = item.x
																					+ item.width;
																					if (!simple
																						&& items.length
																						- 1 is selectedIndex)
																						topRightRect.x += curveWidth
																						- curveIndent;
																				}
																			...
																			? size.y - borderBottom - tabHeight
																			: borderTop + 1;
																			topRightRect.height = tabHeight
																			- 1;
																			break;
																		}
																	case SWT.RIGHT: {
																			Point topRightSize = topRight.computeSize(
																			SWT.DEFAULT, tabHeight,
																			false);
																			int rightEdge = size.x - borderRight - 3
																			- maxRect.width
																			- minRect.width;
																...
																if (oldX !is topRightRect.x
																	|| oldWidth !is topRightRect.width
																	|| oldY !is topRightRect.y
																	|| oldHeight !is topRightRect
																	.height) {
																	int left = Math.min(oldX,
																	topRightRect.x);
																	int right = Math.max(oldX + oldWidth,
																...
																chevronRect.x = chevronRect.y = chevronRect.height
																= chevronRect.width = 0;
																if (single) {
																	if (selectedIndex is -1
																		|| items.length > 1) {
																		...
																		* BUTTON_SIZE / 2;
																		...
																		? size.y - borderBottom - tabHeight + (
																		tabHeight - chevronRect.height) / 2
																		: borderTop + (
																		tabHeight
																		- chevronRect.height) / 2;
																		if (selectedIndex is -1) {
																			chevronRect.x = size.x - borderRight - 3
																			- minRect.width
																			- maxRect.width
																			- topRightRect.width
																			- chevronRect.width;
																			...
																			CTabItem item = items[
																			selectedIndex];
																			int w = size.x - borderRight - 3 - minRect
																			.width - maxRect.width
																			- chevronRect.width;
																			if (
																				topRightRect.width
																				> 0)
																				w -= topRightRect.width
																				+ 3;
																			chevronRect.x = Math.min(
																			item.x + item.width + 3,
																			w);
																		}
																		if (borderRight > 0)
																	...
																	if (showChevron) {
																		...
																		* BUTTON_SIZE / 2;
																		...
																		CTabItem lastItem = items[
																		lastIndex];
																		int w = lastItem.x
																		+ lastItem.width + 3;
																		if (!simple
																			&& lastIndex is selectedIndex)
																			w += curveWidth - 2
																			* curveIndent;
																		chevronRect.x = Math.min(w,
																		getRightItemEdge());
																		chevronRect.y = onBottom
																		? size.y - borderBottom - tabHeight + (
																		tabHeight - chevronRect.height) / 2
																...
																if (oldX !is chevronRect.x
																	|| oldWidth !is chevronRect.width
																	|| oldY !is chevronRect.y
																	|| oldHeight !is chevronRect
																	.height) {
																	int left = Math.min(oldX,
																	chevronRect.x);
																	int right = Math.max(oldX + oldWidth,
 ...
 */
															...
															bool setItemLocation() {
																...
																: borderTop;
																if (single) {
																	int defaultX = getDisplay().getBounds()
																	.width + 10; // off screen
																	for (int i = 0;
																		i < items.length;
																		i++) {
																	...
																	for (int i = 0;
																		...
																		priority[i]];
																		width += item.width;
																		item.showing = i is 0 ? true : item.width > 0
																		&& width <= maxWidth;
																		if (!simple
																			&& priority[i] is selectedIndex)
																			width += curveWidth - 2
																			* curveIndent;
																	}
																	int x = 0;
																	int defaultX = getDisplay().getBounds()
																	.width + 10; // off screen
																	firstIndex = cast(int) /*64bit*/ items.length
																	- 1;
																	for (int i = 0;
																		i < items.length;
																		i++) {
																		...
																		if (!item.showing) {
																			...
																			firstIndex = Math.min(firstIndex,
																			i);
																			if (item.x !is x
																				|| item.y !is y)
																				changed = true;
																			...
																			item.y = y;
																			if (i is selectedIndex) {
																				int edge = Math.min(item.x + item.width,
																				rightItemEdge);
																				item.closeRect.x = edge
																				- CTabItem.RIGHT_MARGIN
																				- BUTTON_SIZE;
																			}
																			else {
																				item.closeRect.x = item.x + item.width
																				- CTabItem.RIGHT_MARGIN
																				- BUTTON_SIZE;
																			}
																			item.closeRect.y = onBottom
															...
															bool setItemSize() {
																...
																+ highlight_margin;
																if (onBottom) {
																	yClient = borderTop + highlight_margin
																	+ marginHeight;
																}
																else {
																...
																if (single) {
																	...
																	if (selectedIndex !is -1) {
																		...
																		getRightItemEdge()
																		- borderLeft);
																		if (tab.height !is tabHeight
																			|| tab.width !is width) {
																			changed = true;
																			tab.shortenedText = null;
																			tab.shortenedTextWidth
																			= 0;
																			tab.height = tabHeight;
																			tab.width = width;
																			tab.closeRect.width = tab.closeRect.height
																			= 0;
																			if (showClose
																				|| tab.showClose) {
																				tab.closeRect.width
																				= BUTTON_SIZE;
																				tab.closeRect.height
																				= BUTTON_SIZE;
																			}
																		}
																	}
																...
																int[] widths;
																GC gc = new GC(this);
																int tabAreaWidth = size.x - borderLeft
																- borderRight - 3;
																if (showMin)
																	tabAreaWidth -= BUTTON_SIZE;
																if (showMax)
																	tabAreaWidth -= BUTTON_SIZE;
																if (topRightAlignment is SWT.RIGHT
																	&& topRight !is null) {
																	Point rightSize = topRight.computeSize(SWT.DEFAULT,
																	SWT.DEFAULT, false);
																	tabAreaWidth -= rightSize.x + 3;
																}
																if (!simple)
																	tabAreaWidth -= curveWidth - 2
																	* curveIndent;
																tabAreaWidth = Math.max(0,
																tabAreaWidth);
																...
																if (minWidth > tabAreaWidth) {
																	...
																	if (showChevron)
																		...
																		* BUTTON_SIZE / 2;
																	widths = minWidths;
																	int index = selectedIndex !is -1 ? selectedIndex
																	: 0;
																	if (tabAreaWidth < widths[index]) {
																		widths[index] = Math.max(0,
																		tabAreaWidth);
																	}
																}
																else {
																	...
																	for (int i = 0;
																		i < items.length;
																		i++) {
																		maxWidths[i] = items[i].preferredWidth(gc,
																		i is selectedIndex, false);
																	...
																	if (maxWidth <= tabAreaWidth) {
																		...
																		while (true) {
																			...
																			for (int i = 0;
																				...
																				i++) {
																				if (
																					maxWidths[i] > minWidths[
																					i] + extra) {
																					totalWidth += minWidths[
																					i] + extra;
																					large++;
																				}
																				else {
																					totalWidth += maxWidths[
																					i];
																				}
																			}
																			if (
																				totalWidth >= tabAreaWidth) {
																				extra--;
																				break;
																			}
																			if (large is 0
																				|| tabAreaWidth - totalWidth
																				< large)
																				break;
																			extra++;
																		}
 ...
 * @since 3.0
 */
															public void setMaximizeVisible(
															bool visible) {
																checkWidget();
 ...
 * @since 3.0
 */
															public void setMinimizeVisible(
															bool visible) {
																checkWidget();
 ...
 * @since 3.0
 */
															public void setMinimumCharacters(
															int count) {
																checkWidget();
																if (count < 0)
																	SWT.error(
 ...
 */
															public void setSelection(int index) {
																...
																selectedIndex = index;
																if (oldIndex !is -1) {
																	items[oldIndex].closeImageState
																	= NONE;
																}
																selection.closeImageState = NORMAL;
																selection.showing = false;

																Control newControl = selection
																.control;
																Control oldControl = null;
																if (oldIndex !is -1) {
																	oldControl = items[oldIndex]
																	.control;
																}

																if (newControl !is oldControl) {
																	if (newControl !is null
																		&& !newControl.isDisposed()) {
																		newControl.setBounds(
																		getClientArea());
																		newControl.setVisible(true);
																	}
																	if (oldControl !is null
																		&& !oldControl.isDisposed()) {
																		oldControl.setVisible(
																		false);
																	}
																}
																showItem(selection);
															...
															void setSelection(int index, bool notify) {
																...
																setSelection(index);
																if (notify && selectedIndex !is oldSelectedIndex
																	&& selectedIndex !is -1) {
 ...
 */
															...
															Color color) {
																...
																if (selectionBackground is color)
																	return;
																if (color is null)
																	color = getDisplay().getSystemColor(
 ...
 */
															...
															int[] percents, bool vertical) {
																...
																if (colors !is null) {
																	...
																	if (percents is null
																		|| !((percents.length is colors.length - 1)
																		|| (
																		percents.length is colors.length
																		- 2))) {
																		SWT.error(
																	...
																	if (
																		percents.length is colors.length
																		- 2) {
																		highlightBeginColor = colors[colors.length
																		- 1];
																		colorsLength = cast(int) /*64bit*/ colors
																		.length - 1;
																	}
																	else {
																		colorsLength = cast(int) /*64bit*/ colors
																...
																if (selectionBgImage is null) {
																	if ((selectionGradientColors !is null)
																		...
																		.length is colorsLength)) {
																		bool same = false;
																		for (int i = 0;
																			i < selectionGradientColors
																			.length;
																			i++) {
																			if (
																				selectionGradientColors[
																		...
																		if (same) {
																			for (int i = 0;
																				i < selectionGradientPercents
																				.length;
																				i++) {
																				same = selectionGradientPercents[
																				i] is percents[i];
																				if (!same)
																					break;
																			}
																		...
																		if (same
																			&& this
																			.selectionGradientVertical is vertical)
																			return;
																	}
																}
																else {
																...
																if (colors is null) {
																	...
																	selectionGradientColors = new Color[
																	colorsLength];
																	for (int i = 0;
																		i < colorsLength;
																		++i) {
																		selectionGradientColors[i] = colors[
																		i];
																	}
																	selectionGradientPercents = new int[
																	percents.length];
																	for (int i = 0;
																		i < percents.length;
																		++i) {
																		selectionGradientPercents[i] = percents[
																		i];
																	}
																	selectionGradientVertical = vertical;
																	setSelectionBackground(selectionGradientColors[
																	selectionGradientColors.length
																	- 1]);
																	setSelectionHighlightGradientColor(
																	highlightBeginColor);
																}
 ...
 */
															...
															void setSelectionHighlightGradientColor(
															Color start) {
																...
																if (!isSelectionHighlightColorsCacheHit(
																	start))
 ...
 */
															bool isSelectionHighlightColorsCacheHit(
															Color start) {

																if (
																	selectionHighlightGradientColorsCache is null)
																	return false;

																//this case should never happen but check to be safe before accessing array indexes
																if (
																	selectionHighlightGradientColorsCache.length
																	< 2)
																	return false;

																Color highlightBegin = selectionHighlightGradientColorsCache[
																0];
																...
																if (selectionHighlightGradientColorsCache
																	.length !is tabHeight)
																	return false;

																//Compare existing highlight end to what it would be (selectionBackground)
																if (
																	highlightEnd != selectionBackground)
																	return false;
 ...
 */
															void createSelectionHighlightGradientColors(
															Color start) {
																...
																selectionHighlightGradientColorsCache = new Color[
																fadeGradientSize];
																int denom = fadeGradientSize - 1;

																for (int i = 0; i < fadeGradientSize;
																	i++) {
																	int propFrom = denom - i;
																	int propTo = i;
																	int red = (
																	to.red * propTo + from.red
																	* propFrom) / denom;
																	int green = (
																	to.green * propTo + from.green
																	* propFrom) / denom;
																	int blue = (
																	to.blue * propTo + from.blue
																	* propFrom) / denom;
																	selectionHighlightGradientColorsCache[
																	i] = new Color(getDisplay(), red,
																	green, blue);
																}
															}
															...
															void disposeSelectionHighlightGradientColors() {
																if (
																	selectionHighlightGradientColorsCache is null)
																	return;
																for (int i = 0; i < selectionHighlightGradientColorsCache
																	.length; i++) {
																	selectionHighlightGradientColorsCache[
																	i].dispose();
																}
 ...
 */
															public void setTabPosition(int position) {
																checkWidget();
																if (position !is SWT.TOP
																	&& position !is SWT.BOTTOM) {
																	SWT.error(
																	SWT.ERROR_INVALID_ARGUMENT);
																}
																if (onBottom !is(
																	position is SWT.BOTTOM)) {
																	onBottom = position is SWT
																	.BOTTOM;
																	borderTop = onBottom ? borderLeft
																	: 0;
																	borderBottom = onBottom ? 0
 ...
 */
															...
															int alignment) {
																checkWidget();
																if (alignment !is SWT.RIGHT
																	&& alignment !is SWT.FILL) {
																	SWT.error(
																	SWT.ERROR_INVALID_ARGUMENT);
																}
																if (control !is null
																	&& control.getParent() !is this) {
																	SWT.error(
																	SWT.ERROR_INVALID_ARGUMENT);
																}
																topRight = control;
 ...
 * @since 3.0
 */
															public void setUnselectedCloseVisible(
															bool visible) {
																checkWidget();
 ...
 * @since 3.0
 */
															public void setUnselectedImageVisible(
															bool visible) {
																checkWidget();
 ...
 */
															public void showItem(CTabItem item) {
																...
																if (mru) {
																	...
																	priority.length];
																	System.arraycopy(priority, 0,
																	newPriority, 1, idx);
																	System.arraycopy(priority, idx + 1, newPriority,
																	idx + 1,
															...
															void showList(Rectangle rect) {
																...
																for (int i = 0; i < items.length;
																	...
																	if (tab.showing)
																		continue;
																	MenuItem item = new MenuItem(showMenu,
																	SWT.NONE);
																	...
																	new class SelectionAdapter {
																		override public void widgetSelected(
																		SelectionEvent e) {
																			MenuItem menuItem = cast(
																			MenuItem) e.widget;
																			int index = indexOf(
																			cast(CTabItem) menuItem.getData(
																			id)); this.outer.setSelection(index,
																			true);
																		}
																	}
);
																...
																Point location = getDisplay().map(this,
																null, x, y);
																showMenu.setLocation(location.x,
																location.y);
 ...
 */
															...
															bool updateItems(int showIndex) {
																if (!single && !mru
																	...
																	if (priority[0] < showIndex) {
																		int maxWidth = getRightItemEdge()
																		- borderLeft;
																		if (!simple)
																			maxWidth -= curveWidth - 2
																			* curveIndent;
																		int width = 0;
																		int[] widths = new int[items
																		.length];
																		GC gc = new GC(this);
																		for (int i = priority[0];
																			i <= showIndex;
																			i++) {
															...
															bool updateTabHeight(bool force) {
																int style = getStyle();
																if (fixedTabHeight is 0 && (style & SWT.FLAT) !is 0
																	&& (style & SWT.BORDER) is 0)
																	highlight_header = 0;
																int oldHeight = tabHeight;
																if (fixedTabHeight !is SWT.DEFAULT) {
																	tabHeight = fixedTabHeight is 0 ? 0
																	: fixedTabHeight + 1; // +1 for line drawn across top of tab
																}
																else {
																	int tempHeight = 0;
																	GC gc = new GC(this);
																	if (items.length is 0) {
																		tempHeight = gc.textExtent("Default",
																		CTabItem.FLAGS).y
																		+ CTabItem.TOP_MARGIN
																		+ CTabItem.BOTTOM_MARGIN; //$NON-NLS-1$
																	}
																	else {
																		for (int i = 0;
																			i < items.length;
																...
																if (onBottom) {
																	int d = tabHeight - 12;
																	curve = [0, 13 + d, 0, 12 + d, 2, 12 + d, 3, 11 + d, 5,
																	11 + d, 6, 10 + d, 7, 10 + d, 9, 8 + d, 10, 8 + d,
																	11, 7 + d, 11 + d, 7, 12 + d, 6, 13 + d, 6, 15 + d,
																	4, 16 + d, 4, 17 + d, 3, 19 + d, 3, 20 + d, 2,
																	22 + d, 2, 23 + d, 1];
																	curveWidth = 26 + d;
																	...
																	int d = tabHeight - 12;
																	curve = [0, 0, 0, 1, 2, 1, 3, 2, 5, 2, 6, 3,
																	7, 3, 9, 5, 10, 5, 11, 6, 11 + d, 6 + d, 12 + d,
																	7 + d, 13 + d, 7 + d, 15 + d, 9 + d, 16 + d, 9 + d,
																	17 + d, 10 + d, 19 + d, 10 + d, 20 + d, 11 + d, 22 + d,
																	11 + d, 23 + d, 12 + d];
																	curveWidth = 26 + d;
																	curveIndent = curveWidth / 3;

																	//this could be static but since values depend on curve, better to keep in one place
																	topCurveHighlightStart = [0, 2, 1, 2, 2, 2, 3, 3,
																	4, 3, 5, 3, 6, 4, 7, 4, 8, 5, 9,
																	6, 10, 6];

																	//also, by adding in 'd' here we save some math cost when drawing the curve
																	topCurveHighlightEnd = [10 + d, 6 + d, 11 + d, 7 + d,
																	12 + d, 8 + d, 13 + d, 8 + d, 14 + d, 9 + d, 15 + d,
																	10 + d, 16 + d, 10 + d, 17 + d, 11 + d, 18 + d, 11 + d,
															...
															String _getToolTip(int x, int y) {
																if (showMin
																	...
																	return minimized ? SWT.getMessage("SWT_Restore")
																	: SWT.getMessage(
																	"SWT_Minimize"); //$NON-NLS-1$ //$NON-NLS-2$
																if (showMax
																	&& maxRect.contains(x, y))
																	return maximized ? SWT.getMessage("SWT_Restore")
																	: SWT.getMessage(
																	"SWT_Maximize"); //$NON-NLS-1$ //$NON-NLS-2$
																if (showChevron
																	&& chevronRect.contains(x, y))
																	return SWT.getMessage(
																	"SWT_ShowList"); //$NON-NLS-1$
																CTabItem item = getItem(new Point(x,
																y));
																if (item is null)
																	return null;
																if (!item.showing)
