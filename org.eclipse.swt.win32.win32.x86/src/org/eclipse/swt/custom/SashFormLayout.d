...
class SashFormLayout : Layout {
	protected override Point computeSize(Composite composite, int wHint, int hHint, bool flushCache) {
		...
		if (ratios[maxIndex] > 0) {
			int sashwidth = sashForm.sashes.length > 0
				? sashForm.SASH_WIDTH + sashForm.sashes[0].getBorderWidth() * 2
				: sashForm.SASH_WIDTH;
			if (vertical) {
				height += cast(int)(total * maxValue / ratios[maxIndex]) + (
						cArray.length - 1) * sashwidth;
			}
			else {
				width += cast(int)(total * maxValue / ratios[maxIndex]) + (
						cArray.length - 1) * sashwidth;
			}
		}
	...
	protected override void layout(Composite composite, bool flushCache) {
		...
		if (sashForm.controls.length is 0 && newControls.length is 0)
			return;
		...
		int sashwidth = sashes.length > 0
			? sashForm.SASH_WIDTH + sashes[0].getBorderWidth() * 2 : sashForm.SASH_WIDTH;
