 ...
 */
	...
	void drawClose(GC gc) {
		...
		case CTabFolder.NORMAL: {
				int[] shape = [x, y, x + 2, y, x + 4, y + 2, x + 5, y + 2, x + 7,
					y, x + 9, y, x + 9, y + 2, x + 7, y + 4, x + 7, y + 5, x + 9,
					y + 7, x + 9, y + 9, x + 7, y + 9, x + 5, y + 7, x + 4, y + 7,
		...
		case CTabFolder.SELECTED: {
				int[] shape = [x + 1, y + 1, x + 3, y + 1, x + 5, y + 3, x + 6,
					y + 3, x + 8, y + 1, x + 10, y + 1, x + 10, y + 3, x + 8, y + 5,
					x + 8, y + 6, x + 10, y + 8, x + 10, y + 10, x + 8, y + 10,
					x + 6, y + 8, x + 5, y + 8, x + 3, y + 10, x + 1, y + 10,
		...
		case CTabFolder.NONE: {
				...
				if (parent.gradientColors !is null && !parent.gradientVertical) {
					...
					parent.drawBackground(gc, shape, x, y, 10, 10,
							defaultBackground, image, colors, percents, vertical);
				}
				break;
			}
		default:
		...
		if (parent.single) {
			...
			if (this.parent.onBottom) {
				TryConst!(int)[] left = parent.simple
					? CTabFolder.SIMPLE_BOTTOM_LEFT_CORNER : CTabFolder.BOTTOM_LEFT_CORNER;
				TryConst!(int[]) right = parent.simple
					? CTabFolder.SIMPLE_BOTTOM_RIGHT_CORNER : parent.curve;
				...
				for (int i = 0; i < right.length / 2; i++) {
					...
					shape[index++] = parent.simple
						? y + height + right[2 * i + 1] - 1 : y + right[2 * i + 1] - 2;
				}
				shape[index++] = parent.simple ? rightEdge - 1
					: rightEdge + parent.curveWidth - parent.curveIndent;
				shape[index++] = y - 1;
				shape[index++] = parent.simple ? rightEdge - 1
					: rightEdge + parent.curveWidth - parent.curveIndent;
				shape[index++] = y - 1;
				...
				TryConst!(int)[] left = parent.simple
					? CTabFolder.SIMPLE_TOP_LEFT_CORNER : CTabFolder.TOP_LEFT_CORNER;
				TryConst!(int[]) right = parent.simple
					? CTabFolder.SIMPLE_TOP_RIGHT_CORNER : parent.curve;
				if (parent.borderLeft is 0 && parent.indexOf(this) is parent.firstIndex) {
					left = [x, y];
				}
				shape = new int[left.length + right.length + 8];
			...
			if (tabInPaint) {
				...
				if (parent.selectionGradientColors !is null && !parent.selectionGradientVertical) {
					...
					parent.drawBackground(gc, shape, xx, yy, ww, hh,
							defaultBackground, image, colors, percents, vertical);
				}
			}
			...
			if (parent.selectionBgImage !is null || (parent.selectionGradientColors !is null
					&& parent.selectionGradientColors.length > 1)) {
				inside = null;
			}
			RGB outside = parent.getBackground().getRGB();
			if (parent.bgImage !is null || (parent.gradientColors !is null
		...
		if (textWidth > 0) {
			...
			if (parent.isFocusControl()) {
				...
				if (parent.simple || parent.single) {
					...
					gc.drawLine(xDraw, textY + extent.y + 1, xDraw + extent.x + 1,
							textY + extent.y + 1);
				}
			}
		}
 ...
 */
	void drawHighlight(GC gc, int rightEdge) {
		...
		gc.drawLine(CTabFolder.TOP_LEFT_CORNER_HILITE[0] + x + 1, //rely on fact that first pair is top/right of curve
				1 + y,
				rightEdge - parent.curveIndent, 1 + y);
 ...
 */
	void drawRightUnselectedBorder(GC gc) {
		...
		if (this.parent.onBottom) {
			TryConst!(int[]) right = parent.simple
				? CTabFolder.SIMPLE_UNSELECTED_INNER_CORNER : CTabFolder.BOTTOM_RIGHT_CORNER;
			...
			TryConst!(int[]) right = parent.simple
				? CTabFolder.SIMPLE_UNSELECTED_INNER_CORNER : CTabFolder.TOP_RIGHT_CORNER;
 ...
 */
	void drawLeftUnselectedBorder(GC gc) {
		...
		if (this.parent.onBottom) {
			TryConst!(int[]) left = parent.simple
				? CTabFolder.SIMPLE_UNSELECTED_INNER_CORNER : CTabFolder.BOTTOM_LEFT_CORNER;
			...
			TryConst!(int[]) left = parent.simple
				? CTabFolder.SIMPLE_UNSELECTED_INNER_CORNER : CTabFolder.TOP_LEFT_CORNER;
	...
	void drawUnselected(GC gc) {
		...
		if (image !is null && parent.showUnselectedImage) {
			...
			if (imageBounds.width < maxImageWidth) {
				...
				gc.drawImage(image, imageBounds.x, imageBounds.y, imageBounds.width,
						imageBounds.height, imageX, imageY, imageWidth, imageHeight);
	...
	int preferredWidth(GC gc, bool isSelected, bool minimum) {
		...
		if (minimum) {
			...
			if (text !is null && toString32(text).length > minChars) {
				if (useEllipses()) {
					int end = minChars < cast(int) /*64bit*/ ELLIPSIS.length + 1
						? minChars : minChars - cast(int) /*64bit*/ ELLIPSIS.length;
 ...
 */
	...
	public override void setImage(Image image) {
		...
		if (image is null && oldImage is null)
			return;
		if (image !is null && image == oldImage)
			return;
					...
					if (showing) {
						...
						if (selected || parent.showUnselectedImage) {
							...
							if (bounds.width < maxImageWidth) {
								int imageY = y + (height - bounds.height) / 2 + (parent.onBottom
										? -1 : 1);
