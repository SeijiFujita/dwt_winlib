 ...
 */
				public void setAlwaysShowScrollBars(bool show) {
					checkWidget();
					if (show is alwaysShowScroll)
						return;
					alwaysShowScroll = show;
					ScrollBar hBar = getHorizontalBar();
					if (hBar !is null && alwaysShowScroll)
						hBar.setVisible(true);
					ScrollBar vBar = getVerticalBar();
					if (vBar !is null && alwaysShowScroll)
 ...
 */
				public void setMinSize(int width, int height) {
					checkWidget();
					if (width is minWidth && height is minHeight)
						return;
 ...
 */
				public void showControl(Control control) {
					...
					if (area.height < itemRect.y + itemRect.height)
						origin.y = Math.max(0, origin.y + itemRect.y + itemRect.height - area
								.height);
					setOrigin(origin);
				}
