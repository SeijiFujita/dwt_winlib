 ...
 */
	public this(Composite parent, int style) {
		...
		int[] events = [
			SWT.Dispose, SWT.MouseDown, SWT.MouseExit, SWT.MouseMove,
			SWT.MouseUp, SWT.Paint, SWT.Resize
		];
 ...
 */
	public void setRightMinimumSize(Point size) {
		checkWidget();
		if (size is null || size.x < SWT.DEFAULT || size.y < SWT.DEFAULT)
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
 ...
 */
	...
	void updateCurve(int height) {
		...
		if (simple) {
			curve = [0, h, 1, h, 2, h - 1, 3, h - 2, 3, 2, 4, 1, 5, 0];
		}
		else {
			curve = bezier(0, h + 1, BEZIER_LEFT, h + 1,
					curve_width - BEZIER_RIGHT, 0, curve_width, 0, curve_width);
		}
	}
}
