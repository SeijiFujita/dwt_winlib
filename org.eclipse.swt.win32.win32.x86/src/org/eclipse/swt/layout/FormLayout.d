 ...
 */
	int computeHeight(Control control, FormData data, bool flushCache) {
		...
		if (height.numerator is 0) {
			if (bottom.numerator is 0)
				return bottom.offset;
			if (bottom.numerator is bottom.denominator)
				return -top.offset;
			if (bottom.offset <= 0) {
 ...
 */
	int computeWidth(Control control, FormData data, bool flushCache) {
		...
		if (width.numerator is 0) {
			if (right.numerator is 0)
				return right.offset;
			if (right.numerator is right.denominator)
				return -left.offset;
			if (right.offset <= 0) {
		...
		for (int i = 0; i < children.length; i++) {
			...
			if (width !is SWT.DEFAULT) {
				...
				if (data.height is SWT.DEFAULT && !data.needed) {
					...
					if (flush is null)
						flush = new bool[children.length];
					flush[i] = true;
				}
				w = Math.max(x2, w);
				if (move) {
 ...
 */
	override public String toString() {
		...
		if (spacing !is 0)
			string ~= "spacing=" ~ String_valueOf(spacing) ~ " ";
		string = string.trim();
		string ~= "}";
		return string;
	}
