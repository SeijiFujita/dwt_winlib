 ...
 */
	...
	Point layout(Composite composite, bool move, int x, int y, int width,
			int height, bool flushCache_) {
		if (numColumns < 1) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (data.grabExcessHorizontalSpace && data.minimumWidth > 0) {
				if (data.cacheWidth < data.minimumWidth) {
					...
					data.computeSize(child, Math.max(0,
							data.minimumWidth - trim), data.heightHint, false);
				}
			}
				...
				if (data !is null) {
					...
					if (hSpan is 1) {
						...
						if (!data.grabExcessHorizontalSpace || data.minimumWidth !is 0) {
							w = !data.grabExcessHorizontalSpace || data.minimumWidth is SWT.DEFAULT
								...
								if (hSpan > 1) {
									if (!data.grabExcessHorizontalSpace || data.minimumWidth !is 0) {
										...
										int w = !data.grabExcessHorizontalSpace
											|| data.minimumWidth is SWT.DEFAULT
											? data.cacheWidth : data.minimumWidth;
				...
				for (int i = 0; i < rowCount; i++) {
					...
					if (data !is null) {
						if (data.heightHint is SWT.DEFAULT) {
							...
							if ((currentWidth !is data.cacheWidth
									&& data.horizontalAlignment is SWT.FILL)
								...
								data.cacheWidth = data.cacheHeight = SWT.DEFAULT;
								data.computeSize(child, Math.max(0,
										currentWidth - trim), data.heightHint, false);
								if (data.grabExcessVerticalSpace && data.minimumHeight > 0) {
				...
				if (data !is null) {
					...
					if (vSpan is 1) {
						...
						if (!data.grabExcessVerticalSpace || data.minimumHeight !is 0) {
							h = !data.grabExcessVerticalSpace || data.minimumHeight is SWT.DEFAULT
			...
			for (int j = 0; j < columnCount; j++) {
				...
				if (data !is null) {
					...
					if (vSpan > 1) {
						...
						int h = data.cacheHeight + data.verticalIndent - spanHeight - (
						...
						if (data !is null) {
							...
							if (vSpan > 1) {
								if (!data.grabExcessVerticalSpace || data.minimumHeight !is 0) {
									...
									int h = !data.grabExcessVerticalSpace || data.minimumHeight is SWT.DEFAULT
										? data.cacheHeight : data.minimumHeight;
		...
		totalDefaultWidth += horizontalSpacing * (
				columnCount - 1) + marginLeft + marginWidth * 2 + marginRight;
		totalDefaultHeight += verticalSpacing * (
				rowCount - 1) + marginTop + marginHeight * 2 + marginBottom;
 ...
 */
	override public String toString() {
		...
		if (numColumns !is 1)
			string ~= "numColumns=" ~ String_valueOf(numColumns) ~ " ";
		if (makeColumnsEqualWidth)
			string ~= "makeColumnsEqualWidth=" ~ String_valueOf(makeColumnsEqualWidth) ~ " ";
		if (marginWidth !is 0)
			string ~= "marginWidth=" ~ String_valueOf(marginWidth) ~ " ";
		if (marginHeight !is 0)
			string ~= "marginHeight=" ~ String_valueOf(marginHeight) ~ " ";
		if (marginLeft !is 0)
			string ~= "marginLeft=" ~ String_valueOf(marginLeft) ~ " ";
		if (marginRight !is 0)
			string ~= "marginRight=" ~ String_valueOf(marginRight) ~ " ";
		if (marginTop !is 0)
			string ~= "marginTop=" ~ String_valueOf(marginTop) ~ " ";
		if (marginBottom !is 0)
			string ~= "marginBottom=" ~ String_valueOf(marginBottom) ~ " ";
		if (horizontalSpacing !is 0)
			string ~= "horizontalSpacing=" ~ String_valueOf(horizontalSpacing) ~ " ";
		if (verticalSpacing !is 0)
			string ~= "verticalSpacing=" ~ String_valueOf(verticalSpacing) ~ " ";
		string = string.trim();
		string ~= "}";
		return string;
	}
