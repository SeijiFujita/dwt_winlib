 ...
 */
	...
	override protected Point computeSize(Composite composite, int wHint, int hHint, bool flushCache_) {
		...
		if (type is SWT.HORIZONTAL) {
			extent = layoutHorizontal(composite, false,
					(wHint !is SWT.DEFAULT) && wrap, wHint, flushCache_);
		}
		else {
			extent = layoutVertical(composite, false, (hHint !is SWT.DEFAULT)
					&& wrap, hHint, flushCache_);
		}
