 ...
 * @since 3.0
 */
	public this(int horizontalAlignment, int verticalAlignment, bool grabExcessHorizontalSpace,
			bool grabExcessVerticalSpace, int horizontalSpan, int verticalSpan) {
	...
	void computeSize(Control control, int wHint, int hHint, bool flushCache) {
		...
		if (wHint is this.widthHint && hHint is this.heightHint) {
			if (defaultWidth is -1 || defaultHeight is -1
					|| wHint !is defaultWhint || hHint !is defaultHhint) {
		...
		if (currentWidth is -1 || currentHeight is -1 || wHint !is currentWhint
				|| hHint !is currentHhint) {
			Point size = control.computeSize(wHint, hHint, flushCache);
			currentWhint = wHint;
			currentHhint = hHint;
			currentWidth = size.x;
 ...
 */
	override public String toString() {
		...
		case CENTER:
			...
			break;
		default:
			hAlign = "Undefined " ~ String_valueOf(horizontalAlignment);
			break;
		}
		String vAlign = "";
		...
		case CENTER:
			...
			break;
		default:
			vAlign = "Undefined " ~ String_valueOf(verticalAlignment);
			break;
		}
		String string = getName() ~ " {";
		string ~= "horizontalAlignment=" ~ hAlign ~ " ";
		if (horizontalIndent !is 0)
			string ~= "horizontalIndent=" ~ String_valueOf(horizontalIndent) ~ " ";
		if (horizontalSpan !is 1)
			string ~= "horizontalSpan=" ~ String_valueOf(horizontalSpan) ~ " ";
		if (grabExcessHorizontalSpace)
			string ~= "grabExcessHorizontalSpace=" ~ String_valueOf(grabExcessHorizontalSpace) ~ " ";
		if (widthHint !is SWT.DEFAULT)
			string ~= "widthHint=" ~ String_valueOf(widthHint) ~ " ";
		if (minimumWidth !is 0)
			string ~= "minimumWidth=" ~ String_valueOf(minimumWidth) ~ " ";
		string ~= "verticalAlignment=" ~ vAlign ~ " ";
		if (verticalIndent !is 0)
			string ~= "verticalIndent=" ~ String_valueOf(verticalIndent) ~ " ";
		if (verticalSpan !is 1)
			string ~= "verticalSpan=" ~ String_valueOf(verticalSpan) ~ " ";
		if (grabExcessVerticalSpace)
			string ~= "grabExcessVerticalSpace=" ~ String_valueOf(grabExcessVerticalSpace) ~ " ";
		if (heightHint !is SWT.DEFAULT)
			string ~= "heightHint=" ~ String_valueOf(heightHint) ~ " ";
		if (minimumHeight !is 0)
			string ~= "minimumHeight=" ~ String_valueOf(minimumHeight) ~ " ";
		if (exclude)
			string ~= "exclude=" ~ String_valueOf(exclude) ~ " ";
		string = string.trim();
		string ~= "}";
		return string;
	}
