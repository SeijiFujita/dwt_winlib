...
alias NotificationHookProc = int function(out ULONG_PTR token);
alias NotificationUnhookProc = void function(in ULONG_PTR token);
...
alias GpDrawImageAbort = int function(void*);
alias GpGetThumbnailImageAbort = GpDrawImageAbort;
...
**************************************************************************/
alias GpBitmap = GpImage;

// alias   uint   ARGB;
alias GpFontFamily = Handle;
alias GpImage = Handle;
alias GpBrush = Handle;
alias GpFont = Handle;
alias GpFontCollection = Handle;
alias GpGraphics = Handle;
alias GpGraphicsPath = Handle;
alias GpImageAttributes = Handle;
alias GpHatchBrush = Handle;
alias GpLinearGradientBrush = Handle;
alias GpMatrix = Handle;
alias GpPen = Handle;
alias GpRegion = Handle;
alias GpSolidBrush = Handle;
alias GpStringFormat = Handle;
alias GpTextureBrush = Handle;
alias GpPath = Handle;
...
alias KnownColor = int;
alias Unit = int;

alias GraphicsState = uint;
...
version (STATIC_GDIPLUS) {
	Status GdiplusStartup(ULONG_PTR* token, GdiplusStartupInput* input,
	...
	Status GdipSetClipRectI(Handle graphics, in int x, in int y, in int width,
			in int height, in CombineMode combineMode);
	Status GdipSetClipRect(Handle graphics, in float x, in float y, in float width,
			in float height, in CombineMode combineMode);
	...
	Status GdipTranslateWorldTransform(Handle graphics, in float dx, in float dy,
	...
	Status GdipBeginContainer(Handle graphics, ref GpRectF dstrect,
			ref GpRectF srcrect, in GraphicsUnit unit, out int state);
	Status GdipBeginContainerI(Handle graphics, ref GpRect dstrect,
			ref GpRect srcrect, in GraphicsUnit unit, out int state);
	...
	Status GdipDrawLine(Handle graphics, Handle pen, in float x1, in float y1, in float x2,
			in float y2);
	...
	Status GdipDrawArc(Handle graphics, Handle pen, in float x, in float y,
			in float width, in float height, in float startAngle, in float sweepAngle);
	Status GdipDrawArcI(Handle graphics, Handle pen, in int x, in int y, in int width,
			in int height, in float startAngle, in float sweepAngle);
	Status GdipDrawBezier(Handle graphics, Handle pen, in float x1, in float y1,
			in float x2, in float y2, in float x3, in float y3, in float x4, in float y4);
	Status GdipDrawBeziers(Handle graphics, Handle pen, GpPointF* points, in int count);
	Status GdipDrawBezierI(Handle graphics, Handle pen, in int x1, in int y1,
			in int x2, in int y2, in int x3, in int y3, in int x4, in int y4);
	Status GdipDrawBeziersI(Handle graphics, Handle pen, GpPoint* points, in int count);
	Status GdipDrawRectangle(Handle graphics, Handle pen, in float x, in float y,
			in float width, in float height);
	Status GdipDrawRectangles(Handle graphics, Handle pen, GpRectF* rects, in int count);
	Status GdipDrawRectangleI(Handle graphics, Handle pen, in int x, in int y,
			in int width, in int height);
	Status GdipDrawRectanglesI(Handle graphics, Handle pen, GpRect* rects, in int count);
	Status GdipDrawEllipse(Handle graphics, Handle pen, in float x, in float y,
			in float width, in float height);
	Status GdipDrawEllipseI(Handle graphics, Handle pen, in int x, in int y,
			in int width, in int height);
	Status GdipDrawPie(Handle graphics, Handle pen, in float x, in float y,
			in float width, in float height, in float startAngle, in float sweepAngle);
	Status GdipDrawPieI(Handle graphics, Handle pen, in int x, in int y, in int width,
			in int height, in float startAngle, in float sweepAngle);
	...
	Status GdipDrawCurve2(Handle graphics, Handle pen, GpPointF* points,
			in int count, in float tension);
	Status GdipDrawCurve3(Handle graphics, Handle pen, GpPointF* points,
			in int count, in int offset, in int numberOfSegments, in float tension);
	Status GdipDrawCurveI(Handle graphics, Handle pen, GpPoint* points, in int count);
	Status GdipDrawCurve2I(Handle graphics, Handle pen, GpPoint* points,
			in int count, in float tension);
	Status GdipDrawCurve3I(Handle graphics, Handle pen, GpPoint* points,
			in int count, in int offset, in int numberOfSegments, in float tension);
	Status GdipDrawClosedCurve(Handle graphics, Handle pen, GpPointF* points, in int count);
	Status GdipDrawClosedCurve2(Handle graphics, Handle pen, GpPointF* points,
			in int count, in float tension);
	Status GdipDrawClosedCurveI(Handle graphics, Handle pen, GpPoint* points, in int count);
	Status GdipDrawClosedCurve2I(Handle graphics, Handle pen, GpPoint* points,
			in int count, in float tension);
	Status GdipFillRectangleI(Handle graphics, Handle brush, in int x, in int y,
			in int width, in int height);
	Status GdipFillRectangle(Handle graphics, Handle brush, in float x, in float y,
			in float width, in float height);
	...
	Status GdipFillPolygon(Handle graphics, Handle brush, GpPointF* rects,
			in int count, in FillMode fillMode);
	Status GdipFillPolygonI(Handle graphics, Handle brush, GpPoint* rects,
			in int count, in FillMode fillMode);
	Status GdipFillEllipse(Handle graphics, Handle brush, in float x, in float y,
			in float width, in float height);
	Status GdipFillEllipseI(Handle graphics, Handle brush, in int x, in int y,
			in int width, in int height);
	Status GdipFillPie(Handle graphics, Handle brush, in float x, in float y,
			in float width, in float height, in float startAngle, in float sweepAngle);
	Status GdipFillPieI(Handle graphics, Handle brush, in int x, in int y,
			in int width, in int height, in float startAngle, in float sweepAngle);
	...
	Status GdipFillClosedCurve2(Handle graphics, Handle brush, GpPointF* points,
			in int count, in FillMode fillMode, in float tension);
	Status GdipFillClosedCurve2I(Handle graphics, Handle brush, GpPoint* points,
			in int count, in FillMode fillMode, in float tension);
	Status GdipFillRegion(Handle graphics, Handle brush, Handle region);
	Status GdipDrawString(Handle graphics, LPCWSTR string, in int length,
			Handle font, ref GpRectF layoutRect, Handle stringFormat, Handle brush);
	Status GdipMeasureString(Handle graphics, LPCWSTR string, in int length, Handle font, ref GpRectF layoutRect,
			Handle stringFormat, ref GpRectF boundingBox, int* codepointsFitted, int* linesFitted);
	...
	Status GdipMeasureCharacterRanges(Handle graphics, LPCWSTR string, in int length, Handle font,
			ref GpRectF layoutRect, Handle stringFormat, in int regionCount, Handle* regions);
	...
	Status GdipDrawImageRect(Handle graphics, Handle image, in float x, in float y,
			in float width, in float height);
	Status GdipDrawImageRectI(Handle graphics, Handle image, in int x, in int y,
			in int width, in int height);
	Status GdipDrawImagePointRect(Handle graphics, Handle image, in float x,
			in float y, in float srcx, in float srcy, in float srcwidth,
			in float srcheight, in GraphicsUnit srcUnit);
	Status GdipDrawImagePointRectI(Handle graphics, Handle image, in int x, in int y,
			in int srcx, in int srcy, in int srcwidth, in int srcheight, in GraphicsUnit srcUnit);
	Status GdipDrawImageRectRect(Handle graphics, Handle image, in float dstx,
			in float dsty, in float dstwidth, in float dstheight, in float srcx, in float srcy, in float srcwidth,
			in float srcheight, in GraphicsUnit srcUnit, Handle imageAttributes,
			GpDrawImageAbort callback, void* callbakcData);
	Status GdipDrawImageRectRectI(Handle graphics, Handle image, in int dstx,
			in int dsty, in int dstwidth, in int dstheight, in int srcx, in int srcy, in int srcwidth,
			in int srcheight, in GraphicsUnit srcUnit, Handle imageAttributes,
			GpDrawImageAbort callback, void* callbakcData);
	...
	Status GdipDrawImagePointsRect(Handle graphics, Handle image, GpPointF* dstpoints,
			in int count, in float srcx, in float srcy, in float srcwidth, in float srcheight, in GraphicsUnit srcUnit,
			Handle imageAttributes, GpDrawImageAbort callback, void* callbakcData);
	Status GdipDrawImagePointsRectI(Handle graphics, Handle image, GpPoint* dstpoints,
			in int count, in int srcx, in int srcy, in int srcwidth, in int srcheight, in GraphicsUnit srcUnit,
			Handle imageAttributes, GpDrawImageAbort callback, void* callbakcData);
	...
	Status GdipIsVisibleRect(Handle graphics, in float x, in float y, in float width,
			in float height, out int result);
	Status GdipIsVisibleRectI(Handle graphics, in int x, in int y, in int width,
			in int height, out int result);
	...
	Status GdipTransformPoints(Handle graphics, in CoordinateSpace destSpace,
			in CoordinateSpace srcSpace, GpPointF* points, in int count);
	Status GdipTransformPointsI(Handle graphics, in CoordinateSpace destSpace,
			in CoordinateSpace srcSpace, GpPoint* points, in int count);

	Status GdipCreateMatrix(out Handle matrix);
	Status GdipCreateMatrix2(in float m11, in float m12, in float m21, in float m22,
			in float dx, in float dy, out Handle matrix);
	...
	Status GdipSetMatrixElements(Handle matrix, in float m11, in float m12,
			in float m21, in float m22, in float xy, in float dy);
	...
	Status GdipTranslateMatrix(Handle matrix, in float offsetX, in float offsetY,
			in MatrixOrder order);
	...
	Status GdipCreateTexture2(Handle image, in WrapMode wrapMode, in float x,
			in float y, in float width, in float height, out Handle texture);
	Status GdipCreateTexture2I(Handle image, in WrapMode wrapMode, in int x, in int y,
			in int width, in int height, out Handle texture);
	...
	Status GdipCreateHatchBrush(in HatchStyle hatchstyle, in int forecol,
			in int backcol, out Handle brush);
	...
	Status GdipCreateLineBrushI(ref GpPoint point1, ref GpPoint point2,
			in int color1, in int color2, in WrapMode wrapMode, out Handle lineGradient);
	Status GdipCreateLineBrush(ref GpPointF point1, ref GpPointF point2,
			in int color1, in int color2, in WrapMode wrapMode, out Handle lineGradient);
	Status GdipCreateLineBrushFromRectI(ref GpRect rect, in int color1, in int color2,
			in LinearGradientMode mode, in WrapMode wrapMode, out Handle lineGradient);
	Status GdipCreateLineBrushFromRect(ref GpRectF rect, in int color1, in int color2,
			in LinearGradientMode mode, in WrapMode wrapMode, out Handle lineGradient);
	Status GdipCreateLineBrushFromRectWithAngleI(ref GpRect rect, in int color1, in int color2,
			in float angle, in int isAngleScalable, in WrapMode wrapMode, out Handle lineGradient);
	Status GdipCreateLineBrushFromRectWithAngle(ref GpRectF rect, in int color1, in int color2,
			in float angle, in int isAngleScalable, in WrapMode wrapMode, out Handle lineGradient);
	...
	Status GdipSetPenLineCap197819(Handle pen, in LineCap startCap,
			in LineCap endCap, in DashCap dashCap);
	...
	Status GdipIsVisibleRegionPoint(Handle region, in float x, in float y,
			Handle graphics, out int result);
	Status GdipIsVisibleRegionRect(Handle region, in float x, in float y,
			in float width, in float height, Handle graphics, out int result);
	Status GdipIsVisibleRegionPointI(Handle region, in int x, in int y,
			Handle graphics, out int result);
	Status GdipIsVisibleRegionRectI(Handle region, in int x, in int y, in int width,
			in int height, Handle graphics, out int result);
	...
	Status GdipGetImageThumbnail(Handle image, in int thumbWidth, in int thumbHeight,
			out Handle thumbImage, GpGetThumbnailImageAbort callback, void* callbackData);
	...
	Status GdipGetPropertyItem(Handle image, in int propId, in uint propSize,
	...
	Status GdipGetAllPropertyItems(Handle image, in uint totalBufferSize,
			in int numProperties, GpPropertyItem* allItems);
	...
	Status GdipCreateBitmapFromScan0(in int width, in int height, in int stride,
			in PixelFormat format, ubyte* scan0, out Handle bitmap);
	...
	Status GdipCreateBitmapFromGraphics(in int width, in int height,
			Handle graphics, out Handle bitmap);
	Status GdipCloneBitmapArea(in float x, in float y, in float width, in float height,
			in PixelFormat format, Handle srcbitmap, out Handle dstbitmap);
	Status GdipCloneBitmapAreaI(in int x, in int y, in int width, in int height,
			in PixelFormat format, Handle srcbitmap, out Handle dstbitmap);
	...
	Status GdipBitmapLockBits(Handle bitmap, GpRect* rect, in ImageLockMode flags,
			in PixelFormat format, GpBitmapData* lockedBitmapData);
	...
	Status GdipSetImageAttributesColorMatrix(Handle imageattr, in ColorAdjustType type, in int enableFlag,
			in GpColorMatrix* colorMatrix, in GpColorMatrix* grayMatrix, in ColorMatrixFlag flags);
	Status GdipSetImageAttributesThreshold(Handle imageattr,
			in ColorAdjustType type, in int enableFlag, in float threshold);
	Status GdipSetImageAttributesGamma(Handle imageattr, in ColorAdjustType type,
			in int enableFlag, in float gamma);
	Status GdipSetImageAttributesNoOp(Handle imageattr, in ColorAdjustType type, in int enableFlag);
	Status GdipSetImageAttributesColorKeys(Handle imageattr,
			in ColorAdjustType type, in int enableFlag, in int colorLow, in int colorHigh);
	Status GdipSetImageAttributesOutputChannel(Handle imageattr,
			in ColorAdjustType type, in int enableFlag, in ColorChannelFlag flags);
	Status GdipSetImageAttributesOutputChannelColorProfile(Handle imageattr,
			in ColorAdjustType type, in int enableFlag, LPCWSTR colorProfileFilename);
	Status GdipSetImageAttributesWrapMode(Handle imageattr, in WrapMode wrap, in int argb,
			in int clamp);
	...
	Status GdipGetFontCollectionFamilyList(Handle fontCollection,
			in int numSought, Handle* gpfamilies, out int numFound);
	...
	Status GdipCreateFont(Handle fontFamily, in float emSize, in int style,
			in int unit, out Handle font);
	...
	alias GdipGetLogFont = GdipGetLogFontW;

	Status GdipCreateStringFormat(in StringFormatFlags formatAttributes,
			in int language, out Handle format);
	...
	Status GdipAddPathArc(Handle path, in float x, in float y, in float width,
			in float height, in float startAngle, in float sweepAngle);
	Status GdipAddPathArcI(Handle path, in int x, in int y, in int width, in int height,
			in float startAngle, in float sweepAngle);
	Status GdipAddPathBezier(Handle path, in float x1, in float y1, in float x2,
			in float y2, in float x3, in float y3, in float x4, in float y4);
	Status GdipAddPathBezierI(Handle path, in int x1, in int y1, in int x2, in int y2,
	...
	Status GdipAddPathCurve3(Handle path, GpPointF* points, in int count,
			in int offset, in int numberOfSegments, in float tension);
	Status GdipAddPathCurve3I(Handle path, GpPoint* points, in int count,
			in int offset, in int numberOfSegments, in float tension);
	...
	Status GdipAddPathPie(Handle path, in float x, in float y, in float width,
			in float height, in float startAngle, in float sweepAngle);
	Status GdipAddPathPieI(Handle path, in int x, in int y, in int width, in int height,
			in float startAngle, in float sweepAngle);
	...
	Status GdipAddPathString(Handle path, LPCWSTR string, in int length, Handle family,
			in FontStyle style, in float emSize, ref GpRectF layoutRect, Handle format);
	Status GdipAddPathStringI(Handle path, LPCWSTR string, in int length, Handle family,
			in FontStyle style, in float emSize, ref GpRect layoutRect, Handle format);
	...
	Status GdipWarpPath(Handle path, Handle matrix, GpPointF* points, in int count,
			in float srcx, in float srcy, in float srcwidth, in float srcwidth,
			in WarpMode warpMode, in float flatness);
	...
	Status GdipIsVisiblePathPoint(Handle path, in float x, in float y, Handle graphics,
			out int result);
	Status GdipIsVisiblePathPointI(Handle path, in int x, in int y, Handle graphics, out int result);
	Status GdipIsOutlineVisiblePathPoint(Handle path, in float x, in float y,
			Handle pen, Handle graphics, out int result);
	Status GdipIsOutlineVisiblePathPointI(Handle path, in int x, in int y,
			Handle pen, Handle graphics, out int result);
	...
	Status GdipPathIterNextSubpath(Handle iterator, out int resultCount,
			out int startIndex, out int endIndex, out int isClosed);
	Status GdipPathIterNextSubpathPath(Handle iterator, out int resultCount,
			Handle path, out int isClosed);
	Status GdipPathIterNextPathType(Handle iterator, out int resultCount,
			out ubyte pathType, out int startIndex, out int endIndex);
	Status GdipPathIterNextMarker(Handle iterator, out int resultCount,
	...
	Status GdipPathIterEnumerate(Handle iterator, out int resultCount,
			GpPointF* points, ubyte* types, in int count);
	Status GdipPathIterCopyData(Handle iterator, out int resultCount,
			GpPointF* points, ubyte* types, in int startIndex, in int endIndex);

	Status GdipCreatePathGradient(GpPointF* points, in int count,
			in WrapMode wrapMode, out Handle polyGradient);
	Status GdipCreatePathGradientI(GpPoint* points, in int count,
			in WrapMode wrapMode, out Handle polyGradient);
	...
	Status GdipSetPathGradientPresetBlend(Handle brush, in int* blend,
			in float* positions, in int count);
	...
	Status GdipTranslatePathGradientTransform(Handle brush, in float dx,
			in float dy, in MatrixOrder order);
	Status GdipScalePathGradientTransform(Handle brush, in float sx, in float sy,
			in MatrixOrder order);
	...
	Status GdipSetStringFormatTabStops(Handle format, in float firstTabOffset,
			in int count, in float* tabStops);
	...
	Status function(Handle graphics, in int x, in int y, in int width, in int height,
			in CombineMode combineMode) GdipSetClipRectI;
	Status function(Handle graphics, in float x, in float y, in float width,
			in float height, in CombineMode combineMode) GdipSetClipRect;
	...
	Status function(Handle graphics, ref GpRectF dstrect, ref GpRectF srcrect,
			in GraphicsUnit unit, out int state) GdipBeginContainer;
	Status function(Handle graphics, ref GpRect dstrect, ref GpRect srcrect,
			in GraphicsUnit unit, out int state) GdipBeginContainerI;
	...
	Status function(Handle graphics, Handle pen, in float x, in float y,
			in float width, in float height, in float startAngle, in float sweepAngle) GdipDrawArc;
	Status function(Handle graphics, Handle pen, in int x, in int y, in int width,
			in int height, in float startAngle, in float sweepAngle) GdipDrawArcI;
	Status function(Handle graphics, Handle pen, in float x1, in float y1, in float x2,
			in float y2, in float x3, in float y3, in float x4, in float y4) GdipDrawBezier;
	Status function(Handle graphics, Handle pen, GpPointF* points, in int count) GdipDrawBeziers;
	Status function(Handle graphics, Handle pen, in int x1, in int y1, in int x2,
			in int y2, in int x3, in int y3, in int x4, in int y4) GdipDrawBezierI;
	Status function(Handle graphics, Handle pen, GpPoint* points, in int count) GdipDrawBeziersI;
	Status function(Handle graphics, Handle pen, in float x, in float y,
			in float width, in float height) GdipDrawRectangle;
	...
	Status function(Handle graphics, Handle pen, in float x, in float y,
			in float width, in float height) GdipDrawEllipse;
	Status function(Handle graphics, Handle pen, in int x, in int y, in int width, in int height) GdipDrawEllipseI;
	Status function(Handle graphics, Handle pen, in float x, in float y,
			in float width, in float height, in float startAngle, in float sweepAngle) GdipDrawPie;
	Status function(Handle graphics, Handle pen, in int x, in int y, in int width,
			in int height, in float startAngle, in float sweepAngle) GdipDrawPieI;
	...
	Status function(Handle graphics, Handle pen, GpPointF* points, in int count,
			in int offset, in int numberOfSegments, in float tension) GdipDrawCurve3;
	...
	Status function(Handle graphics, Handle pen, GpPoint* points, in int count,
			in int offset, in int numberOfSegments, in float tension) GdipDrawCurve3I;
	...
	Status function(Handle graphics, Handle brush, in float x, in float y,
			in float width, in float height) GdipFillRectangle;
	...
	Status function(Handle graphics, Handle brush, GpPointF* rects,
			in int count, in FillMode fillMode) GdipFillPolygon;
	Status function(Handle graphics, Handle brush, GpPoint* rects, in int count, in FillMode fillMode) GdipFillPolygonI;
	Status function(Handle graphics, Handle brush, in float x, in float y,
			in float width, in float height) GdipFillEllipse;
	Status function(Handle graphics, Handle brush, in int x, in int y, in int width, in int height) GdipFillEllipseI;
	Status function(Handle graphics, Handle brush, in float x, in float y,
			in float width, in float height, in float startAngle, in float sweepAngle) GdipFillPie;
	Status function(Handle graphics, Handle brush, in int x, in int y, in int width,
			in int height, in float startAngle, in float sweepAngle) GdipFillPieI;
	...
	Status function(Handle graphics, Handle brush, GpPointF* points, in int count,
			in FillMode fillMode, in float tension) GdipFillClosedCurve2;
	Status function(Handle graphics, Handle brush, GpPoint* points, in int count,
			in FillMode fillMode, in float tension) GdipFillClosedCurve2I;
	Status function(Handle graphics, Handle brush, Handle region) GdipFillRegion;
	Status function(Handle graphics, LPCWSTR string, in int length, Handle font,
			ref GpRectF layoutRect, Handle stringFormat, Handle brush) GdipDrawString;
	Status function(Handle graphics, LPCWSTR string, in int length, Handle font, ref GpRectF layoutRect,
			Handle stringFormat, ref GpRectF boundingBox, int* codepointsFitted, int* linesFitted) GdipMeasureString;
	...
	Status function(Handle graphics, LPCWSTR string, in int length, Handle font,
			ref GpRectF layoutRect, Handle stringFormat, in int regionCount, Handle* regions) GdipMeasureCharacterRanges;
	...
	Status function(Handle graphics, Handle image, in float x, in float y,
			in float width, in float height) GdipDrawImageRect;
	Status function(Handle graphics, Handle image, in int x, in int y, in int width, in int height) GdipDrawImageRectI;
	Status function(Handle graphics, Handle image, in float x, in float y,
			in float srcx, in float srcy, in float srcwidth, in float srcheight,
			in GraphicsUnit srcUnit) GdipDrawImagePointRect;
	Status function(Handle graphics, Handle image, in int x, in int y, in int srcx,
			in int srcy, in int srcwidth, in int srcheight, in GraphicsUnit srcUnit) GdipDrawImagePointRectI;
	Status function(Handle graphics, Handle image, in float dstx, in float dsty,
			in float dstwidth, in float dstheight, in float srcx, in float srcy, in float srcwidth, in float srcheight,
			in GraphicsUnit srcUnit, Handle imageAttributes,
			GpDrawImageAbort callback, void* callbakcData) GdipDrawImageRectRect;
	Status function(Handle graphics, Handle image, in int dstx, in int dsty,
			in int dstwidth, in int dstheight, in int srcx, in int srcy, in int srcwidth, in int srcheight,
			in GraphicsUnit srcUnit, Handle imageAttributes,
			GpDrawImageAbort callback, void* callbakcData) GdipDrawImageRectRectI;
	...
	Status function(Handle graphics, Handle image, GpPointF* dstpoints, in int count,
			in float srcx, in float srcy, in float srcwidth, in float srcheight, in GraphicsUnit srcUnit,
			Handle imageAttributes, GpDrawImageAbort callback, void* callbakcData) GdipDrawImagePointsRect;
	Status function(Handle graphics, Handle image, GpPoint* dstpoints,
			in int count, in int srcx, in int srcy, in int srcwidth, in int srcheight, in GraphicsUnit srcUnit,
			Handle imageAttributes, GpDrawImageAbort callback, void* callbakcData) GdipDrawImagePointsRectI;
	...
	Status function(Handle graphics, in float x, in float y, in float width,
			in float height, out int result) GdipIsVisibleRect;
	...
	Status function(Handle graphics, in CoordinateSpace destSpace,
			in CoordinateSpace srcSpace, GpPointF* points, in int count) GdipTransformPoints;
	Status function(Handle graphics, in CoordinateSpace destSpace,
			in CoordinateSpace srcSpace, GpPoint* points, in int count) GdipTransformPointsI;
	Status function(out Handle matrix) GdipCreateMatrix;
	Status function(in float m11, in float m12, in float m21, in float m22, in float dx,
			in float dy, out Handle matrix) GdipCreateMatrix2;
	...
	Status function(Handle matrix, in float m11, in float m12, in float m21,
			in float m22, in float xy, in float dy) GdipSetMatrixElements;
	...
	Status function(Handle image, in WrapMode wrapMode, in float x, in float y,
			in float width, in float height, out Handle texture) GdipCreateTexture2;
	Status function(Handle image, in WrapMode wrapMode, in int x, in int y,
			in int width, in int height, out Handle texture) GdipCreateTexture2I;
	...
	Status function(ref GpPoint point1, ref GpPoint point2, in int color1,
			in int color2, in WrapMode wrapMode, out Handle lineGradient) GdipCreateLineBrushI;
	Status function(ref GpPointF point1, ref GpPointF point2, in int color1,
			in int color2, in WrapMode wrapMode, out Handle lineGradient) GdipCreateLineBrush;
	Status function(ref GpRect rect, in int color1, in int color2,
			in LinearGradientMode mode, in WrapMode wrapMode, out Handle lineGradient) GdipCreateLineBrushFromRectI;
	Status function(ref GpRectF rect, in int color1, in int color2,
			in LinearGradientMode mode, in WrapMode wrapMode, out Handle lineGradient) GdipCreateLineBrushFromRect;
	Status function(ref GpRect rect, in int color1, in int color2, in float angle,
			in int isAngleScalable, in WrapMode wrapMode, out Handle lineGradient) GdipCreateLineBrushFromRectWithAngleI;
	Status function(ref GpRectF rect, in int color1, in int color2, in float angle,
			in int isAngleScalable, in WrapMode wrapMode, out Handle lineGradient) GdipCreateLineBrushFromRectWithAngle;
	...
	Status function(Handle region, in float x, in float y, in float width,
			in float height, Handle graphics, out int result) GdipIsVisibleRegionRect;
	Status function(Handle region, in int x, in int y, Handle graphics, out int result) GdipIsVisibleRegionPointI;
	Status function(Handle region, in int x, in int y, in int width, in int height,
			Handle graphics, out int result) GdipIsVisibleRegionRectI;
	...
	Status function(Handle image, in int thumbWidth, in int thumbHeight,
			out Handle thumbImage, GpGetThumbnailImageAbort callback, void* callbackData) GdipGetImageThumbnail;
	...
	Status function(Handle image, in uint totalBufferSize, in int numProperties,
			GpPropertyItem* allItems) GdipGetAllPropertyItems;
	...
	Status function(in int width, in int height, in int stride, in PixelFormat format,
			ubyte* scan0, out Handle bitmap) GdipCreateBitmapFromScan0;
	...
	Status function(in float x, in float y, in float width, in float height,
			in PixelFormat format, Handle srcbitmap, out Handle dstbitmap) GdipCloneBitmapArea;
	Status function(in int x, in int y, in int width, in int height,
			in PixelFormat format, Handle srcbitmap, out Handle dstbitmap) GdipCloneBitmapAreaI;
	...
	Status function(Handle bitmap, GpRect* rect, in ImageLockMode flags,
			in PixelFormat format, GpBitmapData* lockedBitmapData) GdipBitmapLockBits;
	...
	Status function(Handle imageattr, in ColorAdjustType type, in int enableFlag,
			in GpColorMatrix* colorMatrix, in GpColorMatrix* grayMatrix, in ColorMatrixFlag flags) GdipSetImageAttributesColorMatrix;
	...
	Status function(Handle imageattr, in ColorAdjustType type, in int enableFlag,
			in int colorLow, in int colorHigh) GdipSetImageAttributesColorKeys;
	Status function(Handle imageattr, in ColorAdjustType type, in int enableFlag,
			in ColorChannelFlag flags) GdipSetImageAttributesOutputChannel;
	Status function(Handle imageattr, in ColorAdjustType type, in int enableFlag,
			LPCWSTR colorProfileFilename) GdipSetImageAttributesOutputChannelColorProfile;
	...
	Status function(Handle path, in float x, in float y, in float width,
			in float height, in float startAngle, in float sweepAngle) GdipAddPathArc;
	Status function(Handle path, in int x, in int y, in int width, in int height,
			in float startAngle, in float sweepAngle) GdipAddPathArcI;
	Status function(Handle path, in float x1, in float y1, in float x2, in float y2,
			in float x3, in float y3, in float x4, in float y4) GdipAddPathBezier;
	Status function(Handle path, in int x1, in int y1, in int x2, in int y2, in int x3,
			in int y3, in int x4, in int y4) GdipAddPathBezierI;
	...
	Status function(Handle path, GpPointF* points, in int count, in int offset,
			in int numberOfSegments, in float tension) GdipAddPathCurve3;
	Status function(Handle path, GpPoint* points, in int count, in int offset,
			in int numberOfSegments, in float tension) GdipAddPathCurve3I;
	...
	Status function(Handle path, in float x, in float y, in float width,
			in float height, in float startAngle, in float sweepAngle) GdipAddPathPie;
	Status function(Handle path, in int x, in int y, in int width, in int height,
			in float startAngle, in float sweepAngle) GdipAddPathPieI;
	...
	Status function(Handle path, LPCWSTR string, in int length, Handle family,
			in FontStyle style, in float emSize, ref GpRectF layoutRect, Handle format) GdipAddPathString;
	Status function(Handle path, LPCWSTR string, in int length, Handle family,
			in FontStyle style, in float emSize, ref GpRect layoutRect, Handle format) GdipAddPathStringI;
	...
	Status function(Handle path, Handle matrix, GpPointF* points, in int count,
			in float srcx, in float srcy, in float srcwidth, in float srcwidth,
			in WarpMode warpMode, in float flatness) GdipWarpPath;
	...
	Status function(Handle iterator, out int resultCount, out int startIndex,
			out int endIndex, out int isClosed) GdipPathIterNextSubpath;
	Status function(Handle iterator, out int resultCount, Handle path, out int isClosed) GdipPathIterNextSubpathPath;
	Status function(Handle iterator, out int resultCount, out ubyte pathType,
			out int startIndex, out int endIndex) GdipPathIterNextPathType;
	...
	Status function(Handle iterator, out int resultCount, GpPointF* points,
			ubyte* types, in int count) GdipPathIterEnumerate;
	Status function(Handle iterator, out int resultCount, GpPointF* points,
			ubyte* types, in int startIndex, in int endIndex) GdipPathIterCopyData;
