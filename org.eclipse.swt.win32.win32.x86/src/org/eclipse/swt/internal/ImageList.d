    ...
    */
		...
		if (bm.bmBitsPixel is 32 && OS.COMCTL32_MAJOR >= 6 && OS.IsAppThemed()) {
			...
			*cast(BITMAPINFOHEADER*) bmi.ptr = bmiHeader;
			...
			void* pBits;
			hBitmap = OS.CreateDIBSection(null, cast(BITMAPINFO*) bmi.ptr,
		...
		if (width !is bm.bmWidth || height !is bm.bmHeight) {
			...
			OS.StretchBlt(hdc2, 0, 0, width, height, hdc1, 0, 0, bm.bmWidth,
					bm.bmHeight, OS.SRCCOPY);
		}
		else {
	...
	HBITMAP copyWithAlpha(HBITMAP hBitmap, int background, byte[] alphaData,
		...
		*cast(BITMAPINFOHEADER*) bmi.ptr = bmiHeader;
		...
		if (alphaData !is null) {
			...
			for (int y = 0; y < srcHeight; ++y) {
				for (int x = 0; x < srcWidth; ++x) {
					srcData[sp] = (srcData[sp - 1] is transRed
							&& srcData[sp - 2] is transGreen && srcData[sp - 3] is transBlue) ? 0
						: cast(byte) 255;
					sp += 4;
				}
				sp += spinc;
		...
		if (srcWidth !is destWidth || srcHeight !is destHeight) {
			...
			*cast(BITMAPINFOHEADER*) bmi2.ptr = bmiHeader2;
			...
			void* pBits2;
			auto memDib2 = OS.CreateDIBSection(null, cast(BITMAPINFO*) bmi2.ptr,
					OS.DIB_RGB_COLORS, &pBits2, null, 0);
			...
			OS.StretchBlt(memHdc2, 0, 0, destWidth, destHeight, memHdc, 0, 0,
	...
	HBITMAP createMask(HBITMAP hBitmap, int destWidth, int destHeight,
			int background, int transparentPixel) {
		BITMAP bm;
		OS.GetObject(hBitmap, BITMAP.sizeof, &bm);
		int srcWidth = bm.bmWidth;
		int srcHeight = bm.bmHeight;
        ...
        */
			...
			if (originalColors !is null)
				OS.SetDIBColorTable(hdc1, 0, 1 << bm.bmBitsPixel,
		...
		for (int i = 0; i < count; i++) {
			if (images[i]!is null) {
				if (images[i].isDisposed())
					images[i] = null;
				if (images[i]!is null && images[i] == /*eq*/ image)
					return i;
			}
		}
		return -1;
		...
		case SWT.BITMAP: {
            ...
            */
				...
				default:
					hBitmap = copyBitmap(hImage, cx, cy);
					if (index !is count)
						hMask = createMask(hImage, cx, cy, -1, -1);
					break;
				}
				if (index is count) {
