...
extern (Windows) {
	...
	WINOLEAPI CreateStreamOnHGlobal(HGLOBAL hGlobal, //Memory handle for the stream object
	...
	WINOLEAPI OleCreate(REFCLSID rclsid, //CLSID of embedded object to be created
	...
	WINOLEAPI OleCreateFromFile(REFCLSID rclsid, //Reserved. Must be CLSID_NULL
	...
	STDAPI OleCreatePropertyFrame(HWND hwndOwner, //Parent window of property sheet dialog box
			...
			LPUNKNOWN* lplpUnk,//Pointer to the objects for property sheet
	...
	WINOLEAPI OleDraw(LPUNKNOWN pUnk, //Pointer to the view object to be drawn
	...
	WINOLEAPI OleGetClipboard(LPDATAOBJECT* ppDataObj //Address of output variable that
			// receives the IDataObject interface
			// pointer
	);
	WINOLEAPI OleIsCurrentClipboard(LPDATAOBJECT pDataObject //Pointer to the data object previously
			// copied or cut
	);
	BOOL OleIsRunning(LPOLEOBJECT pObject //Pointer to the interface
	);
	WINOLEAPI OleLoad(LPSTORAGE pStg, //Pointer to the storage object from which to
			// load
	...
	WINOLEAPI OleRun(LPUNKNOWN pUnknown //Pointer to interface on the object
	);
	WINOLEAPI OleSave(LPPERSISTSTORAGE pPS, //Pointer to the object to be saved
			LPSTORAGE pStg, //Pointer to the destination storage to
			// which pPS is saved
			BOOL fSameAsLoad //Whether the object was loaded from pstg
			// or not
			);
	WINOLEAPI OleSetClipboard(LPDATAOBJECT pDataObj //Pointer to the data object being copied
			// or cut
	);
	WINOLEAPI OleSetContainedObject(LPUNKNOWN pUnk, //Pointer to the interface on the embedded object
			BOOL fContained //Indicates if the object is embedded
			);
	WINOLEAPI OleSetMenuDescriptor(HOLEMENU holemenu, //Handle to the composite menu descriptor
	...
	STDAPI OleTranslateColor(OLE_COLOR clr, //Color to be converted into a COLORREF
	...
	WINOLEAPI RegisterDragDrop(HWND hwnd, //Handle to a window that can accept drops
			LPDROPTARGET pDropTarget//Pointer to object that is to be target of drop
			);
	void ReleaseStgMedium(STGMEDIUM* pmedium //Pointer to storage medium to be freed
	);
	WINOLEAPI RevokeDragDrop(HWND hwnd //Handle to a window that can accept drops
	);
	HRESULT SHDoDragDrop(HWND hwnd, IDataObject* pdtobj, IDropSource* pdsrc,
			DWORD dwEffect, DWORD* pdwEffect);
	...
	HRESULT StgOpenStorage(LPCWSTR pwcsName, LPSTORAGE pstgPriority,
			DWORD grfMode, SNB snbExclude, DWORD reserved, LPSTORAGE* ppstgOpen);
	...
	STDAPI CreateStdAccessibleObject(HWND hwnd, LONG idObject,
			REFCIID riidInterface, void** ppvObject);
