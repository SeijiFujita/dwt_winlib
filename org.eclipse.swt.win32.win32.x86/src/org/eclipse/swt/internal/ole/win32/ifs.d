...
interface IClassFactory2 : IClassFactory {
	...
	HRESULT CreateInstanceLic(LPUNKNOWN pUnkOuter, LPUNKNOWN pUnkReserved,
			REFCIID riid, BSTR bstrKey, void** ppvObject);
}
...
interface IInternetSecurityManager : IUnknown {
	...
	HRESULT GetSecurityId(LPCWSTR pwszUrl, BYTE* pbSecurityId,
			DWORD* pcbSecurityId, DWORD_PTR dwReserved);
	HRESULT ProcessUrlAction(LPCWSTR pwszUrl, DWORD dwAction, BYTE* pPolicy,
			DWORD cbPolicy, BYTE* pContext, DWORD cbContext, DWORD dwFlags, DWORD dwReserved);
	HRESULT QueryCustomPolicy(LPCWSTR pwszUrl, REFGUID guidKey, BYTE** ppPolicy,
...
alias LPINTERNETSECURITYMGRSITE = IInternetSecurityMgrSite;
...
interface IOleControlSite : IUnknown {
	...
	HRESULT LockInPlaceActive(BOOL fLock //Indicates whether to ensure the active state
	);
	HRESULT GetExtendedControl(LPDISPATCH* ppDisp //Address of output variable that receives the
...
interface ISpecifyPropertyPages : IUnknown {
	HRESULT GetPages(CAUUID* pPages //Pointer to structure
	);
}
