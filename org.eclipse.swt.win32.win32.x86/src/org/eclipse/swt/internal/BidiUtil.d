 ...
 */
	static extern (Windows) int EnumSystemLanguageGroupsProc(uint lpLangGrpId,
			wchar* lpLangGrpIdString, wchar* lpLangGrpName, uint options, LONG_PTR lParam) {
 ...
 */
	public static void drawGlyphs(GC gc, wchar[] renderBuffer, int[] renderDx, int x, int y) {
		...
		OS.ExtTextOutW(gc.handle, x, y, ETO_GLYPH_INDEX, null,
				renderBuffer.ptr, cast(int) /*64bit*/ renderBuffer.length, renderDx.ptr);
 ...
 */
	public static String getRenderInfo(GC gc, String text, int[] order,
		...
		auto lpOrder = result.lpOrder = cast(uint*) OS.HeapAlloc(hHeap,
				OS.HEAP_ZERO_MEMORY, byteCount * 4);
		auto lpDx = result.lpDx = cast(int*) OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY, byteCount * 4);
		auto lpClass = result.lpClass = cast(CHAR*) OS.HeapAlloc(hHeap,
				OS.HEAP_ZERO_MEMORY, byteCount);
		auto lpGlyphs = result.lpGlyphs = cast(wchar*) OS.HeapAlloc(hHeap,
				OS.HEAP_ZERO_MEMORY, byteCount * 2);
		...
		for (int i = 0; i < offsets.length - 1; i++) {
			...
			StringT textBuffer2 = StrToTCHARs(lpCs[1], text.substring(offset,
					offset + length_), false);
			OS.GetCharacterPlacement(gc.handle, textBuffer2.ptr,
					cast(int) /*64bit*/ textBuffer2.length, 0, &result, dwFlags);
 ...
 */
	public static int getKeyboardLanguage() {
		...
		if (langID is LANG_HEBREW)
			return KEYBOARD_BIDI;
		if (langID is LANG_ARABIC)
			return KEYBOARD_BIDI;
 ...
 */
	public static bool isBidiPlatform() {
		if (OS.IsWinCE)
			return false;
		if (isBidiPlatform_ !is -1)
			return isBidiPlatform_ is 1; // already set
 ...
 */
	public static void setKeyboardLanguage(int language) {
		...
		if (language is getKeyboardLanguage())
			return;
