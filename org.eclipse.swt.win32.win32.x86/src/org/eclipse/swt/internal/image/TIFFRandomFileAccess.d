...
final class TIFFRandomFileAccess {
	...
	void seek(int pos) {
		if (pos is current)
			return;
		if (pos < start)
			throw new IOException("pos < start");
		current = pos;
		if (current > next) {
			int n = current - next;
			...
			while (n > 0) {
				...
				if (buffers[index] is null)
					buffers[index] = new byte[CHUNK_SIZE];
				int cnt = cast(int) /*64bit*/ inputStream.read(buffers[index],
						offset, Math.min(n, CHUNK_SIZE - offset));
				n -= cnt;
				next += cnt;
				index++;
				offset = 0;
	...
	void read(byte[] b) {
		...
		if (nMissing > 0) {
			...
			while (nMissing > 0) {
				...
				if (buffers[index] is null)
					buffers[index] = new byte[CHUNK_SIZE];
				auto cnt = inputStream.read(buffers[index], offset,
						Math.min(nMissing, CHUNK_SIZE - offset));
				System.arraycopy(buffers[index], offset, b, destNext, cnt);
				nMissing -= cnt;
				next += cnt;
				destNext += cnt;
