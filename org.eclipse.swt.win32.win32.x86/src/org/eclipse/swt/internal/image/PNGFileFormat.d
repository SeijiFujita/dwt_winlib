	...
	override ImageData[] loadFromByteStream() {
		try {
			...
			imageData = ImageData.internal_new(width, height, headerChunk.getSwtBitsPerPixel(),
					new PaletteData(0, 0, 0), 4, data, 0, null, null, -1, -1,
					SWT.IMAGE_PNG, 0, 0, 0, 0);
 ...
 */
	void readNextChunk(PngChunkReader chunkReader) {
		...
		case PngChunk.CHUNK_tRNS:
			...
			if (trnsChunk.getTransparencyType(headerChunk) is PngTrnsChunk.TRANSPARENCY_TYPE_PIXEL) {
	...
	override bool isFileFormat(LEDataInputStream stream) {
		try {
			...
			if ((signature[0] & 0xFF) !is 137)
				return false; //137
			if ((signature[1] & 0xFF) !is 80)
				return false; //P
			if ((signature[2] & 0xFF) !is 78)
				return false; //N
			if ((signature[3] & 0xFF) !is 71)
				return false; //G
			if ((signature[4] & 0xFF) !is 13)
				return false; //<RETURN>
			if ((signature[5] & 0xFF) !is 10)
				return false; //<LINEFEED>
			if ((signature[6] & 0xFF) !is 26)
				return false; //<CTRL/Z>
			if ((signature[7] & 0xFF) !is 10)
				return false; //<LINEFEED>     
			return true;
		}
		catch (Exception e) {
 ...
 */
	void readInterlaceFrame(InputStream inputStream, int rowInterval,
			int columnInterval, int startRow, int startColumn, int frameCount) {
 ...
 */
	static void compress16BitDepthTo8BitDepth(byte[] source, int sourceOffset,
			byte[] destination, int destinationOffset, int numberOfValues) {
