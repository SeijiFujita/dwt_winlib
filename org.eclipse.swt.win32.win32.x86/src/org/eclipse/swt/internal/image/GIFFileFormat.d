...
final class GIFFileFormat : FileFormat {
	...
	byte[] readApplicationExtension() {
		try {
			...
			bool netscape = application[0] is 'N' && application[1] is 'E'
				&& application[2] is 'T' && application[3] is 'S' && application[4] is 'C'
				&& application[5] is 'A' && application[6] is 'P' && application[7] is 'E';
			bool authentic = authentication[0] is '2'
				&& authentication[1] is '.' && authentication[2] is '0';
	...
	ImageData readImageBlock(PaletteData defaultPalette) {
		...
		ImageData image = ImageData.internal_new(width, height, depth, palette,
				4, null, 0, null, null, -1, transparentPixel, SWT.IMAGE_GIF,
				left, top, disposalMethod, delayTime);
		...
		for (int i = 0; i < frameCount; i++) {
			...
			if (multi) {
				if (!(data[i].height <= logicalScreenHeight
						&& data[i].width <= logicalScreenWidth && data[i].depth is depth)) {
					SWT.error(SWT.ERROR_INVALID_IMAGE);
				}
				if (globalTable is 1) {
