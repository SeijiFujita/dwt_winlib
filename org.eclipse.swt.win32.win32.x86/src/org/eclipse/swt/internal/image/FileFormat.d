...
public abstract class FileFormat {
	...
	static const String[] FORMATS = [
		"WinBMP"[
	], "WinBMP", "GIF", "WinICO", "JPEG", "PNG", "TIFF", "OS2BMP"]; //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$//$NON-NLS-5$ //$NON-NLS-6$//$NON-NLS-7$//$NON-NLS-8$
	alias TFormats = Tuple!(WinBMPFileFormat, WinBMPFileFormat, GIFFileFormat,
			WinICOFileFormat, JPEGFileFormat, PNGFileFormat, TIFFFileFormat, OS2BMPFileFormat);
 ...
 */
	public static void save(OutputStream os, int format, ImageLoader loader) {
		if (format < 0 || format >= FORMATS.length)
			SWT.error(SWT.ERROR_UNSUPPORTED_FORMAT);
		if (FORMATS[format] is null)
			SWT.error(SWT.ERROR_UNSUPPORTED_FORMAT);
		if (loader.data is null || loader.data.length < 1)
