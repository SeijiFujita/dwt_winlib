 ...
 */
	...
	int compressRLE4Data(byte[] src, int srcOffset, int numBytes, byte[] dest, bool last) {
		...
		while (sp < end) {
			...
			default:
				dest[dp] = 0;
				dp++;
				dest[dp] = cast(byte)(n + n);
				dp++; /* n bytes = n*2 pixels */
				for (i = n; i > 0; i--) {
					dest[dp] = src[sp];
					dp++;
					sp++;
	...
	void decompressData(byte[] src, byte[] dest, int stride, int cmp) {
		if (cmp is 1) { // BMP_RLE8_COMPRESSION
			if (decompressRLE8Data(src, cast(int) /*64bit*/ src.length, stride,
					dest, cast(int) /*64bit*/ dest.length) <= 0)
				SWT.error(SWT.ERROR_INVALID_IMAGE);
			return;
		}
		if (cmp is 2) { // BMP_RLE4_COMPRESSION
			if (decompressRLE4Data(src, cast(int) /*64bit*/ src.length, stride,
					dest, cast(int) /*64bit*/ dest.length) <= 0)
				SWT.error(SWT.ERROR_INVALID_IMAGE);
			return;
		}
	...
	override bool isFileFormat(LEDataInputStream stream) {
		try {
			...
			int infoHeaderSize = (header[14] & 0xFF) | (
					(header[15] & 0xFF) << 8) | ((header[16] & 0xFF) << 16) | (
					(header[17] & 0xFF) << 24);
	...
	byte[] loadData(byte[] infoHeader, int stride) {
		...
		int cmp = (infoHeader[16] & 0xFF) | ((infoHeader[17] & 0xFF) << 8) | (
				(infoHeader[18] & 0xFF) << 16) | ((infoHeader[19] & 0xFF) << 24);
		if (cmp is 0 || cmp is 3) { // BMP_NO_COMPRESSION
			...
			int compressedSize = (infoHeader[20] & 0xFF) | ((infoHeader[21] & 0xFF) << 8) | (
					(infoHeader[22] & 0xFF) << 16) | ((infoHeader[23] & 0xFF) << 24);
	...
	override ImageData[] loadFromByteStream() {
		...
		int headerSize = (infoHeader[0] & 0xFF) | ((infoHeader[1] & 0xFF) << 8) | (
				(infoHeader[2] & 0xFF) << 16) | ((infoHeader[3] & 0xFF) << 24);
		int width = (infoHeader[4] & 0xFF) | ((infoHeader[5] & 0xFF) << 8) | (
				(infoHeader[6] & 0xFF) << 16) | ((infoHeader[7] & 0xFF) << 24);
		int height = (infoHeader[8] & 0xFF) | ((infoHeader[9] & 0xFF) << 8) | (
				(infoHeader[10] & 0xFF) << 16) | ((infoHeader[11] & 0xFF) << 24);
		if (height < 0)
			height = -height;
		int bitCount = (infoHeader[14] & 0xFF) | ((infoHeader[15] & 0xFF) << 8);
		this.compression = (infoHeader[16] & 0xFF) | ((infoHeader[17] & 0xFF) << 8) | (
				(infoHeader[18] & 0xFF) << 16) | ((infoHeader[19] & 0xFF) << 24);
		...
		this.importantColors = (infoHeader[36] & 0xFF) | (
				(infoHeader[37] & 0xFF) << 8) | ((infoHeader[38] & 0xFF) << 16) | (
				(infoHeader[39] & 0xFF) << 24);
		int xPelsPerMeter = (infoHeader[24] & 0xFF) | ((infoHeader[25] & 0xFF) << 8) | (
				(infoHeader[26] & 0xFF) << 16) | ((infoHeader[27] & 0xFF) << 24);
		int yPelsPerMeter = (infoHeader[28] & 0xFF) | ((infoHeader[29] & 0xFF) << 8) | (
				(infoHeader[30] & 0xFF) << 16) | ((infoHeader[31] & 0xFF) << 24);
		this.pelsPerMeter = new Point(xPelsPerMeter, yPelsPerMeter);
		int type = (this.compression is 1 /*BMP_RLE8_COMPRESSION*/ )
			|| (this.compression is 2 /*BMP_RLE4_COMPRESSION*/ ) ? SWT.IMAGE_BMP_RLE : SWT
			.IMAGE_BMP;
		return [ImageData.internal_new(width, height, bitCount, palette, 4,
				data, 0, null, null, -1, -1, type, 0, 0, 0, 0)];
	}

	PaletteData loadPalette(byte[] infoHeader) {
		int depth = (infoHeader[14] & 0xFF) | ((infoHeader[15] & 0xFF) << 8);
		if (depth <= 8) {
			int numColors = (infoHeader[32] & 0xFF) | ((infoHeader[33] & 0xFF) << 8) | (
					(infoHeader[34] & 0xFF) << 16) | ((infoHeader[35] & 0xFF) << 24);
			if (numColors is 0) {
				numColors = 1 << depth;
			}
			else {
		...
		if (depth is 16) {
			if (this.compression is 3) {
				try {
					return new PaletteData(inputStream.readInt(),
	...
	PaletteData paletteFromBytes(byte[] bytes, int numColors) {
		...
		for (int i = 0; i < numColors; i++) {
			colors[i] = new RGB(bytes[bytesOffset + 2] & 0xFF,
					bytes[bytesOffset + 1] & 0xFF, bytes[bytesOffset] & 0xFF);
			bytesOffset += 4;
		}
 ...
 */
	static byte[] paletteToBytes(PaletteData pal) {
		int n = pal.colors is null ? 0 : (pal.colors.length < 256 ? cast(int) /*64bit*/ pal.colors.length
				: 256);
 ...
 */
	override void unloadIntoByteStream(ImageLoader loader) {
		...
		if (!((comp is 0) || ((comp is 1) && (image.depth is 8)) || ((comp is 2) && (image.depth is 4))))
			SWT.error(SWT.ERROR_INVALID_IMAGE);
