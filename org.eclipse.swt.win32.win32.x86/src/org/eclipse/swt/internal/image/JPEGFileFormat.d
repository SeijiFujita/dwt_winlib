...
final class JPEGFileFormat : FileFormat {
	...
	public static const int[] ZigZag8x8 = [0, 1, 8, 16, 9, 2, 3, 10, 17, 24,
		32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6,
		7, 14, 21, 28, 35, 42, 49, 56, 57, 50, 43, 36, 29, 22, 15, 23, 30, 37,
	...
	void compress(ImageData image, byte[] dataYComp, byte[] dataCbComp, byte[] dataCrComp) {
		...
		for (int iComp = 0; iComp < nComponents; iComp++) {
			...
			if (compressedHeight < componentHeight) {
				...
				for (int yPos = (compressedHeight > 0) ? compressedHeight : 1; yPos <= componentHeight;
						yPos++) {
					int dstOfs = (yPos - 1) * componentWidth;
					System.arraycopy(imageComponent, srcOfs, imageComponent,
							dstOfs, componentWidth);
				}
			}
		}
	}
		...
		if (palette.isDirect) {
			...
			for (int yPos = 0; yPos < srcHeight; yPos++) {
				...
				for (int xPos = 0; xPos < srcWidth; xPos++) {
					...
					dataCbComp[dstDataIndex] = cast(byte)(
							(RCbTable[r] + GCbTable[g] + BCbTable[b]) >> 16);
					dataCrComp[dstDataIndex] = cast(byte)(
							(RCrTable[r] + GCrTable[g] + BCrTable[b]) >> 16);
				}
			}
		}
		else {
	...
	void decodeMCUAtXAndY(int xmcu, int ymcu, int nComponentsInScan, bool first,
			int start, int end, int approxBit) {
				...
				for (int ihi = 0; ihi < hi; ihi++) {
					...
					if (!progressive) {
						...
						if (scanHeader.isACProgressiveScan()) {
							if (first) {
								decodeACFirstCoefficients(dataUnit,
										scanComponent, start, end, approxBit);
							}
							else {
								decodeACRefineCoefficients(dataUnit,
										scanComponent, start, end, approxBit);
							}
						}
	...
	void expandImageComponents() {
		for (int iComp = 0; iComp < nComponents; iComp++) {
			...
			if ((upH * upV) > 1) {
				...
				ImageData src = new ImageData(compWidth, compHeight, 8,
						new PaletteData(RGB16), 4, component);
	...
	bool isZeroInColumn(int[] dataUnit, int col) {
		return dataUnit[col + 8] is 0 && dataUnit[col + 16] is 0
			...
			&& dataUnit[col + 40] is 0 && dataUnit[col + 48] is 0 && dataUnit[col + 56] is 0;
	}
	...
	bool isZeroInRow(int[] dataUnit, int rIndex) {
		return dataUnit[rIndex + 1] is 0 && dataUnit[rIndex + 2] is 0
			...
			&& dataUnit[rIndex + 5] is 0 && dataUnit[rIndex + 6] is 0 && dataUnit[rIndex + 7] is 0;
	}
	...
	override ImageData[] loadFromByteStream() {
		...
		while (!done) {
			...
			if (progressive && loader.hasListeners()) {
				...
				loader.notifyListeners(new ImageLoaderEvent(loader, imageData,
						progressiveScanCount, false));
		...
		if (progressive && loader.hasListeners()) {
			loader.notifyListeners(new ImageLoaderEvent(loader, imageData,
					progressiveScanCount, true));
		}
		return [imageData];
	}

	ImageData createImageData() {
		return ImageData.internal_new(imageWidth, imageHeight, nComponents * samplePrecision,
				setUpPalette(), nComponents is 1 ? 4 : 1, decodeImageComponents(),
	...
	static JPEGSegment seekUnspecifiedMarker(LEDataInputStream byteStream) {
		...
		try {
			while (true) {
				if (byteStream.read(byteArray, 0, 1) !is 1)
					return null;
				if (byteArray[0] is cast(byte) 0xFF) {
					if (byteStream.read(byteArray, 1, 1) !is 1)
						return null;
	...
	override void unloadIntoByteStream(ImageLoader loader) {
		...
		JPEGAppn appn = new JPEGAppn([cast(byte) 0xFF, cast(byte) 0xE0, 0, 0x10,
		...
		JPEGHuffmanTable[] dhtTables = [
			JPEGHuffmanTable.getDefaultDCLuminanceTable(), JPEGHuffmanTable.getDefaultDCChrominanceTable(),
