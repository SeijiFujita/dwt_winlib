...
class PngIhdrChunk : PngChunk {
	...
	this(int width, int height, byte bitDepth, byte colorType,
 ...
 */
	override void validate(PngFileReadState readState, PngIhdrChunk headerChunk) {
		...
		if (length !is IHDR_DATA_LENGTH)
			SWT.error(SWT.ERROR_INVALID_IMAGE);
		if (compressionMethod !is 0)
			SWT.error(SWT.ERROR_INVALID_IMAGE);
		if (interlaceMethod !is INTERLACE_METHOD_NONE && interlaceMethod !is INTERLACE_METHOD_ADAM7) {
			SWT.error(SWT.ERROR_INVALID_IMAGE);
		}
		...
		if ((colorType is COLOR_TYPE_RGB || colorType is COLOR_TYPE_RGB_WITH_ALPHA
				|| colorType is COLOR_TYPE_GRAYSCALE_WITH_ALPHA) && bitDepth < 8) {
			SWT.error(SWT.ERROR_INVALID_IMAGE);
		}
	...
	String getColorTypeString() {
		...
		default:
			return "Unknown - " ~ cast(char) colorType;
		}
	}
	...
	override String contributeToString() {
		return Format("\n\tWidth: {}\n\tHeight: {}\n\tBit Depth: {}\n\tColor Type: {}\n\tCompression Method: {}\n\tFilter Method: {}\n\tInterlace Method: {}",
				width, height, bitDepth, getColorTypeString(), compressionMethod,
				getFilterMethodString(), getInterlaceMethodString());
	}
	...
	bool getCanHavePalette() {
		return colorType !is COLOR_TYPE_GRAYSCALE && colorType !is COLOR_TYPE_GRAYSCALE_WITH_ALPHA;
	}
