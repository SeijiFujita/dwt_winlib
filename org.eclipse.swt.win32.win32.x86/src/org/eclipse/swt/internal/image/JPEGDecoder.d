...
public class JPEGDecoder {
								...
								if (bits_left < HUFF_LOOKAHEAD) {
									...
									if (bits_left < HUFF_LOOKAHEAD) {
										...
										if ((s = jpeg_huff_decode(br_state,
												get_buffer, bits_left, actbl, nb)) < 0) {
											return false;
										}
										get_buffer = br_state.get_buffer;
										bits_left = br_state.bits_left;
		...
		if (need_full_buffer) {
			...
			for (ci = 0; ci < cinfo.num_components; ci++) {
				...
				if (cinfo.progressive_mode)
					access_rows *= 3;
				//#endif
				coef.whole_image[ci] = new short[][][](cast(int) jround_up(compptr.height_in_blocks,
						compptr.v_samp_factor), cast(int) jround_up(compptr.width_in_blocks,
						compptr.h_samp_factor), DCTSIZE2);
			}
	...
	static void jpeg_calc_output_dimensions(jpeg_decompress_struct cinfo) /* Do computations that are needed before master selection phase */ {
		//#ifdef IDCT_SCALING_SUPPORTED
		//  int ci;
		//  jpeg_component_info compptr;
		//#endif
	...
	static bool use_merged_upsample(jpeg_decompress_struct cinfo) {
		...
		if (cinfo.jpeg_color_space !is JCS_YCbCr || cinfo.num_components !is 3
				|| cinfo.out_color_space !is JCS_RGB || cinfo.out_color_components !is RGB_PIXELSIZE)
			return false;
		...
		if (cinfo.comp_info[0].h_samp_factor !is 2
				...
				|| cinfo.comp_info[1].v_samp_factor !is 1 || cinfo.comp_info[2].v_samp_factor !is 1)
			return false;
	...
	static void prepare_range_limit_table(jpeg_decompress_struct cinfo) /* Allocate and fill in the sample_range_limit table */ {
		byte[] table;
		int i;
		...
		System.arraycopy(cinfo.sample_range_limit, cinfo.sample_range_limit_offset,
				table, offset + (4 * (MAXJSAMPLE + 1) - CENTERJSAMPLE), CENTERJSAMPLE);
	}
	...
	static void build_ycc_rgb_table(jpeg_decompress_struct cinfo) {
		...
		for (i = 0, x = -CENTERJSAMPLE; i <= MAXJSAMPLE; i++, x++) {
			...
			cconvert.Cr_r_tab[i] = (
					cast(int)(1.40200f * (1 << SCALEBITS) + 0.5f) * x + ONE_HALF) >> SCALEBITS;
			/* Cb=>B value is nearest int to 1.77200 * x */
			cconvert.Cb_b_tab[i] = (
					cast(int)(1.77200f * (1 << SCALEBITS) + 0.5f) * x + ONE_HALF) >> SCALEBITS;
	...
	static void make_funny_pointers(jpeg_decompress_struct cinfo) /* Create the funny pointer lists discussed in the comments above.
 ...
 */ {
	...
	static void alloc_funny_pointers(jpeg_decompress_struct cinfo) /* Allocate space for the funny pointer lists.
 ...
 */ {
     ...
     */
		...
		for (ci = 0; ci < cinfo.num_components; ci++) {
			compptr = cinfo.comp_info[ci];
			rgroup = (compptr.v_samp_factor * compptr.DCT_scaled_size) / cinfo.min_DCT_scaled_size; /* height of a row group of component */
			main.buffer[ci] = new byte[][](rgroup * ngroups,
					compptr.width_in_blocks * compptr.DCT_scaled_size);
		}
	}

	static long jround_up(long a, long b) /* Compute a rounded up to next multiple of b, ie, ceil(a/b)*b */
	/* Assumes a >= 0, b > 0 */ {
		a += b - 1L;
		return a - (a % b);
	}
     ...
     */
         ...
         */
			h_in_group = (compptr.h_samp_factor * compptr.DCT_scaled_size) / cinfo
				.min_DCT_scaled_size;
			v_in_group = (compptr.v_samp_factor * compptr.DCT_scaled_size) / cinfo
				.min_DCT_scaled_size;
	...
	static void jpeg_idct_islow(jpeg_decompress_struct cinfo, jpeg_component_info compptr,
			short[] coef_block, byte[][] output_buf, int output_buf_offset, int output_col) {
         ...
         */
			...
			if (inptr[DCTSIZE * 1 + inptr_offset] is 0
					&& inptr[DCTSIZE * 2 + inptr_offset] is 0 && inptr[DCTSIZE * 3 + inptr_offset] is 0
					&& inptr[DCTSIZE * 4 + inptr_offset] is 0 && inptr[DCTSIZE * 5 + inptr_offset] is 0
					&& inptr[DCTSIZE * 6 + inptr_offset] is 0 && inptr[DCTSIZE * 7 + inptr_offset] is 0) {
				/* AC terms all zero */
				int dcval = (
						(inptr[DCTSIZE * 0 + inptr_offset]) * quantptr[DCTSIZE * 0 + quantptr_offset]) << PASS1_BITS;
         ...
         */
			...
			wsptr[DCTSIZE * 0 + wsptr_offset] = (((tmp10 + tmp3) + (
					1 << ((CONST_BITS - PASS1_BITS) - 1))) >> (CONST_BITS - PASS1_BITS));
			wsptr[DCTSIZE * 7 + wsptr_offset] = (((tmp10 - tmp3) + (
					1 << ((CONST_BITS - PASS1_BITS) - 1))) >> (CONST_BITS - PASS1_BITS));
			wsptr[DCTSIZE * 1 + wsptr_offset] = (((tmp11 + tmp2) + (
					1 << ((CONST_BITS - PASS1_BITS) - 1))) >> (CONST_BITS - PASS1_BITS));
			wsptr[DCTSIZE * 6 + wsptr_offset] = (((tmp11 - tmp2) + (
					1 << ((CONST_BITS - PASS1_BITS) - 1))) >> (CONST_BITS - PASS1_BITS));
			wsptr[DCTSIZE * 2 + wsptr_offset] = (((tmp12 + tmp1) + (
					1 << ((CONST_BITS - PASS1_BITS) - 1))) >> (CONST_BITS - PASS1_BITS));
			wsptr[DCTSIZE * 5 + wsptr_offset] = (((tmp12 - tmp1) + (
					1 << ((CONST_BITS - PASS1_BITS) - 1))) >> (CONST_BITS - PASS1_BITS));
			wsptr[DCTSIZE * 3 + wsptr_offset] = (((tmp13 + tmp0) + (
					1 << ((CONST_BITS - PASS1_BITS) - 1))) >> (CONST_BITS - PASS1_BITS));
			wsptr[DCTSIZE * 4 + wsptr_offset] = (((tmp13 - tmp0) + (
					1 << ((CONST_BITS - PASS1_BITS) - 1))) >> (CONST_BITS - PASS1_BITS));
         ...
         */
			...
			if (wsptr[1 + wsptr_offset] is 0 && wsptr[2 + wsptr_offset] is 0
					&& wsptr[3 + wsptr_offset] is 0 && wsptr[4 + wsptr_offset] is 0
					&& wsptr[5 + wsptr_offset] is 0
					&& wsptr[6 + wsptr_offset] is 0 && wsptr[7 + wsptr_offset] is 0) {
				/* AC terms all zero */
				//          #define DESCALE(x,n)    RIGHT_SHIFT((x) + (ONE << ((n)-1)), n)
				byte dcval = range_limit[range_limit_offset + (
						(((wsptr[0 + wsptr_offset]) + (1 << ((PASS1_BITS + 3) - 1))) >> PASS1_BITS
						+ 3) & RANGE_MASK)];
         ...
         */
			...
			outptr[0 + outptr_offset] = range_limit[range_limit_offset + (
					(((tmp10 + tmp3) + (1 << (
					(CONST_BITS + PASS1_BITS + 3) - 1))) >> CONST_BITS + PASS1_BITS + 3) & RANGE_MASK)];
	...
	static void upsample(jpeg_decompress_struct cinfo, byte[][][] input_buf,
			int[] input_buf_offset, int[] in_row_group_ctr, int in_row_groups_avail,
	...
	static bool smoothing_ok(jpeg_decompress_struct cinfo) {
		...
		for (ci = 0; ci < cinfo.num_components; ci++) {
			...
			if (qtable.quantval[0] is 0 || qtable.quantval[Q01_POS] is 0
					|| qtable.quantval[Q10_POS] is 0 || qtable.quantval[Q20_POS] is 0
					|| qtable.quantval[Q11_POS] is 0 || qtable.quantval[Q02_POS] is 0)
				return false;
	...
	static void jcopy_sample_rows(byte[][] input_array, int source_row,
			byte[][] output_array, int dest_row, int num_rows, int num_cols) /* Copy some rows of samples from one place to another.
 ...
 */ {
	...
	static void prepare_for_output_pass(jpeg_decompress_struct cinfo) {
		...
		if (master.is_dummy_pass) {
			...
			if (!cinfo.raw_data_out) {
				...
				cinfo.post.start_pass(cinfo, (master.is_dummy_pass
	...
	static bool jpeg_resync_to_restart(jpeg_decompress_struct cinfo, int desired) {
		...
		for (;;) {
			if (marker < M_SOF0)
				...
				else if (marker is(M_RST0 + ((desired - 1) & 7))
						|| marker is(M_RST0 + ((desired - 2) & 7)))
	...
	static bool jpeg_fill_bit_buffer(bitread_working_state state, int get_buffer,
			int bits_left, int nbits) /* Load up the bit buffer to a depth of at least nbits */ {
					...
					for (yindex = 0; yindex < compptr.MCU_height; yindex++) {
						if (cinfo.input_iMCU_row < last_iMCU_row
							...
							for (xindex = 0; xindex < useful_width; xindex++) {
								jpeg_idct_islow(cinfo, compptr, coef.MCU_buffer[blkn + xindex],
										output_ptr, output_ptr_offset, output_col);
		...
		for (ci = 0; ci < cinfo.num_components; ci++) {
			...
			for (block_row = 0; block_row < block_rows; block_row++) {
				...
				for (block_num = 0; block_num < compptr.width_in_blocks; block_num++) {
					jpeg_idct_islow(cinfo, compptr, buffer_ptr[buffer_ptr_offset],
							output_ptr, output_ptr_offset, output_col);
	...
	static void post_process_data(jpeg_decompress_struct cinfo, byte[][][] input_buf,
			int[] input_buf_offset, int[] in_row_group_ctr, int in_row_groups_avail,
			byte[][] output_buf, int[] out_row_ctr, int out_rows_avail) {
	...
	static void set_bottom_pointers(jpeg_decompress_struct cinfo) /* Change the pointer lists to duplicate the last sample row at the bottom
 ...
 */ {
	...
	static void set_wraparound_pointers(jpeg_decompress_struct cinfo) /* Set up the "wraparound" pointers at top and bottom of the pointer lists.
 ...
 */ {
		...
		if (!main.buffer_full) {
			...
			case DECOMPRESS_SMOOTH_DATA:
				result = decompress_smooth_data(cinfo,
						main.xbuffer[main.whichptr], main.xbuffer_offset[main.whichptr]);
				break;
     ...
     */
		...
		case CTX_POSTPONED_ROW:
			...
			post_process_data(cinfo, main.xbuffer[main.whichptr], main.xbuffer_offset[main.whichptr],
					main.rowgroup_ctr, main.rowgroups_avail, output_buf,
     ...
     */
		...
		post_process_data(cinfo, main.buffer, main.buffer_offset, main.rowgroup_ctr,
	...
	static bool get_dht(jpeg_decompress_struct cinfo) /* Process a DHT marker */ {
		int length;
		...
		if (cinfo.bytes_offset is cinfo.bytes_in_buffer)
	...
	static bool get_dqt(jpeg_decompress_struct cinfo) /* Process a DQT marker */ {
		int length;
		int n, i, prec;
		int tmp;
	...
	static bool get_dri(jpeg_decompress_struct cinfo) /* Process a DRI marker */ {
		int length;
		int tmp;
	...
	static bool get_dac(jpeg_decompress_struct cinfo) /* Process a DAC marker */ {
		int length;
		int index, val;
		if (cinfo.bytes_offset is cinfo.bytes_in_buffer)
			fill_input_buffer(cinfo);
	...
	static bool get_sos(jpeg_decompress_struct cinfo) /* Process a SOS marker */ {
		int length;
		int i, ci, n, c, cc;
	...
	static void sep_upsample(jpeg_decompress_struct cinfo, byte[][][] input_buf,
			int[] input_buf_offset, int[] in_row_group_ctr, int in_row_groups_avail,
			byte[][] output_buf, int[] out_row_ctr, int out_rows_avail) {
		jpeg_upsampler upsample = cinfo.upsample;
		int ci;
     ...
     */
		...
		case GRAYSCALE_CONVERT:
			grayscale_convert(cinfo, upsample.color_buf, upsample.color_buf_offset,
					upsample.next_row_out, output_buf, out_row_ctr[0], num_rows);
			break;
		case YCC_RGB_CONVERT:
			ycc_rgb_convert(cinfo, upsample.color_buf, upsample.color_buf_offset,
					upsample.next_row_out, output_buf, out_row_ctr[0], num_rows);
			break;
		case GRAY_RGB_CONVERT:
			gray_rgb_convert(cinfo, upsample.color_buf, upsample.color_buf_offset,
					upsample.next_row_out, output_buf, out_row_ctr[0], num_rows);
			break;
	...
	static void noop_upsample(jpeg_decompress_struct cinfo, jpeg_component_info compptr, byte[][] input_data,
	...
	static void fullsize_upsample(jpeg_decompress_struct cinfo, jpeg_component_info compptr, byte[][] input_data,
			int input_data_offset, byte[][][] output_data_ptr,
			int[] output_data_offset, int output_data_index) {
	...
	static void h2v1_upsample(jpeg_decompress_struct cinfo, jpeg_component_info compptr, byte[][] input_data,
			int input_data_offset, byte[][][] output_data_ptr,
			int[] output_data_offset, int output_data_index) {
	...
	static void h2v2_upsample(jpeg_decompress_struct cinfo, jpeg_component_info compptr, byte[][] input_data,
			int input_data_offset, byte[][][] output_data_ptr,
			int[] output_data_offset, int output_data_index) {
		byte[][] output_data = output_data_ptr[output_data_index];
		byte[] inptr, outptr;
	...
	static void h2v1_fancy_upsample(jpeg_decompress_struct cinfo, jpeg_component_info compptr, byte[][] input_data,
			int input_data_offset, byte[][][] output_data_ptr,
			int[] output_data_offset, int output_data_index) {
		byte[][] output_data = output_data_ptr[output_data_index];
		byte[] inptr, outptr;
		...
		for (inrow = 0; inrow < cinfo.max_v_samp_factor; inrow++) {
			...
			for (colctr = compptr.downsampled_width - 2; colctr > 0; colctr--) {
				...
				outptr[outptr_offset++] = cast(byte)(
						(invalue + (inptr[inptr_offset - 2] & 0xFF) + 1) >> 2);
			...
			invalue = (inptr[inptr_offset] & 0xFF);
			outptr[outptr_offset++] = cast(byte)(
					(invalue * 3 + (inptr[inptr_offset - 1] & 0xFF) + 1) >> 2);
			outptr[outptr_offset++] = cast(byte) invalue;
		}
	}

	static void h2v2_fancy_upsample(jpeg_decompress_struct cinfo, jpeg_component_info compptr, byte[][] input_data,
			int input_data_offset, byte[][][] output_data_ptr,
			int[] output_data_offset, int output_data_index) {
		byte[][] output_data = output_data_ptr[output_data_index];
	...
	static void int_upsample(jpeg_decompress_struct cinfo, jpeg_component_info compptr, byte[][] input_data,
			int input_data_offset, byte[][][] output_data_ptr,
			int[] output_data_offset, int output_data_index) {
		jpeg_upsampler upsample = cinfo.upsample;
		byte[][] output_data = output_data_ptr[output_data_index];
	...
	static void null_convert(jpeg_decompress_struct cinfo, byte[][][] input_buf,
			int[] input_buf_offset, int input_row, byte[][] output_buf,
			int output_buf_offset, int num_rows) {
		byte[] inptr, outptr;
		int count;
	...
	static void grayscale_convert(jpeg_decompress_struct cinfo, byte[][][] input_buf,
			int[] input_buf_offset, int input_row, byte[][] output_buf,
			int output_buf_offset, int num_rows) {
	...
	static void gray_rgb_convert(jpeg_decompress_struct cinfo, byte[][][] input_buf,
			int[] input_buf_offset, int input_row, byte[][] output_buf,
			int output_buf_offset, int num_rows) {
		byte[] inptr, outptr;
		int col;
		...
		while (--num_rows >= 0) {
			...
			for (col = 0; col < num_cols; col++) {
				...
				outptr[RGB_RED + outptr_offset] = outptr[RGB_GREEN + outptr_offset] = outptr[RGB_BLUE
					+ outptr_offset] = inptr[col];
				outptr_offset += RGB_PIXELSIZE;
			}
		}
	}

	static void ycc_rgb_convert(jpeg_decompress_struct cinfo, byte[][][] input_buf,
			int[] input_buf_offset, int input_row, byte[][] output_buf,
			int output_buf_offset, int num_rows) {
		jpeg_color_deconverter cconvert = cinfo.cconvert;
		int y, cb, cr;
		...
		while (--num_rows >= 0) {
			...
			for (col = 0; col < num_cols; col++) {
				...
				outptr[outptr_offset + RGB_GREEN] = range_limit[y + (
						(Cbgtab[cb] + Crgtab[cr] >> SCALEBITS)) + range_limit_offset];
	...
	static bool skip_variable(jpeg_decompress_struct cinfo) /* Skip over an unknown or uninteresting variable-length marker */ {
		int length;
	...
	static bool get_interesting_appn(jpeg_decompress_struct cinfo) /* Process an APP0 or APP14 marker without saving it */ {
		int length;
	...
	static void examine_app0(jpeg_decompress_struct cinfo, byte[] data, int datalen, int remaining) /* Examine first few bytes from an APP0.
 * Take appropriate action if it is a JFIF marker.
 * datalen is # of bytes at data[], remaining is length of rest of marker data.
 */ {
		int totallen = datalen + remaining;

		if (datalen >= APP0_DATA_LEN && (data[0] & 0xFF) is 0x4A
				&& (data[1] & 0xFF) is 0x46 && (data[2] & 0xFF) is 0x49
				&& (data[3] & 0xFF) is 0x46 && (data[4] & 0xFF) is 0) {
		...
		else if (datalen >= 6 && (data[0] & 0xFF) is 0x4A
				&& (data[1] & 0xFF) is 0x46 && (data[2] & 0xFF) is 0x58
				&& (data[3] & 0xFF) is 0x58 && (data[4] & 0xFF) is 0) {
	...
	static void examine_app14(jpeg_decompress_struct cinfo, byte[] data, int datalen, int remaining) /* Examine first few bytes from an APP14.
 * Take appropriate action if it is an Adobe marker.
 * datalen is # of bytes at data[], remaining is length of rest of marker data.
 */ {
		int /*version, flags0, flags1, */ transform;

		if (datalen >= APP14_DATA_LEN && (data[0] & 0xFF) is 0x41
				&& (data[1] & 0xFF) is 0x64 && (data[2] & 0xFF) is 0x6F
				&& (data[3] & 0xFF) is 0x62 && (data[4] & 0xFF) is 0x65) {
	...
	static bool jpeg_finish_output(jpeg_decompress_struct cinfo) {
		if ((cinfo.global_state is DSTATE_SCANNING
				|| cinfo.global_state is DSTATE_RAW_OK) && cinfo.buffered_image) {
			/* Terminate this pass. */
				...
				for (ci = 0; ci < cinfo.comps_in_scan; ci++) {
					...
					for (yindex = 0; yindex < compptr.MCU_height; yindex++) {
						...
						buffer_ptr = coef.whole_image[compptr.component_index][yindex
							+ yoffset + cinfo.input_iMCU_row * compptr.v_samp_factor];
	...
	static long jdiv_round_up(long a, long b) /* Compute a/b rounded up to next integer, ie, ceil(a/b) */
	/* Assumes a >= 0, b > 0 */ {
		return (a + b - 1) / b;
	}

	static void initial_setup(jpeg_decompress_struct cinfo) /* Called once, when first SOS marker is reached */ {
		int ci;
		jpeg_component_info compptr;
     ...
     */
		...
		for (ci = 0; ci < cinfo.num_components; ci++) {
			...
			/* Size in DCT blocks */
			compptr.width_in_blocks = cast(int) jdiv_round_up(cast(long) cinfo.image_width * cast(
					long) compptr.h_samp_factor, (cinfo.max_h_samp_factor * DCTSIZE));
			compptr.height_in_blocks = cast(int) jdiv_round_up(cast(long) cinfo.image_height * cast(
					long) compptr.v_samp_factor, (cinfo.max_v_samp_factor * DCTSIZE));
			/* downsampled_width and downsampled_height will also be overridden by
         * jdmaster.c if we are doing full decompression.   The transcoder library
         * doesn't use these values, but the calling application might.
         */
			/* Size in samples */
			compptr.downsampled_width = cast(int) jdiv_round_up(cast(long) cinfo.image_width * cast(
					long) compptr.h_samp_factor, cinfo.max_h_samp_factor);
			compptr.downsampled_height = cast(int) jdiv_round_up(cast(long) cinfo.image_height * cast(
					long) compptr.v_samp_factor, cinfo.max_v_samp_factor);
		...
		cinfo.total_iMCU_rows = cast(int) jdiv_round_up(cinfo.image_height,
				(cinfo.max_v_samp_factor * DCTSIZE));
	...
	static void per_scan_setup(jpeg_decompress_struct cinfo) /* Do computations that are needed before processing a JPEG scan */
	/* cinfo.comps_in_scan and cinfo.cur_comp_info[] were set from SOS marker */ {
         ...
         */
			...
			/* Overall image size in MCUs */
			cinfo.MCUs_per_row = cast(int) jdiv_round_up(cinfo.image_width,
					(cinfo.max_h_samp_factor * DCTSIZE));
			cinfo.MCU_rows_in_scan = cast(int) jdiv_round_up(cinfo.image_height,
					(cinfo.max_v_samp_factor * DCTSIZE));
	...
	static void jpeg_make_d_derived_tbl(jpeg_decompress_struct cinfo, bool isDC,
			int tblno, d_derived_tbl dtbl) {
		JHUFF_TBL htbl;
	...
	static ImageData[] loadFromByteStream(InputStream inputStream, ImageLoader loader) {
		...
		int scanlinePad = 4;
		int row_stride = (
				((cinfo.output_width * cinfo.out_color_components * 8 + 7) / 8) + (scanlinePad - 1)) / scanlinePad
			* scanlinePad;
		byte[][] buffer = new byte[][](1, row_stride);
		byte[] data = new byte[row_stride * cinfo.output_height];
		ImageData imageData = ImageData.internal_new(cinfo.output_width, cinfo.output_height,
				palette.isDirect ? 24 : 8, palette, scanlinePad, data, 0, null,
				null, -1, -1, SWT.IMAGE_JPEG, 0, 0, 0, 0);
		if (cinfo.buffered_image) {
			bool done;
			do {
				...
				loader.notifyListeners(new ImageLoaderEvent(loader, cast(ImageData) imageData.clone(),
						incrementCount, done = jpeg_input_complete(cinfo)));
			}
			while (!done);
		}
		else {
