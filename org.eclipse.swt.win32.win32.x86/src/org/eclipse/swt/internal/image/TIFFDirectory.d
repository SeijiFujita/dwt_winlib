...
final class TIFFDirectory {
	...
	PaletteData getGrayPalette() {
		...
		for (int i = 0; i < numColors; i++) {
			...
			if (photometricInterpretation is 0)
				value = 0xFF - value;
	...
	int formatStrips(int rowByteSize, int nbrRows, byte[] data,
			int maxStripByteSize, int offsetPostIFD, int extraBytes, int[][] strips) {
		/*
    ...
    */
	...
	void parseEntries(byte[] buffer) {
		for (int offset = 0; offset < buffer.length; offset += IFD_ENTRY_SIZE) {
			...
			case TAG_StripOffsets: {
					if (type !is TYPE_LONG && type !is TYPE_SHORT)
						SWT.error(SWT.ERROR_INVALID_IMAGE);
			...
			case TAG_SamplesPerPixel: {
					...
					if (samplesPerPixel !is 1 && samplesPerPixel !is 3)
	...
	public ImageData read() {
		...
		ImageData image = ImageData.internal_new(imageWidth, imageLength,
				depth, palette, 1, null, 0, null, null, -1, -1, SWT.IMAGE_TIFF, 0, 0, 0, 0);
		decodePixels(image);
		return image;
	}
	...
	int toInt(byte[] buffer, int i, int type) {
		if (type is TYPE_LONG) {
			return isLittleEndian ? (buffer[i] & 0xFF) | (
					(buffer[i + 1] & 0xFF) << 8) | ((buffer[i + 2] & 0xFF) << 16) | (
					(buffer[i + 3] & 0xFF) << 24)
				: (buffer[i + 3] & 0xFF) | ((buffer[i + 2] & 0xFF) << 8) | (
	...
	void write(int photometricInterpretation) {
		...
		if (isColorMap) {
			...
			if (colorMap.length !is 3 * 1 << image.depth)
				SWT.error(SWT.ERROR_UNSUPPORTED_FORMAT);
		...
		writeEntry(TAG_StripByteCounts, TYPE_LONG, cnt, cnt > 1
				? stripByteCountsOffset : stripByteCounts[0]);
		writeEntry(TAG_XResolution, TYPE_RATIONAL, 1, xResolutionOffset);
		writeEntry(TAG_YResolution, TYPE_RATIONAL, 1, yResolutionOffset);
		if (isColorMap)
			writeEntry(TAG_ColorMap, TYPE_SHORT, cast(int) /*64bit*/ colorMap.length,
					colorMapOffset);
		...
		if (cnt > 1) {
			for (int i = 0; i < cnt; i++)
				ostr.writeInt(stripOffsets[i]);
			for (int i = 0; i < cnt; i++)
				ostr.writeInt(stripByteCounts[i]);
		}
    ...
    */
	...
	void writeToStream(LEDataOutputStream byteStream) {
		...
		case 1: {
				...
				if (palette.isDirect || rgbs is null || rgbs.length !is 2)
					SWT.error(SWT.ERROR_UNSUPPORTED_FORMAT);
				RGB rgb0 = rgbs[0];
				RGB rgb1 = rgbs[1];
				if (!(rgb0.red is rgb0.green && rgb0.green is rgb0.blue
						&& rgb1.red is rgb1.green && rgb1.green is rgb1.blue
						&& ((rgb0.red is 0x0 && rgb1.red is 0xFF)
						|| (rgb0.red is 0xFF && rgb1.red is 0x0)))) {
					SWT.error(SWT.ERROR_UNSUPPORTED_FORMAT);
				}
