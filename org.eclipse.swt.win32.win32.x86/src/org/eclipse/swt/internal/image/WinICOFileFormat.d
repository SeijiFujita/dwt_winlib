 ...
 */
	int iconSize(ImageData i) {
		...
		int paletteSize = i.palette.colors !is null ? cast(int) /*64bit*/ i.palette.colors.length * 4
			: 0;
 ...
 */
	ImageData loadIcon(int[] iconHeader) {
		...
		return ImageData.internal_new(width, height, depth, palette, 4,
				shapeData, 2, maskData, null, -1, -1, SWT.IMAGE_ICO, 0, 0, 0, 0);
	}
	...
	byte[] loadInfoHeader(int[] iconHeader) {
		...
		if (((infoHeader[12] & 0xFF) | ((infoHeader[13] & 0xFF) << 8)) !is 1)
			SWT.error(SWT.ERROR_INVALID_IMAGE);
		int infoWidth = (infoHeader[4] & 0xFF) | ((infoHeader[5] & 0xFF) << 8) | (
				(infoHeader[6] & 0xFF) << 16) | ((infoHeader[7] & 0xFF) << 24);
		int infoHeight = (infoHeader[8] & 0xFF) | ((infoHeader[9] & 0xFF) << 8) | (
				(infoHeader[10] & 0xFF) << 16) | ((infoHeader[11] & 0xFF) << 24);
		int bitCount = (infoHeader[14] & 0xFF) | ((infoHeader[15] & 0xFF) << 8);
		if (height is infoHeight && bitCount is 1)
			height /= 2;
		if (!((width is infoWidth) && (height * 2 is infoHeight) && (bitCount is 1
				|| bitCount is 4 || bitCount is 8 || bitCount is 24 || bitCount is 32)))
			SWT.error(SWT.ERROR_INVALID_IMAGE);
 ...
 */
	void unloadIcon(ImageData icon) {
		int sizeImage = (((icon.width * icon.depth + 31) / 32 * 4) + ((icon.width + 31) / 32 * 4))
			* icon.height;
		try {
			...
			outputStream.writeInt(icon.palette.colors !is null
					? cast(int) /*64bit*/ icon.palette.colors.length : 0);
			outputStream.writeInt(0);
		}
		catch (IOException e) {
			SWT.error(SWT.ERROR_IO, e);
 ...
 */
	void unloadIconHeader(ImageData i) {
		...
		try {
			...
			outputStream.writeShort(i.palette.colors !is null ? cast(int) /*64bit*/ i.palette.colors.length
					: 0);
