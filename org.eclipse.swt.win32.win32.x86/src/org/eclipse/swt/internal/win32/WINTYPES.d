...
version (Tango) {
	version (Win32SansUnicode) {
		alias StringT = String;
	}
	else {
		alias StringT = String16;
	}
	alias CWCHAR = wchar;
}
else { // Phobos
    ...
    */
	...
	alias CALTYPE = uint;
	alias CALID = uint;
	alias CCHAR = TryConst!(char);
	alias CWCHAR = TryConst!(wchar);
	alias PCHAR = char*;
	alias COLORREF = uint;
	alias TCOLORREF = uint;
	alias CHAR = char;
	alias BYTE = ubyte;
	alias INT16 = short;
	alias UINT16 = ushort;
	...
	alias WORD = ushort;
	alias DWORD = uint;
	alias INT_PTR = intptr_t;
	alias UINT_PTR = size_t;
	alias LONG_PTR = intptr_t;
	...
	const BOOL FALSE = 0;
	const BOOL TRUE = -1;
	alias PANSICHAR = char*;
	alias PWIDECHAR = wchar*;
	alias PINTEGER = int*;
	alias LONGLONG = long;
	alias PLONGLONG = LONGLONG*;
	...
	alias LCID = DWORD;
	alias LCTYPE = DWORD;
	alias LPARAM = intptr_t;
	alias LP = ushort*;
	alias LPBOOL = WINBOOL*;
	alias LPBYTE = ubyte*;
	alias LPCOLORREF = COLORREF*;
	alias LPCCH = TryConst!(char)*;
	alias LPCH = TryConst!(char)*;
	alias LPCSTR = TryConst!(char)*;
	alias LPCTSTR = TryConst!(TCHAR)*;
	alias LPCWCH = TryConst!(wchar)*;
	alias LPCWSTR = TryConst!(wchar)*;
	alias LPDWORD = DWORD*;
	...
	alias LPVOID = POINTER;
	alias LPCVOID = TryConst!(void)*;
	alias LPWCH = wchar*;
	alias LPWORD = wchar*;
	alias LPWSTR = wchar*;
	alias NWPSTR = wchar*;
	alias PWINBOOL = WINBOOL*;
	alias BOOLEAN = ubyte;
	...
	alias SERVICE_STATUS_HANDLE = DWORD;
	alias TBYTE = ubyte;
	...
	alias _RASCONNSTATE = RASCONNSTATE;
	...
	alias _RASPROJECTION = RASPROJECTION;
	...
	alias TAGTOKEN_TYPE = TOKEN_TYPE;

	extern (Windows) {
		alias BFFCALLBACK = int function(HWND, UINT, LPARAM, LPARAM);
		alias LPCCHOOKPROC = UINT_PTR function(HWND, UINT, WPARAM, LPARAM);
		alias LPCFHOOKPROC = UINT_PTR function(HWND, UINT, WPARAM, LPARAM);
		alias PTHREAD_START_ROUTINE = POINTER;
		alias LPTHREAD_START_ROUTINE = PTHREAD_START_ROUTINE;
		alias EDITSTREAMCALLBACK = DWORD function(DWORD_PTR, LPBYTE, LONG, LONG);
		alias LPFRHOOKPROC = UINT_PTR function(HWND, UINT, WPARAM, LPARAM);
		alias LPOFNHOOKPROC = UINT_PTR function(HWND, UINT, WPARAM, LPARAM);
		alias LPPRINTHOOKPROC = UINT_PTR function(HWND, UINT, WPARAM, LPARAM);
		alias LPSETUPHOOKPROC = UINT_PTR function(HWND, UINT, WPARAM, LPARAM);
		alias DLGPROC = INT_PTR function(HWND, UINT, WPARAM, LPARAM);
		alias PFNPROPSHEETCALLBACK = int function(HWND, UINT, LPARAM);
		alias LPSERVICE_MAIN_FUNCTION = void function(DWORD, LPTSTR);
		alias PFNTVCOMPARE = int function(LPARAM, LPARAM, LPARAM);
		alias WNDPROC = LRESULT function(HWND, UINT, WPARAM, LPARAM);
		alias FARPROC = POINTER;
		alias PROC = FARPROC;
		alias ENUMRESTYPEPROC = WINBOOL function(HANDLE, LPTSTR, LONG_PTR);
		alias ENUMRESNAMEPROC = WINBOOL function(HANDLE, LPCTSTR, LPTSTR, LONG_PTR);
		alias ENUMRESLANGPROC = WINBOOL function(HANDLE, LPCTSTR, LPCTSTR, ushort, LONG_PTR);
		alias DESKTOPENUMPROC = FARPROC;
		alias ENUMWINDOWSPROC = WINBOOL function(HWND, LPARAM);
		alias ENUMWINDOWSTATIONPROC = WINBOOL function(LPTSTR, LPARAM);
		...
		alias DRAWSTATEPROC = WINBOOL function(HDC, LPARAM, WPARAM, int, int);
		alias PROPENUMPROCEX = WINBOOL function(HWND, LPCTSTR, HANDLE, ULONG_PTR);
		alias PROPENUMPROC = WINBOOL function(HWND, LPCTSTR, HANDLE);
		alias HOOKPROC = LRESULT function(int, WPARAM, LPARAM);
		alias ENUMOBJECTSPROC = void function(LPVOID);
		alias LINEDDAPROC = void function(int, int);
		alias TABORTPROC = WINBOOL function(HDC, int);
		alias LPPAGEPAINTHOOK = UINT_PTR function(HWND, UINT, WPARAM, LPARAM);
		alias LPPAGESETUPHOOK = UINT_PTR function(HWND, UINT, WPARAM, LPARAM);
		alias ICMENUMPROC = int function(LPTSTR, LPARAM);
		alias EDITWORDBREAKPROCEX = LONG function(PCHAR, LONG, ubyte, INT);
		alias PFNLVCOMPARE = int function(LPARAM, LPARAM, LPARAM);
		alias LOCALE_ENUMPROC = WINBOOL function(LPTSTR);
		alias CODEPAGE_ENUMPROC = WINBOOL function(LPTSTR);
		alias DATEFMT_ENUMPROC = WINBOOL function(LPTSTR);
		alias TIMEFMT_ENUMPROC = WINBOOL function(LPTSTR);
		alias CALINFO_ENUMPROC = WINBOOL function(LPTSTR);
		alias PHANDLER_ROUTINE = WINBOOL function(DWORD);
		alias LPHANDLER_FUNCTION = WINBOOL function(DWORD);
		alias PTIMERAPCROUTINE = void function(LPVOID, DWORD, DWORD);
		alias PFNGETPROFILEPATH = UINT function(LPCTSTR, LPSTR, UINT);
		alias PFNRECONCILEPROFILE = UINT function(LPCTSTR, LPCTSTR, DWORD);
		alias PFNPROCESSPOLICIES = WINBOOL function(HWND, LPCTSTR, LPCTSTR, LPCTSTR, DWORD);
	}
	...
	const {
		...
		version (Win32SansUnicode) {
			alias SERVICES_ACTIVE_DATABASE = SERVICES_ACTIVE_DATABASEA;
			alias SERVICES_FAILED_DATABASE = SERVICES_FAILED_DATABASEA;
			alias SC_GROUP_IDENTIFIER = SC_GROUP_IDENTIFIERA;
		}
		else {
			alias SERVICES_ACTIVE_DATABASE = SERVICES_ACTIVE_DATABASEW;
			alias SERVICES_FAILED_DATABASE = SERVICES_FAILED_DATABASEW;
			alias SC_GROUP_IDENTIFIER = SC_GROUP_IDENTIFIERW;
		}
	}

	extern (Windows) {
		alias PFNCALLBACK = HDDEDATA function(UINT, UINT, HCONV, HSZ, HSZ,
				HDDEDATA, ULONG_PTR, ULONG_PTR);
	}
	alias CALLB = PFNCALLBACK;
	alias SECURITY___FILE___TRACKING_MODE = WINBOOL;
	alias WNDENUMPROC = FARPROC;
	alias ENHMFENUMPROC = FARPROC;
	alias CCSTYLE = DWORD;
	alias PCCSTYLE = CCSTYLE*;
	alias LPCCSTYLE = CCSTYLE*;
	alias CCSTYLEFLAGA = DWORD;
	alias PCCSTYLEFLAGA = CCSTYLEFLAGA*;
    ...
    */
	...
	alias HTREEITEM = TREEITEM*;
	alias TTREEITEM = TREEITEM;
	alias PTREEITEM = TREEITEM*;
	enum : DWORD {
		TVI_ROOT = (0xFFFF0000),
	...
	enum : DWORD {
		...
		__FILE___DEBUGGER = (__FILE___FULL) | (
				__FILE___FLOATING_POINT),
	...
	alias LPABCFLOAT = ABCFLOAT*;
	alias _ABCFLOAT = ABCFLOAT;
	alias TABCFLOAT = ABCFLOAT;
	alias PABCFLOAT = ABCFLOAT*;
	...
	alias TACCESS_ALLOWED_ACE = ACCESS_ALLOWED_ACE;
	...
	alias TACCESS_DENIED_ACE = ACCESS_DENIED_ACE;
	...
	alias _ACCESSTIMEOUT = ACCESSTIMEOUT;
	alias TACCESSTIMEOUT = ACCESSTIMEOUT;
	alias PACCESSTIMEOUT = ACCESSTIMEOUT*;
	...
	alias TACLREVISIONINFORMATION = ACL_REVISION_INFORMATION;
	alias PACLREVISIONINFORMATION = ACL_REVISION_INFORMATION*;
	...
	alias TACLSIZEINFORMATION = ACL_SIZE_INFORMATION;
	alias PACLSIZEINFORMATION = ACL_SIZE_INFORMATION*;
	...
	alias TACTIONHEADER = ACTION_HEADER;
	alias PACTIONHEADER = ACTION_HEADER*;
	...
	alias TADAPTERSTATUS = ADAPTER_STATUS;
	alias PADAPTERSTATUS = ADAPTER_STATUS*;
	...
	alias _ADDJOB_INFO_1 = ADDJOB_INFO_1;
	alias TADDJOB_INFO_1 = ADDJOB_INFO_1;
	alias PADDJOB_INFO_1 = ADDJOB_INFO_1*;
	...
	alias _ANIMATIONINFO = ANIMATIONINFO;
	alias TANIMATIONINFO = ANIMATIONINFO;
	alias PANIMATIONINFO = ANIMATIONINFO*;
	...
	alias TBITMAPCOREHEADER = BITMAPCOREHEADER;
	alias PBITMAPCOREHEADER = BITMAPCOREHEADER*;
	...
	alias TRGBTRIPLE = RGBTRIPLE;
	alias PRGBTRIPLE = RGBTRIPLE*;
	...
	alias PBITMAPCOREINFO = BITMAPCOREINFO*;
	alias LPBITMAPCOREINFO = BITMAPCOREINFO*;
	alias _BITMAPCOREINFO = BITMAPCOREINFO;
	alias TBITMAPCOREINFO = BITMAPCOREINFO;
	...
	alias TBITMAPINFOHEADER = BITMAPINFOHEADER;
	...
	alias TRGBQUAD = RGBQUAD;
	alias PRGBQUAD = RGBQUAD*;
	...
	alias TBITMAPINFO = BITMAPINFO;
	alias FXPT2DOT30 = int;
	alias LPFXPT2DOT30 = FXPT2DOT30*;
	alias TPFXPT2DOT30 = FXPT2DOT30;
	alias PPFXPT2DOT30 = FXPT2DOT30*;
	...
	alias TPCIEXYZ = CIEXYZ;
	alias PCIEXYZ = CIEXYZ*;
	...
	alias TCIEXYZTRIPLE = CIEXYZTRIPLE;
	alias PCIEXYZTRIPLE = CIEXYZTRIPLE*;
	...
	alias TBITMAPV4HEADER = BITMAPV4HEADER;
	...
	alias _SHITEMID = SHITEMID;
	alias TSHITEMID = SHITEMID;
	...
	alias _ITEMIDLIST = ITEMIDLIST;
	alias TITEMIDLIST = ITEMIDLIST;
	...
	alias TBYHANDLEFILEINFORMATION = BY_HANDLE_FILE_INFORMATION;
	alias PBYHANDLEFILEINFORMATION = BY_HANDLE_FILE_INFORMATION*;
	...
	alias TPOINTFX = POINTFX;
	alias PPOINTFX = POINTFX*;
	...
	alias LPCANDIDATEFORM = CANDIDATEFORM*;
	alias _TAGCANDIDATEFORM = CANDIDATEFORM;
	alias TCANDIDATEFORM = CANDIDATEFORM;
	alias PCANDIDATEFORM = CANDIDATEFORM*;
	...
	alias LPCANDIDATELIST = CANDIDATELIST*;
	alias _TAGCANDIDATELIST = CANDIDATELIST;
	alias TCANDIDATELIST = CANDIDATELIST;
	alias PCANDIDATELIST = CANDIDATELIST*;
	...
	alias TAGCREATESTRUCT = CREATESTRUCT;
	alias TCREATESTRUCT = CREATESTRUCT;
	alias PCREATESTRUCT = CREATESTRUCT*;
	...
	alias TAGCBT_CREATEWND = CBT_CREATEWND;
	alias TCBT_CREATEWND = CBT_CREATEWND;
	alias PCBT_CREATEWND = CBT_CREATEWND*;
	...
	alias TCBTACTIVATESTRUCT = CBTACTIVATESTRUCT;
	alias PCBTACTIVATESTRUCT = CBTACTIVATESTRUCT*;
	...
	alias _CHARFORMAT = CHARFORMAT;
	alias TCHARFORMAT = CHARFORMAT;
	alias PCHARFORMAT = CHARFORMAT*;
	...
	alias _CHARRANGE = CHARRANGE;
	alias TCHARRANGE = CHARRANGE;
	alias PCHARRANGE = CHARRANGE*;
	...
	alias TAGCHARSET = CHARSET;
	alias TCHARSET = CHARSET;
	alias PCHARSET = CHARSET*;
	...
	alias LPFONTSIGNATURE = FONTSIGNATURE*;
	alias TAGFONTSIGNATURE = FONTSIGNATURE;
	alias TFONTSIGNATURE = FONTSIGNATURE;
	alias PFONTSIGNATURE = FONTSIGNATURE*;
	...
	alias LPCHARSETINFO = CHARSETINFO*;
	alias TCHARSETINFO = CHARSETINFO;
	alias PCHARSETINFO = CHARSETINFO*;
	...
	alias PCHOOSECOLORA = CHOOSECOLORA*, LPCHOOSECOLORA = CHOOSECOLORA*;
	struct CHOOSECOLORW {
		DWORD lStructSize;
		HWND hwndOwner;
		HWND hInstance;
	...
	alias PCHOOSECOLORW = CHOOSECOLORW*, LPCHOOSECOLORW = CHOOSECOLORW*;
	version (Win32SansUnicode) {
	...
	alias PCHOOSECOLOR = CHOOSECOLOR*, LPCHOOSECOLOR = CHOOSECOLOR*;
	...
	alias TLOGFONTW = LOGFONTW;
	alias LPLOGFONTW = LOGFONTW*;
	alias PLOGFONTW = LOGFONTW*;

	version (Win32SansUnicode) {
		alias LOGFONT = LOGFONTA;
		alias TLOGFONT = LOGFONTA;
		alias PLOGFONT = LOGFONTA*;
		alias LPLOGFONT = LOGFONTA*;
	}
	else {
		alias LOGFONT = LOGFONTW;
		alias TLOGFONT = LOGFONTW;
		alias PLOGFONT = LOGFONTW*;
		alias LPLOGFONT = LOGFONTW*;
	}
	...
	alias PCHOOSEFONT = CHOOSEFONT*;
	alias TCHOOSEFONT = CHOOSEFONT;
	...
	alias TCLIENTCREATESTRUCT = CLIENTCREATESTRUCT;
	alias PCLIENTCREATESTRUCT = CLIENTCREATESTRUCT*;
	...
	alias _CMINVOKECOMMANDINFO = CMINVOKECOMMANDINFO;
	alias TCMINVOKECOMMANDINFO = CMINVOKECOMMANDINFO;
	alias PCMINVOKECOMMANDINFO = CMINVOKECOMMANDINFO*;
	...
	alias LPCOLORADJUSTMENT = COLORADJUSTMENT*;
	alias TAGCOLORADJUSTMENT = COLORADJUSTMENT;
	alias TCOLORADJUSTMENT = COLORADJUSTMENT;
	alias PCOLORADJUSTMENT = COLORADJUSTMENT*;
	...
	alias TCOMMCONFIG = COMMCONFIG;
	alias PCOMMCONFIG = COMMCONFIG*;
	...
	alias TCOMMPROP = COMMPROP;
	alias PCOMMPROP = COMMPROP*;
	...
	alias TCOMMTIMEOUTS = COMMTIMEOUTS;
	alias PCOMMTIMEOUTS = COMMTIMEOUTS*;
	...
	alias TCOMPAREITEMSTRUCT = COMPAREITEMSTRUCT;
	alias PCOMPAREITEMSTRUCT = COMPAREITEMSTRUCT*;
	...
	alias TCOMPCOLOR = COMPCOLOR;
	alias PCOMPCOLOR = COMPCOLOR*;
	...
	alias LPCOMPOSITIONFORM = COMPOSITIONFORM*;
	alias _TAGCOMPOSITIONFORM = COMPOSITIONFORM;
	alias TCOMPOSITIONFORM = COMPOSITIONFORM;
	alias PCOMPOSITIONFORM = COMPOSITIONFORM*;
	...
	alias TCOMSTAT = COMSTAT;
	alias PCOMSTAT = COMSTAT*;
	enum : DWORD {
	...
	alias TCONSOLECURSORINFO = CONSOLE_CURSOR_INFO;
	alias PCONSOLECURSORINFO = CONSOLE_CURSOR_INFO*;
	alias TCURSORINFO = CONSOLE_CURSOR_INFO;
	...
	alias TCONSOLESCREENBUFFERINFO = CONSOLE_SCREEN_BUFFER_INFO;
	alias PCONSOLESCREENBUFFERINFO = CONSOLE_SCREEN_BUFFER_INFO*;
	...
	alias TFLOATINGSAVEAREA = FLOATING_SAVE_AREA;
	alias PFLOATINGSAVEAREA = FLOATING_SAVE_AREA*;
	...
	enum : DWORD {
		...
		CONTEXT_FLOATING_POINT = (
				CONTEXT_i386 | 0x00000008), // 387 state
	...
	alias TLISTENTRY = LIST_ENTRY;
	alias PLISTENTRY = LIST_ENTRY*;
	...
	alias LPCRITICAL_SECTION_DEBUG = CRITICAL_SECTION_DEBUG*;
	alias PCRITICAL_SECTION_DEBUG = CRITICAL_SECTION_DEBUG*;
	alias _CRITICAL_SECTION_DEBUG = CRITICAL_SECTION_DEBUG;
	alias TCRITICALSECTIONDEBUG = CRITICAL_SECTION_DEBUG;
	alias PCRITICALSECTIONDEBUG = CRITICAL_SECTION_DEBUG*;
	...
	alias TCRITICALSECTION = CRITICAL_SECTION;
	alias PCRITICALSECTION = CRITICAL_SECTION*;
	...
	alias TSECURITYQUALITYOFSERVICE = SECURITY_QUALITY_OF_SERVICE;
	alias PSECURITYQUALITYOFSERVICE = SECURITY_QUALITY_OF_SERVICE*;
	...
	alias TCONVCONTEXT = CONVCONTEXT;
	...
	alias TCOPYDATASTRUCT = COPYDATASTRUCT;
	...
	alias TCPINFO = CPINFO;
	alias PCPINFO = CPINFO*;
	...
	alias TAGCPLINFO = CPLINFO;
	alias TCPLINFO = CPLINFO;
	alias PCPLINFO = CPLINFO*;
	...
	alias TCREATEPROCESSDEBUGINFO = CREATE_PROCESS_DEBUG_INFO;
	alias PCREATEPROCESSDEBUGINFO = CREATE_PROCESS_DEBUG_INFO*;
	...
	alias TCREATETHREADDEBUGINFO = CREATE_THREAD_DEBUG_INFO;
	alias PCREATETHREADDEBUGINFO = CREATE_THREAD_DEBUG_INFO*;
	...
	alias _CURRENCYFMT = CURRENCYFMT;
	alias TCURRENCYFMT = CURRENCYFMT;
	alias PCURRENCYFMT = CURRENCYFMT*;
	...
	alias TCURSORSHAPE = CURSORSHAPE;
	alias PCURSORSHAPE = CURSORSHAPE*;
	...
	alias TCWPRETSTRUCT = CWPRETSTRUCT;
	alias PCWPRETSTRUCT = CWPRETSTRUCT*;
	...
	alias _DATATYPES_INFO_1 = DATATYPES_INFO_1;
	alias TDATATYPESINFO1 = DATATYPES_INFO_1;
	alias PDATATYPESINFO1 = DATATYPES_INFO_1*;

	struct DDEACK {
		ushort flag0;
	}

	alias TDDEACK = DDEACK;
	alias PDDEACK = DDEACK*;
	enum : DWORD {
	...
	alias TDDEADVISE = DDEADVISE;
	alias PDDEADVISE = DDEADVISE*;
	enum : DWORD {
	...
	alias TDDEMLMSGHOOKDATA = DDEML_MSG_HOOK_DATA;
	alias PDDEMLMSGHOOKDATA = DDEML_MSG_HOOK_DATA*;
	...
	alias TDDEPOKE = DDEPOKE;
	alias PDDEPOKE = DDEPOKE*;
	enum : DWORD {
	...
	alias TEXCEPTIONRECORD = EXCEPTION_RECORD;
	alias PEXCEPTIONRECORD = EXCEPTION_RECORD*;
	...
	alias PEXCEPTION_DEBUG_INFO = EXCEPTION_DEBUG_INFO*;
	alias _EXCEPTION_DEBUG_INFO = EXCEPTION_DEBUG_INFO;
	alias TEXCEPTIONDEBUGINFO = EXCEPTION_DEBUG_INFO;
	alias PEXCEPTIONDEBUGINFO = EXCEPTION_DEBUG_INFO*;
	...
	alias TEXITPROCESSDEBUGINFO = EXIT_PROCESS_DEBUG_INFO;
	alias PEXITPROCESSDEBUGINFO = EXIT_PROCESS_DEBUG_INFO*;
	...
	alias TEXITTHREADDEBUGINFO = EXIT_THREAD_DEBUG_INFO;
	alias PEXITTHREADDEBUGINFO = EXIT_THREAD_DEBUG_INFO*;
	...
	alias TLOADDLLDEBUGINFO = LOAD_DLL_DEBUG_INFO;
	alias PLOADDLLDEBUGINFO = LOAD_DLL_DEBUG_INFO*;
	...
	alias TUNLOADDLLDEBUGINFO = UNLOAD_DLL_DEBUG_INFO;
	alias PUNLOADDLLDEBUGINFO = UNLOAD_DLL_DEBUG_INFO*;
	...
	alias TOUTPUTDEBUGSTRINGINFO = OUTPUT_DEBUG_STRING_INFO;
	alias POUTPUTDEBUGSTRINGINFO = OUTPUT_DEBUG_STRING_INFO*;
	...
	alias TRIPINFO = RIP_INFO;
	alias PRIPINFO = RIP_INFO*;
	...
	alias TDEBUGEVENT = DEBUG_EVENT;
	alias PDEBUGEVENT = DEBUG_EVENT*;
	...
	alias TDEBUGHOOKINFO = DEBUGHOOKINFO;
	alias PDEBUGHOOKINFO = DEBUGHOOKINFO*;
	...
	alias TDELETEITEMSTRUCT = DELETEITEMSTRUCT;
	alias PDELETEITEMSTRUCT = DELETEITEMSTRUCT*;
	...
	alias _DEV_BROADCAST_HDR = DEV_BROADCAST_HDR;
	alias TDEVBROADCASTHDR = DEV_BROADCAST_HDR;
	alias PDEVBROADCASTHDR = DEV_BROADCAST_HDR*;
	...
	alias _DEV_BROADCAST_OEM = DEV_BROADCAST_OEM;
	alias TDEVBROADCASTOEM = DEV_BROADCAST_OEM;
	alias PDEVBROADCASTOEM = DEV_BROADCAST_OEM*;
	...
	alias _DEV_BROADCAST_PORT = DEV_BROADCAST_PORT;
	alias TDEVBROADCASTPORT = DEV_BROADCAST_PORT;
	alias PDEVBROADCASTPORT = DEV_BROADCAST_PORT*;
	...
	alias TDEVBROADCASTUSERDEFINED = _DEV_BROADCAST_USERDEFINED;
	alias PDEVBROADCASTUSERDEFINED = _DEV_BROADCAST_USERDEFINED*;
	...
	alias _DEV_BROADCAST_VOLUME = DEV_BROADCAST_VOLUME;
	alias TDEVBROADCASTVOLUME = DEV_BROADCAST_VOLUME;
	alias PDEVBROADCASTVOLUME = DEV_BROADCAST_VOLUME*;
	...
	alias LPDEVMODE = DEVMODE*;
	alias _DEVICEMODE = DEVMODE;
	alias DEVICEMODE = DEVMODE;
	alias TDEVICEMODE = DEVMODE;
	alias TDEVICEMODEA = DEVMODE;
	alias TDEVMODE = DEVMODE;
	alias PDEVMODE = DEVMODE*;
	...
	alias TDEVNAMES = DEVNAMES;
	alias PDEVNAMES = DEVNAMES*;
	...
	alias TLARGEINTEGER = LARGE_INTEGER;
	alias PLARGEINTEGER = LARGE_INTEGER*;
	...
	alias TDISKGEOMETRY = DISK_GEOMETRY;
	alias PDISKGEOMETRY = DISK_GEOMETRY*;
	...
	alias TDISKPERFORMANCE = DISK_PERFORMANCE;
	alias PDISKPERFORMANCE = DISK_PERFORMANCE*;
	...
	alias TDLGITEMTEMPLATE = DLGITEMTEMPLATE;
	...
	alias TDLGTEMPLATE = DLGTEMPLATE;
	alias PDLGTEMPLATE = DLGTEMPLATE*;
	...
	alias _DOC_INFO_1 = DOC_INFO_1;
	alias TDOCINFO1 = DOC_INFO_1;
	alias PDOCINFO1 = DOC_INFO_1*;
	...
	alias _DOC_INFO_2 = DOC_INFO_2;
	alias TDOCINFO2 = DOC_INFO_2;
	alias PDOCINFO2 = DOC_INFO_2*;
	...
	alias TDOCINFO = DOCINFO;
	alias LPDOCINFO = DOCINFO*;
	alias PDOCINFO = DOCINFO*;
	...
	alias TDRAGLISTINFO = DRAGLISTINFO;
	alias PDRAGLISTINFO = DRAGLISTINFO*;
	...
	alias TDRAWITEMSTRUCT = DRAWITEMSTRUCT;
	alias PDRAWITEMSTRUCT = DRAWITEMSTRUCT*;
	...
	alias TDRAWTEXTPARAMS = DRAWTEXTPARAMS;
	alias PDRAWTEXTPARAMS = DRAWTEXTPARAMS*;
	...
	alias TPARTITIONINFORMATION = PARTITION_INFORMATION;
	alias PPARTITIONINFORMATION = PARTITION_INFORMATION*;
	...
	alias TDRIVELAYOUTINFORMATION = DRIVE_LAYOUT_INFORMATION;
	alias PDRIVELAYOUTINFORMATION = DRIVE_LAYOUT_INFORMATION*;

	struct DRIVER_INFO_1 {
		LPTSTR pName;
	}

	alias _DRIVER_INFO_1 = DRIVER_INFO_1;
	alias TDRIVERINFO1 = DRIVER_INFO_1;
	alias PDRIVERINFO1 = DRIVER_INFO_1*;
	...
	alias _DRIVER_INFO_2 = DRIVER_INFO_2;
	alias TDRIVERINFO2 = DRIVER_INFO_2;
	alias PDRIVERINFO2 = DRIVER_INFO_2*;
	...
	alias _DRIVER_INFO_3 = DRIVER_INFO_3;
	alias TDRIVERINFO3 = DRIVER_INFO_3;
	alias PDRIVERINFO3 = DRIVER_INFO_3*;
	...
	alias _EDITSTREAM = EDITSTREAM;
	alias TEDITSTREAM = EDITSTREAM;
	alias PEDITSTREAM = EDITSTREAM*;
	...
	alias TEMRANGLEARC = EMRANGLEARC;
	...
	alias TEMRARC = EMRARC;
	alias PEMRARC = EMRARC*;
	alias EMRARCTO = EMRARC;
	alias TEMRARCTO = EMRARC;
	alias PEMRARCTO = EMRARC*;
	alias EMRCHORD = EMRARC;
	alias TEMRCHORD = EMRARC;
	alias PEMRCHORD = EMRARC*;
	alias EMRPIE = EMRARC;
	alias TEMRPIE = EMRARC;
	alias PEMRPIE = EMRARC*;
	...
	alias TLOGBRUSH = LOGBRUSH;
	alias PLOGBRUSH = LOGBRUSH*;
	...
	alias TEMRCREATEBRUSHINDIRECT = EMRCREATEBRUSHINDIRECT;
	...
	alias TAGLOGCOLORSPACE = LOGCOLORSPACE;
	alias TLOGCOLORSPACE = LOGCOLORSPACE;
	alias TLOGCOLORSPACEA = LOGCOLORSPACE;
	alias PLOGCOLORSPACE = LOGCOLORSPACE*;
	...
	alias TEMRCREATECOLORSPACE = EMRCREATECOLORSPACE;
	...
	alias TEMRCREATEDIBPATTERNBRUSHPT = EMRCREATEDIBPATTERNBRUSHPT;
	...
	alias TEMRCREATEMONOBRUSH = EMRCREATEMONOBRUSH;
	...
	alias TPALETTEENTRY = PALETTEENTRY;
	...
	alias TEMRCREATEPALETTE = EMRCREATEPALETTE;
	...
	alias TEMRCREATEPEN = EMRCREATEPEN;
	...
	alias TEMRELLIPSE = EMRELLIPSE;
	alias PEMRELLIPSE = EMRELLIPSE*;
	alias EMRRECTANGLE = EMRELLIPSE;
	alias TEMRRECTANGLE = EMRELLIPSE;
	alias PEMRRECTANGLE = EMRELLIPSE*;
	...
	alias TEMREXCLUDECLIPRECT = EMREXCLUDECLIPRECT;
	alias PEMREXCLUDECLIPRECT = EMREXCLUDECLIPRECT*;
	alias EMRINTERSECTCLIPRECT = EMREXCLUDECLIPRECT;
	alias TEMRINTERSECTCLIPRECT = EMREXCLUDECLIPRECT;
	alias PEMRINTERSECTCLIPRECT = EMREXCLUDECLIPRECT*;
	...
	alias TPANOSE = PANOSE;
	alias PPANOSE = PANOSE*;
	...
	alias TAGEXTLOGFONT = EXTLOGFONT;
	alias TEXTLOGFONT = EXTLOGFONT;
	alias PEXTLOGFONT = EXTLOGFONT*;
	...
	alias TEMREXTCREATEFONTINDIRECTW = EMREXTCREATEFONTINDIRECTW;
	...
	alias TEXTLOGPEN = EXTLOGPEN;
	alias PEXTLOGPEN = EXTLOGPEN*;
	...
	alias TEMREXTCREATEPEN = EMREXTCREATEPEN;
	...
	alias TEMREXTFLOODFILL = EMREXTFLOODFILL;
	...
	alias TEMREXTSELECTCLIPRGN = EMREXTSELECTCLIPRGN;
	...
	alias TEMREXTTEXTOUTA = EMREXTTEXTOUTA;
	alias PEMREXTTEXTOUTA = EMREXTTEXTOUTA*;
	alias EMREXTTEXTOUTW = EMREXTTEXTOUTA;
	alias TEMREXTTEXTOUTW = EMREXTTEXTOUTA;
	alias PEMREXTTEXTOUTW = EMREXTTEXTOUTA*;
	...
	alias TEMRFILLPATH = EMRFILLPATH;
	alias PEMRFILLPATH = EMRFILLPATH*;
	alias EMRSTROKEANDFILLPATH = EMRFILLPATH;
	alias TEMRSTROKEANDFILLPATH = EMRFILLPATH;
	alias PEMRSTROKEANDFILLPATH = EMRFILLPATH*;
	alias EMRSTROKEPATH = EMRFILLPATH;
	alias TEMRSTROKEPATH = EMRFILLPATH;
	alias PEMRSTROKEPATH = EMRFILLPATH*;
	...
	alias TEMRFRAMERGN = EMRFRAMERGN;
	...
	alias TEMRGDICOMMENT = EMRGDICOMMENT;
	...
	alias TEMRINVERTRGN = EMRINVERTRGN;
	alias PEMRINVERTRGN = EMRINVERTRGN*;
	alias EMRPAINTRGN = EMRINVERTRGN;
	alias TEMRPAINTRGN = EMRINVERTRGN;
	alias PEMRPAINTRGN = EMRINVERTRGN*;
	...
	alias TEMRLINETO = EMRLINETO;
	alias PEMRLINETO = EMRLINETO*;
	alias EMRMOVETOEX = EMRLINETO;
	alias TEMRMOVETOEX = EMRLINETO;
	alias PEMRMOVETOEX = EMRLINETO*;
	...
	alias TEMRMODIFYWORLDTRANSFORM = EMRMODIFYWORLDTRANSFORM;
	...
	alias TEMROFFSETCLIPRGN = EMROFFSETCLIPRGN;
	...
	alias TEMRPOLYDRAW = EMRPOLYDRAW;
	...
	alias TEMRPOLYDRAW16 = EMRPOLYDRAW16;
	...
	alias TEMRPOLYLINE = EMRPOLYLINE;
	alias PEMRPOLYLINE = EMRPOLYLINE*;
	alias EMRPOLYBEZIER = EMRPOLYLINE;
	alias TEMRPOLYBEZIER = EMRPOLYLINE;
	alias PEMRPOLYBEZIER = EMRPOLYLINE*;
	alias EMRPOLYGON = EMRPOLYLINE;
	alias TEMRPOLYGON = EMRPOLYLINE;
	alias PEMRPOLYGON = EMRPOLYLINE*;
	alias EMRPOLYBEZIERTO = EMRPOLYLINE;
	alias TEMRPOLYBEZIERTO = EMRPOLYLINE;
	alias PEMRPOLYBEZIERTO = EMRPOLYLINE*;
	alias EMRPOLYLINETO = EMRPOLYLINE;
	alias TEMRPOLYLINETO = EMRPOLYLINE;
	alias PEMRPOLYLINETO = EMRPOLYLINE*;
	...
	alias TEMRPOLYLINE16 = EMRPOLYLINE16;
	alias PEMRPOLYLINE16 = EMRPOLYLINE16*;
	alias EMRPOLYBEZIER16 = EMRPOLYLINE16;
	alias TEMRPOLYBEZIER16 = EMRPOLYLINE16;
	alias PEMRPOLYBEZIER16 = EMRPOLYLINE16*;
	alias EMRPOLYGON16 = EMRPOLYLINE16;
	alias TEMRPOLYGON16 = EMRPOLYLINE16;
	alias PEMRPOLYGON16 = EMRPOLYLINE16*;
	alias EMRPOLYBEZIERTO16 = EMRPOLYLINE16;
	alias TEMRPOLYBEZIERTO16 = EMRPOLYLINE16;
	alias PEMRPOLYBEZIERTO16 = EMRPOLYLINE16*;
	alias EMRPOLYLINETO16 = EMRPOLYLINE16;
	alias TEMRPOLYLINETO16 = EMRPOLYLINE16;
	alias PEMRPOLYLINETO16 = EMRPOLYLINE16*;
	...
	alias TEMRPOLYPOLYLINE = EMRPOLYPOLYLINE;
	alias PEMRPOLYPOLYLINE = EMRPOLYPOLYLINE*;
	alias EMRPOLYPOLYGON = EMRPOLYPOLYLINE;
	alias TEMRPOLYPOLYGON = EMRPOLYPOLYLINE;
	alias PEMRPOLYPOLYGON = EMRPOLYPOLYLINE*;
	...
	alias TEMRPOLYPOLYLINE16 = EMRPOLYPOLYLINE16;
	alias PEMRPOLYPOLYLINE16 = EMRPOLYPOLYLINE16*;
	alias EMRPOLYPOLYGON16 = EMRPOLYPOLYLINE16;
	alias TEMRPOLYPOLYGON16 = EMRPOLYPOLYLINE16;
	alias PEMRPOLYPOLYGON16 = EMRPOLYPOLYLINE16*;
	...
	alias TEMRPOLYTEXTOUTA = EMRPOLYTEXTOUTA;
	alias PEMRPOLYTEXTOUTA = EMRPOLYTEXTOUTA*;
	alias EMRPOLYTEXTOUTW = EMRPOLYTEXTOUTA;
	alias TEMRPOLYTEXTOUTW = EMRPOLYTEXTOUTA;
	alias PEMRPOLYTEXTOUTW = EMRPOLYTEXTOUTA*;
	...
	alias TEMRRESIZEPALETTE = EMRRESIZEPALETTE;
	...
	alias TEMRRESTOREDC = EMRRESTOREDC;
	...
	alias TEMRROUNDRECT = EMRROUNDRECT;
	...
	alias TEMRSCALEVIEWPORTEXTEX = EMRSCALEVIEWPORTEXTEX;
	alias PEMRSCALEVIEWPORTEXTEX = EMRSCALEVIEWPORTEXTEX*;
	alias EMRSCALEWINDOWEXTEX = EMRSCALEVIEWPORTEXTEX;
	alias TEMRSCALEWINDOWEXTEX = EMRSCALEVIEWPORTEXTEX;
	alias PEMRSCALEWINDOWEXTEX = EMRSCALEVIEWPORTEXTEX*;
	...
	alias TEMRSELECTCOLORSPACE = EMRSELECTCOLORSPACE;
	alias PEMRSELECTCOLORSPACE = EMRSELECTCOLORSPACE*;
	alias EMRDELETECOLORSPACE = EMRSELECTCOLORSPACE;
	alias TEMRDELETECOLORSPACE = EMRSELECTCOLORSPACE;
	alias PEMRDELETECOLORSPACE = EMRSELECTCOLORSPACE*;
	...
	alias TEMRSELECTOBJECT = EMRSELECTOBJECT;
	alias PEMRSELECTOBJECT = EMRSELECTOBJECT*;
	alias EMRDELETEOBJECT = EMRSELECTOBJECT;
	alias TEMRDELETEOBJECT = EMRSELECTOBJECT;
	alias PEMRDELETEOBJECT = EMRSELECTOBJECT*;
	...
	alias TEMRSELECTPALETTE = EMRSELECTPALETTE;
	...
	alias TEMRSETARCDIRECTION = EMRSETARCDIRECTION;
	...
	alias TEMRSETBKCOLOR = EMRSETBKCOLOR;
	alias PEMRSETBKCOLOR = EMRSETBKCOLOR*;
	alias EMRSETTEXTCOLOR = EMRSETBKCOLOR;
	alias TEMRSETTEXTCOLOR = EMRSETBKCOLOR;
	alias PEMRSETTEXTCOLOR = EMRSETBKCOLOR*;
	...
	alias TEMRSETCOLORADJUSTMENT = EMRSETCOLORADJUSTMENT;
	...
	alias TEMRSETDIBITSTODEVICE = EMRSETDIBITSTODEVICE;
	...
	alias TEMRSETMAPPERFLAGS = EMRSETMAPPERFLAGS;
	...
	alias TEMRSETMITERLIMIT = EMRSETMITERLIMIT;
	...
	alias TEMRSETPALETTEENTRIES = EMRSETPALETTEENTRIES;
	...
	alias TEMRSETPIXELV = EMRSETPIXELV;
	...
	alias TEMRSETVIEWPORTEXTEX = EMRSETVIEWPORTEXTEX;
	alias PEMRSETVIEWPORTEXTEX = EMRSETVIEWPORTEXTEX*;
	alias EMRSETWINDOWEXTEX = EMRSETVIEWPORTEXTEX;
	alias TEMRSETWINDOWEXTEX = EMRSETVIEWPORTEXTEX;
	alias PEMRSETWINDOWEXTEX = EMRSETVIEWPORTEXTEX*;
	...
	alias TEMRSETVIEWPORTORGEX = EMRSETVIEWPORTORGEX;
	alias PEMRSETVIEWPORTORGEX = EMRSETVIEWPORTORGEX*;
	alias EMRSETWINDOWORGEX = EMRSETVIEWPORTORGEX;
	alias TEMRSETWINDOWORGEX = EMRSETVIEWPORTORGEX;
	alias PEMRSETWINDOWORGEX = EMRSETVIEWPORTORGEX*;
	alias EMRSETBRUSHORGEX = EMRSETVIEWPORTORGEX;
	alias TEMRSETBRUSHORGEX = EMRSETVIEWPORTORGEX;
	alias PEMRSETBRUSHORGEX = EMRSETVIEWPORTORGEX*;
	...
	alias TEMRSETWORLDTRANSFORM = EMRSETWORLDTRANSFORM;
	...
	alias TEMRSTRETCHBLT = EMRSTRETCHBLT;
	...
	alias TEMRSTRETCHDIBITS = EMRSTRETCHDIBITS;
	...
	alias TEMRABORTPATH = EMRABORTPATH;
	alias PEMRABORTPATH = EMRABORTPATH*;
	alias TAGABORTPATH = EMRABORTPATH;
	alias TABORTPATH = EMRABORTPATH;
	alias EMRBEGINPATH = EMRABORTPATH;
	alias TEMRBEGINPATH = EMRABORTPATH;
	alias PEMRBEGINPATH = EMRABORTPATH*;
	alias EMRENDPATH = EMRABORTPATH;
	alias TEMRENDPATH = EMRABORTPATH;
	alias PEMRENDPATH = EMRABORTPATH*;
	alias EMRCLOSEFIGURE = EMRABORTPATH;
	alias TEMRCLOSEFIGURE = EMRABORTPATH;
	alias PEMRCLOSEFIGURE = EMRABORTPATH*;
	alias EMRFLATTENPATH = EMRABORTPATH;
	alias TEMRFLATTENPATH = EMRABORTPATH;
	alias PEMRFLATTENPATH = EMRABORTPATH*;
	alias EMRWIDENPATH = EMRABORTPATH;
	alias TEMRWIDENPATH = EMRABORTPATH;
	alias PEMRWIDENPATH = EMRABORTPATH*;
	alias EMRSETMETARGN = EMRABORTPATH;
	alias TEMRSETMETARGN = EMRABORTPATH;
	alias PEMRSETMETARGN = EMRABORTPATH*;
	alias EMRSAVEDC = EMRABORTPATH;
	alias TEMRSAVEDC = EMRABORTPATH;
	alias PEMRSAVEDC = EMRABORTPATH*;
	alias EMRREALIZEPALETTE = EMRABORTPATH;
	alias TEMRREALIZEPALETTE = EMRABORTPATH;
	alias PEMRREALIZEPALETTE = EMRABORTPATH*;
	...
	alias TEMRSELECTCLIPPATH = EMRSELECTCLIPPATH;
	alias PEMRSELECTCLIPPATH = EMRSELECTCLIPPATH*;
	alias EMRSETBKMODE = EMRSELECTCLIPPATH;
	alias TEMRSETBKMODE = EMRSELECTCLIPPATH;
	alias PEMRSETBKMODE = EMRSELECTCLIPPATH*;
	alias EMRSETMAPMODE = EMRSELECTCLIPPATH;
	alias TEMRSETMAPMODE = EMRSELECTCLIPPATH;
	alias PEMRSETMAPMODE = EMRSELECTCLIPPATH*;
	alias EMRSETPOLYFILLMODE = EMRSELECTCLIPPATH;
	alias TEMRSETPOLYFILLMODE = EMRSELECTCLIPPATH;
	alias PEMRSETPOLYFILLMODE = EMRSELECTCLIPPATH*;
	alias EMRSETROP2 = EMRSELECTCLIPPATH;
	alias TEMRSETROP2 = EMRSELECTCLIPPATH;
	alias PEMRSETROP2 = EMRSELECTCLIPPATH*;
	alias EMRSETSTRETCHBLTMODE = EMRSELECTCLIPPATH;
	alias TEMRSETSTRETCHBLTMODE = EMRSELECTCLIPPATH;
	alias PEMRSETSTRETCHBLTMODE = EMRSELECTCLIPPATH*;
	alias EMRSETTEXTALIGN = EMRSELECTCLIPPATH;
	alias TEMRSETTEXTALIGN = EMRSELECTCLIPPATH;
	alias PEMRSETTEXTALIGN = EMRSELECTCLIPPATH*;
	alias EMRENABLEICM = EMRSELECTCLIPPATH;
	alias TEMRENABLEICM = EMRSELECTCLIPPATH;
	alias PEMRENABLEICM = EMRSELECTCLIPPATH*;
	...
	alias _ENCORRECTTEXT = ENCORRECTTEXT;
	alias TENCORRECTTEXT = ENCORRECTTEXT;
	alias PENCORRECTTEXT = ENCORRECTTEXT*;
	...
	alias _ENDROPFILES = ENDROPFILES;
	alias TENDROPFILES = ENDROPFILES;
	alias PENDROPFILES = ENDROPFILES*;
	...
	alias TENSAVECLIPBOARD = ENSAVECLIPBOARD;
	alias PENSAVECLIPBOARD = ENSAVECLIPBOARD*;
	...
	alias TENOLEOPFAILED = ENOLEOPFAILED;
	alias PENOLEOPFAILED = ENOLEOPFAILED*;
	...
	alias LPENHMETAHEADER = ENHMETAHEADER*;
	alias TAGENHMETAHEADER = ENHMETAHEADER;
	alias TENHMETAHEADER = ENHMETAHEADER;
	alias PENHMETAHEADER = ENHMETAHEADER*;
	...
	alias LPENHMETARECORD = ENHMETARECORD*;
	alias TAGENHMETARECORD = ENHMETARECORD;
	alias TENHMETARECORD = ENHMETARECORD;
	alias PENHMETARECORD = ENHMETARECORD*;
	...
	alias _ENPROTECTED = ENPROTECTED;
	alias TENPROTECTED = ENPROTECTED;
	alias PENPROTECTED = ENPROTECTED*;
	...
	alias TSERVICESTATUS = SERVICE_STATUS;
	alias PSERVICESTATUS = SERVICE_STATUS*;
	...
	alias _ENUM_SERVICE_STATUS = ENUM_SERVICE_STATUS;
	alias TENUMSERVICESTATUS = ENUM_SERVICE_STATUS;
	alias PENUMSERVICESTATUS = ENUM_SERVICE_STATUS*;
	...
	alias TAGENUMLOGFONT = ENUMLOGFONT;
	alias TENUMLOGFONT = ENUMLOGFONT;
	alias PENUMLOGFONT = ENUMLOGFONT*;
	alias TAGENUMLOGFONTEX = ENUMLOGFONTEX;
	alias TENUMLOGFONTEX = ENUMLOGFONTEX;
	alias PENUMLOGFONTEX = ENUMLOGFONTEX*;
	...
	alias TEVENTLOGRECORD = EVENTLOGRECORD;
	...
	alias TEVENTMSG = EVENTMSG;
	alias PEVENTMSG = EVENTMSG*;
	...
	alias TEXCEPTIONPOINTERS = EXCEPTION_POINTERS;
	alias PEXCEPTIONPOINTERS = EXCEPTION_POINTERS*;
	...
	alias LPEXT_BUTTON = EXT_BUTTON*;
	alias _EXT_BUTTON = EXT_BUTTON;
	alias TEXTBUTTON = EXT_BUTTON;
	alias PEXTBUTTON = EXT_BUTTON*;
	...
	alias TFILTERKEYS = FILTERKEYS;
	alias PFILTERKEYS = FILTERKEYS*;
	...
	alias TFINDNAMEBUFFER = FIND_NAME_BUFFER;
	alias PFINDNAMEBUFFER = FIND_NAME_BUFFER*;
	...
	alias TFINDNAMEHEADER = FIND_NAME_HEADER;
	alias PFINDNAMEHEADER = FIND_NAME_HEADER*;
	...
	alias TFINDREPLACE = FINDREPLACE;
	alias PFINDREPLACE = FINDREPLACE*;
	...
	alias _FINDTEXT = TFINDTEXT;
	alias FINDTEXT = TFINDTEXT;
	alias PFINDTEXT = TFINDTEXT*;
	...
	alias _FINDTEXTEX = FINDTEXTEX;
	alias TFINDTEXTEX = FINDTEXTEX;
	alias PFINDTEXTEX = FINDTEXTEX*;
	...
	alias _FMS_GETDRIVEINFO = FMS_GETDRIVEINFO;
	alias TFMSGETDRIVEINFO = FMS_GETDRIVEINFO;
	alias PFMSGETDRIVEINFO = FMS_GETDRIVEINFO*;
	...
	alias _FMS_GETFILESEL = FMS_GETFILESEL;
	alias TFMSGETFILESEL = FMS_GETFILESEL;
	alias PFMSGETFILESEL = FMS_GETFILESEL*;
	...
	alias _FMS_LOAD = FMS_LOAD;
	alias TFMSLOAD = FMS_LOAD;
	alias PFMSLOAD = FMS_LOAD*;
	...
	alias _FMS_TOOLBARLOAD = FMS_TOOLBARLOAD;
	alias TFMSTOOLBARLOAD = FMS_TOOLBARLOAD;
	alias PFMSTOOLBARLOAD = FMS_TOOLBARLOAD*;
	...
	alias TFOCUSEVENTRECORD = FOCUS_EVENT_RECORD;
	alias PFOCUSEVENTRECORD = FOCUS_EVENT_RECORD*;
	...
	alias _FORM_INFO_1 = FORM_INFO_1;
	alias TFORMINFO1 = FORM_INFO_1;
	alias PFORMINFO1 = FORM_INFO_1*;
	...
	alias TFORMATPARAMETERS = FORMAT_PARAMETERS;
	alias PFORMATPARAMETERS = FORMAT_PARAMETERS*;
	...
	alias _FORMATRANGE = FORMATRANGE;
	alias TFORMATRANGE = FORMATRANGE;
	alias PFORMATRANGE = FORMATRANGE*;
	...
	alias TAGGCP_RESULTS = GCP_RESULTS;
	alias TGCPRESULTS = GCP_RESULTS;
	alias PGCPRESULTS = GCP_RESULTS*;
	...
	alias TGENERICMAPPING = GENERIC_MAPPING;
	alias PGENERICMAPPING = GENERIC_MAPPING*;
	...
	alias TGLYPHMETRICS = GLYPHMETRICS;
	alias PGLYPHMETRICS = GLYPHMETRICS*;
	...
	alias THANDLETABLE = HANDLETABLE;
	alias LPHANDLETABLE = HANDLETABLE*;
	...
	alias _HD_HITTESTINFO = HD_HITTESTINFO;
	alias THDHITTESTINFO = HD_HITTESTINFO;
	alias HDHITTESTINFO = HD_HITTESTINFO;
	alias PHDHITTESTINFO = HD_HITTESTINFO*;
	...
	alias _HD_ITEM = HD_ITEM;
	alias THDITEM = HD_ITEM;
	alias PHDITEM = HD_ITEM*;
	...
	alias _HD_LAYOUT = HD_LAYOUT;
	alias THDLAYOUT = HD_LAYOUT;
	alias HDLAYOUT = HD_LAYOUT;
	alias PHDLAYOUT = HD_LAYOUT*;
	...
	alias _HD_NOTIFY = HD_NOTIFY;
	alias THDNOTIFY = HD_NOTIFY;
	alias PHDNOTIFY = HD_NOTIFY*;
	...
	alias THELPINFO = HELPINFO;
	alias PHELPINFO = HELPINFO*;
	...
	alias THELPWININFO = HELPWININFO;
	alias PHELPWININFO = HELPWININFO*;
	...
	alias TAGHIGHCONTRAST = HIGHCONTRAST;
	alias THIGHCONTRAST = HIGHCONTRAST;
	alias PHIGHCONTRAST = HIGHCONTRAST*;
	...
	alias TAGICONMETRICS = ICONMETRICS;
	alias TICONMETRICS = ICONMETRICS;
	alias PICONMETRICS = ICONMETRICS*;
	...
	alias TIMAGEINFO = IMAGEINFO;
	alias PIMAGEINFO = IMAGEINFO*;
	...
	alias TKEYEVENTRECORD = KEY_EVENT_RECORD;
	alias PKEYEVENTRECORD = KEY_EVENT_RECORD*;
	...
	alias TMOUSEEVENTRECORD = MOUSE_EVENT_RECORD;
	alias PMOUSEEVENTRECORD = MOUSE_EVENT_RECORD*;
	...
	alias TWINDOWBUFFERSIZERECORD = WINDOW_BUFFER_SIZE_RECORD;
	alias PWINDOWBUFFERSIZERECORD = WINDOW_BUFFER_SIZE_RECORD*;
	...
	alias TMENUEVENTRECORD = MENU_EVENT_RECORD;
	alias PMENUEVENTRECORD = MENU_EVENT_RECORD*;
	...
	alias TINPUTRECORD = INPUT_RECORD;
	alias PINPUTRECORD = INPUT_RECORD*;
	...
	alias TSYSTEMTIME = SYSTEMTIME;
	alias PSYSTEMTIME = SYSTEMTIME*;
	...
	alias _JOB_INFO_1 = JOB_INFO_1;
	alias TJOBINFO1 = JOB_INFO_1;
	alias PJOBINFO1 = JOB_INFO_1*;
	...
	alias TSIDIDENTIFIERAUTHORITY = SID_IDENTIFIER_AUTHORITY;
	alias PSIDIDENTIFIERAUTHORITY = SID_IDENTIFIER_AUTHORITY*;
	...
	alias TSECURITYDESCRIPTORCONTROL = SECURITY_DESCRIPTOR_CONTROL;
	alias PSECURITYDESCRIPTORCONTROL = SECURITY_DESCRIPTOR_CONTROL*;
	...
	alias TSECURITYDESCRIPTOR = SECURITY_DESCRIPTOR;
	alias PSECURITYDESCRIPTOR = SECURITY_DESCRIPTOR*;
	...
	alias _JOB_INFO_2 = JOB_INFO_2;
	alias TJOBINFO2 = JOB_INFO_2;
	alias PJOBINFO2 = JOB_INFO_2*;
	...
	alias TKERNINGPAIR = KERNINGPAIR;
	alias PKERNINGPAIR = KERNINGPAIR*;
	...
	alias TLANAENUM = LANA_ENUM;
	alias PLANAENUM = LANA_ENUM*;
	...
	alias LPLDT_ENTRY = LDT_ENTRY*;
	alias PLDT_ENTRY = LDT_ENTRY*;
	alias _LDT_ENTRY = LDT_ENTRY;
	alias TLDTENTRY = LDT_ENTRY;
	alias PLDTENTRY = LDT_ENTRY*;
	enum : DWORD {
	...
	alias TLOCALESIGNATURE = LOCALESIGNATURE;
	alias PLOCALESIGNATURE = LOCALESIGNATURE*;

	struct LOCALGROUP_MEMBERS_INFO_0 {
		PSID lgrmi0_sid;
	}

	alias _LOCALGROUP_MEMBERS_INFO_0 = LOCALGROUP_MEMBERS_INFO_0;
	alias TLOCALGROUPMEMBERSINFO0 = LOCALGROUP_MEMBERS_INFO_0;
	alias PLOCALGROUPMEMBERSINFO0 = LOCALGROUP_MEMBERS_INFO_0*;

	struct LOCALGROUP_MEMBERS_INFO_3 {
		LPWSTR lgrmi3_domainandname;
	}

	alias _LOCALGROUP_MEMBERS_INFO_3 = LOCALGROUP_MEMBERS_INFO_3;
	alias TLOCALGROUPMEMBERSINFO3 = LOCALGROUP_MEMBERS_INFO_3;
	alias PLOCALGROUPMEMBERSINFO3 = LOCALGROUP_MEMBERS_INFO_3*;
	alias FXPT16DOT16 = int;
	alias LPFXPT16DOT16 = FXPT16DOT16*;
	alias TFXPT16DOT16 = FXPT16DOT16;
	alias PFXPT16DOT16 = FXPT16DOT16*;
	struct LUID {
		DWORD LowPart;
		LONG HighPart;
	}
	...
	alias TLUIDANDATTRIBUTES = LUID_AND_ATTRIBUTES;
	alias PLUIDANDATTRIBUTES = LUID_AND_ATTRIBUTES*;
	alias LUID_AND_ATTRIBUTES_ARRAY = LUID_AND_ATTRIBUTES[1 + ANYSIZE_ARRAY - 1];
	alias PLUID_AND_ATTRIBUTES_ARRAY = LUID_AND_ATTRIBUTES_ARRAY*;
	alias TLUIDANDATTRIBUTESARRAY = LUID_AND_ATTRIBUTES_ARRAY;
	alias PLUIDANDATTRIBUTESARRAY = LUID_AND_ATTRIBUTES_ARRAY*;
	...
	alias LPLVCOLUMN = LVCOLUMN*;
	alias LV_COLUMN = LVCOLUMN;
	alias _LV_COLUMN = LV_COLUMN;
	alias TLVCOLUMN = LV_COLUMN;
	alias PLVCOLUMN = LV_COLUMN*;
	...
	alias LV_ITEM = LVITEM;

	alias _LV_ITEM = LV_ITEM;
	alias TLVITEM = LV_ITEM;
	alias PLVITEM = LV_ITEM*;
	...
	alias TAGLV_DISPINFO = LV_DISPINFO;
	alias TLVDISPINFO = LV_DISPINFO;
	alias PLVDISPINFO = LV_DISPINFO*;
	...
	alias _LV_FINDINFO = LV_FINDINFO;
	alias TLVFINDINFO = LV_FINDINFO;
	alias PLVFINDINFO = LV_FINDINFO*;
	...
	alias _LV_HITTESTINFO = LV_HITTESTINFO;
	alias TLVHITTESTINFO = LV_HITTESTINFO;
	alias PLVHITTESTINFO = LV_HITTESTINFO*;
	...
	alias TAGLV_KEYDOWN = LV_KEYDOWN;
	alias TLVKEYDOWN = LV_KEYDOWN;
	alias PLVKEYDOWN = LV_KEYDOWN*;
	...
	alias TAGMDICREATESTRUCT = MDICREATESTRUCT;
	alias TMDICREATESTRUCT = MDICREATESTRUCT;
	alias PMDICREATESTRUCT = MDICREATESTRUCT*;
	...
	alias TMEASUREITEMSTRUCT = MEASUREITEMSTRUCT;
	alias PMEASUREITEMSTRUCT = MEASUREITEMSTRUCT*;
	...
	alias TMEMORYBASICINFORMATION = MEMORY_BASIC_INFORMATION;
	alias PMEMORYBASICINFORMATION = MEMORY_BASIC_INFORMATION*;
	...
	alias TMEMORYSTATUS = MEMORYSTATUS;
	alias PMEMORYSTATUS = MEMORYSTATUS*;
	...
	alias TMENUXTEMPLATEHEADER = MENUEX_TEMPLATE_HEADER;
	alias PMENUXTEMPLATEHEADER = MENUEX_TEMPLATE_HEADER*;
	...
	alias TMENUEXTEMPLATEITEM = MENUEX_TEMPLATE_ITEM;
	alias PMENUEXTEMPLATEITEM = MENUEX_TEMPLATE_ITEM*;
    ...
    */
	...
	alias PMENUITEMINFOA = MENUITEMINFOA*, LPMENUITEMINFOA = MENUITEMINFOA*;
	...
	alias PMENUITEMINFOW = MENUITEMINFOW*, LPMENUITEMINFOW = MENUITEMINFOW*;
	version (Win32SansUnicode) {
	...
	alias LPCMENUITEMINFO = MENUITEMINFO*;
	alias TAGMENUITEMINFO = MENUITEMINFO;
	alias TMENUITEMINFO = MENUITEMINFO;
	alias TMENUITEMINFOA = MENUITEMINFO;
	alias PMENUITEMINFO = MENUITEMINFO*;
	...
	alias TMENUITEMTEMPLATE = MENUITEMTEMPLATE;
	...
	alias TMENUITEMTEMPLATEHEADER = MENUITEMTEMPLATEHEADER;
	alias PMENUITEMTEMPLATEHEADER = MENUITEMTEMPLATEHEADER*;

	struct MENUTEMPLATE {
	}

	alias LPMENUTEMPLATE = MENUTEMPLATE*;
	alias TMENUTEMPLATE = MENUTEMPLATE;
	alias PMENUTEMPLATE = MENUTEMPLATE*;
	...
	alias TMETAFILEPICT = METAFILEPICT;
	alias PMETAFILEPICT = METAFILEPICT*;
	...
	alias TMINIMIZEDMETRICS = MINIMIZEDMETRICS;
	alias PMINIMIZEDMETRICS = MINIMIZEDMETRICS*;
	...
	alias LPMODEMDEVCAPS = MODEMDEVCAPS*;
	alias TMODEMDEVCAPS = MODEMDEVCAPS;
	alias PMODEMDEVCAPS = MODEMDEVCAPS*;
	alias MODEMDEVCAPS_TAG = MODEMDEVCAPS;
	...
	alias LPMODEMSETTINGS = MODEMSETTINGS*;
	alias TMODEMSETTINGS = MODEMSETTINGS;
	alias PMODEMSETTINGS = MODEMSETTINGS*;
	alias MODEMSETTINGS_TAG = MODEMSETTINGS;
	...
	alias TMONCBSTRUCT = MONCBSTRUCT;
	...
	alias TMONCONVSTRUCT = MONCONVSTRUCT;
	...
	alias TMONERRSTRUCT = MONERRSTRUCT;
	...
	alias TAGMONHSZSTRUCT = MONHSZSTRUCT;
	alias TMONHSZSTRUCT = MONHSZSTRUCT;
	...
	alias _MONITOR_INFO_1 = MONITOR_INFO_1;
	alias TMONITORINFO1 = MONITOR_INFO_1;
	alias PMONITORINFO1 = MONITOR_INFO_1*;
	...
	alias _MONITOR_INFO_2 = MONITOR_INFO_2;
	alias TMONITORINFO2 = MONITOR_INFO_2;
	alias PMONITORINFO2 = MONITOR_INFO_2*;
	...
	alias TMONLINKSTRUCT = MONLINKSTRUCT;
	...
	alias TMONMSGSTRUCT = MONMSGSTRUCT;
	...
	alias LPMOUSEHOOKSTRUCT = MOUSEHOOKSTRUCT*;
	alias TAGMOUSEHOOKSTRUCT = MOUSEHOOKSTRUCT;
	alias TMOUSEHOOKSTRUCT = MOUSEHOOKSTRUCT;
	alias PMOUSEHOOKSTRUCT = MOUSEHOOKSTRUCT*;
	...
	alias TMOUSEKEYS = MOUSEKEYS;
	alias PMOUSEKEYS = MOUSEKEYS*;
	...
	alias TMSGBOXCALLBACK = MSGBOXCALLBACK;
	...
	alias LPMSGBOXPARAMS = MSGBOXPARAMS*;
	alias TMSGBOXPARAMS = MSGBOXPARAMS;
	alias TMSGBOXPARAMSA = MSGBOXPARAMS;
	alias PMSGBOXPARAMS = MSGBOXPARAMS*;
	...
	alias _MSGFILTER = MSGFILTER;
	alias TMSGFILTER = MSGFILTER;
	alias PMSGFILTER = MSGFILTER*;
	...
	alias TAGMULTIKEYHELP = MULTIKEYHELP;
	alias TMULTIKEYHELP = MULTIKEYHELP;
	alias PMULTIKEYHELP = MULTIKEYHELP*;
	...
	alias TNAMEBUFFER = NAME_BUFFER;
	alias PNAMEBUFFER = NAME_BUFFER*;
	alias P_NCB = _NCB*;
	...
	alias _NCCALCSIZE_PARAMS = NCCALCSIZE_PARAMS;
	alias TNCCALCSIZEPARAMS = NCCALCSIZE_PARAMS;
	alias PNCCALCSIZEPARAMS = NCCALCSIZE_PARAMS*;
	...
	alias _NDDESHAREINFO = NDDESHAREINFO;
	alias TNDDESHAREINFO = NDDESHAREINFO;
	alias PNDDESHAREINFO = NDDESHAREINFO*;
	...
	alias _NETRESOURCE = NETRESOURCE;
	alias TNETRESOURCE = NETRESOURCE;
	alias TNETRESOURCEA = NETRESOURCE;
	alias PNETRESOURCE = NETRESOURCE*;
	alias PNETRESOURCEA = NETRESOURCE*;
	...
	alias TAGNEWCPLINFO = NEWCPLINFO;
	alias TNEWCPLINFO = NEWCPLINFO;
	alias PNEWCPLINFO = NEWCPLINFO*;
	...
	alias TNEWTEXTMETRIC = NEWTEXTMETRIC;
	alias PNEWTEXTMETRIC = NEWTEXTMETRIC*;
	alias LPNEWTEXTMETRIC = NEWTEXTMETRIC*;
	alias TAGNEWTEXTMETRIC = NEWTEXTMETRIC;
	alias TAGNEWTEXTMETRICEX = NEWTEXTMETRICEX;
	alias TNEWTEXTMETRICEX = NEWTEXTMETRICEX;
	alias PNEWTEXTMETRICEX = NEWTEXTMETRICEX*;
	...
	alias TAGNM_LISTVIEW = NM_LISTVIEW;
	alias TNMLISTVIEW = NM_LISTVIEW;
	alias PNMLISTVIEW = NM_LISTVIEW*;
	...
	alias LPTVITEM = TVITEM*;
	alias LPTV_ITEM = TVITEM*;
	alias _TV_ITEM = TVITEM;
	alias TTVITEM = TVITEM;
	alias TV_ITEM = TVITEM;
	alias PTVITEM = TVITEM*;
	...
	alias PNMTREEVIEW = NMTREEVIEW*;
	alias LPNMTREEVIEW = NMTREEVIEW*;
	alias NM_TREEVIEW = NMTREEVIEW;
	alias LPNM_TREEVIEW = NM_TREEVIEW*;
	alias _NM_TREEVIEW = NM_TREEVIEW;
	alias TNMTREEVIEW = NM_TREEVIEW;
	...
	alias _NM_UPDOWN = NM_UPDOWNW;
	alias TNMUPDOWN = NM_UPDOWNW;
	alias PNMUPDOWN = NM_UPDOWNW*;
	alias NMUPDOWN = NM_UPDOWNW;
	...
	alias TAGNONCLIENTMETRICS = NONCLIENTMETRICS;
	alias TNONCLIENTMETRICS = NONCLIENTMETRICS;
	alias PNONCLIENTMETRICS = NONCLIENTMETRICS*;
	...
	alias _SERVICE_ADDRESS = SERVICE_ADDRESS;
	alias TSERVICEADDRESS = SERVICE_ADDRESS;
	alias PSERVICEADDRESS = SERVICE_ADDRESS*;
	...
	alias LPSERVICE_ADDRESSES = SERVICE_ADDRESSES*;
	alias _SERVICE_ADDRESSES = SERVICE_ADDRESSES;
	alias TSERVICEADDRESSES = SERVICE_ADDRESSES;
	alias PSERVICEADDRESSES = SERVICE_ADDRESSES*;
	...
	alias TGUID = __GUID;
	alias PGUID = __GUID*;
	alias __CLSID = __GUID;
	alias LPCLSID = __CLSID*;
	alias TCLSID = __CLSID;
	alias PCLSID = __CLSID*;
	...
	alias TSERVICEINFO = SERVICE_INFO;
	alias PSERVICEINFO = SERVICE_INFO*;
	...
	alias _NS_SERVICE_INFO = NS_SERVICE_INFO;
	alias TNSSERVICEINFO = NS_SERVICE_INFO;
	alias PNSSERVICEINFO = NS_SERVICE_INFO*;
	...
	alias _NUMBERFMT = NUMBERFMT;
	alias TNUMBERFMT = NUMBERFMT;
	alias PNUMBERFMT = NUMBERFMT*;
	...
	alias LPOFSTRUCT = OFSTRUCT*;
	alias _OFSTRUCT = OFSTRUCT;
	alias TOFSTRUCT = OFSTRUCT;
	alias POFSTRUCT = OFSTRUCT*;
	...
	alias TOPENFILENAME = OPENFILENAME;
	alias POPENFILENAME = OPENFILENAME*;
	alias TAGOFN = OPENFILENAME;
	alias TOFN = OPENFILENAME;
	alias POFN = OPENFILENAME*;
	...
	alias _OFNOTIFY = OFNOTIFY;
	alias TOFNOTIFY = OFNOTIFY;
	alias POFNOTIFY = OFNOTIFY*;
	...
	alias POSVERSIONINFOA = OSVERSIONINFOA*, LPOSVERSIONINFOA = OSVERSIONINFOA*;
	...
	alias POSVERSIONINFOW = OSVERSIONINFOW*, LPOSVERSIONINFOW = OSVERSIONINFOW*;
	version (Win32SansUnicode) {
	...
	alias _OSVERSIONINFO = OSVERSIONINFO;
	alias TOSVERSIONINFO = OSVERSIONINFO;
	...
	alias LPTEXTMETRIC = TEXTMETRIC*;
	alias TAGTEXTMETRIC = TEXTMETRIC;
	alias TTEXTMETRIC = TEXTMETRIC;
	alias PTEXTMETRIC = TEXTMETRIC*;
	...
	alias LPOUTLINETEXTMETRIC = OUTLINETEXTMETRIC*;
	alias _OUTLINETEXTMETRIC = OUTLINETEXTMETRIC;
	alias TOUTLINETEXTMETRIC = OUTLINETEXTMETRIC;
	alias POUTLINETEXTMETRIC = OUTLINETEXTMETRIC*;
	version (Win32SansUnicode) {
		alias OUTLINETEXTMETRIC = OUTLINETEXTMETRICA;
		alias LPOUTLINETEXTMETRICA = OUTLINETEXTMETRICA*;
	}
	else {
		alias OUTLINETEXTMETRIC = OUTLINETEXTMETRICW;
		alias LPOUTLINETEXTMETRICW = OUTLINETEXTMETRICW*;
	}
	...
	alias TOVERLAPPED = OVERLAPPED;
	alias POVERLAPPED = OVERLAPPED*;
	...
	alias PAGESETUPDLG = TPAGESETUPDLG;
	alias LPPAGESETUPDLG = TPAGESETUPDLG*;
	alias PPAGESETUPDLG = TPAGESETUPDLG*;
	alias TAGPSD = TPAGESETUPDLG;
	alias TPSD = TPAGESETUPDLG;
	alias PPSD = TPAGESETUPDLG*;
	...
	alias TPAINTSTRUCT = PAINTSTRUCT;
	...
	alias _PARAFORMAT = PARAFORMAT;
	alias TPARAFORMAT = PARAFORMAT;
	alias PPARAFORMAT = PARAFORMAT*;
	...
	alias TPERFCOUNTERBLOCK = PERF_COUNTER_BLOCK;
	alias PPERFCOUNTERBLOCK = PERF_COUNTER_BLOCK*;
	...
	alias TPERFCOUNTERDEFINITION = PERF_COUNTER_DEFINITION;
	alias PPERFCOUNTERDEFINITION = PERF_COUNTER_DEFINITION*;
	...
	alias TPERFDATABLOCK = PERF_DATA_BLOCK;
	alias PPERFDATABLOCK = PERF_DATA_BLOCK*;
	...
	alias TPERFINSTANCEDEFINITION = PERF_INSTANCE_DEFINITION;
	alias PPERFINSTANCEDEFINITION = PERF_INSTANCE_DEFINITION;
	...
	alias TPERFOBJECTTYPE = PERF_OBJECT_TYPE;
	alias PPERFOBJECTTYPE = PERF_OBJECT_TYPE*;
	...
	alias _POLYTEXT = POLYTEXT;
	alias TPOLYTEXT = POLYTEXT;
	alias PPOLYTEXT = POLYTEXT*;

	struct PORT_INFO_1 {
		LPTSTR pName;
	}

	alias _PORT_INFO_1 = PORT_INFO_1;
	alias TPORTINFO1 = PORT_INFO_1;
	alias PPORTINFO1 = PORT_INFO_1*;
	...
	alias _PORT_INFO_2 = PORT_INFO_2;
	alias TPORTINFO2 = PORT_INFO_2;
	alias PPORTINFO2 = PORT_INFO_2*;
	...
	alias TPREVENTMEDIAREMOVAL = PREVENT_MEDIA_REMOVAL;
	alias PPREVENTMEDIAREMOVAL = PREVENT_MEDIA_REMOVAL*;
	...
	alias PPRINTDLG = PRINTDLG*;
	alias TAGPD = PRINTDLG;
	alias TPD = PRINTDLG;
	alias PPD = PRINTDLG*;
	...
	alias _PRINTER_DEFAULTS = PRINTER_DEFAULTS;
	alias TPRINTERDEFAULTS = PRINTER_DEFAULTS;
	alias PPRINTERDEFAULTS = PRINTER_DEFAULTS*;
	...
	alias LPPRINTER_INFO_1 = PRINTER_INFO_1*;
	alias PPRINTER_INFO_1 = PRINTER_INFO_1*;
	alias _PRINTER_INFO_1 = PRINTER_INFO_1;
	alias TPRINTERINFO1 = PRINTER_INFO_1;
	alias PPRINTERINFO1 = PRINTER_INFO_1*;
	...
	alias _PRINTER_INFO_2 = PRINTER_INFO_2;
	alias TPRINTERINFO2 = PRINTER_INFO_2;
	alias PPRINTERINFO2 = PRINTER_INFO_2*;
	...
	alias TPRINTERINFO3 = PRINTER_INFO_3;
	alias PPRINTERINFO3 = PRINTER_INFO_3*;
	...
	alias _PRINTER_INFO_4 = PRINTER_INFO_4;
	alias TPRINTERINFO4 = PRINTER_INFO_4;
	alias PPRINTERINFO4 = PRINTER_INFO_4*;
	...
	alias _PRINTER_INFO_5 = PRINTER_INFO_5;
	alias TPRINTERINFO5 = PRINTER_INFO_5;
	alias PPRINTERINFO5 = PRINTER_INFO_5*;
	...
	alias TPRINTERNOTIFYINFODATA = PRINTER_NOTIFY_INFO_DATA;
	alias PPRINTERNOTIFYINFODATA = PRINTER_NOTIFY_INFO_DATA*;
	...
	alias TPRINTERNOTIFYINFO = PRINTER_NOTIFY_INFO;
	alias PPRINTERNOTIFYINFO = PRINTER_NOTIFY_INFO*;
	...
	alias PPRINTER_NOTIFY_OPTIONS_TYPE = PRINTER_NOTIFY_OPTIONS_TYPE*;
	alias _PRINTER_NOTIFY_OPTIONS_TYPE = PRINTER_NOTIFY_OPTIONS_TYPE;
	alias TPRINTERNOTIFYOPTIONSTYPE = PRINTER_NOTIFY_OPTIONS_TYPE;
	alias PPRINTERNOTIFYOPTIONSTYPE = PRINTER_NOTIFY_OPTIONS_TYPE*;
	...
	alias TPRINTERNOTIFYOPTIONS = PRINTER_NOTIFY_OPTIONS;
	alias PPRINTERNOTIFYOPTIONS = PRINTER_NOTIFY_OPTIONS*;

	struct PRINTPROCESSOR_INFO_1 {
		LPTSTR pName;
	}

	alias _PRINTPROCESSOR_INFO_1 = PRINTPROCESSOR_INFO_1;
	alias TPRINTPROCESSORINFO1 = PRINTPROCESSOR_INFO_1;
	alias PPRINTPROCESSORINFO1 = PRINTPROCESSOR_INFO_1*;
	...
	alias LPPRIVILEGE_SET = PRIVILEGE_SET*;
	alias PPRIVILEGE_SET = PRIVILEGE_SET*;
	alias _PRIVILEGE_SET = PRIVILEGE_SET;
	alias TPRIVILEGESET = PRIVILEGE_SET;
	alias PPRIVILEGESET = PRIVILEGE_SET*;
	...
	alias LPPROCESS_HEAP_ENTRY = PROCESS_HEAPENTRY*;
	alias _PROCESS_HEAP_ENTRY = PROCESS_HEAPENTRY;
	alias TPROCESSHEAPENTRY = PROCESS_HEAPENTRY;
	alias PPROCESSHEAPENTRY = PROCESS_HEAPENTRY*;
	...
	alias TPROCESSINFORMATION = PROCESS_INFORMATION;
	alias PPROCESSINFORMATION = PROCESS_INFORMATION*;
	extern (Windows) {
		alias LPFNPSPCALLBACK = UINT function(HWND, UINT, LPVOID);
	}
	alias TFNPSPCALLBACK = LPFNPSPCALLBACK;
	...
	alias LPPROPSHEETPAGE = PROPSHEETPAGE*;
	alias LPCPROPSHEETPAGE = PROPSHEETPAGE*;
	alias _PROPSHEETPAGE = PROPSHEETPAGE;
	alias TPROPSHEETPAGE = PROPSHEETPAGE;
	alias PPROPSHEETPAGE = PROPSHEETPAGE*;

	struct EMPTYRECORD {
	}

	alias HPROPSHEETPAGE = EMPTYRECORD*;
	...
	alias LPPROPSHEETHEADER = PROPSHEETHEADER*;
	alias LPCPROPSHEETHEADER = PROPSHEETHEADER*;
	alias _PROPSHEETHEADER = PROPSHEETHEADER;
	alias TPROPSHEETHEADER = PROPSHEETHEADER;
	alias PPROPSHEETHEADER = PROPSHEETHEADER*;
	extern (Windows) {
		alias LPFNADDPROPSHEETPAGE = WINBOOL function(HPROPSHEETPAGE, LPARAM);
		alias LPFNADDPROPSHEETPAGES = WINBOOL function(LPVOID, LPFNADDPROPSHEETPAGE, LPARAM);
	}
	alias TFNADDPROPSHEETPAGE = LPFNADDPROPSHEETPAGE;
	alias TFNADDPROPSHEETPAGES = LPFNADDPROPSHEETPAGES;
	...
	alias _PROTOCOL_INFO = PROTOCOL_INFO;
	alias TPROTOCOLINFO = PROTOCOL_INFO;
	alias PPROTOCOLINFO = PROTOCOL_INFO*;
	...
	alias _PROVIDOR_INFO_1 = PROVIDOR_INFO_1;
	alias TPROVIDORINFO1 = PROVIDOR_INFO_1;
	alias PPROVIDORINFO1 = PROVIDOR_INFO_1*;
	...
	alias TPSHNOTIFY = PSHNOTIFY;
	alias PPSHNOTIFY = PSHNOTIFY*;
	...
	alias _PUNCTUATION = PUNCTUATION;
	alias TPUNCTUATION = PUNCTUATION;
	alias PPUNCTUATION = PUNCTUATION*;
	...
	alias _QUERY_SERVICE_CONFIG = QUERY_SERVICE_CONFIG;
	alias TQUERYSERVICECONFIG = QUERY_SERVICE_CONFIG;
	alias PQUERYSERVICECONFIG = QUERY_SERVICE_CONFIG*;
	...
	alias _QUERY_SERVICE_LOCK_STATUS = QUERY_SERVICE_LOCK_STATUS;
	alias TQUERYSERVICELOCKSTATUS = QUERY_SERVICE_LOCK_STATUS;
	alias PQUERYSERVICELOCKSTATUS = QUERY_SERVICE_LOCK_STATUS*;
	...
	alias _RASAMB = RASAMB;
	alias TRASAMB = RASAMB;
	alias PRASAMB = RASAMB*;
	...
	alias _RASCONN = RASCONN;
	alias TRASCONN = RASCONN;
	alias PRASCONN = RASCONN*;
	...
	alias _RASCONNSTATUS = RASCONNSTATUS;
	alias TRASCONNSTATUS = RASCONNSTATUS;
	alias PRASCONNSTATUS = RASCONNSTATUS*;
	...
	alias _RASDIALEXTENSIONS = RASDIALEXTENSIONS;
	alias TRASDIALEXTENSIONS = RASDIALEXTENSIONS;
	alias PRASDIALEXTENSIONS = RASDIALEXTENSIONS*;
	...
	alias _RASDIALPARAMS = RASDIALPARAMS;
	alias TRASDIALPARAMS = RASDIALPARAMS;
	alias PRASDIALPARAMS = RASDIALPARAMS*;
	...
	alias _RASENTRYNAME = RASENTRYNAME;
	alias TRASENTRYNAME = RASENTRYNAME;
	alias PRASENTRYNAME = RASENTRYNAME*;
	...
	alias _RASPPPIP = RASPPPIP;
	alias TRASPPPIP = RASPPPIP;
	alias PRASPPPIP = RASPPPIP*;
	...
	alias _RASPPPIPX = RASPPPIPX;
	alias TRASPPPIPX = RASPPPIPX;
	alias PRASPPPIPX = RASPPPIPX*;
	...
	alias _RASPPPNBF = RASPPPNBF;
	alias TRASPPPNBF = RASPPPNBF;
	alias PRASPPPNBF = RASPPPNBF*;
	...
	alias TRASTERIZERSTATUS = RASTERIZER_STATUS;
	alias PRASTERIZERSTATUS = RASTERIZER_STATUS*;
	...
	alias TREASSIGNBLOCKS = REASSIGN_BLOCKS;
	alias PREASSIGNBLOCKS = REASSIGN_BLOCKS*;
	...
	alias _REMOTE_NAME_INFO = REMOTE_NAME_INFO;
	alias TREMOTENAMEINFO = REMOTE_NAME_INFO;
	alias PREMOTENAMEINFO = REMOTE_NAME_INFO*;
	...
	alias _REPASTESPECIAL = REPASTESPECIAL;
	alias TREPASTESPECIAL = REPASTESPECIAL;
	alias PREPASTESPECIAL = REPASTESPECIAL*;
	...
	alias _REQRESIZE = REQRESIZE;
	alias TREQRESIZE = REQRESIZE;
	alias PREQRESIZE = REQRESIZE*;
	...
	alias TRGNDATAHEADER = RGNDATAHEADER;
	alias PRGNDATAHEADER = RGNDATAHEADER*;
	alias LPRGNDATAHEADER = RGNDATAHEADER*;
	...
	alias LPRGNDATA = RGNDATA*;
	alias _RGNDATA = RGNDATA;
	alias TRGNDATA = RGNDATA;
	alias PRGNDATA = RGNDATA*;
	...
	alias LPCSCROLLINFO = SCROLLINFO*;
	alias TAGSCROLLINFO = SCROLLINFO;
	alias TSCROLLINFO = SCROLLINFO;
	alias PSCROLLINFO = SCROLLINFO*;
	...
	alias TSECURITYATTRIBUTES = SECURITY_ATTRIBUTES;
	alias PSECURITYATTRIBUTES = SECURITY_ATTRIBUTES*;
	alias SECURITY_INFORMATION = DWORD;
	alias PSECURITY_INFORMATION = SECURITY_INFORMATION*;
	alias TSECURITYINFORMATION = SECURITY_INFORMATION;
	alias PSECURITYINFORMATION = SECURITY_INFORMATION*;
	...
	alias _SELCHANGE = SELCHANGE;
	alias TSELCHANGE = SELCHANGE;
	alias PSELCHANGE = SELCHANGE*;
	...
	alias TAGSERIALKEYS = SERIALKEYS;
	alias TSERIALKEYS = SERIALKEYS;
	alias PSERIALKEYS = SERIALKEYS*;
	...
	alias TSERVICETABLEENTRY = SERVICE_TABLE_ENTRY;
	alias PSERVICETABLEENTRY = SERVICE_TABLE_ENTRY*;
	...
	alias _SERVICE_TYPE_VALUE_ABS = SERVICE_TYPE_VALUE_ABS;
	alias TSERVICETYPEVALUEABS = SERVICE_TYPE_VALUE_ABS;
	alias PSERVICETYPEVALUEABS = SERVICE_TYPE_VALUE_ABS*;
	...
	alias _SERVICE_TYPE_INFO_ABS = SERVICE_TYPE_INFO_ABS;
	alias TSERVICETYPEINFOABS = SERVICE_TYPE_INFO_ABS;
	alias PSERVICETYPEINFOABS = SERVICE_TYPE_INFO_ABS*;
	...
	alias TSESSIONBUFFER = SESSION_BUFFER;
	alias PSESSIONBUFFER = SESSION_BUFFER*;
	...
	alias TSESSIONHEADER = SESSION_HEADER;
	alias PSESSIONHEADER = SESSION_HEADER*;
	...
	alias TSETPARTITIONINFORMATION = SET_PARTITION_INFORMATION;
	alias PSETPARTITIONINFORMATION = SET_PARTITION_INFORMATION*;
	alias SHCONTF = int;
	enum {
		SHCONTF_FOLDERS = 32,
	...
	alias TAGSHCONTF = SHCONTF;
	alias TSHCONTF = SHCONTF;
	...
	alias _SHFILEINFO = SHFILEINFO;
	alias TSHFILEINFO = SHFILEINFO;
	alias PSHFILEINFO = SHFILEINFO*;

	alias FILEOP_FLAGS = ushort;
	alias TFILEOPFLAGS = FILEOP_FLAGS;
	alias PFILEOPFLAGS = FILEOP_FLAGS*;

	const FILEOP_FLAGS FOF_MULTIDESTFILES = 0x0001, FOF_CONFIRMMOUSE = 0x0002,
		FOF_SILENT = 0x0004, FOF_RENAMEONCOLLISION = 0x0008,
		FOF_NOCONFIRMATION = 0x0010,
		FOF_WANTMAPPINGHANDLE = 0x0020, FOF_ALLOWUNDO = 0x0040, FOF_FILESONLY = 0x0080, FOF_SIMPLEPROGRESS = 0x0100,
		FOF_NOCONFIRMMKDIR = 0x0200, FOF_NOERRORUI = 0x0400, FOF_NOCOPYSECURITYATTRIBS = 0x0800;
	...
	version (Win32SansUnicode) {
		alias LPSHFILEOPSTRUCT = SHFILEOPSTRUCTA*;
		alias _SHFILEOPSTRUCT = SHFILEOPSTRUCTA;
		alias TSHFILEOPSTRUCT = SHFILEOPSTRUCTA;
		alias PSHFILEOPSTRUCT = SHFILEOPSTRUCTA*;
		alias SHFILEOPSTRUCT = SHFILEOPSTRUCTA;
	}
	else {
		alias LPSHFILEOPSTRUCT = SHFILEOPSTRUCTW*;
		alias _SHFILEOPSTRUCT = SHFILEOPSTRUCTW;
		alias TSHFILEOPSTRUCT = SHFILEOPSTRUCTW;
		alias PSHFILEOPSTRUCT = SHFILEOPSTRUCTW*;
	...
	alias LPSHNAMEMAPPING = SHNAMEMAPPING*;
	alias _SHNAMEMAPPING = SHNAMEMAPPING;
	alias TSHNAMEMAPPING = SHNAMEMAPPING;
	alias PSHNAMEMAPPING = SHNAMEMAPPING*;
	...
	alias TSIDANDATTRIBUTES = SID_AND_ATTRIBUTES;
	alias PSIDANDATTRIBUTES = SID_AND_ATTRIBUTES*;
	alias SID_AND_ATTRIBUTES_ARRAY = SID_AND_ATTRIBUTES[1 + ANYSIZE_ARRAY - 1];
	alias PSID_AND_ATTRIBUTES_ARRAY = SID_AND_ATTRIBUTES_ARRAY*;
	alias TSIDANDATTRIBUTESARRAY = SID_AND_ATTRIBUTES_ARRAY;
	alias PSIDANDATTRIBUTESARRAY = SID_AND_ATTRIBUTES_ARRAY*;
	...
	alias TSINGLELISTENTRY = SINGLE_LIST_ENTRY;
	alias PSINGLELISTENTRY = SINGLE_LIST_ENTRY*;
	...
	alias TAGSOUNDSENTRY = SOUNDSENTRY;
	alias TSOUNDSENTRY = SOUNDSENTRY;
	alias PSOUNDSENTRY = SOUNDSENTRY*;
	...
	alias _STARTUPINFO = STARTUPINFO;
	alias TSTARTUPINFO = STARTUPINFO;
	alias PSTARTUPINFO = STARTUPINFO*;
	...
	alias TSTICKYKEYS = STICKYKEYS;
	alias PSTICKYKEYS = STICKYKEYS*;
	...
	alias _STRRET = STRRET;
	alias TSTRRET = STRRET;
	alias PSTRRET = STRRET*;
	...
	alias LPSTYLEBUF = STYLEBUF*;
	alias _TAGSTYLEBUF = STYLEBUF;
	alias TSTYLEBUF = STYLEBUF;
	alias PSTYLEBUF = STYLEBUF*;
	...
	alias TSTYLESTRUCT = STYLESTRUCT;
	alias PSTYLESTRUCT = STYLESTRUCT*;
	...
	alias TSYSTEMAUDITACE = SYSTEM_AUDIT_ACE;
	alias PSYSTEMAUDITACE = SYSTEM_AUDIT_ACE*;
	...
	alias TSYSTEMINFO = SYSTEM_INFO;
	alias PSYSTEMINFO = SYSTEM_INFO*;
	...
	alias TSYSTEMPOWERSTATUS = SYSTEM_POWER_STATUS;
	alias PSYSTEMPOWERSTATUS = SYSTEM_POWER_STATUS*;
	alias LPSYSTEM_POWER_STATUS = EMPTYRECORD*;
	...
	alias TTAPEERASE = TAPE_ERASE;
	alias PTAPEERASE = TAPE_ERASE*;
	...
	alias TTAPEGETDRIVEPARAMETERS = TAPE_GET_DRIVE_PARAMETERS;
	alias PTAPEGETDRIVEPARAMETERS = TAPE_GET_DRIVE_PARAMETERS*;
	...
	alias TTAPEGETMEDIAPARAMETERS = TAPE_GET_MEDIA_PARAMETERS;
	alias PTAPEGETMEDIAPARAMETERS = TAPE_GET_MEDIA_PARAMETERS*;
	...
	alias TTAPEGETPOSITION = TAPE_GET_POSITION;
	alias PTAPEGETPOSITION = TAPE_GET_POSITION*;
	...
	alias TTAPEPREPARE = TAPE_PREPARE;
	alias PTAPEPREPARE = TAPE_PREPARE*;
	...
	alias TTAPESETDRIVEPARAMETERS = TAPE_SET_DRIVE_PARAMETERS;
	alias PTAPESETDRIVEPARAMETERS = TAPE_SET_DRIVE_PARAMETERS*;
	...
	alias TTAPESETMEDIAPARAMETERS = TAPE_SET_MEDIA_PARAMETERS;
	alias PTAPESETMEDIAPARAMETERS = TAPE_SET_MEDIA_PARAMETERS*;
	...
	alias TTAPESETPOSITION = TAPE_SET_POSITION;
	alias PTAPESETPOSITION = TAPE_SET_POSITION*;
	...
	alias TTAPEWRITEMARKS = TAPE_WRITE_MARKS;
	alias PTAPEWRITEMARKS = TAPE_WRITE_MARKS*;
	...
	alias TTBADDBITMAP = TBADDBITMAP;
	alias PTBADDBITMAP = TBADDBITMAP*;
	...
	alias LPNMTOOLBAR = NMTOOLBAR*;

	alias TBNOTIFY = NMTOOLBAR;

	alias LPTBNOTIFY = TBNOTIFY*;
	alias TTBNOTIFY = TBNOTIFY;
	alias PTBNOTIFY = TBNOTIFY*;
	...
	alias TTBSAVEPARAMS = TBSAVEPARAMS;
	alias PTBSAVEPARAMS = TBSAVEPARAMS*;
	...
	alias _TC_HITTESTINFO = TC_HITTESTINFO;
	alias TTCHITTESTINFO = TC_HITTESTINFO;
	alias PTCHITTESTINFO = TC_HITTESTINFO*;
	...
	alias _TC_ITEM = TC_ITEM;
	alias TTCITEM = TC_ITEM;
	alias PTCITEM = TC_ITEM*;
	...
	alias _TC_ITEMHEADER = TC_ITEMHEADER;
	alias TTCITEMHEADER = TC_ITEMHEADER;
	alias PTCITEMHEADER = TC_ITEMHEADER*;
	...
	alias TC_KEYDOWN = NMTCKEYDOWN;
	alias _TC_KEYDOWN = TC_KEYDOWN;
	alias TTCKEYDOWN = TC_KEYDOWN;
	alias PTCKEYDOWN = TC_KEYDOWN*;
	...
	alias _TEXTRANGE = TEXTRANGE;
	alias TTEXTRANGE = TEXTRANGE;
	alias PTEXTRANGE = TEXTRANGE*;
	...
	alias TTIMEZONEINFORMATION = TIME_ZONE_INFORMATION;
	alias PTIMEZONEINFORMATION = TIME_ZONE_INFORMATION*;
	...
	alias TTOGGLEKEYS = TOGGLEKEYS;
	alias PTOGGLEKEYS = TOGGLEKEYS*;
	...
	alias TTOKENSOURCE = TOKEN_SOURCE;
	alias PTOKENSOURCE = TOKEN_SOURCE*;
	...
	alias TTOKENCONTROL = TOKEN_CONTROL;
	alias PTOKENCONTROL = TOKEN_CONTROL*;
	...
	alias TTOKENDEFAULTDACL = TOKEN_DEFAULT_DACL;
	alias PTOKENDEFAULTDACL = TOKEN_DEFAULT_DACL*;
	...
	alias TTOKENGROUPS = TOKEN_GROUPS;
	alias PTOKENGROUPS = TOKEN_GROUPS*;
	...
	alias TTOKENOWNER = TOKEN_OWNER;
	alias PTOKENOWNER = TOKEN_OWNER*;
	...
	alias TTOKENPRIMARYGROUP = TOKEN_PRIMARY_GROUP;
	alias PTOKENPRIMARYGROUP = TOKEN_PRIMARY_GROUP*;
	...
	alias TTOKENPRIVILEGES = TOKEN_PRIVILEGES;
	alias PTOKENPRIVILEGES = TOKEN_PRIVILEGES*;
	...
	alias TTOKENSTATISTICS = TOKEN_STATISTICS;
	alias PTOKENSTATISTICS = TOKEN_STATISTICS*;
	...
	alias TTOKENUSER = TOKEN_USER;
	alias PTOKENUSER = TOKEN_USER*;
	...
	alias LPTOOLINFO = TOOLINFO*;
	alias TTOOLINFO = TOOLINFO;
	alias PTOOLINFO = TOOLINFO*;
	...
	alias LPTOOLTIPTEXT = TOOLTIPTEXT*;
	alias TTOOLTIPTEXT = TOOLTIPTEXT;
	alias PTOOLTIPTEXT = TOOLTIPTEXT*;
	...
	alias TTPMPARAMS = TPMPARAMS;
	alias PTPMPARAMS = TPMPARAMS*;
	...
	alias TTRANSMITFILEBUFFERS = TRANSMIT_FILE_BUFFERS;
	alias PTRANSMITFILEBUFFERS = TRANSMIT_FILE_BUFFERS*;
	...
	alias LPHITTESTINFO = TTHITTESTINFO*;
	alias _TT_HITTESTINFO = TTHITTESTINFO;
	alias TTTHITTESTINFO = TTHITTESTINFO;
	alias PTTHITTESTINFO = TTHITTESTINFO*;
	...
	alias TTTPOLYCURVE = TTPOLYCURVE;
	alias PTTPOLYCURVE = TTPOLYCURVE*;
	...
	alias _TTPOLYGONHEADER = TTPOLYGONHEADER;
	alias TTTPOLYGONHEADER = TTPOLYGONHEADER;
	alias PTTPOLYGONHEADER = TTPOLYGONHEADER*;
	...
	alias _TV_DISPINFO = TV_DISPINFO;
	alias TTVDISPINFO = TV_DISPINFO;
	alias PTVDISPINFO = TV_DISPINFO*;
	...
	alias LPTV_HITTESTINFO = TV_HITTESTINFO*;
	alias _TVHITTESTINFO = TV_HITTESTINFO;
	alias TTVHITTESTINFO = TV_HITTESTINFO;
	alias PTVHITTESTINFO = TV_HITTESTINFO*;
	...
	alias LPTV_INSERTSTRUCT = TVINSERTSTRUCT*;
	alias _TV_INSERTSTRUCT = TVINSERTSTRUCT;
	alias TTVINSERTSTRUCT = TVINSERTSTRUCT;
	alias PTVINSERTSTRUCT = TVINSERTSTRUCT*;
	...
	alias _TV_KEYDOWN = TV_KEYDOWN;
	alias TTVKEYDOWN = TV_KEYDOWN;
	alias PTVKEYDOWN = TV_KEYDOWN*;
	...
	alias LPTV_SORTCB = TV_SORTCB*;
	alias _TV_SORTCB = TV_SORTCB;
	alias TTVSORTCB = TV_SORTCB;
	alias PTVSORTCB = TV_SORTCB*;
	...
	alias TUDACCEL = UDACCEL;
	alias PUDACCEL = UDACCEL*;
	...
	alias TULARGEINTEGER = ULARGE_INTEGER;
	alias PULARGEINTEGER = ULARGE_INTEGER*;

	struct UNIVERSAL_NAME_INFO {
		LPTSTR lpUniversalName;
	}

	alias _UNIVERSAL_NAME_INFO = UNIVERSAL_NAME_INFO;
	alias TUNIVERSALNAMEINFO = UNIVERSAL_NAME_INFO;
	alias PUNIVERSALNAMEINFO = UNIVERSAL_NAME_INFO*;
	...
	alias TUSEROBJECTFLAGS = USEROBJECTFLAGS;
	alias PUSEROBJECTFLAGS = USEROBJECTFLAGS*;
	...
	alias TVALENT = VALENT;
	alias PVALENT = VALENT*;
	alias VALUE_ENT = VALENT;
	alias TVALUE_ENT = VALENT;
	alias PVALUE_ENT = VALENT*;
	...
	alias TVERIFYINFORMATION = VERIFY_INFORMATION;
	alias PVERIFYINFORMATION = VERIFY_INFORMATION*;
	...
	alias _VS_FIXEDFILEINFO = VS_FIXEDFILEINFO;
	alias TVSFIXEDFILEINFO = VS_FIXEDFILEINFO;
	alias PVSFIXEDFILEINFO = VS_FIXEDFILEINFO*;
	...
	alias PWIN32_FIND_DATA = WIN32_FIND_DATA*;
	alias _WIN32_FIND_DATA = WIN32_FIND_DATA;
	alias TWIN32FINDDATA = WIN32_FIND_DATA;
	alias TWIN32FINDDATAA = WIN32_FIND_DATA;
	alias PWIN32FINDDATA = WIN32_FIND_DATA*;
	...
	alias LPWIN32_FIND_DATAW = WIN32_FIND_DATAW*;
	alias PWIN32_FIND_DATAW = WIN32_FIND_DATAW*;
	...
	alias TWIN32STREAMID = WIN32_STREAM_ID;
	alias PWIN32STREAMID = WIN32_STREAM_ID*;
	...
	alias TWINDOWPLACEMENT = WINDOWPLACEMENT;
	alias PWINDOWPLACEMENT = WINDOWPLACEMENT*;
	...
	alias PWNDCLASSA = WNDCLASSA*, LPWNDCLASSA = WNDCLASSA*;
	...
	alias PWNDCLASSW = WNDCLASSW*, LPWNDCLASSW = WNDCLASSW*;
	...
	alias LPWNDCLASS = WNDCLASS*;
	alias _WNDCLASS = WNDCLASS;
	alias TWNDCLASS = WNDCLASS;
	alias WNDCLASS_T = WNDCLASS;
	alias PWNDCLASS = WNDCLASS*;
	...
	alias LPWNDCLASSEX = WNDCLASSEX*;
	alias _WNDCLASSEX = WNDCLASSEX;
	alias TWNDCLASSEX = WNDCLASSEX;
	alias TWNDCLASSEXA = WNDCLASSEX;
	alias PWNDCLASSEX = WNDCLASSEX*;
	...
	alias _CONNECTDLGSTRUCT = CONNECTDLGSTRUCT;
	alias TCONNECTDLGSTRUCT = CONNECTDLGSTRUCT;
	alias PCONNECTDLGSTRUCT = CONNECTDLGSTRUCT*;
	...
	alias _DISCDLGSTRUCT = DISCDLGSTRUCT;
	alias TDISCDLGSTRUCT = DISCDLGSTRUCT;
	alias TDISCDLGSTRUCTA = DISCDLGSTRUCT;
	alias PDISCDLGSTRUCT = DISCDLGSTRUCT*;
	...
	alias TNETINFOSTRUCT = NETINFOSTRUCT;
	alias PNETINFOSTRUCT = NETINFOSTRUCT*;
	...
	alias TNETCONNECTINFOSTRUCT = NETCONNECTINFOSTRUCT;
	alias PNETCONNECTINFOSTRUCT = NETCONNECTINFOSTRUCT*;
	extern (Windows) {
		alias ENUMMETAFILEPROC = int function(HDC, HANDLETABLE*, METARECORD*, int, LPARAM);
		alias ENHMETAFILEPROC = int function(HDC, HANDLETABLE*, ENHMETARECORD*, int, LPARAM);
		alias ENUMFONTSPROC = int function(LPLOGFONT, LPTEXTMETRIC, DWORD, LPARAM);
		alias FONTENUMPROC = int function(ENUMLOGFONT*, NEWTEXTMETRIC*, int, LPARAM);
		alias FONTENUMEXPROC = int function(ENUMLOGFONTEX*, NEWTEXTMETRICEX*, int, LPARAM);
		alias LPOVERLAPPED_COMPLETION_ROUTINE = void function(DWORD, DWORD);
	}
	...
	alias TGLYPHMETRICSFLOAT = GLYPHMETRICSFLOAT;
	...
	alias TLAYERPLANEDESCRIPTOR = LAYERPLANEDESCRIPTOR;
	alias PLAYERPLANEDESCRIPTOR = LAYERPLANEDESCRIPTOR*;
	...
	alias TPIXELFORMATDESCRIPTOR = PIXELFORMATDESCRIPTOR;
	...
	alias PUSER_INFO_2 = USER_INFO_2*;
	alias LPUSER_INFO_2 = USER_INFO_2*;
	alias TUSERINFO2 = USER_INFO_2;
	alias PUSERINFO2 = USER_INFO_2*;

	struct USER_INFO_0 {
		LPWSTR usri0_name;
	}

	alias PUSER_INFO_0 = USER_INFO_0*;
	alias LPUSER_INFO_0 = USER_INFO_0*;
	alias TUSERINFO0 = USER_INFO_0;
	alias PUSERINFO0 = USER_INFO_0*;
	...
	alias PUSER_INFO_3 = USER_INFO_3*;
	alias LPUSER_INFO_3 = USER_INFO_3*;
	alias TUSERINFO3 = USER_INFO_3;
	alias PUSERINFO3 = USER_INFO_3*;
	...
	alias TGROUPINFO2 = GROUP_INFO_2;
	alias PGROUPINFO2 = GROUP_INFO_2*;

	struct LOCALGROUP_INFO_0 {
		LPWSTR lgrpi0_name;
	}

	alias PLOCALGROUP_INFO_0 = LOCALGROUP_INFO_0*;
	alias LPLOCALGROUP_INFO_0 = LOCALGROUP_INFO_0*;
	alias TLOCALGROUPINFO0 = LOCALGROUP_INFO_0;
	alias PLOCALGROUPINFO0 = LOCALGROUP_INFO_0*;
	...
	alias TIMAGEDOSHEADER = IMAGE_DOS_HEADER;
	alias PIMAGEDOSHEADER = IMAGE_DOS_HEADER*;
	alias TVARTYPE = ushort;
	alias PVARIANT = TVARIANT*;
	...
	alias TRTLCRITICALSECTION = CRITICAL_SECTION;
	alias PRTLCRITICALSECTION = PCRITICAL_SECTION;
	alias PIID = PGUID;
	alias TIID = TGUID;
	alias THANDLE = HANDLE;
	alias PSMALLRECT = TSMALLRECT*;
	alias TSMALLRECT = SMALL_RECT;
	alias PCHARINFO = TCHARINFO*;
	alias TCHARINFO = _CHAR_INFO;
	alias TFARPROC = POINTER;
	alias TFNDLGPROC = POINTER;
	alias TFNTHREADSTARTROUTINE = POINTER;
	...
	alias TOBJECTTYPELIST = _OBJECT_TYPE_LIST;
	alias POBJECTTYPELIST = TOBJECTTYPELIST*;
	alias OBJECT_TYPE_LIST = _OBJECT_TYPE_LIST;
	alias AUDIT_EVENT_TYPE = DWORD;
	...
	alias TBLENDFUNCTION = _BLENDFUNCTION;
	alias PBLENDFUNCTION = TBLENDFUNCTION*;
	...
	alias PWINDOWINFO = WINDOWINFO*;
	alias LPWINDOWINFO = WINDOWINFO*;
 ...
 */
	...
	alias MESSAGEPROC = LRESULT function(int code, // hook code
	...
	alias LANGUAGEGROUP_ENUMPROC = BOOL function(LGRPID LanguageGroup, // language-group identifier
	...
	alias PINITCOMMONCONTROLSEX = INITCOMMONCONTROLSEX*,
		LPINITCOMMONCONTROLSEX = INITCOMMONCONTROLSEX*;
	...
	alias LPMENUBARINFO = MENUBARINFO*;
	...
	alias PCMENUINFO = MENUINFO*;
	alias LPCMENUINFO = MENUINFO*;
...
+/
	...
	alias PNOTIFYICONDATAA = NOTIFYICONDATAA*, LPNOTIFYICONDATAA = NOTIFYICONDATAA*;
	...
	alias PNOTIFYICONDATAW = NOTIFYICONDATAW*, LPNOTIFYICONDATAW = NOTIFYICONDATAW*;
	version (Win32SansUnicode) {
...
*/
	...
	alias btvs = BITWISE!(BYTE).btvs;
	alias btvs = BITWISE!(WORD).btvs;
	alias btvs = BITWISE!(DWORD).btvs;
	alias btvg = BITWISE!(BYTE).btvg;
	alias btvg = BITWISE!(WORD).btvg;
	alias btvg = BITWISE!(DWORD).btvg;
	...
	struct SCRIPT_ANALYSIS {
		...
		void fLayoutRTL(uint val) {
			btvs(&BITS, 11, val);
		}

		void fLinkBefore(uint val) {
			btvs(&BITS, 12, val);
		}

		void fLinkAfter(uint val) {
			btvs(&BITS, 13, val);
		}

		void fLogicalOrder(uint val) {
			btvs(&BITS, 14, val);
		}

		void fNoGlyphIndex(uint val) {
			btvs(&BITS, 15, val);
		}
	}
	...
	align(1):
		...
		uint fInvertPreBoundDir() {
			return btvg(&BITS, 1);
		}

		uint fInvertPostBoundDir() {
			return btvg(&BITS, 2);
		}

		uint fLinkStringBefore() {
			return btvg(&BITS, 3);
		}

		uint fLinkStringAfter() {
			return btvg(&BITS, 4);
		}

		uint fNeutralOverride() {
			return btvg(&BITS, 5);
		}

		uint fNumericOverride() {
			return btvg(&BITS, 6);
		}

		uint fLegacyBidiClass() {
			return btvg(&BITS, 7);
		}

		void fContextDigits(uint val) {
			btvs(&BITS, 0, val);
		}

		void fInvertPreBoundDir(uint val) {
			btvs(&BITS, 1, val);
		}

		void fInvertPostBoundDir(uint val) {
			btvs(&BITS, 2, val);
		}

		void fLinkStringBefore(uint val) {
			btvs(&BITS, 3, val);
		}

		void fLinkStringAfter(uint val) {
			btvs(&BITS, 4, val);
		}

		void fNeutralOverride(uint val) {
			btvs(&BITS, 5, val);
		}

		void fNumericOverride(uint val) {
			btvs(&BITS, 6, val);
		}

		void fLegacyBidiClass(uint val) {
			btvs(&BITS, 7, val);
		}
	}
	...
	align(1):
		...
		uint fReserved() {
			return cast(BYTE)(BITS >> 5);
		}

		void fSoftBreak(uint val) {
			btvs(&BITS, 0, val);
		}

		void fWhiteSpace(uint val) {
			btvs(&BITS, 1, val);
		}

		void fCharStop(uint val) {
			btvs(&BITS, 2, val);
		}

		void fWordStop(uint val) {
			btvs(&BITS, 3, val);
		}

		void fInvalid(uint val) {
			btvs(&BITS, 4, val);
		}

		void fReserved(uint val) {
			BITS &= 0x1F;
			BITS |= (val & 0x07) << 5;
		}
	}
	...
	struct SCRIPT_PROPERTIES {
		...
		uint fNumeric() {
			return btvg(&BITS1, 16);
		}

		uint fComplex() {
			return btvg(&BITS1, 17);
		}

		uint fNeedsWordBreaking() {
			return btvg(&BITS1, 18);
		}

		uint fNeedsCaretInfo() {
			return btvg(&BITS1, 19);
		}

		uint bCharSet() {
			return (BITS1 >> 20) & 0xFF;
		}

		uint fControl() {
			return btvg(&BITS1, 28);
		}

		uint fPrivateUseArea() {
			return btvg(&BITS1, 29);
		}

		uint fNeedsCharacterJustify() {
			return btvg(&BITS1, 30);
		}

		uint fInvalidGlyph() {
			return btvg(&BITS1, 31);
		}

		uint fInvalidLogAttr() {
			return btvg(&BITS2, 0);
		}
		...
		uint fAmbiguousCharSet() {
			return btvg(&BITS2, 2);
		}

		uint fClusterSizeVaries() {
			return btvg(&BITS2, 3);
		}
		...
		void langid(uint val) {
			BITS1 &= 0xFFFF0000;
			BITS1 |= (val & 0xFFFF);
		}

		void fNumeric(uint val) {
			btvs(&BITS1, 16, val);
		}

		void fComplex(uint val) {
			btvs(&BITS1, 17, val);
		}

		void fNeedsWordBreaking(uint val) {
			btvs(&BITS1, 18, val);
		}

		void fNeedsCaretInfo(uint val) {
			btvs(&BITS1, 19, val);
		}

		void bCharSet(uint val) {
			BITS1 &= 0xF00FFFFF;
			BITS1 |= (val & 0xFF) << 20;
		}

		void fControl(uint val) {
			btvs(&BITS1, 28, val);
		}

		void fPrivateUseArea(uint val) {
			btvs(&BITS1, 29, val);
		}

		void fNeedsCharacterJustify(uint val) {
			btvs(&BITS1, 30, val);
		}

		void fInvalidGlyph(uint val) {
			btvs(&BITS1, 31, val);
		}

		void fInvalidLogAttr(uint val) {
			btvs(&BITS2, 0, val);
		}
		...
		void fAmbiguousCharSet(uint val) {
			btvs(&BITS2, 2, val);
		}

		void fClusterSizeVaries(uint val) {
			btvs(&BITS2, 3, val);
		}

		void fRejectInvalid(uint val) {
			btvs(&BITS2, 4, val);
		}
	}
	...
	struct SCRIPT_STATE {
		...
		uint fOverrideDirection() {
			return btvg(&BITS, 5);
		}

		uint fInhibitSymSwap() {
			return btvg(&BITS, 6);
		}
		...
		void fOverrideDirection(uint val) {
			btvs(&BITS, 5, val);
		}

		void fInhibitSymSwap(uint val) {
			btvs(&BITS, 6, val);
		}

		void fCharShape(uint val) {
			btvs(&BITS, 7, val);
		}

		void fDigitSubstitute(uint val) {
			btvs(&BITS, 8, val);
		}

		void fInhibitLigate(uint val) {
			btvs(&BITS, 9, val);
		}

		void fDisplayZWG(uint val) {
			btvs(&BITS, 10, val);
		}

		void fArabicNumContext(uint val) {
			btvs(&BITS, 11, val);
		}

		void fGcpClusters(uint val) {
			btvs(&BITS, 12, val);
		}
	...
	align(1):
		...
		// setter
		void uJustification(uint val) {
			BITS &= 0xF0;
			BITS |= (val & 0x0F);
		}

		void fClusterStart(uint val) {
			btvs(&BITS, 4, val);
		}

		void fDiacritic(uint val) {
			btvs(&BITS, 5, val);
		}

		void fZeroWidth(uint val) {
			btvs(&BITS, 6, val);
		}
	...
	alias PSHELLEXECUTEINFOA = SHELLEXECUTEINFOA*, LPSHELLEXECUTEINFOA = SHELLEXECUTEINFOA*;
	...
	alias PSHELLEXECUTEINFOW = SHELLEXECUTEINFOW*, LPSHELLEXECUTEINFOW = SHELLEXECUTEINFOW*;
	version (Win32SansUnicode) {
	...
	alias LPTBBUTTONINFO = TBBUTTONINFO*;
	...
	alias PTRACKMOUSEEVENT = TRACKMOUSEEVENT*, LPTRACKMOUSEEVENT = TRACKMOUSEEVENT*;
	...
	alias LPNMTTCUSTOMDRAW = NMTTCUSTOMDRAW*;
	...
	alias PSCROLLBARINFO = SCROLLBARINFO*;
