...
public class Accessible {
	...
	this(Control control) {
         ...
         */
		HRESULT result = COM.CreateStdAccessibleObject(control.handle,
				COM.OBJID_CLIENT, &COM.IIDIAccessible, cast(void**)&iaccessible);
...
+/
	...
	public ptrdiff_t internal_WM_GETOBJECT(WPARAM wParam, LPARAM lParam) {
		...
		if (lParam is COM.OBJID_CLIENT) {
             ...
             */
			return COM.LresultFromObject(&COM.IIDIAccessible, wParam,
	...
	public void selectionChanged() {
		checkWidget();
		COM.NotifyWinEvent(COM.EVENT_OBJECT_SELECTIONWITHIN, control.handle,
				COM.OBJID_CLIENT, COM.CHILDID_SELF);
	}
	...
	public void setFocus(int childID) {
		checkWidget();
		COM.NotifyWinEvent(COM.EVENT_OBJECT_FOCUS, control.handle,
				COM.OBJID_CLIENT, childIDToOs(childID));
	}
	...
	public void textCaretMoved(int index) {
		checkWidget();
		COM.NotifyWinEvent(COM.EVENT_OBJECT_LOCATIONCHANGE, control.handle,
				COM.OBJID_CARET, COM.CHILDID_SELF);
	}
	...
	public void textChanged(int type, int startIndex, int length) {
		checkWidget();
		COM.NotifyWinEvent(COM.EVENT_OBJECT_VALUECHANGE, control.handle,
				COM.OBJID_CLIENT, COM.CHILDID_SELF);
	}
	...
	HRESULT accHitTest(LONG xLeft, LONG yTop, VARIANT* pvarChild) {
		...
		for (int i = 0; i < accessibleControlListeners.size(); i++) {
			AccessibleControlListener listener = cast(AccessibleControlListener) accessibleControlListeners.elementAt(
					i);
	...
	HRESULT accLocation(LONG* pxLeft, LONG* pyTop, LONG* pcxWidth,
			LONG* pcyHeight, VARIANT variant) {
		if (iaccessible is null)
			return COM.CO_E_OBJNOTCONNECTED;
		VARIANT* v = &variant;
		//COM.MoveMemory(v, variant, VARIANT.sizeof);
		if ((v.vt & 0xFFFF) !is COM.VT_I4)
	...
	HRESULT get_accDescription(VARIANT variant, BSTR* pszDescription) {
		...
		if (accessibleListeners.size() is 0 && !(null is cast(Tree) control))
			return code;
		if (code is COM.S_OK) {
					...
					if (widget !is null)
						if (auto item = cast(TreeItem) widget) {
							for (int i = 1; i < columnCount; i++) {
								event.result ~= tree.getColumn(i).getText() ~ ": " ~ item.getText(
										i);
								if (i + 1 < columnCount)
									event.result ~= ", ";
							}
	...
	HRESULT get_accHelp(VARIANT variant, BSTR* pszHelp) {
		...
		if (code is COM.E_INVALIDARG)
			code = COM.S_FALSE; // proxy doesn't know about app childID
		if (accessibleListeners.size() is 0)
			return code;
		if (code is COM.S_OK) {
	...
	HRESULT get_accRole(VARIANT variant, VARIANT* pvarRole) {
		...
		if (accessibleControlListeners.size() is 0
				&& !(null !is cast(Tree) control || null !is cast(Table) control))
			return code;
		if (code is COM.S_OK) {
			//TODO - use VARIANT structure
			short[1] pvt;
		...
		if (v.lVal !is COM.CHILDID_SELF) {
			if (null !is cast(Tree) control || null !is cast(Table) control) {
				if ((control.getStyle() & SWT.CHECK) !is 0)
			...
			if (null !is cast(Tree) control && (control.getStyle() & SWT.CHECK) !is 0) {
				...
				if (OS.COMCTL32_MAJOR >= 6) {
					tvItem.hItem = cast(HTREEITEM) OS.SendMessage(hwnd,
							OS.TVM_MAPACCIDTOHTREEITEM, v.lVal, 0);
				}
				else {
	...
	HRESULT Next(ULONG celt, VARIANT* rgvar, ULONG* pceltFetched) {
		...
		if (accessibleControlListeners.size() is 0) {
			...
			int code = iaccessible.QueryInterface(&COM.IIDIEnumVARIANT,
					cast(void**)&ienumvariant);
			if (code !is COM.S_OK)
				return code;
			uint[1] celtFetched;
		...
		if (rgvar is null)
			return COM.E_INVALIDARG;
		if (pceltFetched is null && celt !is 1)
			return COM.E_INVALIDARG;
		if (enumIndex is 0) {
			AccessibleControlEvent event = new AccessibleControlEvent(this);
			event.childID = ACC.CHILDID_SELF;
			for (int i = 0; i < accessibleControlListeners.size(); i++) {
				AccessibleControlListener listener = cast(AccessibleControlListener) accessibleControlListeners
					.elementAt(i);
		...
		if (variants !is null && celt >= 1) {
			...
			if (endIndex > (variants.length - 1))
				endIndex = cast(int) /*64bit*/ variants.length - 1;
	...
	int childIDToOs(int childID) {
        ...
        */
		...
		if (OS.COMCTL32_MAJOR < 6)
			return childID;
		return cast(int) /*64bit*/ OS.SendMessage(control.handle,
				OS.TVM_MAPHTREEITEMTOACCID, childID, 0);
	}

	int osToChildID(int osChildID) {
		if (osChildID is COM.CHILDID_SELF)
        ...
        */
		...
		if (OS.COMCTL32_MAJOR < 6)
			return osChildID;
		return cast(int) /*64bit*/ OS.SendMessage(control.handle,
				OS.TVM_MAPACCIDTOHTREEITEM, osChildID, 0);
	}

	int stateToOs(int state) {
		int osState = 0;
		if ((state & ACC.STATE_SELECTED) !is 0)
			osState |= COM.STATE_SYSTEM_SELECTED;
		if ((state & ACC.STATE_SELECTABLE) !is 0)
			osState |= COM.STATE_SYSTEM_SELECTABLE;
		if ((state & ACC.STATE_MULTISELECTABLE) !is 0)
			osState |= COM.STATE_SYSTEM_MULTISELECTABLE;
		if ((state & ACC.STATE_FOCUSED) !is 0)
			osState |= COM.STATE_SYSTEM_FOCUSED;
		if ((state & ACC.STATE_FOCUSABLE) !is 0)
			osState |= COM.STATE_SYSTEM_FOCUSABLE;
		if ((state & ACC.STATE_PRESSED) !is 0)
			osState |= COM.STATE_SYSTEM_PRESSED;
		if ((state & ACC.STATE_CHECKED) !is 0)
			osState |= COM.STATE_SYSTEM_CHECKED;
		if ((state & ACC.STATE_EXPANDED) !is 0)
			osState |= COM.STATE_SYSTEM_EXPANDED;
		if ((state & ACC.STATE_COLLAPSED) !is 0)
			osState |= COM.STATE_SYSTEM_COLLAPSED;
		if ((state & ACC.STATE_HOTTRACKED) !is 0)
			osState |= COM.STATE_SYSTEM_HOTTRACKED;
		if ((state & ACC.STATE_BUSY) !is 0)
			osState |= COM.STATE_SYSTEM_BUSY;
		if ((state & ACC.STATE_READONLY) !is 0)
			osState |= COM.STATE_SYSTEM_READONLY;
		if ((state & ACC.STATE_INVISIBLE) !is 0)
			osState |= COM.STATE_SYSTEM_INVISIBLE;
		if ((state & ACC.STATE_OFFSCREEN) !is 0)
			osState |= COM.STATE_SYSTEM_OFFSCREEN;
		if ((state & ACC.STATE_SIZEABLE) !is 0)
			osState |= COM.STATE_SYSTEM_SIZEABLE;
		if ((state & ACC.STATE_LINKED) !is 0)
	...
	int osToState(int osState) {
		...
		if ((osState & COM.STATE_SYSTEM_SELECTED) !is 0)
			state |= ACC.STATE_SELECTED;
		if ((osState & COM.STATE_SYSTEM_SELECTABLE) !is 0)
			state |= ACC.STATE_SELECTABLE;
		if ((osState & COM.STATE_SYSTEM_MULTISELECTABLE) !is 0)
			state |= ACC.STATE_MULTISELECTABLE;
		if ((osState & COM.STATE_SYSTEM_FOCUSED) !is 0)
			state |= ACC.STATE_FOCUSED;
		if ((osState & COM.STATE_SYSTEM_FOCUSABLE) !is 0)
			state |= ACC.STATE_FOCUSABLE;
		if ((osState & COM.STATE_SYSTEM_PRESSED) !is 0)
			state |= ACC.STATE_PRESSED;
		if ((osState & COM.STATE_SYSTEM_CHECKED) !is 0)
			state |= ACC.STATE_CHECKED;
		if ((osState & COM.STATE_SYSTEM_EXPANDED) !is 0)
			state |= ACC.STATE_EXPANDED;
		if ((osState & COM.STATE_SYSTEM_COLLAPSED) !is 0)
			state |= ACC.STATE_COLLAPSED;
		if ((osState & COM.STATE_SYSTEM_HOTTRACKED) !is 0)
			state |= ACC.STATE_HOTTRACKED;
		if ((osState & COM.STATE_SYSTEM_BUSY) !is 0)
			state |= ACC.STATE_BUSY;
		if ((osState & COM.STATE_SYSTEM_READONLY) !is 0)
			state |= ACC.STATE_READONLY;
		if ((osState & COM.STATE_SYSTEM_INVISIBLE) !is 0)
			state |= ACC.STATE_INVISIBLE;
		if ((osState & COM.STATE_SYSTEM_OFFSCREEN) !is 0)
			state |= ACC.STATE_OFFSCREEN;
		if ((osState & COM.STATE_SYSTEM_SIZEABLE) !is 0)
			state |= ACC.STATE_SIZEABLE;
		if ((osState & COM.STATE_SYSTEM_LINKED) !is 0)
...
extern (Windows):
	...
	HRESULT get_accParent(LPDISPATCH* ppdispParent) {
		return parent.get_accParent(ppdispParent);
	}

	HRESULT get_accChildCount(LONG* pcountChildren) {
		return parent.get_accChildCount(pcountChildren);
	}
	...
	HRESULT get_accFocus(VARIANT* pvarID) {
		return parent.get_accFocus(pvarID);
	}

	HRESULT get_accSelection(VARIANT* pvarChildren) {
		return parent.get_accSelection(pvarChildren);
	}
...
extern (Windows):
	...
	HRESULT Next(ULONG celt, VARIANT* rgvar, ULONG* pceltFetched) {
		return parent.Next(celt, rgvar, pceltFetched);
	}

	HRESULT Skip(ULONG celt) {
		return parent.Skip(celt);
	}
