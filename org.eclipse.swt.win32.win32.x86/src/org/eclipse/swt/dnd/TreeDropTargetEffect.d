		...
		if ((effect & DND.FEEDBACK_SCROLL) is 0) {
			...
			if (hItem !is -1 && scrollIndex is hItem && scrollBeginTime !is 0) {
				if (System.currentTimeMillis() >= scrollBeginTime) {
					auto topItem = cast(HTREEITEM) OS.SendMessage(handle,
							OS.TVM_GETNEXTITEM, OS.TVGN_FIRSTVISIBLE, 0);
					auto nextItem = cast(HTREEITEM) OS.SendMessage(handle, OS.TVM_GETNEXTITEM,
							cast(HTREEITEM) hItem is topItem ? OS.TVGN_PREVIOUSVISIBLE
			...
			if (hItem !is -1 && expandIndex is hItem && expandBeginTime !is 0) {
				if (System.currentTimeMillis() >= expandBeginTime) {
					if (OS.SendMessage(handle, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, hItem) !is 0) {
						TreeItem item = cast(TreeItem) tree.getDisplay()
							.findWidget(tree.handle, hItem);
