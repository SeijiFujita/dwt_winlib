 ...
 */
		...
		private void drag(Event dragEvent) {
			...
			if (!event.doit || transferAgents is null || transferAgents.length is 0)
				return;
         ...
         */
				...
				if ((topControl.getStyle() & SWT.RIGHT_TO_LEFT) !is 0) {
					...
					hwndDrag = OS.CreateWindowEx(OS.WS_EX_TRANSPARENT | OS.WS_EX_NOINHERITLAYOUT,
							WindowClass.ptr, null, OS.WS_CHILD | OS.WS_CLIPSIBLINGS,
							0, 0, rect.right - rect.left, rect.bottom - rect.top,
							topControl.handle, null, OS.GetModuleHandle(null), null);
        ...
        */
			...
			try {
				...
				if (hwndDrag !is null) {
					...
					imagelist.dispose();
					if (hwndDrag !is topControl.handle)
			...
			if (dataEffect is DND.DROP_MOVE) {
				operation = (operation is DND.DROP_NONE || operation is DND.DROP_COPY) ? DND.DROP_TARGET_MOVE
					: DND.DROP_MOVE;
			}
			else {
 ...
 */
		private int EnumFormatEtc(int dwDirection, IEnumFORMATETC* ppenumFormatetc) {
			...
			if (dwDirection is COM.DATADIR_SET)
			...
			for (int i = 0; i < transferAgents.length; i++) {
				...
				if (transferAgent !is null) {
					...
					TransferData[] newAllowedDataTypes = new TransferData[allowedDataTypes.length
						+ formats.length];
					System.arraycopy(allowedDataTypes, 0, newAllowedDataTypes,
							0, allowedDataTypes.length);
					System.arraycopy(formats, 0, newAllowedDataTypes,
							allowedDataTypes.length, formats.length);
 ...
 */
    ...
    */
			...
			if (QueryGetData(pFormatetc) !is COM.S_OK)
			...
			if (transferData.result !is COM.S_OK)
 ...
 */
		...
		package.HRESULT QueryContinueDrag(int fEscapePressed, DWORD grfKeyState) {
			if (topControl !is null && topControl.isDisposed())
 ...
 */
		private HRESULT QueryInterface(REFCIID riid, void** ppvObject) {
			...
			if (COM.IsEqualGUID(riid, &COM.IIDIUnknown)
					|| COM.IsEqualGUID(riid, &COM.IIDIDropSource)) {
 ...
 */
		...
		HRESULT SetData(FORMATETC* pFormatetc, STGMEDIUM* pmedium, int fRelease) {
			...
			if (formatetc.cfFormat is CFSTR_PERFORMEDDROPEFFECT
					&& formatetc.tymed is COM.TYMED_HGLOBAL) {
 ...
 */
	...
	extern (Windows):
		...
		HRESULT QueryContinueDrag(BOOL fEscapePressed, DWORD grfKeyState) {
			return parent.QueryContinueDrag(fEscapePressed, grfKeyState);
		}

		HRESULT GiveFeedback(DWORD dwEffect) {
			return parent.GiveFeedback(dwEffect);
		}
	}
	...
	extern (Windows):
		...
		HRESULT GetData(FORMATETC* pFormatetc, STGMEDIUM* pmedium) {
			return parent.GetData(pFormatetc, pmedium);
		}
		...
		HRESULT QueryGetData(FORMATETC* pFormatetc) {
			return parent.QueryGetData(pFormatetc);
		}
		...
		HRESULT SetData(FORMATETC* pFormatetc, STGMEDIUM* pmedium, BOOL fRelease) {
			return parent.SetData(pFormatetc, pmedium, fRelease);
		}

		HRESULT EnumFormatEtc(DWORD dwDirection, IEnumFORMATETC* ppenumFormatetc) {
			return parent.EnumFormatEtc(dwDirection, ppenumFormatetc);
		}
