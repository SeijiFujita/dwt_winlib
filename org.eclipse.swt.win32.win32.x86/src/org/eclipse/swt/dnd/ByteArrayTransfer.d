...
public abstract class ByteArrayTransfer : Transfer {
	...
	override public bool isSupportedType(TransferData transferData) {
		if (transferData is null)
			return false;
		int[] types = getTypeIds();
		for (int i = 0; i < types.length; i++) {
			FORMATETC* format = transferData.formatetc;
			if (format.cfFormat is types[i] && (format.dwAspect & COM.DVASPECT_CONTENT) is COM.DVASPECT_CONTENT
 ...
 */
	override protected void javaToNative(Object object, TransferData transferData) {
 ...
 */
	...
	bool checkByteArray(Object object) {
		return (object !is null && (null !is cast(ArrayWrapperByte) object)
				&& (cast(ArrayWrapperByte) object).array.length > 0);
	}
}
