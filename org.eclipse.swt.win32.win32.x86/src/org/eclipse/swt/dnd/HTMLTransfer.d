 ...
 */
	override public void javaToNative(Object object, TransferData transferData) {
		...
		int cchMultiByte = OS.WideCharToMultiByte(OS.CP_UTF8, 0,
				StrToTCHARz(string), -1, null, 0, null, null);
		...
		auto lpMultiByteStr = cast(PCHAR) OS.GlobalAlloc(OS.GMEM_FIXED | OS.GMEM_ZEROINIT,
				cchMultiByte);
		OS.WideCharToMultiByte(OS.CP_UTF8, 0, wstrz, -1, lpMultiByteStr,
 ...
 */
	override public Object nativeToJava(TransferData transferData) {
		if (!isSupportedType(transferData) || transferData.pIDataObject is null)
			return null;
		...
		try {
             ...
             * Otherwise, the function fails with ERROR_INVALID_FLAGS. */
				auto cchWideChar = OS.MultiByteToWideChar(OS.CP_UTF8, 0,
						lpMultiByteStr, -1, null, 0);
				if (cchWideChar is 0)
					return null;
				wchar[] lpWideCharStr = new wchar[cchWideChar - 1];
				OS.MultiByteToWideChar(OS.CP_UTF8, 0, lpMultiByteStr, -1,
						lpWideCharStr.ptr, cast(int) /*64bit*/ lpWideCharStr.length);
				String string = WCHARzToStr(lpWideCharStr.ptr);
				int fragmentStart = 0, fragmentEnd = 0;
				int start = string.indexOf(StartFragment) + cast(int) /*64bit*/ StartFragment
					.length;
				int end = start + 1;
				...
				if (fragmentEnd <= fragmentStart || fragmentEnd > OS.strlen(lpMultiByteStr))
					return null;
				cchWideChar = OS.MultiByteToWideChar(OS.CP_UTF8, 0, lpMultiByteStr + fragmentStart,
						fragmentEnd - fragmentStart, lpWideCharStr.ptr,
						cast(int) /*64bit*/ lpWideCharStr.length);
				if (cchWideChar is 0)
					return null;
	...
	override protected bool validate(Object object) {
