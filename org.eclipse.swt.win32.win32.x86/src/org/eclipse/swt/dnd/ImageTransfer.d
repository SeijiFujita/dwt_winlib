 ...
 */
	override public void javaToNative(Object object, TransferData transferData) {
		...
		auto newPtr = OS.GlobalAlloc(OS.GMEM_FIXED | OS.GMEM_ZEROINIT,
				BITMAPINFOHEADER.sizeof + colorSize + imageSize);
 ...
 */
	override public Object nativeToJava(TransferData transferData) {
		...
		try {
			...
			try {
				...
				void* pBits;
				auto memDib = OS.CreateDIBSection(null, cast(BITMAPINFO*) ptr,
	...
	bool checkImage(Object object) {
		if (object is null || !(null !is cast(ImageData) object))
			return false;
		return true;
	}
