...
public class DropTargetEffect : DropTargetAdapter {
	...
	TreeItem nextItem(Tree tree, TreeItem item) {
		...
		while (true) {
			if (index + 1 < count)
				return parentItem is null ? tree.getItem(index + 1) : parentItem.getItem(index + 1);
	...
	TreeItem previousItem(Tree tree, TreeItem item) {
		...
		if (index is 0)
			return parentItem;
		TreeItem nextItem = parentItem is null ? tree.getItem(index - 1)
			: parentItem.getItem(index - 1);
