 ...
 */
		...
		HRESULT DragEnter(IDataObject pDataObject, DWORD grfKeyState, POINTL pt, DWORD* pdwEffect) {
			...
			if (event.detail is DND.DROP_DEFAULT) {
				event.detail = (allowedOperations & DND.DROP_MOVE) !is 0 ? DND.DROP_MOVE
					: DND.DROP_NONE;
			}
 ...
 */
		HRESULT QueryInterface(REFCIID riid, void** ppvObject) {
			...
			if (COM.IsEqualGUID(riid, &COM.IIDIUnknown)
					|| COM.IsEqualGUID(riid, &COM.IIDIDropTarget)) {
 ...
 */
		...
		bool setEventData(DNDEvent event, IDataObject pDataObject,
				int grfKeyState, POINTL pt, DWORD* pdwEffect) {
			if (pDataObject is null || pdwEffect is null)
				return false;
			...
			if (operations[0] is DND.DROP_NONE)
				return false;
			...
			if (operation is DND.DROP_DEFAULT) {
				if ((style & DND.DROP_DEFAULT) is 0) {
					operation = (operations[0] & DND.DROP_MOVE) !is 0 ? DND.DROP_MOVE
						: DND.DROP_NONE;
				}
			}
			else {
				if ((operation & operations[0]) is 0)
			...
			try {
				...
				try {
					...
					auto rgelt = cast(FORMATETC*) OS.GlobalAlloc(OS.GMEM_FIXED | OS.GMEM_ZEROINIT,
							FORMATETC.sizeof);
					try {
						uint[1] pceltFetched;
						enumFormatetc.Reset();
						while (enumFormatetc.Next(1, rgelt,
								pceltFetched.ptr) is COM.S_OK && pceltFetched[0] is 1) {
							...
							for (int i = 0; i < transferAgents.length; i++) {
								...
								if (transfer !is null && transfer.isSupportedType(transferData)) {
									TransferData[] newDataTypes = new TransferData[dataTypes.length
										+ 1];
									System.arraycopy(dataTypes, 0,
											newDataTypes, 0, dataTypes.length);
