...
public class MessageBox : Dialog {
	...
	alias checkStyle = Dialog.checkStyle;
 ...
 */
 ...
 +/
	...
	static int checkStyle(int style) {
		...
		int bits = style & mask;
		if (bits is SWT.OK || bits is SWT.CANCEL || bits is(SWT.OK | SWT.CANCEL))
			return style;
		if (bits is SWT.YES || bits is SWT.NO || bits is(SWT.YES | SWT.NO)
				|| bits is(SWT.YES | SWT.NO | SWT.CANCEL))
			return style;
		if (bits is(SWT.RETRY | SWT.CANCEL) || bits is(SWT.ABORT | SWT.RETRY | SWT.IGNORE))
			return style;
 ...
 */
	public int open() {
		...
		int buttonBits = 0;
		if ((style & SWT.OK) is SWT.OK)
			buttonBits = OS.MB_OK;
		if ((style & (SWT.OK | SWT.CANCEL)) is(SWT.OK | SWT.CANCEL))
			buttonBits = OS.MB_OKCANCEL;
		if ((style & (SWT.YES | SWT.NO)) is(SWT.YES | SWT.NO))
			buttonBits = OS.MB_YESNO;
		if ((style & (SWT.YES | SWT.NO | SWT.CANCEL)) is(SWT.YES | SWT.NO | SWT.CANCEL))
			buttonBits = OS.MB_YESNOCANCEL;
		if ((style & (SWT.RETRY | SWT.CANCEL)) is(SWT.RETRY | SWT.CANCEL))
			buttonBits = OS.MB_RETRYCANCEL;
		if ((style & (SWT.ABORT | SWT.RETRY | SWT.IGNORE)) is(SWT.ABORT | SWT.RETRY | SWT.IGNORE))
			buttonBits = OS.MB_ABORTRETRYIGNORE;
		if (buttonBits is 0)
			buttonBits = OS.MB_OK;

		int iconBits = 0;
		if ((style & SWT.ICON_ERROR) !is 0)
			iconBits = OS.MB_ICONERROR;
		if ((style & SWT.ICON_INFORMATION) !is 0)
			iconBits = OS.MB_ICONINFORMATION;
		if ((style & SWT.ICON_QUESTION) !is 0)
			iconBits = OS.MB_ICONQUESTION;
		if ((style & SWT.ICON_WARNING) !is 0)
			iconBits = OS.MB_ICONWARNING;
		if ((style & SWT.ICON_WORKING) !is 0)
			iconBits = OS.MB_ICONINFORMATION;
    ...
    */
		...
		if ((bits & OS.MB_TASKMODAL) !is 0) {
			display = (allowNullParent && parent is null) ? Display.getCurrent()
				: parent.getDisplay();
    ...
    */
		...
		if (code !is 0) {
			...
			if (type is OS.MB_ABORTRETRYIGNORE) {
				if (code is OS.IDRETRY)
					return SWT.RETRY;
				if (code is OS.IDABORT)
					return SWT.ABORT;
				return SWT.IGNORE;
			}
		}
