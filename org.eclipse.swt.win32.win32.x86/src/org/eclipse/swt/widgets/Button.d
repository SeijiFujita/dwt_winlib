...
public class Button : Control {
	...
	alias TWindowProc = extern (Windows).LRESULT function(HWND, uint, WPARAM, LPARAM);
	String text = "", message = "";
				...
				if ((style & SWT.RIGHT_TO_LEFT) !is 0) {
					if (!OS.IsWinCE && OS.WIN32_VERSION >= OS.VERSION(4, 10)) {
						...
						if (fImageType is OS.IMAGE_BITMAP) {
							...
							OS.BitBlt(dstHdc, 0, 0, rect.width, rect.height,
									srcHdc, 0, 0, OS.SRCCOPY);
 ...
 */
	public bool getSelection() {
		checkWidget();
		if ((style & (SWT.CHECK | SWT.RADIO | SWT.TOGGLE)) is 0)
			return false;
 ...
 */
	public void setAlignment(int alignment) {
		checkWidget();
		if ((style & SWT.ARROW) !is 0) {
			if ((style & (SWT.UP | SWT.DOWN | SWT.LEFT | SWT.RIGHT)) is 0)
				return;
    ...
    */
		if ((style & SWT.RADIO) !is 0 && !getSelection())
			return false;
		...
		if (!OS.IsWinCE && OS.WIN32_VERSION >= OS.VERSION(6, 0)) {
			if ((style & (SWT.RADIO | SWT.CHECK | SWT.TOGGLE)) !is 0) {
				...
				if (!redraw) {
					...
					if (!redraw)
						redraw = findBackgroundControl() !is null;
				}
				if (redraw) {
    ...
    */
	...
	override LRESULT wmDrawChild(WPARAM wParam, LPARAM lParam) {
		...
		OS.SetRect(&rect, struct_.rcItem.left, struct_.rcItem.top,
				struct_.rcItem.right, struct_.rcItem.bottom);
		if (OS.COMCTL32_MAJOR >= 6 && OS.IsAppThemed()) {
        ...
        */
			...
			if ((struct_.itemState & OS.ODS_SELECTED) !is 0)
				iStateId += OS.ABS_UPPRESSED - OS.ABS_UPNORMAL;
			OS.DrawThemeBackground(display.hScrollBarTheme(), struct_.hDC,
