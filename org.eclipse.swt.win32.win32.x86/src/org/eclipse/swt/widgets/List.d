...
public class List : Scrollable {
	...
	alias windowProc = Scrollable.windowProc;
    ...
    */
		...
		do {
			index = cast(int) /*64bit*/ OS.SendMessage(handle,
					OS.LB_FINDSTRINGEXACT, last = index, cast(void*) buffer);
			if (index is OS.LB_ERR || index <= last)
				return -1;
		}
 ...
 */
	public void remove(int index) {
		...
		if ((style & SWT.H_SCROLL) !is 0)
			setScrollWidth(buffer.ptr, false);
		if (index < topIndex) {
 ...
 */
	public void setItems(String[] items) {
		...
		while (index < length) {
			...
			auto result = OS.SendMessage(handle, OS.LB_ADDSTRING, 0, cast(void*) buffer);
			if (result is OS.LB_ERR || result is OS.LB_ERRSPACE)
				break;
			if ((style & SWT.H_SCROLL) !is 0) {
				int flags = OS.DT_CALCRECT | OS.DT_SINGLELINE | OS.DT_NOPREFIX;
