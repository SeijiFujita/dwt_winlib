...
public abstract class Control : Widget, Drawable {
	...
	alias dragDetect = Widget.dragDetect;
	alias callWindowProc = Widget.callWindowProc;
 ...
 */
	...
	void checkMirrored() {
		if ((style & SWT.RIGHT_TO_LEFT) !is 0) {
			...
			if ((bits & OS.WS_EX_LAYOUTRTL) !is 0)
				style |= SWT.MIRRORED;
		}
	}
 ...
 */
	...
	void createHandle() {
		...
		handle = OS.CreateWindowEx(widgetExtStyle(), StrToTCHARz(windowClass()),
				null, widgetStyle(), OS.CW_USEDEFAULT, 0, OS.CW_USEDEFAULT, 0,
				hwndParent, null, OS.GetModuleHandle(null), widgetCreateStruct());
		if (handle is null)
 ...
 */
	...
	bool dragDetect(int button, int count, int stateMask, int x, int y) {
        ...
        */
			if (button is 1 && OS.GetKeyState(OS.VK_ESCAPE) >= 0) {
				WPARAM wParam = 0;
				if ((stateMask & SWT.CTRL) !is 0)
					wParam |= OS.MK_CONTROL;
				if ((stateMask & SWT.SHIFT) !is 0)
					wParam |= OS.MK_SHIFT;
				if ((stateMask & SWT.ALT) !is 0)
					wParam |= OS.MK_ALT;
				if ((stateMask & SWT.BUTTON1) !is 0)
					wParam |= OS.MK_LBUTTON;
				if ((stateMask & SWT.BUTTON2) !is 0)
					wParam |= OS.MK_MBUTTON;
				if ((stateMask & SWT.BUTTON3) !is 0)
					wParam |= OS.MK_RBUTTON;
				if ((stateMask & SWT.BUTTON4) !is 0)
					wParam |= OS.MK_XBUTTON1;
				if ((stateMask & SWT.BUTTON5) !is 0)
	...
	Control findBackgroundControl() {
		if (background !is -1 || backgroundImage !is null)
			return this;
	...
	Menu[] findMenus(Control control) {
		if (menu !is null && this !is control)
			return [menu];
		return new Menu[0];
	}
	...
	char findMnemonic(String string) {
		...
		do {
			while (index < length_ && string.charAt(index) !is '&')
				index++;
			if (++index >= length_)
				return '\0';
	...
	void fixChildren(Shell newShell, Shell oldShell, Decorations newDecorations,
			Decorations oldDecorations, Menu[] menus) {
 ...
 */
	public HDC internal_new_GC(GCData data) {
		...
		if (data !is null) {
			...
			if (foreground !is OS.GetTextColor(hDC))
				data.foreground = foreground;
			Control control = findBackgroundControl();
			if (control is null)
				control = this;
			auto background = control.getBackgroundPixel();
			if (background !is OS.GetBkColor(hDC))
				data.background = background;
			data.font = font !is null ? font : Font.win32_new(display,
					cast(HFONT) OS.SendMessage(hwnd, OS.WM_GETFONT, 0, 0));
 ...
 */
    ...
    */
	...
	bool isTabItem() {
		...
		int bits = OS.GetWindowLong(handle, OS.GWL_STYLE);
		if ((bits & OS.WS_TABSTOP) !is 0)
			return false;
		auto code = OS.SendMessage(handle, OS.WM_GETDLGCODE, 0, 0);
		if ((code & OS.DLGC_STATIC) !is 0)
			return false;
		if ((code & OS.DLGC_WANTALLKEYS) !is 0)
			return false;
		if ((code & OS.DLGC_WANTARROWS) !is 0)
			return false;
		if ((code & OS.DLGC_WANTTAB) !is 0)
			return false;
		return true;
	}
 ...
 */
	public void moveAbove(Control control) {
		...
		if (control !is null) {
			...
			if (hwnd is null || hwnd is topHandle_)
				return;
        ...
        */
            ...
            */
				...
				while (hwndAbove !is null && hwndAbove !is hwnd) {
					if (OS.GetWindow(hwndAbove, OS.GW_OWNER) is hwndParent)
						break;
					hwndAbove = OS.GetWindow(hwnd = hwndAbove, OS.GW_HWNDPREV);
				}
				if (hwndAbove is hwnd)
					return;
			}
		}
		if (hwndAbove is null || hwndAbove is topHandle_)
			return;
		int flags = OS.SWP_NOSIZE | OS.SWP_NOMOVE | OS.SWP_NOACTIVATE;
		SetWindowPos(topHandle_, hwndAbove, 0, 0, 0, 0, flags);
	}
    ...
    */
	...
	void setBackground() {
		...
		if (control.backgroundImage !is null) {
			...
			setBackgroundPixel(control.background is -1
					? control.defaultBackground() : control.background);
		}
	}
 ...
 */
	public void setCursor(Cursor cursor) {
		checkWidget();
		if (cursor !is null && cursor.isDisposed())
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
		this.cursor = cursor;
		static if (OS.IsWinCE) {
 ...
 */
	public void setVisible(bool visible) {
		checkWidget();
		if (drawCount !is 0) {
			if (((state & HIDDEN) is 0) is visible)
				return;
		}
		else {
			int bits = OS.GetWindowLong(handle, OS.GWL_STYLE);
			if (((bits & OS.WS_VISIBLE) !is 0) is visible)
				return;
		}
		if (visible) {
			sendEvent(SWT.Show);
 ...
 */
	...
	bool translateTraversal(MSG* msg) {
		...
		case OS.VK_RIGHT: {
            ...
            */
				if (OS.IsSP) {
					if (key is OS.VK_LEFT || key is OS.VK_RIGHT)
						return false;
				}
				lastVirtual = true;
				auto code = OS.SendMessage(hwnd, OS.WM_GETDLGCODE, 0, 0);
				if ((code & (OS.DLGC_WANTARROWS /*| OS.DLGC_WANTALLKEYS*/ )) !is 0)
					doit = false;
    ...
    */
		...
		case SWT.TRAVERSE_MNEMONIC:
			return traverseMnemonic(cast(char) event.character);
 ...
 */
	public bool setParent(Composite parent) {
		...
		auto topHandle_ = topHandle();
		if (OS.SetParent(topHandle_, parent.handle) is null)
			return false;
		this.parent = parent;
		int flags = OS.SWP_NOSIZE | OS.SWP_NOMOVE | OS.SWP_NOACTIVATE;
	...
	.LRESULT windowProc(HWND hwnd, int msg, WPARAM wParam, LPARAM lParam) {
		...
		case OS.WM_CAPTURECHANGED:
			...
			break;
		case OS.WM_CHANGEUISTATE:
			result = WM_CHANGEUISTATE(wParam, lParam);
			break;
		case OS.WM_CHAR:
		...
		case OS.WM_IME_COMPOSITION:
			...
			break;
		case OS.WM_IME_COMPOSITION_START:
			result = WM_IME_COMPOSITION_START(wParam, lParam);
			break;
		case OS.WM_IME_ENDCOMPOSITION:
			result = WM_IME_ENDCOMPOSITION(wParam, lParam);
			break;
		...
		case OS.WM_NCLBUTTONDOWN:
			...
			break;
		case OS.WM_NCPAINT:
			result = WM_NCPAINT(wParam, lParam);
			break;
		case OS.WM_NOTIFY:
		...
		case OS.WM_UPDATEUISTATE:
			result = WM_UPDATEUISTATE(wParam, lParam);
			break;
		case OS.WM_VSCROLL:
	...
	LRESULT WM_CHANGEUISTATE(WPARAM wParam, LPARAM lParam) {
		if ((state & IGNORE_WM_CHANGEUISTATE) !is 0)
			return LRESULT.ZERO;
		return null;
	}
    ...
    */
				...
				if ((lpwp.flags & OS.SWP_NOMOVE) is 0 || (lpwp.flags & OS.SWP_NOSIZE) is 0) {
					...
					if (width !is 0 && height !is 0) {
						...
						static if (OS.IsWinCE) {
							...
							auto rgn1 = OS.CreateRectRgn(rect.left, rect.top,
									rect.right, rect.bottom);
							auto rgn2 = OS.CreateRectRgn(lpwp.x, lpwp.y,
	...
	LRESULT wmColorChild(WPARAM wParam, LPARAM lParam) {
		...
		if (control.backgroundImage !is null) {
			...
			OS.SetBrushOrgEx(cast(HANDLE) wParam, -rect.left - lpPoint.x,
					-rect.top - lpPoint.y, &lpPoint);
			auto hBrush = findBrush(cast(ptrdiff_t) hBitmap, OS.BS_PATTERN);
			if ((state & DRAW_BACKGROUND) !is 0) {
				auto hOldBrush = OS.SelectObject(cast(HANDLE) wParam, hBrush);
				OS.MapWindowPoints(hwnd, handle, cast(POINT*)&rect, 2);
				OS.PatBlt(cast(HANDLE) wParam, rect.left, rect.top,
						rect.right - rect.left, rect.bottom - rect.top, OS.PATCOPY);
