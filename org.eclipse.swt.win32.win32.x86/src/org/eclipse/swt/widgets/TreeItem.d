 ...
 */
	public this(TreeItem parentItem, int style) {
		this(checkNull(parentItem).parent, style, parentItem.handle,
				cast(HANDLE) OS.TVI_LAST, null);
	}
 ...
 */
	public this(TreeItem parentItem, int style, int index) {
		this(checkNull(parentItem).parent, style, parentItem.handle,
				findPrevious(parentItem, index), null);
	}
	...
	static HANDLE findPrevious(Tree parent, int index) {
		...
		if (index is 0)
			return cast(HANDLE) OS.TVI_FIRST;
	...
	static HANDLE findPrevious(TreeItem parentItem, int index) {
		...
		HANDLE hFirstItem = cast(HANDLE) OS.SendMessage(hwnd,
 ...
 */
	public void clear(int index, bool all) {
		checkWidget();
		auto hwnd = parent.handle;
		HANDLE hItem = cast(HANDLE) OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM,
 ...
 */
	...
	HFONT fontHandle(int index) {
		if (cellFont !is null && cellFont[index]!is null)
 ...
 */
	...
	RECT* getBounds(int index, bool getText, bool getImage, bool fullText,
			bool fullImage, bool clip, HDC hDC) {
		if (!getText && !getImage)
			return new RECT();
		auto hwnd = parent.handle;
					...
					if (fullText && clip) {
						...
						if (string !is null) {
							...
							if (getImage) {
								...
								rect.left = rect.right + Tree.INSET;
								rect.right = rect.left + (textRect.right - textRect.left)
									+ Tree.INSET;
							}
						}
					}
 ...
 */
	public TreeItem getItem(int index) {
		...
		if (!parent.checkData(this, true))
			error(SWT.ERROR_WIDGET_DISPOSED);
		auto hwnd = parent.handle;
		auto hFirstItem = cast(HANDLE) OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM,
				OS.TVGN_CHILD, handle);
 ...
 */
	...
	void redraw() {
		if (parent.currentItem is this || parent.drawCount !is 0)
			return;
		auto hwnd = parent.handle;
 ...
 */
	public void removeAll() {
		...
		TVITEM tvItem;
		tvItem.mask = OS.TVIF_HANDLE | OS.TVIF_PARAM;
		tvItem.hItem = cast(HTREEITEM) OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM,
				OS.TVGN_CHILD, handle);
		while (tvItem.hItem !is null) {
			...
			tvItem.hItem = cast(HTREEITEM) OS.SendMessage(hwnd,
					OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, handle);
		}
	}
 ...
 */
	public void setBackground(int index, Color color) {
		...
		if (cellBackground[index] is pixel)
			return;
 ...
 */
	public void setExpanded(bool expanded) {
        ...
        */
		...
		if (((state & OS.TVIS_EXPANDED) !is 0) is expanded)
			return;
    ...
    */
		...
		HANDLE hTopItem = cast(HANDLE) OS.SendMessage(hwnd, OS.TVM_GETNEXTITEM,
		...
		if (noScroll && hTopItem !is null) {
			...
			if (parent.drawCount is 0 && OS.IsWindowVisible(hwnd)) {
				...
				while (hItem !is null && (noAnimate || hItem !is handle) && index < count) {
					...
					hItem = cast(HANDLE) OS.SendMessage(hwnd,
    ...
    */
		...
		if (noScroll && hTopItem !is null) {
			...
			if (!expanded) {
				RECT rect;
				while (hTopItem !is null && !OS.TreeView_GetItemRect(hwnd,
						cast(HTREEITEM) hTopItem, &rect, false)) {
					hTopItem = cast(HANDLE) OS.SendMessage(hwnd,
							OS.TVM_GETNEXTITEM, OS.TVGN_PARENT, hTopItem);
					collapsed = true;
				}
			}
			bool scrolled = true;
			if (hTopItem !is null) {
				...
				scrolled = hTopItem !is cast(HANDLE) OS.SendMessage(hwnd,
            ...
            */
				...
				if (fixScroll) {
					...
					if (handle is hBottomItem) {
						RECT rect;
						if (OS.TreeView_GetItemRect(hwnd,
								cast(HTREEITEM) hBottomItem, &rect, false)) {
							OS.InvalidateRect(hwnd, &rect, true);
						}
					}
				}
 ...
 */
	public void setForeground(int index, Color color) {
		...
		if (cellForeground[index] is pixel)
			return;
 ...
 */
	public void setImage(int index, Image image) {
        ...
        */
			...
			bool drawText = (image is null && oldImage !is null) || (image !is null
					&& oldImage is null);
