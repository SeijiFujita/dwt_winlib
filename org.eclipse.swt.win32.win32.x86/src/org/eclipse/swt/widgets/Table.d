...
public class Table : Composite {
	...
	bool ignoreCustomDraw, ignoreDrawForeground, ignoreDrawBackground,
		ignoreDrawFocus, ignoreDrawSelection, ignoreDrawHot;
	bool customDraw, dragStarted, explorerTheme, firstColumnImage,
		fixScrollWidth, tipRequested, wasSelected, wasResized;
	bool ignoreActivate, ignoreSelect, ignoreShrink, ignoreResize,
 ...
 */
			...
			if (OS.IsWindowVisible(handle) && OS.IsWindowEnabled(handle)) {
				if (!explorerTheme && (style & SWT.FULL_SELECTION) !is 0) {
					if (OS.SendMessage(handle, OS.LVM_GETBKCOLOR, 0, 0) is OS.CLR_NONE) {
						auto dwExStyle = OS.SendMessage(handle,
								OS.LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
						if ((dwExStyle & OS.LVS_EX_FULLROWSELECT) is 0) {
							//                      if ((nmcd.uItemState & OS.CDIS_FOCUS) !is 0) {
							if (OS.SendMessage(handle, OS.LVM_GETNEXTITEM, -1,
									OS.LVNI_FOCUSED) is nmcd.nmcd.dwItemSpec) {
								if (handle is OS.GetFocus()) {
									...
									if ((uiState & OS.UISF_HIDEFOCUS) is 0) {
										...
										OS.SendMessage(handle, OS.LVM_GETITEMRECT,
												nmcd.nmcd.dwItemSpec, &rect);
										auto hwndHeader = cast(HWND) OS.SendMessage(handle,
												OS.LVM_GETHEADER, 0, 0);
										auto index = OS.SendMessage(hwndHeader,
												OS.HDM_ORDERTOINDEX, 0, 0);
										RECT itemRect;
										if (index is 0) {
											itemRect.left = OS.LVIR_LABEL;
											OS.SendMessage(handle,
													OS.LVM_GETITEMRECT, index, &itemRect);
										}
										else {
											itemRect.top = cast(int) /*64bit*/ index;
											itemRect.left = OS.LVIR_ICON;
											OS.SendMessage(handle, OS.LVM_GETSUBITEMRECT,
													nmcd.nmcd.dwItemSpec, &itemRect);
										}
    ...
    */
		...
		if (explorerTheme && !ignoreCustomDraw) {
			hotIndex = (nmcd.nmcd.uItemState & OS.CDIS_HOT) !is 0 ? cast(int) /*64bit*/ nmcd.nmcd.dwItemSpec
				: -1;
			if (hooks(SWT.EraseItem) && nmcd.nmcd.rc.left !is nmcd.nmcd.rc.right) {
		...
		if (--customCount is 0 && OS.IsWindowVisible(handle)) {
			if (!explorerTheme && (style & SWT.FULL_SELECTION) !is 0) {
				if (OS.SendMessage(handle, OS.LVM_GETBKCOLOR, 0, 0) is OS.CLR_NONE) {
                    ...
                    */
						hwndToolTip = OS.SendMessage(handle,
								OS.LVM_SETTOOLTIPS, hwndToolTip, hwndToolTip);
					}
				}
			}
		}
    ...
    */
							...
							if (OS.COMCTL32_MAJOR >= 6 && OS.IsAppThemed()) {
								if (sortColumn !is null && sortDirection !is SWT.NONE) {
									...
									if (index !is -1) {
										...
										auto hwndHeader = cast(HWND) OS.SendMessage(handle,
												OS.LVM_GETHEADER, 0, 0);
										if (OS.SendMessage(hwndHeader,
												OS.HDM_GETITEMRECT, index, &headerRect) !is 0) {
											OS.MapWindowPoints(hwndHeader, handle,
													cast(POINT*)&headerRect, 2);
											columnRect.left = headerRect.left;
											columnRect.right = headerRect.right;
											if (OS.IntersectRect(&columnRect,
													&columnRect, &rect)) {
												fillBackground(nmcd.nmcd.hdc,
														clrSortBk, &columnRect);
											}
										}
									}
								}
	...
	LRESULT CDDS_SUBITEMPREPAINT(NMLVCUSTOMDRAW* nmcd, WPARAM wParam, LPARAM lParam) {
		auto hDC = nmcd.nmcd.hdc;
		if (explorerTheme && !ignoreCustomDraw && hooks(SWT.EraseItem)
				&& (nmcd.nmcd.rc.left !is nmcd.nmcd.rc.right)) {
			OS.RestoreDC(hDC, -1);
		}
		/*
    ...
    */
		...
		if (OS.IsWindowVisible(handle)) {
			...
			if (hooks(SWT.MeasureItem)) {
				measureEvent = sendMeasureItemEvent(item,
						cast(int) /*64bit*/ nmcd.nmcd.dwItemSpec, nmcd.iSubItem, nmcd.nmcd.hdc);
    ...
    */
                ...
                */
					...
					if ((result !is 0 && (lvItem.state & OS.LVIS_SELECTED) !is 0)) {
						...
						if (clrSelection !is -1) {
							RECT rect = item.getBounds(cast(int) /*64bit*/ nmcd.nmcd.dwItemSpec,
									nmcd.iSubItem, true, nmcd.iSubItem !is 0, true, false, hDC);
        ...
        */
			...
			if (hFont is cast(HFONT)-1 && clrText is -1 && clrTextBk is -1) {
				if (item.cellForeground is null && item.cellBackground is null
						&& item.cellFont is null) {
					auto hwndHeader = cast(HWND) OS.SendMessage(handle, OS.LVM_GETHEADER, 0, 0);
 ...
 */
	...
	override public Point computeSize(int wHint, int hHint, bool changed) {
		...
		auto result = OS.SendMessage(handle, OS.LVM_APPROXIMATEVIEWRECT, -1,
				OS.MAKELPARAM(bits, 0xFFFF));
    ...
    */
	...
	void createItem(TableItem item, int index) {
        ...
        */
			...
			int length = small ? cast(int) /*64bit*/ items.length + 4
				: Math.max(4, cast(int) /*64bit*/ items.length * 3 / 2);
 ...
 */
    ...
    */
		if ((style & SWT.CHECK) is 0)
			return;
		HANDLE hImageList = cast(HANDLE) OS.SendMessage(handle,
				OS.LVM_GETIMAGELIST, OS.LVSIL_SMALL, 0);
		if (hImageList is null)
			return;
		int cx, cy;
		OS.ImageList_GetIconSize(hImageList, &cx, &cy);
		HANDLE hStateList = cast(HANDLE) OS.SendMessage(handle,
				OS.LVM_GETIMAGELIST, OS.LVSIL_STATE, 0);
		if (hStateList is null)
			return;
		int stateCx, stateCy;
		OS.ImageList_GetIconSize(hStateList, &stateCx, &stateCy);
		if (cx is stateCx && cy is stateCy)
			return;
 ...
 */
	...
	int getFocusIndex() {
		//  checkWidget ();
		return cast(int) /*64bit*/ OS.SendMessage(handle, OS.LVM_GETNEXTITEM,
				-1, OS.LVNI_FOCUSED);
	}
 ...
 */
	public int getSelectionIndex() {
		...
		if (focusIndex is selectedIndex)
 ...
 */
	public int indexOf(TableItem item) {
		...
		if (1 <= lastIndexOf && lastIndexOf < count - 1) {
			if (items[lastIndexOf] is item)
				return lastIndexOf;
			if (items[lastIndexOf + 1] is item)
				return ++lastIndexOf;
			if (items[lastIndexOf - 1] is item)
				return --lastIndexOf;
		}
 ...
 */
	public void remove(int start, int end) {
		...
		if (start is 0 && end is count - 1) {
			...
			for (auto i = count - (index - start); i < count; i++)
				items[i] = null;
			if (index <= end)
				error(SWT.ERROR_ITEM_NOT_REMOVED);
			/*
 ...
 */
	public void select(int start, int end) {
		checkWidget();
		if (end < 0 || start > end || ((style & SWT.SINGLE) !is 0 && start !is end))
			return;
		auto count = OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0, 0);
		if (count is 0 || start >= count)
			return;
 ...
 */
    ...
    */
		...
		RECT cellRect = item.getBounds(cast(int) /*64bit*/ nmcd.nmcd.dwItemSpec,
				nmcd.iSubItem, true, true, true, true, hDC);
		Event event = new Event();
		event.item = item;
		event.gc = gc;
		...
		if (ignoreDrawForeground && ignoreDrawHot) {
			if (!ignoreDrawBackground && drawBackground) {
				RECT backgroundRect = item.getBounds(cast(int) /*64bit*/ nmcd.nmcd.dwItemSpec,
						nmcd.iSubItem, true, false, true, false, hDC);
		...
		if (!ignoreDrawHot || !ignoreDrawSelection || !ignoreDrawFocus) {
			...
			RECT textRect = item.getBounds(cast(int) /*64bit*/ nmcd.nmcd.dwItemSpec,
					nmcd.iSubItem, true, false, fullText, false, hDC);
			if ((style & SWT.FULL_SELECTION) is 0) {
			...
			if (explorerTheme) {
				if (!ignoreDrawHot || (!ignoreDrawSelection && clrSelectionBk !is -1)) {
					bool hot = drawHot;
					RECT pClipRect;
					OS.SetRect(&pClipRect, nmcd.nmcd.rc.left,
							nmcd.nmcd.rc.top, nmcd.nmcd.rc.right, nmcd.nmcd.rc.bottom);
					RECT rect;
					OS.SetRect(&rect, nmcd.nmcd.rc.left, nmcd.nmcd.rc.top,
							nmcd.nmcd.rc.right, nmcd.nmcd.rc.bottom);
					...
					OS.DrawThemeBackground(hTheme, hDC, OS.LVP_LISTITEM,
							iStateId, &rect, &pClipRect);
					OS.CloseThemeData(hTheme);
				}
			}
			else {
				if (!ignoreDrawSelection && clrSelectionBk !is -1)
		...
		if (ignoreDrawForeground) {
			RECT clipRect = item.getBounds(cast(int) /*64bit*/ nmcd.nmcd.dwItemSpec,
					nmcd.iSubItem, true, true, true, false, hDC);
			OS.SaveDC(hDC);
    ...
    */
		if (fullRowSelect) {
			...
			OS.SendMessage(handle, OS.LVM_SETEXTENDEDLISTVIEWSTYLE,
					OS.LVS_EX_FULLROWSELECT, OS.LVS_EX_FULLROWSELECT);
		}
		dragStarted = false;
    ...
    */
		...
		if (OS.IsWindowEnabled(handle)) {
			if (selected && (nmcd.iSubItem is 0 || (style & SWT.FULL_SELECTION) !is 0)) {
				...
				if (explorerTheme && selectionForeground is -1) {
					int clrText = item.cellForeground !is null
						? item.cellForeground[nmcd.iSubItem] : -1;
					if (clrText is -1)
						clrText = item.foreground;
					data.foreground = clrText !is -1 ? clrText : getForegroundPixel();
				}
				...
				int clrText = item.cellForeground !is null ? item.cellForeground[nmcd.iSubItem]
					: -1;
				if (clrText is -1)
					clrText = item.foreground;
				int clrTextBk = item.cellBackground !is null
					? item.cellBackground[nmcd.iSubItem] : -1;
				if (clrTextBk is -1)
					clrTextBk = item.background;
				drawBackground = clrTextBk !is -1;
				/*
		...
		auto nSavedDC = OS.SaveDC(hDC);
		GC gc = GC.win32_new(hDC, data);
		RECT itemRect = item.getBounds(cast(int) /*64bit*/ nmcd.nmcd.dwItemSpec,
				nmcd.iSubItem, true, true, false, false, hDC);
		Event event = new Event();
		event.item = item;
		event.gc = gc;
		event.index = nmcd.iSubItem;
 ...
 */
	...
	void setCustomDraw(bool customDraw) {
		if (this.customDraw is customDraw)
			return;
				...
				if (hooks(SWT.MeasureItem) || hooks(SWT.EraseItem) || hooks(SWT.PaintItem)) {
					if (--drawCount is 0 /*&& OS.IsWindowVisible (handle)*/ ) {
						...
						static if (OS.IsWinCE) {
							...
							int flags = OS.RDW_ERASE | OS.RDW_FRAME
    ...
    */
		if (!OS.IsWinCE && OS.WIN32_VERSION >= OS.VERSION(6, 0)) {
			auto hImageList = cast(HANDLE) OS.SendMessage(handle,
					OS.LVM_GETIMAGELIST, OS.LVSIL_SMALL, 0);
			OS.SendMessage(handle, OS.LVM_SETIMAGELIST, OS.LVSIL_SMALL, hImageList);
		}
		if (fixScroll && topIndex !is 0) {
    ...
    */
			...
			while (index < itemCount) {
				...
				if (string !is null && string.length !is 0) {
					if (hFont !is cast(HFONT)-1) {
						...
						newWidth = cast(int) /*64bit*/ Math.max(newWidth, OS.SendMessage(handle,
								OS.LVM_GETSTRINGWIDTH, 0, cast(void*) buffer));
					}
				}
				if (item !is null)
					break;
        ...
        */
			if (newWidth is 0) {
				...
				newWidth = cast(int) /*64bit*/ Math.max(newWidth, OS.SendMessage(handle,
						OS.LVM_GETSTRINGWIDTH, 0, cast(void*) buffer.ptr));
			}
			auto hStateList = cast(HANDLE) OS.SendMessage(handle,
					OS.LVM_GETIMAGELIST, OS.LVSIL_STATE, 0);
			if (hStateList !is null) {
 ...
 */
	...
	void setSubImagesVisible(bool visible) {
		auto dwExStyle = OS.SendMessage(handle, OS.LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
		if (((dwExStyle & OS.LVS_EX_SUBITEMIMAGES) !is 0) is visible)
			return;
 ...
 */
    ...
    */
		...
		if (fixSubclass) {
			oldTableProc = cast(WNDPROC) OS.SetWindowLongPtr(handle,
					OS.GWLP_WNDPROC, cast(LONG_PTR) TableProc);
			oldHeaderProc = cast(WNDPROC) OS.SetWindowLongPtr(hwndHeader,
					OS.GWLP_WNDPROC, cast(LONG_PTR) HeaderProc);
		}
		super.update(all);
		if (fixSubclass) {
    ...
    */
	...
	override.LRESULT windowProc(HWND hwnd, int msg, WPARAM wParam, LPARAM lParam) {
                ...
                */
					if (OS.COMCTL32_MAJOR < 6) {
						if (lParam !is 0) {
							auto hwndHeader = cast(HWND) OS.SendMessage(handle,
									OS.LVM_GETHEADER, 0, 0);
							if (lParam !is cast(ptrdiff_t) hwndHeader)
								OS.InvalidateRect(hwndHeader, null, true);
						}
					}
					break;
        ...
        */
			if ((!OS.IsWinCE && OS.WIN32_VERSION >= OS.VERSION(6, 0))
					|| (style & SWT.VIRTUAL) !is 0 || hooks(SWT.EraseItem) || hooks(SWT.PaintItem)) {
				auto topIndex = OS.SendMessage(handle, OS.LVM_GETTOPINDEX, 0, 0);
				auto selection = OS.SendMessage(handle, OS.LVM_GETNEXTITEM,
						topIndex - 1, OS.LVNI_SELECTED);
				if (selection is -1)
					return 0;
				POINT mousePos;
				...
				while ((selection = OS.SendMessage(handle, OS.LVM_GETNEXTITEM,
						selection, OS.LVNI_SELECTED)) !is -1) {
					if (rect.bottom - rect.top > DRAG_IMAGE_SIZE)
						break;
					if (rect.bottom > clientRect.bottom)
						break;
					RECT itemRect = item.getBounds(cast(int) /*64bit*/ selection,
							0, true, true, true);
					auto rectRgn = OS.CreateRectRgn(rect.left, itemRect.top,
							rect.right, itemRect.bottom);
	...
	override LRESULT WM_CHAR(WPARAM wParam, LPARAM lParam) {
		...
		case ' ':
			if ((style & SWT.CHECK) !is 0) {
				int index = cast(int) /*64bit*/ OS.SendMessage(handle,
						OS.LVM_GETNEXTITEM, -1, OS.LVNI_FOCUSED);
				if (index !is -1) {
					TableItem item = _getItem(index);
					item.setChecked(!item.getChecked(), true);
					static if (!OS.IsWinCE) {
						OS.NotifyWinEvent(OS.EVENT_OBJECT_FOCUS, handle,
								OS.OBJID_CLIENT, index + 1);
					}
				}
			}
			/*
	...
	override LRESULT WM_KEYDOWN(WPARAM wParam, LPARAM lParam) {
		...
		case OS.VK_END:
            ...
            */
			...
			bool fixSubclass = isOptimizedRedraw();
			if (fixSubclass) {
				oldTableProc = OS.SetWindowLongPtr(handle, OS.GWLP_WNDPROC,
						cast(LONG_PTR) TableProc);
				oldHeaderProc = OS.SetWindowLongPtr(hwndHeader,
						OS.GWLP_WNDPROC, cast(LONG_PTR) HeaderProc);
			}
			auto code = callWindowProc(handle, OS.WM_KEYDOWN, wParam, lParam);
		...
		case OS.VK_DOWN:
			OS.SendMessage(handle,
					OS.WM_CHANGEUISTATE, OS.UIS_INITIALIZE, 0);
			break;
		default:
		}
		return result;
			...
			if ((style & SWT.DOUBLE_BUFFERED) !is 0 || findImageControl() !is null) {
				...
				if ((bits & OS.LVS_EX_DOUBLEBUFFER) is 0) {
					...
					if (width !is 0 && height !is 0) {
						...
						OS.BitBlt(paintDC, ps.rcPaint.left, ps.rcPaint.top,
								width, height, hDC, 0, 0, OS.SRCCOPY);
			...
			if (OS.COMCTL32_MAJOR >= 6) {
				if (columnCount > H_SCROLL_LIMIT) {
					...
					if (rowCount > V_SCROLL_LIMIT)
						fixScroll = drawCount is 0 && OS.IsWindowVisible(handle);
				}
			}
		}
		if (fixScroll)
	...
	override LRESULT wmNotifyChild(NMHDR* hdr, WPARAM wParam, LPARAM lParam) {
		...
		case OS.LVN_GETDISPINFOW: {
            ...
            */
				if ((style & SWT.VIRTUAL) !is 0) {
					if (ignoreShrink) {
						OS.SendMessage(handle, OS.LVM_REDRAWITEMS,
								plvfi.item.iItem, plvfi.item.iItem);
						break;
					}
				}
            ...
            */
				...
				if ((plvfi.item.mask & OS.LVIF_TEXT) !is 0) {
					...
					if (plvfi.item.iSubItem is 0) {
						...
						String[] strings = item.strings;
						if (strings !is null)
							string = strings[plvfi.item.iSubItem];
					}
					if (string !is null) {
						/*
                    ...
                    */
						...
						static if (OS.IsUnicode) {
							...
							OS.WideCharToMultiByte(getCodePage(), 0, buffer.ptr, length_,
									plvfi.item.pszText, plvfi.item.cchTextMax, null, null);
							byte dummy;
							OS.MoveMemory(plvfi.item.pszText + plvfi.item.cchTextMax - 1,
									&dummy, 1);
						}
					}
				}
				...
				if ((plvfi.item.mask & OS.LVIF_IMAGE) !is 0) {
					...
					if (plvfi.item.iSubItem is 0) {
						...
						Image[] images = item.images;
						if (images !is null)
							image = images[plvfi.item.iSubItem];
					}
					if (image !is null) {
		...
		case OS.NM_CUSTOMDRAW: {
                ...
                */
				...
				case OS.CDDS_SUBITEMPREPAINT:
					return CDDS_SUBITEMPREPAINT(nmcd, wParam, lParam);
				case OS.CDDS_SUBITEMPOSTPAINT:
					return CDDS_SUBITEMPOSTPAINT(nmcd, wParam, lParam);
    ...
    */
		...
		case OS.HDN_BEGINDRAG: {
				if (ignoreColumnMove)
					return LRESULT.ONE;
				auto bits = OS.SendMessage(handle, OS.LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
				if ((bits & OS.LVS_EX_HEADERDRAGDROP) is 0)
					break;
				if (columnCount is 0)
					return LRESULT.ONE;
				NMHEADER* phdn = cast(NMHEADER*) lParam;
					...
					if (phdn.pitem !is null) {
						...
						if ((pitem.mask & OS.HDI_WIDTH) !is 0) {
                            ...
                            */
								...
								System.arraycopy(columns, 0, newColumns, 0, columnCount);
								int[] order = new int[columnCount];
								OS.SendMessage(handle, OS.LVM_GETCOLUMNORDERARRAY,
						...
						if (!isDisposed() && !item.isDisposed()) {
							...
							if (hdr.code is OS.TTN_SHOW) {
								...
								if (hdr.code is OS.TTN_GETDISPINFOA) {
									...
									OS.MoveMemory(cast(void*) lpnmtdi,
											cast(void*) lParam, NMTTDISPINFOA.sizeof);
								...
								RECT cellRect = item.getBounds(pinfo.iItem,
										pinfo.iSubItem, true, true, true, true, hDC);
								if (itemRect.right > cellRect.right) {
									...
									if (string !is null) {
										...
										if (hdr.code is OS.TTN_GETDISPINFOA) {
											...
											OS.WideCharToMultiByte(getCodePage(), 0, chars.ptr,
													cast(int) /*64bit*/ chars.length, bytes.ptr,
													cast(int) /*64bit*/ bytes.length, null, null);
											shell.setToolTipText(lpnmtdi, bytes);
											OS.MoveMemory(lParam, cast(NMTTDISPINFOA*) lpnmtdi,
													NMTTDISPINFOA.sizeof);
										}
										else {
											shell.setToolTipText(lpnmtdi, chars);
											OS.MoveMemory(lParam, cast(NMTTDISPINFOW*) lpnmtdi,
													NMTTDISPINFOW.sizeof);
										}
									}
								}
							}
	...
	LRESULT wmNotifyToolTip(NMTTCUSTOMDRAW* nmcd, LPARAM lParam) {
		...
		case OS.CDDS_POSTPAINT: {
				...
				if (OS.SendMessage(handle, OS.LVM_SUBITEMHITTEST, 0, &pinfo) !is -1) {
					...
					RECT cellRect = item.getBounds(pinfo.iItem, pinfo.iSubItem,
							true, true, false, false, hDC);
					if (hooks(SWT.EraseItem)) {
					...
					if (drawForeground) {
						...
						if (image !is null) {
							...
							RECT imageRect = item.getBounds(pinfo.iItem,
									pinfo.iSubItem, false, true, false, false, hDC);
							Point size = imageList is null ? new Point(rect.width, rect.height)
					...
					if (hooks(SWT.PaintItem)) {
						RECT itemRect = item.getBounds(pinfo.iItem,
								pinfo.iSubItem, true, true, false, false, hDC);
