 ...
 */
	...
	RECT getBounds(int row, int column, bool getText, bool getImage, bool fullText,
			...
			if (parent.explorerTheme) {
				...
				if (getText) {
					...
					if (hFont is cast(HFONT)-1 && hDC is null) {
						...
						OS.DrawText(textDC, buffer.ptr, cast(int) /*64bit*/ buffer.length,
								&textRect, flags);
						width = textRect.right - textRect.left;
						if (hDC is null) {
							if (oldFont !is cast(HFONT)-1)
        ...
        */
			bool hasImage = (column is 0 && image !is null) || (images !is null
					&& images[column]!is null);
			rect.top = column;
