 ...
 */
	...
	override void fixChildren(Shell newShell, Shell oldShell,
			Decorations newDecorations, Decorations oldDecorations, Menu[] menus) {
			...
			if ((style & SWT.TRANSPARENT) !is 0) {
				...
				while (hwndChild !is null) {
					if (hwndChild !is handle) {
						int bits = OS.GetWindowLong(hwndParent, OS.GWL_EXSTYLE);
						if ((bits & OS.WS_EX_TRANSPARENT) !is 0)
							return;
					}
					hwndChild = OS.GetWindow(hwndChild, OS.GW_HWNDNEXT);
				}
	...
	bool resizeChildren(bool defer, WINDOWPOS*[] pwp) {
		...
		for (int i = 0; i < pwp.length; i++) {
            ...
            */
				...
				//          }
				if (defer) {
					hdwp = DeferWindowPos(hdwp, wp._hwnd, null, wp.x, wp.y,
			...
			if (processID is OS.GetCurrentProcessId()) {
				if (display.msgHook is null) {
					static if (!OS.IsWinCE) {
						...
						display.msgHook = OS.SetWindowsHookEx(OS.WH_GETMESSAGE,
								&Display.getMsgFunc, OS.GetLibraryHandle(), threadId);
		...
		if ((state & CANVAS) !is 0) {
			...
			case OS.VK_NEXT:
				...
				if ((uiState & OS.UISF_HIDEFOCUS) !is 0) {
					OS.SendMessage(msg.hwnd, OS.WM_UPDATEUISTATE,
    ...
    */
	...
	override LRESULT WM_PARENTNOTIFY(WPARAM wParam, LPARAM lParam) {
		if ((state & CANVAS) !is 0 && (style & SWT.EMBEDDED) !is 0) {
			if (OS.LOWORD(wParam) is OS.WM_CREATE) {
				RECT rect;
				OS.GetClientRect(handle, &rect);
				resizeEmbeddedHandle(cast(HANDLE) lParam, rect.right - rect.left,
		...
		if (hooks(SWT.Paint)) {
			...
			if (bufferedPaint) {
				...
				if (width !is 0 && height !is 0) {
					...
					RECT prcTarget;
					OS.SetRect(&prcTarget, ps.rcPaint.left, ps.rcPaint.top,
							ps.rcPaint.right, ps.rcPaint.bottom);
					auto hBufferedPaint = OS.BeginBufferedPaint(hDC,
							&prcTarget, flags, null, &phdc);
					...
					data.font = Font.win32_new(display,
							cast(HANDLE) OS.SendMessage(handle, OS.WM_GETFONT, 0, 0));
				...
				if ((style & (SWT.DOUBLE_BUFFERED | SWT.TRANSPARENT)) !is 0
						|| (style & SWT.NO_MERGE_PAINTS) !is 0) {
					...
					if (OS.GetRandomRgn(gc.handle, sysRgn, OS.SYSRGN) is 1) {
						if (OS.WIN32_VERSION >= OS.VERSION(4, 10)) {
							if ((OS.GetLayout(gc.handle) & OS.LAYOUT_RTL) !is 0) {
								...
								HRGN newSysRgn = OS.ExtCreateRegion(cast(XFORM*)[-1.0f,
				...
				if (width !is 0 && height !is 0) {
					...
					if ((style & (SWT.DOUBLE_BUFFERED | SWT.TRANSPARENT)) !is 0) {
						...
						if ((style & SWT.TRANSPARENT) !is 0) {
							OS.BitBlt(gc.handle, 0, 0, width, height,
									paintGC.handle, ps.rcPaint.left, ps.rcPaint.top, OS.SRCCOPY);
						}
					...
					if ((style & SWT.NO_MERGE_PAINTS) !is 0
							&& OS.GetRgnBox(sysRgn, &rect) is OS.COMPLEXREGION) {
						int nBytes = OS.GetRegionData(sysRgn, 0, null);
						int[] lpRgnData = new int[nBytes / 4];
						OS.GetRegionData(sysRgn, nBytes, cast(RGNDATA*) lpRgnData.ptr);
						...
						for (int i = 0; i < count; i++) {
							int offset = 8 + (i << 2);
							OS.SetRect(&rect, lpRgnData[offset], lpRgnData[offset + 1],
									lpRgnData[offset + 2], lpRgnData[offset + 3]);
							if (
								(style & (SWT.DOUBLE_BUFFERED | SWT.NO_BACKGROUND | SWT.TRANSPARENT)) is 0) {
								drawBackground(gc.handle, &rect);
							}
							event.x = rect.left;
							event.y = rect.top;
					...
					if ((style & (SWT.DOUBLE_BUFFERED | SWT.TRANSPARENT)) !is 0) {
						...
						gc.dispose();
						if (!isDisposed())
							paintGC.drawImage(image, ps.rcPaint.left, ps.rcPaint.top);
						image.dispose();
						gc = paintGC;
					}
				}
	...
	override LRESULT WM_SIZE(WPARAM wParam, LPARAM lParam) {
        ...
        */
		...
		if ((state & CANVAS) !is 0 && (style & SWT.EMBEDDED) !is 0) {
			resizeEmbeddedHandle(OS.GetWindow(handle, OS.GW_CHILD),
					OS.LOWORD(lParam), OS.HIWORD(lParam));
		}
		return result;
	}
		...
		if ((state & CANVAS) !is 0 || (hwnd is borderHandle && handle !is borderHandle)) {
			if (OS.COMCTL32_MAJOR >= 6 && OS.IsAppThemed()) {
				...
				if ((bits1 & OS.WS_EX_CLIENTEDGE) !is 0) {
					...
					OS.ExcludeClipRect(hDC, border, border, rect.right - border,
							rect.bottom - border);
					OS.DrawThemeBackground(display.hEditTheme(), hDC,
            ...
            */
			...
			case OS.TTN_POP: {
                ...
                */
					...
					HWND hwndInsertAfter = cast(HWND)(hdr.code is OS.TTN_SHOW
							? OS.HWND_TOPMOST : OS.HWND_NOTOPMOST);
			...
			case OS.TTN_GETDISPINFOW: {
                    ...
                    */
						...
						if ((lpnmtdi.uFlags & OS.TTF_IDISHWND) !is 0) {
							...
							if (hdr.hwndFrom is shell.toolTipHandle
									|| hdr.hwndFrom is shell.balloonTipHandle) {
