 ...
 */
	...
	private static extern (Windows) UINT_PTR OFNHookProc(HWND hdlg, uint uiMsg,
			...
			if (ofn.hdr.code is OS.CDN_SELCHANGE) {
				...
				if (lResult > 0) {
					...
					if (lpofn.nMaxFile < lResult) {
						...
						auto lpstrFile = cast(TCHAR*) OS.HeapAlloc(hHeap,
								OS.HEAP_ZERO_MEMORY, lResult * TCHAR.sizeof);
						if (lpstrFile !is null) {
							if (lpofn.lpstrFile !is null)
 ...
 */
    ...
    */
		...
		if (!OS.IsWinCE && OS.WIN32_VERSION >= OS.VERSION(4, 10)) {
			...
			if (dialogOrientation !is parentOrientation) {
				...
				if (dialogOrientation is SWT.RIGHT_TO_LEFT)
					exStyle |= OS.WS_EX_LAYOUTRTL;
				hwndOwner = OS.CreateWindowEx(exStyle, Shell.DialogClass.ptr, null, 0,
						OS.CW_USEDEFAULT, 0, OS.CW_USEDEFAULT, 0, hwndParent,
    ...
    */
		...
		if (save && overwrite)
			struct_.Flags |= OS.OFN_OVERWRITEPROMPT;
		if ((style & SWT.MULTI) !is 0) {
    ...
    */
		bool success = cast(bool)((save) ? OS.GetSaveFileName(&struct_)
				: OS.GetOpenFileName(&struct_));
		switch (OS.CommDlgExtendedError()) {
		case OS.FNERR_INVALIDFILENAME:
			OS.MoveMemory(lpstrFile, StrToTCHARz(""), TCHAR.sizeof);
			success = cast(bool)((save) ? OS.GetSaveFileName(&struct_)
					: OS.GetOpenFileName(&struct_));
			break;
        ...
        */
            ...
            */
				...
				do {
					int end = start;
					while (end < buffer.length && buffer[end]!is 0)
						end++;
