 ...
 */
	...
	bool fillAccel(ACCEL* accel) {
		...
		if (accelerator is 0 || !getEnabled())
			return false;
		if ((accelerator & SWT.COMMAND) !is 0)
			return false;
		...
		if ((accelerator & SWT.ALT) !is 0)
			accel.fVirt |= OS.FALT;
		if ((accelerator & SWT.SHIFT) !is 0)
			accel.fVirt |= OS.FSHIFT;
		if ((accelerator & SWT.CONTROL) !is 0)
 ...
 */
	...
	void selectRadio() {
		...
		while (index < items.length && items[index]!is this)
			index++;
		int i = index - 1;
 ...
 */
	public void setAccelerator(int accelerator) {
		checkWidget();
		if (this.accelerator is accelerator)
			return;
	...
	public void setEnabled(bool enabled) {
        ...
        */
			...
			static if (OS.IsWinCE) {
				...
				if (enabled) {
					if ((info.fState & bits) is 0)
						return;
					info.fState &= ~bits;
				}
				else {
					if ((info.fState & bits) is bits)
						return;
					info.fState |= bits;
				}
                ...
                */
					if (!OS.IsWinCE && OS.WIN32_VERSION >= OS.VERSION(6, 0)) {
						success = id is OS.GetMenuDefaultItem(hMenu,
 ...
 */
	...
	LRESULT wmDrawChild(WPARAM wParam, LPARAM lParam) {
        ...
        */
		...
		if (parent.foreground !is -1)
			OS.SetTextColor(struct_.hDC, parent.foreground);
		return null;
	}
