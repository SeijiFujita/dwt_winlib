...
public class Shell : Decorations {
	...
	alias setParent = Decorations.setParent;
	...
	const static int[] SYSTEM_COLORS = [
		OS.COLOR_BTNFACE, OS.COLOR_WINDOW, OS.COLOR_BTNTEXT,
		OS.COLOR_WINDOWTEXT, OS.COLOR_HIGHLIGHT, OS.COLOR_SCROLLBAR,
	];
 ...
 */
	...
	static int checkStyle(int style) {
		...
		int bits = style & ~mask;
		if ((style & SWT.SYSTEM_MODAL) !is 0)
			return bits | SWT.SYSTEM_MODAL;
		if ((style & SWT.APPLICATION_MODAL) !is 0)
			return bits | SWT.APPLICATION_MODAL;
		if ((style & SWT.PRIMARY_MODAL) !is 0)
 ...
 */
	...
	void createBalloonTipHandle() {
		balloonTipHandle_ = OS.CreateWindowEx(0, OS.TOOLTIPS_CLASS.dup.ptr, null,
				OS.TTS_ALWAYSTIP | OS.TTS_NOPREFIX | OS.TTS_BALLOON, OS.CW_USEDEFAULT,
    ...
    */
		...
		if (!embedded) {
			...
			static if (!OS.IsWinCE)
				bits |= OS.WS_POPUP;
			if ((style & SWT.TITLE) !is 0)
				bits |= OS.WS_CAPTION;
			if ((style & SWT.NO_TRIM) is 0) {
				if ((style & (SWT.BORDER | SWT.RESIZE)) is 0)
					bits |= OS.WS_BORDER;
			}
			/*
        ...
        */
			...
			int flags = OS.SWP_DRAWFRAME | OS.SWP_NOMOVE | OS.SWP_NOSIZE
	...
	void createToolTip(ToolTip toolTip) {
		...
		while (id < toolTips.length && toolTips[id]!is null)
			id++;
	...
	void createToolTipHandle() {
		toolTipHandle_ = OS.CreateWindowEx(0, OS.TOOLTIPS_CLASS.dup.ptr, null,
				OS.TTS_ALWAYSTIP | OS.TTS_NOPREFIX, OS.CW_USEDEFAULT, 0,
				OS.CW_USEDEFAULT, 0, handle, null, OS.GetModuleHandle(null), null);
    ...
    */
	...
	override void deregister() {
		super.deregister();
		if (toolTipHandle_ !is null)
			display.removeControl(toolTipHandle_);
		if (balloonTipHandle_ !is null)
			display.removeControl(balloonTipHandle_);
	}
    ...
    */
	...
	override HBRUSH findBrush(ptrdiff_t value, int lbStyle) {
		...
		if (brushes is null)
			brushes = new HBRUSH[BRUSHES_SIZE];
		LOGBRUSH logBrush;
		for (int i = 0; i < brushes.length; i++) {
			...
			case OS.BS_SOLID:
				if (lbStyle is OS.BS_SOLID) {
					if (logBrush.lbColor is value)
						return hBrush;
				}
				break;
			case OS.BS_PATTERN:
				if (lbStyle is OS.BS_PATTERN) {
					if (logBrush.lbHatch is value)
						return hBrush;
				}
				break;
			default:
 ...
 */
	public int getImeInputMode() {
		...
		if (open)
			open = cast(bool) OS.ImmGetConversionStatus(hIMC, &lpfdwConversion, &lpfdwSentence);
		OS.ImmReleaseContext(handle, hIMC);
		if (!open)
			return SWT.NONE;
		int result = 0;
		if ((lpfdwConversion & OS.IME_CMODE_ROMAN) !is 0)
			result |= SWT.ROMAN;
		if ((lpfdwConversion & OS.IME_CMODE_FULLSHAPE) !is 0)
			result |= SWT.DBCS;
		if ((lpfdwConversion & OS.IME_CMODE_KATAKANA) !is 0)
			return result | SWT.PHONETIC;
		if ((lpfdwConversion & OS.IME_CMODE_NATIVE) !is 0)
	...
	override public bool getMaximized() {
		checkWidget();
 ...
 */
	...
	HWND hwndMDIClient() {
		if (hwndMDIClient_ is null) {
			int widgetStyle = OS.MDIS_ALLCHILDSTYLES | OS.WS_CHILD
				| OS.WS_CLIPCHILDREN | OS.WS_CLIPSIBLINGS;
			hwndMDIClient_ = OS.CreateWindowEx(0, StrToTCHARz("MDICLIENT"), null,
					widgetStyle, 0, 0, 0, 0, handle, null,
					OS.GetModuleHandle(null), new CREATESTRUCT());
 ...
 */
    ...
    */
	...
	override public bool print(GC gc) {
		checkWidget();
		if (gc is null)
			error(SWT.ERROR_NULL_ARGUMENT);
		if (gc.isDisposed())
	...
	void releaseBrushes() {
		if (brushes !is null) {
			for (int i = 0; i < brushes.length; i++) {
				if (brushes[i]!is null)
					OS.DeleteObject(brushes[i]);
			}
		}
		brushes = null;
	}
 ...
 */
	...
	void setActiveControl(Control control) {
		if (control !is null && control.isDisposed())
			control = null;
		if (lastActive !is null && lastActive.isDisposed())
			lastActive = null;
		if (lastActive is control)
			return;
    ...
    */
		...
		lastActive = control;
		int index = 0, length = Math.min(cast(int) /*64bit*/ activate.length,
				cast(int) /*64bit*/ deactivate.length);
		while (index < length) {
			if (activate[index]!is deactivate[index])
				break;
			index++;
		}
 ...
 */
    ...
    */
	...
	override public void setEnabled(bool enabled) {
		checkWidget();
		if (((state & DISABLED) is 0) is enabled)
			return;
 ...
 */
	public void setMinimumSize(int width, int height) {
		...
		if (newWidth !is size.x || newHeight !is size.y)
 ...
 */
    ...
    */
	...
	override bool translateAccelerator(MSG* msg) {
		if (!isEnabled() || !isActive())
			return false;
		if (menuBar !is null && !menuBar.isEnabled())
			return false;
    ...
    */
	...
	override String windowClass() {
		...
		if ((style & SWT.TOOL) !is 0) {
			int trim = SWT.TITLE | SWT.CLOSE | SWT.MIN | SWT.MAX | SWT.BORDER | SWT.RESIZE;
			if ((style & trim) is 0)
				return TCHARsToStr(display.windowShadowClass);
		}
	...
	override ptrdiff_t windowProc() {
		if (windowProc_ !is null)
			return cast(ptrdiff_t) windowProc_;
		static if (OS.IsSP)
			return cast(ptrdiff_t) DialogProc;
		if ((style & SWT.TOOL) !is 0) {
			int trim = SWT.TITLE | SWT.CLOSE | SWT.MIN | SWT.MAX | SWT.BORDER | SWT.RESIZE;
			if ((style & trim) is 0)
    ...
    */
		...
		if (OS.LOWORD(wParam) is 0) {
			if (lParam is 0 || (cast(HANDLE) lParam !is toolTipHandle_
					&& cast(HANDLE) lParam !is balloonTipHandle_)) {
    ...
    */
	...
	override LRESULT WM_GETMINMAXINFO(WPARAM wParam, LPARAM lParam) {
		...
		if (minWidth !is SWT.DEFAULT || minHeight !is SWT.DEFAULT) {
			...
			if (minWidth !is SWT.DEFAULT)
				info.ptMinTrackSize.x = minWidth;
			if (minHeight !is SWT.DEFAULT)
    ...
    */
	...
	override LRESULT WM_NCHITTEST(WPARAM wParam, LPARAM lParam) {
		...
		if (!isEnabled() || !isActive()) {
			...
			if (hittest is OS.HTCLIENT || hittest is OS.HTMENU)
				hittest = OS.HTBORDER;
