 ...
 */
	...
	private void drawChevron(HDC hDC, RECT* rect) {
		...
		if (expanded) {
			int px = rect.left + 5;
			int py = rect.top + 7;
			polyline1 = [px, py, px + 1, py, px + 1, py - 1, px + 2, py - 1,
				px + 2, py - 2, px + 3, py - 2, px + 3, py - 3, px + 3, py - 2,
				px + 4, py - 2, px + 4, py - 1, px + 5, py - 1, px + 5, py, px + 7, py];
			py += 4;
			polyline2 = [px, py, px + 1, py, px + 1, py - 1, px + 2, py - 1,
				px + 2, py - 2, px + 3, py - 2, px + 3, py - 3, px + 3, py - 2,
				px + 4, py - 2, px + 4, py - 1, px + 5, py - 1, px + 5, py, px + 7, py];
		}
		else {
		...
		if (hover) {
			...
			int[] points2 = [rect.right, rect.top, rect.right, rect.bottom,
	...
	void drawItem(GC gc, HTHEME hTheme, RECT* clipRect, bool drawFocus) {
		...
		if (text.length > 0) {
			...
			if (hTheme !is null) {
				OS.DrawThemeText(hTheme, hDC, OS.EBP_NORMALGROUPHEAD, 0,
						buffer.ptr, cast(int) /*64bit*/ buffer.length,
						OS.DT_VCENTER | OS.DT_SINGLELINE, 0, &rect);
			}
			else {
				int oldBkMode = OS.SetBkMode(hDC, OS.TRANSPARENT);
				OS.DrawText(hDC, buffer.ptr, cast(int) /*64bit*/ buffer.length, &rect,
						OS.DT_VCENTER | OS.DT_SINGLELINE);
		...
		if (expanded) {
			if (!parent.isAppThemed()) {
				...
				int[] points = [
					x, y + headerHeight, x, y + headerHeight + height,
					x + width - 1, y + headerHeight + height, x + width - 1, y + headerHeight - 1
				];
 ...
 */
	...
	int getPreferredWidth(HTHEME hTheme, HDC hDC) {
		...
		if (text.length > 0) {
			...
			if (hTheme !is null) {
				OS.GetThemeTextExtent(hTheme, hDC, OS.EBP_NORMALGROUPHEAD, 0,
						buffer.ptr, cast(int) /*64bit*/ buffer.length,
						OS.DT_SINGLELINE, null, &rect);
			}
			else {
	...
	void redraw(bool all) {
		...
		if (imageHeight > headerHeight) {
			OS.SetRect(&rect, x + ExpandItem.TEXT_INSET,
					y + headerHeight - imageHeight, x + ExpandItem.TEXT_INSET + imageWidth, y);
			OS.InvalidateRect(parentHandle, &rect, true);
		}
		if (!parent.isAppThemed()) {
	...
	void setBounds(int x, int y, int width, int height, bool move, bool size) {
		...
		if (control !is null && !control.isDisposed()) {
			...
			if (move && !size)
				control.setLocation(x, y + headerHeight);
			if (!move && size)
