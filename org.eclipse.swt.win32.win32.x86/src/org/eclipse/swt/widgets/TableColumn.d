 ...
 */
	public void pack() {
		...
		auto headerWidth = OS.SendMessage(hwnd, OS.LVM_GETSTRINGWIDTH, 0,
				cast(void*) buffer) + Table.HEADER_MARGIN;
		...
		if ((index is 0 && !parent.firstColumnImage) || parent.hooks(SWT.MeasureItem)) {
            ...
            */
				if ((parent.style & SWT.CHECK) !is 0) {
					auto hStateList = cast(HANDLE) OS.SendMessage(hwnd,
	...
	void setSortDirection(int direction) {
        ...
        */
			...
			if (OS.SendMessage(hwnd, OS.LVM_GETBKCOLOR, 0, 0) !is OS.CLR_NONE) {
				...
				RECT headerRect;
				if (oldColumn !is -1) {
					if (OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT,
							oldColumn, &headerRect) !is 0) {
						OS.MapWindowPoints(hwndHeader, hwnd, cast(POINT*)&headerRect, 2);
						rect.left = headerRect.left;
						rect.right = headerRect.right;
						OS.InvalidateRect(hwnd, &rect, true);
