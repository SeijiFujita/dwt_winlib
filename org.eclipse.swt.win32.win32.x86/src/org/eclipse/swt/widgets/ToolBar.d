 ...
 */
    ...
    */
		...
		int bits = OS.TBSTYLE_EX_DRAWDDARROWS | OS.TBSTYLE_EX_MIXEDBUTTONS
			| OS.TBSTYLE_EX_HIDECLIPPEDBUTTONS;
		if (OS.COMCTL32_MAJOR >= 6)
    ...
    */
		...
		if ((style & SWT.VERTICAL) !is 0)
			setRowCount(cast(int) /*64bit*/ count + 1);
		layoutItems();
	}
 ...
 */
		...
		if (OS.COMCTL32_MAJOR >= 6 && OS.IsAppThemed()) {
			if ((style & SWT.RIGHT) !is 0 && (style & SWT.VERTICAL) is 0) {
				...
				for (int i = 0; i < items.length; i++) {
					...
					if (item !is null) {
						if (!hasText)
							hasText = item.text.length !is 0;
						if (!hasImage)
							hasImage = item.image !is null;
						if (hasText && hasImage)
							break;
					}
				}
    ...
    */
	...
	override bool mnemonicHit(wchar ch) {
		...
		if ((style & SWT.FLAT) !is 0 && !setTabGroupFocus())
			return false;
    ...
    */
	...
	void setDisabledImageList(ImageList imageList) {
		if (disabledImageList is imageList)
			return;
		HBITMAP hImageList;
    ...
    */
		...
		while (index < items.length) {
			ToolItem item = items[index];
			if (item !is null && (item.style & mask) !is 0)
				break;
			index++;
		}
	...
	void setImageList(ImageList imageList) {
		if (this.imageList is imageList)
			return;
		HBITMAP hImageList;
			...
			case OS.CDDS_PREERASE: {
                    ...
                    */
					...
					if ((bits & OS.TBSTYLE_FLAT) is 0) {
						...
						RECT rect;
						OS.SetRect(&rect, nmcd.rc.left, nmcd.rc.top,
