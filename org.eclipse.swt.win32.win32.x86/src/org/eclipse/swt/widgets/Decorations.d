...
public class Decorations : Canvas {
	...
	alias setBounds = Canvas.setBounds;
	alias setParent = Canvas.setParent;
	alias setSavedFocus = Canvas.setSavedFocus;
	alias sort = Canvas.sort;
	alias windowProc = Canvas.windowProc;
 ...
 */
	...
	void _setMaximized(bool maximized) {
        ...
        */
			...
			if (!OS.IsWindowVisible(handle))
				return;
			if (maximized is OS.IsZoomed(handle))
				return;
	...
	void _setMinimized(bool minimized) {
		...
		if (!OS.IsWindowVisible(handle))
			return;
		if (minimized is OS.IsIconic(handle))
			return;
		int flags = swFlags;
    ...
    */
	...
	int compare(ImageData data1, ImageData data2, int width, int height, int depth) {
		...
		if (value1 is value2) {
			...
			if (transparent1 is transparent2) {
				if (data1.depth is data2.depth)
					return 0;
				return data1.depth > data2.depth && data1.depth <= depth ? -1 : 1;
			}
			if (!OS.IsWinCE && OS.WIN32_VERSION >= OS.VERSION(5, 1)) {
				if (transparent1 is SWT.TRANSPARENCY_ALPHA)
					return -1;
				if (transparent2 is SWT.TRANSPARENCY_ALPHA)
					return 1;
			}
			if (transparent1 is SWT.TRANSPARENCY_MASK)
				return -1;
			if (transparent2 is SWT.TRANSPARENCY_MASK)
				return 1;
			if (transparent1 is SWT.TRANSPARENCY_PIXEL)
				return -1;
			if (transparent2 is SWT.TRANSPARENCY_PIXEL)
				return 1;
			return 0;
		}
	...
	override public Rectangle computeTrim(int x, int y, int width, int height) {
		...
		if (hasMenu) {
			...
			while ((testRect.bottom - testRect.top) < height) {
				if (testRect.bottom - testRect.top is 0)
					break;
	...
	void createAccelerators() {
		...
		if (menuBar is null || items is null) {
			...
			maxAccel = OS.IsPPC ? cast(int) /*64bit*/ items.length + 1 : cast(int) /*64bit*/ items
				.length;
		}
		ACCEL accel;
					...
					if (menu.parent is this) {
						...
						if (menu is menuBar && item.fillAccel(&accel)) {
							*cast(ACCEL*) buffer1.ptr = accel;
							...
							System.arraycopy(buffer1, 0, buffer2,
									nAccel * ACCEL.sizeof, ACCEL.sizeof);
							nAccel++;
						}
					}
				}
        ...
        */
		...
		if (nAccel !is 0)
			hAccel = OS.CreateAcceleratorTable(cast(ACCEL*) buffer2.ptr, nAccel);
	}
	...
	void destroyAccelerators() {
		if (hAccel !is null && hAccel !is cast(HACCEL)-1)
	...
	Menu findMenu(HMENU hMenu) {
		...
		for (int i = 0; i < menus.length; i++) {
			Menu menu = menus[i];
			if (menu !is null && hMenu is menu.handle)
				return menu;
		}
		return null;
	}
	...
	void fixDecorations(Decorations newDecorations, Control control, Menu[] menus) {
		if (this is newDecorations)
			return;
		if (control is savedFocus)
			savedFocus = null;
		if (control is defaultButton)
			defaultButton = null;
		if (control is saveDefault)
			saveDefault = null;
		if (menus is null)
			return;
	...
	override public Rectangle getBounds() {
		...
		static if (!OS.IsWinCE) {
			if (OS.IsIconic(handle)) {
				...
				return new Rectangle(lpwndpl.rcNormalPosition.left,
						lpwndpl.rcNormalPosition.top, width, height);
			}
		}
 ...
 */
	public bool getMaximized() {
		...
		if (OS.IsWindowVisible(handle))
			return cast(bool) OS.IsZoomed(handle);
		return swFlags is OS.SW_SHOWMAXIMIZED;
	}
 ...
 */
	public bool getMinimized() {
		...
		static if (OS.IsWinCE)
			return false;
		if (OS.IsWindowVisible(handle))
			return cast(bool) OS.IsIconic(handle);
    ...
    */
	...
	bool restoreFocus() {
		if (display.ignoreRestoreFocus)
			return true;
		if (savedFocus !is null && savedFocus.isDisposed())
			savedFocus = null;
		if (savedFocus !is null && savedFocus.setSavedFocus())
			return true;
		/*
 ...
 */
	...
	void setDefaultButton(Button button, bool save) {
		...
		if ((defaultButton = button) is null)
		...
		if (saveDefault !is null && saveDefault.isDisposed())
			saveDefault = null;
	}
		...
		if (image !is null) {
			...
			if (images !is null && images.length > 0) {
				...
				if (images.length > 1) {
					...
					images = bestImages;
					sort(images, datas, OS.GetSystemMetrics(OS.SM_CXSMICON),
							OS.GetSystemMetrics(OS.SM_CYSMICON), depth);
				}
				smallIcon = images[0];
				if (images.length > 1) {
					sort(images, datas, OS.GetSystemMetrics(OS.SM_CXICON),
							OS.GetSystemMetrics(OS.SM_CYICON), depth);
				}
				largeIcon = images[0];
			}
		}
    ...
    */
	...
	void setPlacement(int x, int y, int width, int height, int flags) {
		...
		if ((flags & OS.SWP_NOMOVE) is 0) {
			...
			lpwndpl.rcNormalPosition.right = x + (
					lpwndpl.rcNormalPosition.right - lpwndpl.rcNormalPosition.left);
			lpwndpl.rcNormalPosition.bottom = y + (
		...
		if ((flags & OS.SWP_NOSIZE) is 0) {
			sameExtent = lpwndpl.rcNormalPosition.right - lpwndpl.rcNormalPosition.left !is width
				|| lpwndpl.rcNormalPosition.bottom - lpwndpl.rcNormalPosition.top !is height;
 ...
 */
	...
	override public void setVisible(bool visible) {
		...
		if (drawCount !is 0) {
			...
			if (visible is OS.IsWindowVisible(handle))
				return;
		}
		if (visible) {
			/*
        ...
        */
			...
			if (drawCount !is 0) {
				...
				static if (OS.IsWinCE) {
					...
					if (lpStartUpInfo !is null
							&& (lpStartUpInfo.dwFlags & OS.STARTF_USESHOWWINDOW) !is 0) {
