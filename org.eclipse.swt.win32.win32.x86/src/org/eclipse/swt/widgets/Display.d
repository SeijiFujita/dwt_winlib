 ...
 */
	...
	void addControl(HANDLE handle, Control control) {
		...
		if (freeSlot is -1) {
			...
			for (auto i = freeSlot; i < length_ - 1; i++)
				newIndexTable[i] = i + 1;
 ...
 */
	...
	int asciiKey(int key) {
		...
		for (int i = 0; i < keyboard.length; i++)
			keyboard[i] = 0;
		if (!OS.GetKeyboardState(keyboard.ptr))
			return 0;

		/* Translate the key to ASCII or UNICODE using the virtual keyboard */
		static if (OS.IsUnicode) {
			wchar result;
			if (OS.ToUnicode(key, key, keyboard.ptr, &result, 1, 0) is 1)
				return result;
		}
		else {
			short[] result = new short[1];
			if (OS.ToAscii(key, key, keyboard, result, 0) is 1)
				return result[0];
		}
		return 0;
	}
 ...
 */
		...
		synchronized (Device.classinfo) {
			for (int i = 0; i < Displays.length; i++) {
				if (Displays[i]!is null) {
					...
					if (Displays[i].thread is thread)
	...
	void clearModal(Shell shell) {
		...
		while (index < length_) {
			if (modalShells[index] is shell)
				break;
			if (modalShells[index] is null)
				return;
			index++;
		}
		if (index is length_)
		...
		if (index is 0 && modalShells[0] is null)
			modalShells = null;
 ...
 */
	...
	static HBITMAP create32bitDIB(Image image) {
			...
			for (int y = 0, dp = 0; y < imgHeight; ++y) {
				for (int x = 0; x < imgWidth; ++x) {
					if (OS.GetPixel(srcHdc, x, y) !is 0) {
						srcData[dp + 0] = srcData[dp + 1] = srcData[dp + 2] = srcData[dp + 3] = cast(
								byte) 0;
					}
					else {
		...
		if (hBitmap !is image.handle && hBitmap !is null)
	...
	static Image createIcon(Image image) {
		...
		if (!OS.IsWinCE && OS.WIN32_VERSION >= OS.VERSION(5, 1)) {
			hBitmap = Display.create32bitDIB(image.handle, data.alpha,
 ...
 */
	...
	private static extern (Windows).LRESULT embeddedFunc(HWND hwnd, int msg,
 ...
 */
	...
	bool filterMessage(MSG* msg) {
		...
		if (OS.WM_KEYFIRST <= message && message <= OS.WM_KEYLAST) {
			...
			if (control !is null) {
				if (translateAccelerator(msg, control) || translateMnemonic(msg,
						control) || translateTraversal(msg, control)) {
					lastAscii = lastKey = 0;
 ...
 */
	...
	private static extern (Windows).LRESULT foregroundIdleProcFunc(int code,
			WPARAM wParam, LPARAM lParam) {
 ...
 */
	...
	int getClickCount(int type, int button, HWND hwnd, LPARAM lParam) {
		...
		case SWT.MouseDown:
			...
			if (clickRect is null)
				clickRect = new RECT();
			int deltaTime = Math.abs(lastTime - getLastEventTime());
			POINT pt;
			OS.POINTSTOPOINT(pt, lParam);
			if (lastClickHwnd is hwnd && lastButton is button
					&& (deltaTime <= doubleClick) && OS.PtInRect(clickRect, pt)) {
				clickCount++;
			}
			else {
				clickCount = 1;
 ...
 */
	...
	String getFontName(LOGFONT* logFont) {
		...
		static if (OS.IsUnicode) {
			...
			OS.MultiByteToWideChar(OS.CP_ACP, OS.MB_PRECOMPOSED, bytes,
					cast(int) /*64bit*/ bytes.length, chars, cast(int) /*64bit*/ chars.length);
		}
 ...
 */
	public int getIconDepth() {
		...
		void* phkResult;
		int result = OS.RegOpenKeyEx(cast(void*) OS.HKEY_CURRENT_USER, buffer1,
				0, OS.KEY_READ, &phkResult);
		if (result !is 0)
			return 4;
		int depth = 4;
		uint lpcbData;
 ...
 */
	public Point[] getIconSizes() {
		checkDevice();
		return [new Point(OS.GetSystemMetrics(OS.SM_CXSMICON),
				OS.GetSystemMetrics(OS.SM_CYSMICON)), new Point(OS.GetSystemMetrics(OS.SM_CXICON),
				OS.GetSystemMetrics(OS.SM_CYICON))];
	}
	...
	ImageList getImageListToolBarDisabled(int style, int width, int height) {
		if (toolDisabledImageList is null)
 ...
 */
	public org.eclipse.swt.widgets.Monitor.Monitor[] getMonitors() {
		...
		org.eclipse.swt.widgets.Monitor.Monitor[] result
			= new org.eclipse.swt.widgets.Monitor.Monitor[monitorCount];
	...
	.LRESULT getMsgProc(int code, WPARAM wParam, LPARAM lParam) {
		if (embeddedHwnd is null) {
			...
			embeddedHwnd = OS.CreateWindowEx(0, windowClass_.ptr, null,
					OS.WS_OVERLAPPED, 0, 0, 0, 0, null, null, hInstance, null);
		...
		if (code >= 0 && (wParam & OS.PM_REMOVE) !is 0) {
			...
			case OS.WM_SYSKEYUP: {
					...
					if (control !is null) {
						auto hHeap = OS.GetProcessHeap();
						MSG* keyMsg = cast(MSG*) OS.HeapAlloc(hHeap,
								OS.HEAP_ZERO_MEMORY, MSG.sizeof);
						*keyMsg = *msg;
	...
	public Image getSystemImage(int id) {
		...
		case SWT.ICON_ERROR: {
				if (errorImage !is null)
					return errorImage;
				auto hIcon = OS.LoadImage(null, cast(wchar*) OS.OIC_HAND,
						OS.IMAGE_ICON, 0, 0, OS.LR_SHARED);
		...
		case SWT.ICON_INFORMATION: {
				if (infoImage !is null)
					return infoImage;
				auto hIcon = OS.LoadImage(null, cast(wchar*) OS.OIC_INFORMATION,
						OS.IMAGE_ICON, 0, 0, OS.LR_SHARED);
		...
		case SWT.ICON_QUESTION: {
				...
				auto hIcon = OS.LoadImage(null, cast(wchar*) OS.OIC_QUES,
						OS.IMAGE_ICON, 0, 0, OS.LR_SHARED);
				return questionImage = Image.win32_new(this, SWT.ICON, hIcon);
			}
		case SWT.ICON_WARNING: {
				if (warningIcon !is null)
					return warningIcon;
				auto hIcon = OS.LoadImage(null, cast(wchar*) OS.OIC_BANG,
						OS.IMAGE_ICON, 0, 0, OS.LR_SHARED);
 ...
 */
	override protected void init_() {
		...
		windowShadowClass = StrToTCHARs(0, WindowShadowName ~ String_valueOf(WindowClassCount),
				true);
		WindowClassCount++;
    ...
    */
    ...
    +/
		...
		hwndMessage = OS.CreateWindowEx(0, windowClass_.ptr, null,
				OS.WS_OVERLAPPED, 0, 0, 0, 0, null, null, hInstance, null);
		...
		static if (!OS.IsWinCE) {
			...
			idleHook = OS.SetWindowsHookEx(OS.WH_FOREGROUNDIDLE,
					&foregroundIdleProcFunc, null, threadId);
		}
 ...
 */
    ...
    */
		...
		void* phKey;
		int result = OS.RegOpenKeyEx(cast(void*) OS.HKEY_CURRENT_USER, key, 0,
				OS.KEY_READ, &phKey);
		if (result is 0) {
			LPCTSTR lpValueName = StrToTCHARz("UserPreferencesMask"); //$NON-NLS-1$
			uint[4] lpcbData;
			uint lpData;
			result = OS.RegQueryValueEx(phKey, lpValueName, null, null,
					cast(ubyte*)&lpData, lpcbData.ptr);
			if (result is 0)
				xMouseActive = (lpData & 0x01) !is 0;
 ...
 */
	public Point map(Control from, Control to, int x, int y) {
		checkDevice();
		if (from !is null && from.isDisposed())
			error(SWT.ERROR_INVALID_ARGUMENT);
		if (to !is null && to.isDisposed())
			error(SWT.ERROR_INVALID_ARGUMENT);
		if (from is to)
 ...
 */
	static wchar mbcsToWcs(int ch, int codePage) {
		...
		int count = OS.MultiByteToWideChar(cp, OS.MB_PRECOMPOSED, buffer.ptr,
				cast(int) /*64bit*/ buffer.length, unicode.ptr, 1);
		if (count is 0)
			return 0;
		return unicode[0];
	}

	private static extern (Windows).LRESULT messageProcFunc(HWND hwnd, uint msg,
			WPARAM wParam, LPARAM lParam) {
	...
	.LRESULT messageProc(HWND hwnd, uint msg, WPARAM wParam, LPARAM lParam) {
                ...
                */
					...
					case OS.WM_SYSKEYDOWN: {
							static if (!OS.IsWinCE) {
								...
								default: {
                                    ...
                                    */
										int mapKey = OS.MapVirtualKey(cast(int) /*64bit*/ keyMsg.wParam,
												2);
										if (mapKey !is 0) {
											accentKey = (mapKey & (OS.IsWinNT ? 0x80000000 : 0x8000)) !is 0;
											if (!accentKey) {
												for (int i = 0; i < ACCENTS.length;
														i++) {
													int value = OS.VkKeyScan(ACCENTS[i]);
													if (value !is -1 && (value & 0xFF) is keyMsg
															.wParam) {
														int state = value >> 8;
														if ((OS.GetKeyState(OS.VK_SHIFT) < 0) is((state & 0x1) !is 0)
																&& (OS.GetKeyState(
																	OS.VK_CONTROL) < 0) is(
																	(state & 0x2) !is 0) && (OS.GetKeyState(
																	OS.VK_MENU) < 0) is(
																	(state & 0x4) !is 0)) {
															if ((state & 0x7) !is 0)
																accentKey = true;
					...
					if (!accentKey && !ignoreNextKey) {
						...
						int flags = OS.PM_REMOVE | OS.PM_NOYIELD
							| OS.PM_QS_INPUT | OS.PM_QS_POSTMESSAGE;
						do {
						...
						while (OS.PeekMessage(keyMsg, keyMsg.hwnd,
								OS.WM_KEYFIRST, OS.WM_KEYLAST, flags));
					}
					switch (keyMsg.message) {
					case OS.WM_KEYDOWN:
					case OS.WM_SYSKEYDOWN: {
            ...
            */
				if (wParam !is 0) {
					if (!isXMouseActive()) {
						...
						if (hwndActive !is null && OS.IsWindowEnabled(hwndActive))
							break;
		...
		case OS.WM_SETTINGCHANGE: {
				...
				case OS.SPI_SETHIGHCONTRAST:
					OS.SetTimer(hwndMessage,
							SETTINGS_ID, SETTINGS_DELAY, null);
					goto default;
				default:
				}
				break;
		...
		case OS.WM_THEMECHANGED: {
				if (OS.COMCTL32_MAJOR >= 6) {
					if (hButtonTheme_ !is null)
						OS.CloseThemeData(hButtonTheme_);
					if (hEditTheme_ !is null)
						OS.CloseThemeData(hEditTheme_);
					if (hExplorerBarTheme_ !is null)
						OS.CloseThemeData(hExplorerBarTheme_);
					if (hScrollBarTheme_ !is null)
						OS.CloseThemeData(hScrollBarTheme_);
					if (hTabTheme_ !is null)
						OS.CloseThemeData(hTabTheme_);
	...
	private static extern (Windows) BOOL monitorEnumFunc(HMONITOR hmonitor,
			HDC hdc, RECT* lprcMonitor, LPARAM dwData) {
	...
	int monitorEnumProc(HMONITOR hmonitor, HDC hdc, RECT* lprcMonitor) {
		if (monitorCount >= monitors.length) {
			org.eclipse.swt.widgets.Monitor.Monitor[] newMonitors
				= new org.eclipse.swt.widgets.Monitor.Monitor[monitors.length + 4];
		...
		org.eclipse.swt.widgets.Monitor.Monitor monitor
			= new org.eclipse.swt.widgets.Monitor.Monitor();
		...
		synchronized (Device.classinfo) {
			...
			case SWT.KeyUp: {
					...
					if (inputs.wVk is 0) {
						...
						switch (key) {
						case SWT.BS:
							inputs.wVk = cast(short) OS.VK_BACK;
							break;
						case SWT.CR:
							inputs.wVk = cast(short) OS.VK_RETURN;
							break;
						case SWT.DEL:
							inputs.wVk = cast(short) OS.VK_DELETE;
							break;
						case SWT.ESC:
							inputs.wVk = cast(short) OS.VK_ESCAPE;
							break;
						case SWT.TAB:
							inputs.wVk = cast(short) OS.VK_TAB;
							break;
							/*
					...
					auto hHeap = OS.GetProcessHeap();
					auto pInputs = cast(INPUT*) OS.HeapAlloc(hHeap,
							OS.HEAP_ZERO_MEMORY, INPUT.sizeof);
			...
			case SWT.MouseWheel: {
					...
					if (type is SWT.MouseMove) {
						...
						if (type is SWT.MouseWheel) {
							...
							switch (event.button) {
							case 1:
								inputs.dwFlags = type is SWT.MouseDown
									? OS.MOUSEEVENTF_LEFTDOWN : OS.MOUSEEVENTF_LEFTUP;
								break;
							case 2:
								inputs.dwFlags = type is SWT.MouseDown
									? OS.MOUSEEVENTF_MIDDLEDOWN : OS.MOUSEEVENTF_MIDDLEUP;
								break;
							case 3:
								inputs.dwFlags = type is SWT.MouseDown
									? OS.MOUSEEVENTF_RIGHTDOWN : OS.MOUSEEVENTF_RIGHTUP;
								break;
							case 4: {
									if (OS.WIN32_VERSION < OS.VERSION(5, 0))
										return false;
									inputs.dwFlags = type is SWT.MouseDown
										? OS.MOUSEEVENTF_XDOWN : OS.MOUSEEVENTF_XUP;
		...
		while (i < length_) {
			if (toolDisabledImageList[i] is list) {
				...
				System.arraycopy(toolDisabledImageList, i + 1,
						toolDisabledImageList, i, --length_ - i);
				toolDisabledImageList[length_] = null;
				for (int j = 0; j < length_; j++) {
					if (toolDisabledImageList[j]!is null)
						return;
				}
	...
	void saveResources() {
		...
		if (systemFont !is null) {
			static if (!OS.IsWinCE) {
				...
				if (OS.SystemParametersInfo(OS.SPI_GETNONCLIENTMETRICS, 0, &info, 0)) {
					LOGFONT* logFont = &info.lfMessageFont;
					if (lfSystemFont is null || logFont.lfCharSet !is lfSystemFont.lfCharSet
							|| logFont.lfHeight !is lfSystemFont.lfHeight || logFont.lfWidth !is lfSystemFont.lfWidth
	...
	public void setData(String key, Object value) {
		...
		if (value is null) {
			if (keys is null)
				return;
			int index = 0;
			while (index < keys.length && keys[index] != /*eq*/ key)
				index++;
 ...
 */
	public void setSynchronizer(Synchronizer synchronizer) {
		...
		if (synchronizer is null)
			error(SWT.ERROR_NULL_ARGUMENT);
		if (synchronizer is this.synchronizer)
			return;
	...
	int shiftedKey(int key) {
		...
		static if (OS.IsUnicode) {
			...
			wchar result;
			if (OS.ToAscii(key, key, keyboard.ptr, &result, 0) is 1)
				return result;
		}
		return 0;
	}
 ...
 */
	public bool sleep() {
		...
		if (runMessages && getMessageCount() !is 0)
			return true;
		static if (OS.IsWinCE) {
			OS.MsgWaitForMultipleObjectsEx(0, null, OS.INFINITE,
 ...
 */
	public void timerExec(int milliseconds, Runnable runnable) {
		...
		if (runnable is null)
			error(SWT.ERROR_NULL_ARGUMENT);
		assert(runnable);
		if (timerList is null)
			timerList = new Runnable[4];
		if (timerIds is null)
			timerIds = new UINT_PTR[4];
		int index = 0;
		while (index < timerList.length) {
			if (timerList[index] is runnable)
				break;
			index++;
		}
		UINT_PTR timerId = 0;
 ...
 */
	...
	private static extern (Windows).LRESULT windowProcFunc(HWND hwnd, uint msg,
			WPARAM wParam, LPARAM lParam) {
		auto d = Display.getCurrent();
			...
			if (msg is OS.WM_NOTIFY && hwndParent is hwnd) {
				...
				case OS.LVN_GETDISPINFOW: {
						...
						if (0 <= plvfi.item.iSubItem && plvfi.item.iSubItem < columnCount) {
							if (!columnVisible[plvfi.item.iSubItem])
								return 0;
						}
						break;
					}
