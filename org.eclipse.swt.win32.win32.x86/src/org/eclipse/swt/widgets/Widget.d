...
public abstract class Widget {
	...
	public alias LRESULT = org.eclipse.swt.internal.win32.OS.LDWTRESULT;
		...
		synchronized {
			...
			static if (!OS.IsWinCE) {
				if (OS.COMCTL32_VERSION < OS.VERSION(MAJOR, MINOR)) {
					getDwtLogger().warn(__FILE__, __LINE__,
							"***WARNING: SWT requires comctl32.dll version {}.{} or greater",
							MAJOR, MINOR); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					getDwtLogger().warn(__FILE__, __LINE__, "***WARNING: Detected: {}.{}",
							OS.COMCTL32_MAJOR, OS.COMCTL32_MINOR); //$NON-NLS-1$ //$NON-NLS-2$
				}
			}
 ...
 */
	...
	HDWP DeferWindowPos(HDWP hWinPosInfo, HWND hWnd, HWND hWndInsertAfter, int X,
 ...
 */
	...
	bool sendMouseEvent(int type, int button, HWND hwnd, int msg, WPARAM wParam, LPARAM lParam) {
		return sendMouseEvent(type, button, display.getClickCount(type, button,
				hwnd, lParam), 0, false, hwnd, msg, wParam, lParam);
	}

	bool sendMouseEvent(int type, int button, int count, int detail, bool send,
 ...
 */
	...
	bool setInputState(Event event, int type) {
		...
		case SWT.KeyDown:
		case SWT.Traverse:
			if (event.keyCode is SWT.ALT)
				event.stateMask &= ~SWT.ALT;
			if (event.keyCode is SWT.SHIFT)
				event.stateMask &= ~SWT.SHIFT;
			if (event.keyCode is SWT.CONTROL)
				event.stateMask &= ~SWT.CONTROL;
			break;
		case SWT.KeyUp:
			if (event.keyCode is SWT.ALT)
    ...
    */
		...
		case SWT.DEL:
			if (display.lastKey is SWT.BS)
				display.lastAscii = SWT.BS;
			break;
		case SWT.LF:
			if (display.lastKey is SWT.CR)
    ...
    */
        ...
        */
			if (display.lastKey is OS.VK_DELETE)
        ...
        * to zero.
        */
			if (display.lastKey is OS.VK_CANCEL)
 ...
 */
    ...
    */
		if (OS.IsWinNT) {
			if ((mapKey & 0x80000000) !is 0)
				return null;
		}
		else {
			if ((mapKey & 0x8000) !is 0)
				return null;
		}
		MSG msg;
    ...
    */
        ...
        */
            ...
            */
				if (display.asciiKey(display.lastKey) !is 0)
					return null;
    ...
    */
	...
	LRESULT wmLButtonDown(HWND hwnd, WPARAM wParam, LPARAM lParam) {
		...
		display.captureChanged = false;
		bool dispatch = sendMouseEvent(SWT.MouseDown, 1, count, 0, false, hwnd,
				OS.WM_LBUTTONDOWN, wParam, lParam);
    ...
    */
				...
				if (trackMouse || mouseEnter || mouseExit || mouseHover) {
					...
					if (lpEventTrack.dwFlags is 0) {
                        ...
                        */
							...
							while (OS.PeekMessage(&msg, null,
									OS.WM_MOUSELEAVE, OS.WM_MOUSELEAVE, flags)) {
								OS.TranslateMessage(&msg);
								OS.DispatchMessage(&msg);
							}
							sendMouseEvent(SWT.MouseEnter, 0, hwnd,
									OS.WM_MOUSEMOVE, wParam, lParam);
						}
					}
					else {
	...
	LRESULT wmMouseWheel(HWND hwnd, WPARAM wParam, LPARAM lParam) {
		...
		OS.ScreenToClient(hwnd, &pt);
		lParam = OS.MAKELPARAM(pt.x, pt.y);
		if (!sendMouseEvent(SWT.MouseWheel, 0, count, detail, true, hwnd,
				OS.WM_MOUSEWHEEL, wParam, lParam)) {
			return LRESULT.ZERO;
		}
		return null;
	}
		...
		if ((lParam & OS.PRF_NONCLIENT) !is 0) {
			if (OS.COMCTL32_MAJOR >= 6 && OS.IsAppThemed()) {
				...
				if ((bits & OS.WS_EX_CLIENTEDGE) !is 0) {
					...
					OS.ExcludeClipRect(cast(void*) wParam, border, border,
							rect.right - border, rect.bottom - border);
					OS.DrawThemeBackground(display.hEditTheme(), cast(HDC) wParam,
							OS.EP_EDITTEXT, OS.ETS_NORMAL, &rect, null);
					return new LRESULT(code);
				}
			}
		}
    ...
    */
		...
		case OS.VK_F4: {
				...
				while (OS.GetParent(hwndShell) !is null) {
					if (OS.GetWindow(hwndShell, OS.GW_OWNER) !is null)
						break;
    ...
    */
		...
		if (sendMouseEvent(SWT.MouseDoubleClick, button, hwnd,
				OS.WM_XBUTTONDBLCLK, wParam, lParam)) {
