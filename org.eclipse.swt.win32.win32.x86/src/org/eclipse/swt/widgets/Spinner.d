 ...
 */
    ...
    */
	...
	override void createHandle() {
		...
		hwndText = OS.CreateWindowEx(textExStyle, EditClass.ptr, null,
				textStyle, 0, 0, 0, 0, handle, null, hInstance, null);
		if (hwndText is null)
		...
		hwndUpDown = OS.CreateWindowEx(0, UpDownClass.ptr, null, upDownStyle,
				0, 0, 0, 0, handle, null, hInstance, null);
		if (hwndUpDown is null)
			error(SWT.ERROR_NO_HANDLES);
		...
		try {
			...
			if (digits > 0) {
				...
				if (index !is -1) {
					...
					String wholePart = startIndex !is index ? string.substring(startIndex,
							index) : "0";
 ...
 */
	public void setIncrement(int value) {
		...
		auto udaccels = cast(UDACCEL*) OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY,
				UDACCEL.sizeof * count);
 ...
 */
	...
	String verifyText(String string, int start, int end, Event keyEvent) {
		...
		if (string.length > 0) {
			int min;
			OS.SendMessage(hwndUpDown, OS.UDM_GETRANGE32, &min, null);
			if (min < 0 && string.charAt(0) is '-')
				index++;
		}
		while (index < string.length) {
			if (!CharacterIsDigit(string.charAt(index)))
				break;
			index++;
		}
	...
	override.LRESULT windowProc(HWND hwnd, int msg, WPARAM wParam, LPARAM lParam) {
		if (hwnd is hwndText || hwnd is hwndUpDown) {
			...
			case OS.WM_PAINT:
				...
				break;
			case OS.WM_PRINT:
				result = wmPrint(hwnd, wParam, lParam);
				break;
	...
	override LRESULT wmKeyDown(HWND hwnd, WPARAM wParam, LPARAM lParam) {
		...
		if (delta !is 0) {
			...
			if (value !is newValue)
				setSelection(cast(int) /*64bit*/ newValue, true, true, true);
		}
