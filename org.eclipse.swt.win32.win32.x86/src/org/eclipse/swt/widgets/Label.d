 ...
 */
    ...
    */
			...
			if (drawImage || drawSeparator) {
				...
				if (gc !is null) {
					...
					if (drawSeparator) {
						...
						if ((style & SWT.HORIZONTAL) !is 0) {
							int bottom = clientRect.top + Math.max(lineWidth * 2,
									(clientRect.bottom - clientRect.top) / 2);
							OS.SetRect(&rect, clientRect.left, clientRect.top,
									clientRect.right, bottom);
							OS.DrawEdge(gc.handle, &rect, flags, OS.BF_BOTTOM);
						}
						else {
							int right = clientRect.left + Math.max(lineWidth * 2,
					...
					if (drawImage) {
						...
						gc.drawImage(image, x, Math.max(0,
								(clientRect.bottom - imageBounds.height) / 2));
						result = LRESULT.ONE;
					}
	...
	override LRESULT wmDrawChild(WPARAM wParam, LPARAM lParam) {
		...
		if ((style & SWT.SEPARATOR) !is 0) {
			if ((style & SWT.SHADOW_NONE) !is 0)
				return null;
			RECT rect;
			int lineWidth = OS.GetSystemMetrics(OS.SM_CXBORDER);
			int flags = (style & SWT.SHADOW_IN) !is 0 ? OS.EDGE_SUNKEN : OS.EDGE_ETCHED;
			if ((style & SWT.HORIZONTAL) !is 0) {
				int bottom = struct_.rcItem.top + Math.max(lineWidth * 2,
						(struct_.rcItem.bottom - struct_.rcItem.top) / 2);
				OS.SetRect(&rect, struct_.rcItem.left, struct_.rcItem.top,
						struct_.rcItem.right, bottom);
				OS.DrawEdge(struct_.hDC, &rect, flags, OS.BF_BOTTOM);
			}
			else {
				int right = struct_.rcItem.left + Math.max(lineWidth * 2,
						(struct_.rcItem.right - struct_.rcItem.left) / 2);
				OS.SetRect(&rect, struct_.rcItem.left, struct_.rcItem.top,
						right, struct_.rcItem.bottom);
			...
			if (width !is 0 && height !is 0) {
				...
				if (drawImage) {
					...
					Image image = getEnabled() ? this.image : new Image(display,
