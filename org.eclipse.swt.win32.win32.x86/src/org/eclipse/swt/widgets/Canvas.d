...
public class Canvas : Composite {
	...
	alias drawBackground = Composite.drawBackground;
 ...
 */
	...
	void clearArea(int x, int y, int width, int height) {
		...
		if (OS.IsWindowVisible(handle)) {
			...
			auto hDC = OS.GetDCEx(handle, null,
					OS.DCX_CACHE | OS.DCX_CLIPCHILDREN | OS.DCX_CLIPSIBLINGS);
		...
		if (all) {
			...
			for (int i = 0; i < children.length; i++) {
				...
				if (Math.min(x + width, rect.x + rect.width) >= Math.max(x,
						rect.x) && Math.min(y + height, rect.y + rect.height) >= Math.max(y,
						rect.y)) {
