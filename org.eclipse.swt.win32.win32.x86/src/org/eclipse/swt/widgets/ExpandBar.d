 ...
 */
	...
	override void drawThemeBackground(HDC hDC, HWND hwnd, RECT* rect) {
		...
		OS.DrawThemeBackground(display.hExplorerBarTheme(), hDC,
				OS.EBP_NORMALGROUPBACKGROUND, 0, &rect2, null);
	}
	...
	void drawWidget(GC gc, RECT* clipRect) {
		...
		if (hTheme !is null) {
			...
			OS.DrawThemeBackground(hTheme, gc.handle,
					OS.EBP_HEADERBACKGROUND, 0, &rect, clipRect);
		}
		else {
 ...
 */
	...
	void showFocus(bool up) {
		...
		if (updateY !is 0) {
			...
			OS.ScrollWindowEx(handle, 0, updateY, null, null, null, null,
					OS.SW_SCROLLCHILDREN | OS.SW_INVALIDATE);
			for (int i = 0; i < itemCount; i++) {
				items[i].y += updateY;
			}
		}
	...
	override LRESULT wmScroll(ScrollBar bar, bool update, HWND hwnd, int msg,
