...
public class Text : Scrollable {
	...
	alias sendKeyEvent = Scrollable.sendKeyEvent;
 ...
 */
	...
	override.LRESULT callWindowProc(HWND hwnd, int msg, WPARAM wParam, LPARAM lParam) {
		...
		case OS.WM_HSCROLL:
		case OS.WM_VSCROLL: {
				redraw = findImageControl() !is null && drawCount is 0 && OS.IsWindowVisible(
						handle);
				if (redraw)
    ...
    */
	...
	static int checkStyle(int style) {
		...
		if ((style & SWT.MULTI) !is 0)
			style &= ~SWT.PASSWORD;
		if ((style & (SWT.SINGLE | SWT.MULTI)) !is 0)
			return style;
 ...
 */
	...
	override public Point computeSize(int wHint, int hHint, bool changed) {
		...
		if (wHint is SWT.DEFAULT || hHint is SWT.DEFAULT) {
			...
			int count = (style & SWT.SINGLE) !is 0 ? 1
				: cast(int) /*64bit*/ OS.SendMessage(handle, OS.EM_GETLINECOUNT, 0, 0);
					...
					if (OS.GetGUIThreadInfo(idThread, &lpgui)) {
						if (lpgui.hwndCaret is handle || lpgui.hwndCaret is null) {
							...
							if (OS.GetCaretPos(&ptCurrentPos)) {
								...
								if (endPos is -1) {
									auto startPos = OS.SendMessage(handle,
											OS.EM_POSFROMCHAR, start, 0);
 ...
 */
	public char getEchoChar() {
		...
		if (echo !is 0 && (echo = Display.mbcsToWcs(echo, getCodePage())) is 0)
			echo = '*';
	...
	int mbcsToWcsPos(int mbcsPos) {
		...
		for (int line = 0; line < count; line++) {
			...
			if (mbcsSize !is 0) {
				...
				wcsSize = OS.MultiByteToWideChar(cp, OS.MB_PRECOMPOSED,
						buffer.ptr, cast(int) /*64bit*/ mbcsSize, null, 0);
			}
			if (line - 1 !is count) {
 ...
 */
	public void setEchoChar(char echo) {
		...
		if ((style & SWT.MULTI) !is 0)
			return;
		if (echo !is 0) {
			if ((echo = cast(char) Display.wcsToMbcs(echo, getCodePage())) is 0)
				echo = '*';
		}
		...
		if (OS.COMCTL32_MAJOR >= 6) {
			if ((style & SWT.SEARCH) !is 0) {
				...
				if ((bits & OS.ES_MULTILINE) is 0) {
					OS.SendMessage(handle, OS.EM_SETCUEBANNER, 0,
							cast(void*) StrToTCHARz(0, message));
				}
			}
		}
	}
 ...
 */
    ...
    */
		...
		if (!redraw) {
			...
			if (oldStart is start && oldEnd is end)
				return;
			OS.SendMessage(handle, OS.EM_SCROLLCARET, 0, 0);
		}
	}
 ...
 */
	public void setText(String string) {
		...
		int limit = cast(int) /*64bit*/ OS.SendMessage(handle,
				OS.EM_GETLIMITTEXT, 0, 0) & 0x7FFFFFFF;
	...
	override LRESULT WM_LBUTTONDOWN(WPARAM wParam, LPARAM lParam) {
		static if (OS.IsWinCE)
			if (OS.IsPPC) {
				...
				bool dispatch = sendMouseEvent(SWT.MouseDown, 1, handle,
