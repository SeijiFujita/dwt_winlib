...
public class Combo : Composite {
	...
	alias sendKeyEvent = Composite.sendKeyEvent;
 ...
 */
	...
	override bool checkHandle(HWND hwnd) {
		return hwnd is handle || hwnd is OS.GetDlgItem(handle, CBID_EDIT)
			|| hwnd is OS.GetDlgItem(handle, CBID_LIST);
	}
	...
	override public Point computeSize(int wHint, int hHint, bool changed) {
				...
				for (int i = 0; i < count; i++) {
					...
					if (length_ !is OS.CB_ERR) {
						if (length_ + 1 > buffer.length)
							buffer = new TCHAR[length_ + 1], buffer[] = 0;
		...
		if (hHint is SWT.DEFAULT) {
			if ((style & SWT.SIMPLE) !is 0) {
				...
				int itemHeight = cast(int) /*64bit*/ OS.SendMessage(handle,
						OS.CB_GETITEMHEIGHT, 0, 0);
		...
		if (((style & SWT.SIMPLE) is 0) && !OS.IsWinCE && OS.GetComboBoxInfo(handle, &pcbi)) {
			...
			int textHeight = cast(int) /*64bit*/ OS.SendMessage(handle,
					OS.CB_GETITEMHEIGHT, -1, 0);
		...
		if (filter && (style & SWT.READ_ONLY) is 0) {
			...
			if (hwndText !is null) {
				...
				if (start !is end) {
					...
					int position = OS.LOWORD(OS.SendMessage(hwndText,
							OS.EM_CHARFROMPOS, 0, lParam));
 ...
 */
	...
	override bool hasFocus() {
		...
		if (hwndFocus is handle)
			return true;
		if (hwndFocus is null)
			return false;
		auto hwndText = OS.GetDlgItem(handle, CBID_EDIT);
		if (hwndFocus is hwndText)
			return true;
		auto hwndList = OS.GetDlgItem(handle, CBID_LIST);
		if (hwndFocus is hwndList)
			return true;
		return false;
	}
 ...
 */
    ...
    */
		...
		do {
			index = cast(int) /*64bit*/ OS.SendMessage(handle,
					OS.CB_FINDSTRINGEXACT, last = index, cast(void*) buffer);
			if (index is OS.CB_ERR || index <= last)
				return -1;
		}
	...
	int mbcsToWcsPos(int mbcsPos) {
		...
		return OS.MultiByteToWideChar(getCodePage(), OS.MB_PRECOMPOSED,
				buffer.ptr, mbcsPos, null, 0);
	}
 ...
 */
	...
	void remove(int index, bool notify) {
		...
		if ((style & SWT.H_SCROLL) !is 0)
			setScrollWidth(cast(StringT) buffer, true);
 ...
 */
    ...
    */
		switch (msg) {
		case OS.WM_CHAR:
			if (key !is 0x08 && key !is 0x7F && key !is '\r'
					&& key !is '\t' && key !is '\n')
				break;
			// FALL THROUGH
			goto case;
		case OS.WM_KEYDOWN:
			if ((stateMask & (SWT.ALT | SWT.SHIFT | SWT.CONTROL)) !is 0)
				return false;
			break;
		default:
		}
        ...
        */
			...
			if (rect.right - rect.left !is 0) {
				if (OS.SendMessage(handle, OS.CB_GETDROPPEDCONTROLRECT, 0, &rect) !is 0) {
					...
					if (oldWidth is width && oldHeight is height)
						flags |= OS.SWP_NOSIZE;
				}
			}
 ...
 */
	public void setOrientation(int orientation) {
		...
		if ((orientation & flags) is 0 || (orientation & flags) is flags)
			return;
		style &= ~flags;
    ...
    */
	...
	override int widgetStyle() {
		int bits = super.widgetStyle() | OS.CBS_AUTOHSCROLL
			| OS.CBS_NOINTEGRALHEIGHT | OS.WS_HSCROLL | OS.WS_VSCROLL;
		if ((style & SWT.SIMPLE) !is 0)
				...
				if (width !is 0 && height !is 0) {
					...
					while (hwndChild !is null) {
						if (hwndChild !is handle) {
							...
							OS.MapWindowPoints(null, hwndParent, cast(POINT*)&rect, 2);
							auto rgn2 = OS.CreateRectRgn(rect.left, rect.top,
		...
		if (newText !is null) {
			...
			if (newText != /*eq*/ oldText) {
				...
				if (msg is OS.WM_SETTEXT) {
					...
					auto code = OS.CallWindowProc(EditProc, hwndText, msg,
							wParam, cast(ptrdiff_t) pszText);
