 ...
 */
    ...
    */
		...
		while (index < originalItems.length) {
			if (originalItems[index] is item)
				break;
			index++;
		}
 ...
 */
	public int[] getItemOrder() {
		...
		for (int i = 0; i < count; i++) {
			...
			if (index is originalItems.length)
 ...
 */
	void setItemOrder(int[] itemOrder) {
		...
		if (itemOrder.length !is itemCount)
			error(SWT.ERROR_INVALID_ARGUMENT);
	...
	override LRESULT wmNotifyChild(NMHDR* hdr, WPARAM wParam, LPARAM lParam) {
		...
		case OS.RBN_HEIGHTCHANGE: {
				if (!ignoreResize) {
					Point size = getSize();
					int border = getBorderWidth();
					int barHeight = cast(int) /*64bit*/ OS.SendMessage(handle,
							OS.RB_GETBARHEIGHT, 0, 0);
					if ((style & SWT.VERTICAL) !is 0) {
