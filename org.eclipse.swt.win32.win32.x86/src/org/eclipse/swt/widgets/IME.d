 ...
 */
	...
	TF_DISPLAYATTRIBUTE* getDisplayAttribute(short langid, int attInfo) {
		void* pProfiles;
		auto hr = OS.CoCreateInstance(CLSID_TF_InputProcessorProfiles.ptr, null,
				OS.CLSCTX_INPROC_SERVER, IID_ITfInputProcessorProfiles.ptr, &pProfiles);
		TF_DISPLAYATTRIBUTE* pda;
		if (hr is OS.S_OK) {
			byte[16] pclsid;
			byte[16] pguidProfile;
			/* pProfiles.GetDefaultLanguageProfile () */
			hr = OS.VtblCall(8, pProfiles, langid, GUID_TFCAT_TIP_KEYBOARD.ptr,
					pclsid.ptr, pguidProfile.ptr);
			if (hr is OS.S_OK) {
				void* pProvider;
				hr = OS.CoCreateInstance(pclsid.ptr, null, OS.CLSCTX_INPROC_SERVER,
						IID_ITfDisplayAttributeProvider.ptr, &pProvider);
				if (hr is OS.S_OK) {
					void* pEnum;
			...
			if ((lParam & OS.GCS_COMPSTR) !is 0) {
				...
				if (length_ > 0) {
					...
					if ((lParam & OS.GCS_COMPCLAUSE) !is 0) {
						length_ = OS.ImmGetCompositionStringW(hIMC, OS.GCS_COMPCLAUSE, /+cast(int [])+/ null,
								0);
						if (length_ > 0) {
							clauses = new int[length_ / 4];
							OS.ImmGetCompositionStringW(hIMC,
									OS.GCS_COMPCLAUSE, clauses.ptr, length_);
						}
					}
					if ((lParam & OS.GCS_COMPATTR) !is 0 && clauses !is null) {
						length_ = OS.ImmGetCompositionStringA(hIMC, OS.GCS_COMPATTR, /+cast(byte [])+/ null,
								0);
						if (length_ > 0) {
							...
							for (int i = 0; i < length_; i++) {
								...
								if (attr !is null) {
									...
									case OS.TF_CT_COLORREF:
										style.underlineColor = Color.win32_new(display,
												attr.crLine.cr);
										break;
									case OS.TF_CT_SYSCOLOR:
									...
									case OS.TF_LS_DOT:
										...
										break;
									case OS.TF_LS_SOLID:
										style.underlineStyle = attr.fBoldLine
											? UNDERLINE_IME_THICK : SWT.UNDERLINE_SINGLE;
										break;
									default:
									}
								}
		...
		if (hIMC !is null) {
			if (OS.ImmGetOpenStatus(hIMC)) {
				if (OS.ImmGetCompositionString(hIMC, OS.GCS_COMPSTR, null, 0) > 0) {
					...
					if (offset !is -1 && startOffset !is -1
							&& startOffset <= offset && offset < startOffset + length_) {
						auto imeWnd = OS.ImmGetDefaultIMEWnd(hwnd);
						offset = event.index + event.count - startOffset;
						int trailing = event.count > 0 ? 1 : 2;
						auto param = OS.MAKEWPARAM(OS.MAKEWORD(OS.IMEMOUSE_LDOWN,
								trailing), offset);
