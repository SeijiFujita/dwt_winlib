 ...
 */
    ...
    * setting the state.
    */
		if (((fsState & OS.TBSTATE_ENABLED) !is 0) is enabled)
			return;
		if (enabled) {
 ...
 */
    ...
    * setting the state.
    */
		if (((fsState & OS.TBSTATE_CHECKED) !is 0) is selected)
			return;
		if (selected) {
 ...
 */
	...
	void updateImages(bool enabled) {
		...
		info.dwMask = OS.TBIF_IMAGE;
		OS.SendMessage(hwnd, OS.TB_GETBUTTONINFO, id, &info);
		if (info.iImage is OS.I_IMAGENONE && image is null)
			return;
		ImageList imageList = parent.getImageList();
		ImageList hotImageList = parent.getHotImageList();
		ImageList disabledImageList = parent.getDisabledImageList();
		if (info.iImage is OS.I_IMAGENONE) {
			...
			if (disabledImageList is null) {
				disabledImageList = display.getImageListToolBarDisabled(listStyle,
						bounds.width, bounds.height);
			}
			if (hotImageList is null) {
				hotImageList = display.getImageListToolBarHot(listStyle,
						bounds.width, bounds.height);
			}
