...
public class Link : Control {
	...
	alias computeSize = Control.computeSize;
	alias windowProc = Control.windowProc;
	...
	private static void static_this() {
		...
		synchronized {
            ...
            */
				...
				auto lpszClassName = cast(TCHAR*) OS.HeapAlloc(hHeap,
						OS.HEAP_ZERO_MEMORY, byteCount);
			...
			accessible.addAccessibleControlListener(new class() AccessibleControlAdapter {
				override public void getChildAtPoint(AccessibleControlEvent e) {
					...
					override public void getLocation(AccessibleControlEvent e) {
						...
						e.x = rect.x; e.y = rect.y; e.width = rect.width; e.height = rect.height;
					}
					...
					override public void getChildCount(AccessibleControlEvent e) {
						...
						override public void getRole(AccessibleControlEvent e) {
							...
							override public void getState(AccessibleControlEvent e) {
								e.detail = ACC.STATE_FOCUSABLE; if (hasFocus())
								...
								override public void getSelection(AccessibleControlEvent e) {
									if (hasFocus())
										e.childID = ACC.CHILDID_SELF;}

									override public void getFocus(AccessibleControlEvent e) {
										if (hasFocus())
											e.childID = ACC.CHILDID_SELF;}
									}
);
								...
								Rectangle[] getRectangles(int linkIndex) {
									...
									if (lineStart is lineEnd) {
										...
										rects[index++] = layout.getBounds(point.x,
										lineOffsets[lineStart] - 1);
										rects[index++] = layout.getBounds(lineOffsets[lineEnd - 1],
										point.y);
 ...
 */
								...
								String parse(String string) {
									...
									int start = 0, tagStart = 0, linkStart = 0,
									endtagStart = 0, refStart = 0;
									...
									while (index < length_) {
										...
										case 6:
											if (c is '>') {
												...
												start, tagStart, result);
												int offset = cast(int) /*64bit*/ result.length;
												parseMnemonics(buffer, linkStart,
												endtagStart, result);
												offsets[linkIndex] = new Point(offset,
												cast(int) /*64bit*/ result.length - 1);
									...
									if (start < length_) {
										...
										Math.max(tagStart, linkStart), length_, result);
										if (mnemonic is -1)
											mnemonic = tmp;
									...
									if (offsets.length !is linkIndex) {
										...
										System.arraycopy(mnemonics, 0,
										newMnemonics, 0, linkIndex + 1);
										mnemonics = newMnemonics;
									}
									return result.toString();
								}

								int parseMnemonics(char[] buffer, int start,
								int end, StringBuffer result) {
 ...
 */
        ...
        */
										...
										bool enabled = cast(bool) OS.IsWindowEnabled(handle);
										TextStyle linkStyle = new TextStyle(null,
										enabled ? linkColor : disabledColor, null);
										linkStyle.underline = true;
										for (int i = 0; i < offsets.length; i++) {
											Point point = offsets[i];
											layout.setStyle(linkStyle, point.x, point.y);
								...
								override String windowClass() {
									...
									? TCHARsToStr(LinkClass) : display.windowClass();
								}

								override ptrdiff_t windowProc() {
									return LinkProc !is null ? cast(ptrdiff_t) LinkProc
									: display.windowProc();
								}
                ...
                */
											auto code = callWindowProc(handle,
											OS.WM_KEYDOWN, wParam, lParam);
								...
								override LRESULT WM_GETDLGCODE(WPARAM wParam, LPARAM lParam) {
									...
									if (OS.COMCTL32_MAJOR >= 6) {
										...
										index = 0;
										while (OS.SendMessage(handle,
											OS.LM_GETITEM, 0, &item) !is 0) {
										...
										OS.WM_GETDLGCODE, wParam, lParam);
									}
									else {
										index = focusIndex;
								...
								override LRESULT WM_LBUTTONDOWN(WPARAM wParam, LPARAM lParam) {
									...
									if (OS.COMCTL32_MAJOR < 6) {
										...
										if (oldSelectionX !is -1 && oldSelectionY !is -1) {
											...
											Rectangle rect = layout.getBounds(oldSelectionX,
											oldSelectionY);
									...
									if (OS.COMCTL32_MAJOR < 6) {
										...
										if (OS.GetKeyState(OS.VK_LBUTTON) < 0) {
											...
											if (selection.y !is oldSelection) {
												...
												Rectangle rect = layout.getBounds(oldSelection,
												newSelection);
											...
											for (int j = 0; j < offsets.length; j++) {
												Rectangle[] rects = getRectangles(j);
												for (int i = 0; i < rects.length;
													i++) {
													Rectangle rect = rects[i];
													if (rect.contains(x, y)) {
														setCursor(display.getSystemCursor(
														SWT.CURSOR_HAND));
														return result;
													}
												}
    ...
    */
									if (OS.COMCTL32_MAJOR >= 6) {
										if (!OS.IsWindowEnabled(handle)) {
											OS.SetTextColor(cast(HANDLE) wParam,
								...
								override LRESULT wmNotifyChild(NMHDR* hdr,
