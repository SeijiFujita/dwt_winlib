 ...
 */
	void drawRectangles(Rectangle[] rects, bool stippled) {
		...
		for (int i = 0; i < rects.length; i++) {
			...
			OS.PatBlt(hDC, rect.x, rect.y + bandWidth, bandWidth,
					rect.height - (bandWidth * 2), OS.PATINVERT);
			OS.PatBlt(hDC, rect.x + rect.width - bandWidth, rect.y + bandWidth,
					bandWidth, rect.height - (bandWidth * 2), OS.PATINVERT);
			OS.PatBlt(hDC, rect.x, rect.y + rect.height - bandWidth, rect.width,
					bandWidth, OS.PATINVERT);
		}
		if (stippled) {
 ...
 */
	...
	void moveRectangles(int xChange, int yChange) {
		if (bounds is null)
			return;
		if (xChange < 0 && ((style & SWT.LEFT) is 0))
			xChange = 0;
		if (xChange > 0 && ((style & SWT.RIGHT) is 0))
			xChange = 0;
		if (yChange < 0 && ((style & SWT.UP) is 0))
			yChange = 0;
		if (yChange > 0 && ((style & SWT.DOWN) is 0))
			yChange = 0;
 ...
 */
    ...
    */
		...
		if ((parent is null && isVista) || !mouseDown) {
			...
			hwndTransparent = OS.CreateWindowEx(isVista ? OS.WS_EX_LAYERED | OS.WS_EX_NOACTIVATE
					: OS.WS_EX_TRANSPARENT, display.windowClass_.ptr, null, OS.WS_POPUP,
					0, 0, width, height, null, null, OS.GetModuleHandle(null), null);
			...
			version (VISTA)
				if (isVista) {
					OS.SetLayeredWindowAttributes(hwndTransparent, 0xFFFFFF,
							cast(byte) 0xFF, OS.LWA_COLORKEY | OS.LWA_ALPHA);
				}
 ...
 */
    ...
    */
     ...
     */
		...
		for (int i = 0; i < rectangles.length; i++) {
			Rectangle proportion = proportions[i];
			newRects[i] = new Rectangle(proportion.x * bounds.width / 100 + bounds.x,
					proportion.y * bounds.height / 100 + bounds.y,
					proportion.width * bounds.width / 100, proportion.height * bounds.height / 100);
		}
		rectangles = newRects;
	}
 ...
 */
	...
	private static extern (Windows).LRESULT transparentFunc(HWND hwnd, int msg,
        ...
        */
		...
		case OS.WM_PAINT:
			if (parent is null && !OS.IsWinCE && OS.WIN32_VERSION >= OS.VERSION(6, 0)) {
				...
				OS.PatBlt(hDC, ps.rcPaint.left, ps.rcPaint.top,
						ps.rcPaint.right - ps.rcPaint.left,
						ps.rcPaint.bottom - ps.rcPaint.top, OS.PATCOPY);
				OS.SelectObject(hDC, oldBrush);
				...
				for (int i = 0; i < rects.length; i++) {
					...
					OS.PatBlt(hDC, rect.x, rect.y + bandWidth, bandWidth,
							rect.height - (bandWidth * 2), OS.PATCOPY);
					OS.PatBlt(hDC, rect.x + rect.width - bandWidth,
							rect.y + bandWidth, bandWidth,
							rect.height - (bandWidth * 2), OS.PATCOPY);
					OS.PatBlt(hDC, rect.x, rect.y + rect.height - bandWidth,
							rect.width, bandWidth, OS.PATCOPY);
				}
				OS.SelectObject(hDC, oldBrush);
				if (stippled) {
					OS.DeleteObject(hBrush);
	...
	override LRESULT wmKeyDown(HWND hwnd, WPARAM wParam, LPARAM lParam) {
		...
		if (xChange !is 0 || yChange !is 0) {
			...
			for (int i = 0; i < rectangles.length; i++) {
				Rectangle current = rectangles[i];
				rectsToErase[i] = new Rectangle(current.x, current.y,
						current.width, current.height);
			}
