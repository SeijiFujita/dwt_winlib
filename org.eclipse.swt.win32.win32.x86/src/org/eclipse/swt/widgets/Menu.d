 ...
 */
    ...
    */
	...
	void _setVisible(bool visible) {
		if ((style & (SWT.BAR | SWT.DROP_DOWN)) !is 0)
			return;
        ...
        */
			bool success = cast(bool) OS.TrackPopupMenu(handle, flags, nX,
					nY, 0, hwndParent, null);
 ...
 */
	...
	int imageIndex(Image image) {
		if (hwndCB is null || image is null)
				...
				for (int i = 0; i < items.length; i++) {
					...
					if ((style & SWT.SEPARATOR) is 0) {
						if (item.image is null || foreground !is -1) {
							info.hbmpItem = hasImage || foreground !is -1 ? OS.HBMMENU_CALLBACK
								: null;
