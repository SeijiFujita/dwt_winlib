 ...
 */
    ...
    */
		if (horizontalBar !is null && (lParam is 0 || lParam is cast(size_t) handle)) {
			return wmScroll(horizontalBar, (state & CANVAS) !is 0, handle,
					OS.WM_HSCROLL, wParam, lParam);
		}
		return result;
	}
    ...
    */
		if ((state & CANVAS) !is 0) {
			if ((wParam & (OS.MK_SHIFT | OS.MK_CONTROL)) !is 0)
				return result;
    ...
    */
		if (verticalBar !is null && (lParam is 0 || lParam is cast(size_t) handle)) {
			return wmScroll(verticalBar, (state & CANVAS) !is 0, handle,
					OS.WM_VSCROLL, wParam, lParam);
		}
		return result;
	}
		...
		if (OS.COMCTL32_MAJOR >= 6 && OS.IsAppThemed()) {
			if (!OS.IsWinCE && OS.WIN32_VERSION < OS.VERSION(6, 0)) {
				...
				if ((bits1 & (OS.WS_HSCROLL | OS.WS_VSCROLL)) !is 0) {
					...
					cornerRect.left = cornerRect.right - (hVisible
							? OS.GetSystemMetrics(OS.SM_CXVSCROLL) : 0);
					cornerRect.top = cornerRect.bottom - (vVisible
							? OS.GetSystemMetrics(OS.SM_CYHSCROLL) : 0);
					if (cornerRect.left !is cornerRect.right && cornerRect.top !is cornerRect
							.bottom) {
						...
						if ((shell.style & SWT.RESIZE) !is 0) {
							...
							if (!drawGripper) {
								...
								drawGripper = shellRect.right is windowRect.right
									&& shellRect.bottom is windowRect.bottom;
							}
							if (drawGripper) {
								OS.DrawThemeBackground(display.hScrollBarTheme(),
										hDC, OS.SBP_SIZEBOX, 0, &cornerRect, null);
							}
						}
