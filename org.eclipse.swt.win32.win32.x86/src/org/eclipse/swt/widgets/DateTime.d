...
public class DateTime : Composite {
	...
	alias windowProc = Composite.windowProc;
	...
	static const int[] MONTH_NAMES = [
		OS.LOCALE_SMONTHNAME1, OS.LOCALE_SMONTHNAME2, OS.LOCALE_SMONTHNAME3,
		OS.LOCALE_SMONTHNAME4, OS.LOCALE_SMONTHNAME5,
		OS.LOCALE_SMONTHNAME6, OS.LOCALE_SMONTHNAME7, OS.LOCALE_SMONTHNAME8, OS.LOCALE_SMONTHNAME9,
		OS.LOCALE_SMONTHNAME10, OS.LOCALE_SMONTHNAME11, OS.LOCALE_SMONTHNAME12
	];
 ...
 */
	public this(Composite parent, int style) {
		...
		if ((this.style & SWT.SHORT) !is 0) {
			String buffer = ((this.style & SWT.DATE) !is 0) ? getCustomShortDateFormat()
    ...
    */
			...
			if ((style & SWT.CALENDAR) !is 0) {
				...
				if ((style & SWT.DATE) !is 0) {
					...
					for (int i = 0; i <= MAX_DIGIT; i++) {
						...
						int size = OS.GetDateFormat(OS.LOCALE_USER_DEFAULT, OS.DATE_SHORTDATE,
								&systime, null, buffer.ptr, cast(int) /*64bit*/ buffer.length);
						if (size is 0) {
							buffer = new TCHAR[size];
							OS.GetDateFormat(OS.LOCALE_USER_DEFAULT, OS.DATE_SHORTDATE,
									&systime, null, buffer.ptr, cast(int) /*64bit*/ buffer.length);
						}
					...
					else if (secondWidest > 1)
						widest = secondWidest * 1000 + widest * 100 + widest * 10 + widest;
					else
						widest = thirdWidest * 1000 + widest * 100 + widest * 10 + widest;
					...
					for (short i = 0; i < MONTH_NAMES.length; i++) {
						...
						int size = OS.GetLocaleInfo(OS.LOCALE_USER_DEFAULT,
								name, buffer.ptr, cast(int) /*64bit*/ buffer.length);
						if (size is 0) {
							buffer = new TCHAR[size];
							OS.GetLocaleInfo(OS.LOCALE_USER_DEFAULT, name,
									buffer.ptr, cast(int) /*64bit*/ buffer.length);
						}
						rect.left = rect.top = rect.right = rect.bottom = 0;
						OS.DrawText(hDC, buffer.ptr, size, &rect, flags);
						if (rect.right - rect.left > width) {
					...
					int dwFlags = ((style & SWT.MEDIUM) !is 0) ? OS.DATE_SHORTDATE
						: ((style & SWT.SHORT) !is 0) ? OS.DATE_YEARMONTH : OS.DATE_LONGDATE;
					width = 0;
					for (short i = 1; i <= MAX_DAY; i++) {
						systime.wDay = i;
						int size = OS.GetDateFormat(OS.LOCALE_USER_DEFAULT, dwFlags,
								&systime, null, buffer.ptr, cast(int) /*64bit*/ buffer.length);
						if (size is 0) {
							buffer = new TCHAR[size];
							OS.GetDateFormat(OS.LOCALE_USER_DEFAULT, dwFlags,
									&systime, null, buffer.ptr, cast(int) /*64bit*/ buffer.length);
						}
						rect.left = rect.top = rect.right = rect.bottom = 0;
						OS.DrawText(hDC, buffer.ptr, size, &rect, flags);
					...
					for (short i = 0; i < max; i++) {
						systime.wHour = i;
						int size = OS.GetTimeFormat(OS.LOCALE_USER_DEFAULT, dwFlags,
								&systime, null, buffer.ptr, cast(int) /*64bit*/ buffer.length);
						if (size is 0) {
							buffer = new TCHAR[size];
							OS.GetTimeFormat(OS.LOCALE_USER_DEFAULT, dwFlags,
									&systime, null, buffer.ptr, cast(int) /*64bit*/ buffer.length);
						}
						rect.left = rect.top = rect.right = rect.bottom = 0;
						OS.DrawText(hDC, buffer.ptr, size, &rect, flags);
	...
	String getComputeSizeString() {
		...
		if ((style & SWT.DATE) !is 0) {
			...
			if ((style & SWT.MEDIUM) !is 0)
				return getShortDateFormat();
			if ((style & SWT.LONG) !is 0)
	...
	String getCustomShortDateFormat() {
		if (true) {
			...
			int size = OS.GetLocaleInfo(OS.LOCALE_USER_DEFAULT,
					OS.LOCALE_SYEARMONTH, tchar.ptr, 80);
		...
		while (start < length) {
			...
			if (ch is SINGLE_QUOTE)
				inQuotes = !inQuotes;
			else if (ch is DAY_FORMAT_CONSTANT && !inQuotes) {
				end = start + 1;
				while (end < length && buffer.charAt(end) is DAY_FORMAT_CONSTANT)
					end++;
				int ordering = getShortDateFormatOrdering();
				switch (ordering) {
				case MONTH_DAY_YEAR:
					// skip the following separator
					while (end < length && buffer.charAt(end) !is YEAR_FORMAT_CONSTANT)
						end++;
					break;
				case DAY_MONTH_YEAR:
					// skip the following separator
					while (end < length && buffer.charAt(end) !is MONTH_FORMAT_CONSTANT)
						end++;
					break;
				case YEAR_MONTH_DAY:
					// skip the preceding separator
					while (start > 0 && buffer.charAt(start) !is MONTH_FORMAT_CONSTANT)
						start--;
					break;
				default:
				}
	...
	String getCustomShortTimeFormat() {
		...
		while (start < length) {
			...
			if (ch is SINGLE_QUOTE)
				...
				while (end < length && buffer.charAt(end) is SECONDS_FORMAT_CONSTANT)
					end++;
				// skip the preceding separator
				while (start > 0 && buffer.charAt(start) !is MINUTES_FORMAT_CONSTANT)
					start--;
				start++;
				break;
			}
 ...
 */
	...
	override String getNameText() {
		return (style & SWT.TIME) !is 0 ? Format("{}:{}:{}", getHours(),
				getMinutes(), getSeconds()) : Format("{}/{}/{}",
 ...
 */
	public void setHours(int hours) {
		...
		OS.SendMessage(handle, msg, 0, &systime);
		if ((style & SWT.CALENDAR) !is 0 && hours >= 0 && hours <= 23)
 ...
 */
	public void setMinutes(int minutes) {
		...
		OS.SendMessage(handle, msg, 0, &systime);
		if ((style & SWT.CALENDAR) !is 0 && minutes >= 0 && minutes <= 59)
 ...
 */
	public void setSeconds(int seconds) {
		...
		OS.SendMessage(handle, msg, 0, &systime);
		if ((style & SWT.CALENDAR) !is 0 && seconds >= 0 && seconds <= 59)
 ...
 */
	public void setTime(int hours, int minutes, int seconds) {
		...
		systime.wSecond = cast(short) seconds;
		OS.SendMessage(handle, msg, 0, &systime);
		if ((style & SWT.CALENDAR) !is 0 && hours >= 0 && hours <= 23 && minutes >= 0
 ...
 */
    ...
    */
		...
		if ((style & SWT.DATE) !is 0)
			bits |= ((style & SWT.MEDIUM) !is 0 ? OS.DTS_SHORTDATECENTURYFORMAT
					: OS.DTS_LONGDATEFORMAT) | OS.DTS_UPDOWN;
		return bits;
	}

	override String windowClass() {
		return (style & SWT.CALENDAR) !is 0 ? TCHARsToStr(CalendarClass)
			: TCHARsToStr(DateTimeClass);
	}

	override ptrdiff_t windowProc() {
		return (style & SWT.CALENDAR) !is 0 ? cast(ptrdiff_t) CalendarProc
			: cast(ptrdiff_t) DateTimeProc;
	}
	...
	override LRESULT wmNotifyChild(NMHDR* hdr, WPARAM wParam, LPARAM lParam) {
		...
		case OS.DTN_DATETIMECHANGE:
			...
			int msg = (style & SWT.CALENDAR) !is 0 ? OS.MCM_GETCURSEL : OS.DTM_GETSYSTEMTIME;
			OS.SendMessage(handle, msg, 0, &systime);
			if (lastSystemTime is null || systime.wDay !is lastSystemTime.wDay
					|| systime.wMonth !is lastSystemTime.wMonth
					|| systime.wYear !is lastSystemTime.wYear) {
