 ...
 */
	...
	override LRESULT WM_KEYDOWN(WPARAM wParam, LPARAM lParam) {
		...
		case OS.VK_DOWN:
			...
			if ((style & SWT.VERTICAL) !is 0) {
				if (wParam is OS.VK_UP || wParam is OS.VK_DOWN)
					break;
				pt.x = wParam is OS.VK_LEFT ? -step : step;
			}
			else {
				if (wParam is OS.VK_LEFT || wParam is OS.VK_RIGHT)
					break;
			...
			if (newX is lastX && newY is lastY)
				return result;
	...
	override LRESULT WM_MOUSEMOVE(WPARAM wParam, LPARAM lParam) {
		...
		if (result !is null)
			return result;
		if (!dragging || (wParam & OS.MK_LBUTTON) is 0)
			return result;
