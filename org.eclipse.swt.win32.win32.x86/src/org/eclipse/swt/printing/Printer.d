	...
	public static PrinterData[] getPrinterList() {
		...
		for (int p = 0; p < nameCount; p++) {
			...
			if (OS.GetProfileString(TCHARsToStr(profile), device, null, buf, length) > 0) {
				int commaIndex = 0;
				while (buf[commaIndex]!is ',' && commaIndex < length)
					commaIndex++;
 ...
 */
	override protected void create(DeviceData deviceData) {
 ...
 */
	public bool startJob(String jobName) {
		...
		if (lpszDocName !is null)
			OS.HeapFree(hHeap, 0, lpszDocName);
		if (lpszOutput !is null)
