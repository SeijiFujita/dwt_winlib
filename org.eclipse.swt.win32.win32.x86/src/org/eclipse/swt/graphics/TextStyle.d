 ...
 */
	public override equals_t opEquals(Object object) {
		...
		if (object is null)
			return false;
		if (!(cast(TextStyle) object))
			return false;
		TextStyle style = cast(TextStyle) object;
		if (foreground !is null) {
			if (foreground !is style.foreground)
				return false;
		}
		else if (style.foreground !is null)
			return false;
		if (background !is null) {
			if (background !is style.background)
				return false;
		}
		else if (style.background !is null)
			return false;
		if (font !is null) {
			if (font !is style.font)
				return false;
		}
		else if (style.font !is null)
			return false;
		if (metrics !is null || style.metrics !is null)
			return false;
		if (underline !is style.underline)
			return false;
		if (underlineStyle !is style.underlineStyle)
			return false;
		if (borderStyle !is style.borderStyle)
			return false;
		if (strikeout !is style.strikeout)
			return false;
		if (rise !is style.rise)
			return false;
		if (underlineColor !is null) {
			if (underlineColor != (style.underlineColor))
				return false;
		}
		else if (style.underlineColor !is null)
			return false;
		if (strikeoutColor !is null) {
			if (strikeoutColor != (style.strikeoutColor))
				return false;
		}
		else if (style.strikeoutColor !is null)
			return false;
		if (underlineStyle !is style.underlineStyle)
			return false;
		...
		else if (style.borderColor !is null)
			return false;
		return true;
	}
 ...
 */
	public override hash_t toHash() {
		int hash = 0;
		if (foreground !is null)
			hash ^= foreground.toHash();
		if (background !is null)
			hash ^= background.toHash();
		if (font !is null)
			hash ^= font.toHash();
		if (metrics !is null)
			hash ^= metrics.toHash();
		...
		hash ^= rise;
		if (underlineColor !is null)
			hash ^= underlineColor.toHash();
		if (strikeoutColor !is null)
			hash ^= strikeoutColor.toHash();
		if (borderColor !is null)
			hash ^= borderColor.toHash();
		hash ^= underlineStyle;
		return hash;
	}
