 ...
 */
	public this(Device device, ImageData source, ImageData mask, int hotspotX, int hotspotY) {
		...
		byte[] sourceData = ImageData.convertPad(source.data, source.width,
				source.height, source.depth, source.scanlinePad, 2);
		byte[] maskData = ImageData.convertPad(mask.data, mask.width,
				mask.height, mask.depth, mask.scanlinePad, 2);

		/* Create the cursor */
		auto hInst = OS.GetModuleHandle(null);
		static if (OS.IsWinCE)
			SWT.error(SWT.ERROR_NOT_IMPLEMENTED);
		handle = OS.CreateCursor(hInst, hotspotX, hotspotY, source.width,
				source.height, sourceData.ptr, maskData.ptr);
		if (handle is null)
			SWT.error(SWT.ERROR_NO_HANDLES);
		init_();
	}
