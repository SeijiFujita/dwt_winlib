 ...
 */
	public RGB getRGB() {
		if (isDisposed())
			SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
		return new RGB(cast(int) handle & 0xFF, cast(int)(handle & 0xFF00) >> 8,
				cast(int)(handle & 0xFF0000) >> 16);
	}
 ...
 */
	void init_(int red, int green, int blue) {
		...
		if ((entry.peRed is cast(byte) red)
				&& (entry.peGreen is cast(byte) green) && (entry.peBlue is cast(byte) blue)) {
