...
"));
	private static void static_this() {
		...
		synchronized {
			if (!static_this_completed) {
				OS.IIDFromString("{275c23e2-3747-11d0-9fea-00aa003f8646}\0"w.ptr,
						CLSID_CMultiLanguage.ptr);
				OS.IIDFromString("{DCCFC162-2B38-11d2-B7EC-00C04F8F5D9A}\0"w.ptr,
						IID_IMLangFontLink2.ptr);
 ...
 */
	public this(Device device) {
		...
		if (OS.CoCreateInstance(CLSID_CMultiLanguage.ptr, null, OS.CLSCTX_INPROC_SERVER,
				IID_IMLangFontLink2.ptr, cast(void*)&ppv) is OS.S_OK) {
			mLangFontLink2 = ppv;
		}
		init_();
	}
	...
	void breakRun(StyleItem run) {
		if (run.psla !is null)
			return;
		String16 wchars = segmentsWText[getUTF16index(
				run.UTF8start) .. getUTF16index(run.UTF8start + run.UTF8length)];
		auto hHeap = OS.GetProcessHeap();
		run.psla = cast(SCRIPT_LOGATTR*) OS.HeapAlloc(hHeap,
				OS.HEAP_ZERO_MEMORY, SCRIPT_LOGATTR.sizeof * wchars.length);
		if (run.psla is null)
		...
		for (int i = 0; i < allRuns.length - 1; i++) {
			...
			if (run.UTF8length.internalValue is 1) {
				...
				case '\t': {
						...
						if (j is tabsLength) {
							...
							int lastTabWidth = tabsLength > 1 ? tabs[tabsLength - 1] - tabs[tabsLength - 2]
								: tabs[0];
							if (lastTabWidth > 0) {
				...
				case '\r': {
						...
						if (next.UTF8length.internalValue !is 0
								&& segmentsText.charAt(cast(int) /*64bit*/ next
									.UTF8start.internalValue) is '\n') {
							...
							System.arraycopy(allRuns, i + 2, newAllRuns, i + 1,
									allRuns.length - i - 2);
							allRuns = newAllRuns;
						}
						break;
			...
			if (wrapWidth !is -1 && lineWidth + run.width > wrapWidth && !run.tab) {
				...
				if (run.style !is null && run.style.metrics !is null) {
					...
					OS.ScriptGetLogicalWidths(&run.analysis, cChars, run.glyphCount,
							run.advances, run.clusters, run.visAttrs, piDx.ptr);
				}
                ...
                */
					if (wstart is 0 && i !is lineStart && !run.tab) {
						if (logAttr.fSoftBreak && !logAttr.fWhiteSpace) {
							...
							if (properties.langid is langID || langID is OS.LANG_NEUTRAL
									|| properties.langid is OS.LANG_NEUTRAL) {
								breakRun(pRun);
								...
								if (!logAttr.fWhiteSpace)
									wstart = cast(UTF16index)-1;
							}
						}
					}
					if (wstart >= 0 || i is lineStart)
						break;
					run = allRuns[--i];
				...
				if (0 < wstart && wstart < runWlength) {
					...
					UTF8shift UTF8startShift = getUTF8index(getUTF16index(run.UTF8start) + wstart)
						- run.UTF8start;
				...
				if (justify && wrapWidth !is -1 && run.softBreak && lineWidth > 0) {
					...
					for (int j = 0; j < runs[line].length; j++) {
						...
						if (iDx !is item.width) {
							item.justify = cast(int*) OS.HeapAlloc(hHeap,
									OS.HEAP_ZERO_MEMORY, item.glyphCount * 4);
							if (item.justify is null)
								SWT.error(SWT.ERROR_NO_HANDLES);
							OS.ScriptJustify(item.visAttrs, item.advances,
									item.glyphCount, iDx - item.width, 2, item.justify);
							item.width = iDx;
						}
 ...
 * </ul>
 */
	public void draw(GC gc, int x, int y, int i_selectionStart, int i_selectionEnd,
			Color selectionForeground, Color selectionBackground) {
		draw(gc, x, y, i_selectionStart, i_selectionEnd, selectionForeground,
 ...
 */
	public void draw(GC gc, int x, int y, int i_selectionStart, int i_selectionEnd,
		...
		if (selectionForeground !is null && selectionForeground.isDisposed())
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
		if (selectionBackground !is null && selectionBackground.isDisposed())
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
		int length = cast(int) /*64bit*/ text.length;
		int wlength = cast(int) /*64bit*/ wtext.length;
		if (length is 0 && flags is 0)
			return;
		auto hdc = gc.handle;
		...
		bool hasSelection = selectionStart <= selectionEnd
			&& selectionStart.internalValue !is -1 && selectionEnd.internalValue !is -1;
		if (hasSelection || (flags & SWT.LAST_LINE_SELECTION) !is 0) {
			selectionStart = Math.min(Math.max(text.firstIndex(),
					selectionStart), text.beforeEndIndex());
			selectionEnd = Math.min(Math.max(text.firstIndex(), selectionEnd),
					text.beforeEndIndex());
		...
		if (hasSelection || (flags & SWT.LAST_LINE_SELECTION) !is 0) {
			if (gdip) {
				...
				auto argb = ((alpha & 0xFF) << 24) | ((bg >> 16) & 0xFF) | (
						bg & 0xFF00) | ((bg & 0xFF) << 16);
				...
				argb = ((alpha & 0xFF) << 24) | ((fg >> 16) & 0xFF) | (
						fg & 0xFF00) | ((fg & 0xFF) << 16);
		...
		for (int line = 0; line < runs.length; line++) {
			...
			if (flags !is 0 && (hasSelection || (flags & SWT.LAST_LINE_SELECTION) !is 0)) {
				...
				if (line is runs.length - 1 && (flags & SWT.LAST_LINE_SELECTION) !is 0) {
					...
					if (run.lineBreak && !run.softBreak) {
						if (selectionStart <= run.UTF8start && run.UTF8start <= selectionEnd)
							extents = true;
					}
					else {
						UTF8index endOffset = segmentsText.offsetBefore(
								run.UTF8start + run.UTF8length);
						if (selectionStart <= endOffset && endOffset < selectionEnd
								&& (flags & SWT.FULL_SELECTION) !is 0) {
							extents = true;
						}
					}
				}
				if (extents) {
					...
					if (gdip) {
						Gdip.Graphics_FillRectangle(gdipGraphics, cast(Gdip.Brush) selBrush,
								drawX + lineWidth[line], drawY, width, lineHeight);
					}
					else {
						OS.SelectObject(hdc, selBrush);
						OS.PatBlt(hdc, drawX + lineWidth[line], drawY, width,
								lineHeight, OS.PATCOPY);
					}
				}
			}
			...
			for (int i = 0; i < lineRuns.length; i++) {
				...
				if (run.UTF8length.internalValue is 0)
					continue;
				if (drawX > clip.x + clip.width)
					break;
				if (drawX + run.width >= clip.x) {
					if (!run.lineBreak || run.softBreak) {
						...
						bool fullSelection = hasSelection
							&& selectionStart <= run.UTF8start && selectionEnd >= end;
						if (fullSelection) {
							if (gdip) {
								Gdip.Graphics_FillRectangle(gdipGraphics,
										cast(Gdip.Brush) selBrush, drawX,
										drawY, run.width, lineHeight);
							}
							else {
								OS.SelectObject(hdc, selBrush);
							...
							if (run.style !is null && run.style.background !is null) {
								...
								if (gdip) {
									int argb = ((alpha & 0xFF) << 24) | (
											(bg >> 16) & 0xFF) | (bg & 0xFF00) | ((bg & 0xFF) << 16);
									auto color = Gdip.Color_new(argb);
									auto brush = Gdip.SolidBrush_new(color);
									Gdip.Graphics_FillRectangle(gdipGraphics,
											cast(Gdip.Brush) brush, drawX,
											drawY, run.width, lineHeight);
									Gdip.Color_delete(color);
									...
									OS.PatBlt(hdc, drawX, drawY, run.width,
											lineHeight, OS.PATCOPY);
									OS.SelectObject(hdc, oldBrush);
									OS.DeleteObject(hBrush);
								}
							}
							bool partialSelection = hasSelection
								&& !(selectionStart > end || run.UTF8start > selectionEnd);
							if (partialSelection) {
								UTF16index selStart = getUTF16index(Math.max(selectionStart,
										run.UTF8start)) - getUTF16index(run.UTF8start);
								UTF16index selEnd = getUTF16index(Math.min(selectionEnd, end)) - getUTF16index(
										run.UTF8start);
								...
								OS.ScriptCPtoX(cast(int) /*64bit*/ selStart, false, cChars, gGlyphs,
										run.clusters, run.visAttrs, advances,
										&run.analysis, &piX);
								int runX = (orientation & SWT.RIGHT_TO_LEFT) !is 0 ? run.width - piX
									: piX;
								rect.left = drawX + runX;
								rect.top = drawY;
								OS.ScriptCPtoX(cast(int) /*64bit*/ selEnd, true, cChars, gGlyphs,
										run.clusters, run.visAttrs, advances,
										&run.analysis, &piX);
								runX = (orientation & SWT.RIGHT_TO_LEFT) !is 0 ? run.width - piX
									: piX;
								...
								if (gdip) {
									...
									Gdip.Graphics_FillRectangle(gdipGraphics, cast(Gdip.Brush) selBrush, rect.left,
			...
			for (int i = 0; i < lineRuns.length; i++) {
				...
				if (drawX + run.width >= clip.x) {
					if (!run.tab && (!run.lineBreak || run.softBreak)
							&& !(run.style !is null && run.style.metrics !is null)) {
						UTF8index end = segmentsText.offsetBefore(run.UTF8start + run.UTF8length);
						bool fullSelection = hasSelection
							&& selectionStart <= run.UTF8start && selectionEnd >= end;
						bool partialSelection = hasSelection && !fullSelection
							&& !(selectionStart > end || run.UTF8start > selectionEnd);
						OS.SelectObject(hdc, getItemFont(run));
						int drawRunY = drawY + (baseline - run.ascent);
						if (partialSelection) {
							UTF16index selStart = Math.max(getUTF16index(selectionStart),
									getUTF16index(run.UTF8start)) - getUTF16index(run.UTF8start);
							UTF16index selEnd = Math.min(getUTF16index(selectionEnd),
									getUTF16index(end)) - getUTF16index(run.UTF8start);
							UTF16shift cChars = getUTF16length(run); // make it wchar
							int gGlyphs = run.glyphCount;
							int piX;
							int* advances = run.justify !is null ? run.justify : run.advances;
						...
						if (gdip) {
							OS.BeginPath(hdc);
							OS.ScriptTextOut(hdc, run.psc, drawX, drawRunY, 0,
									null, &run.analysis, null, 0, run.glyphs, run.glyphCount,
							...
							for (int typeIndex = 0; typeIndex < types.length; typeIndex++) {
								...
								case OS.PT_MOVETO:
									newType = Gdip.PathPointTypeStart;
									break;
								case OS.PT_LINETO:
									newType = Gdip.PathPointTypeLine;
									break;
								case OS.PT_BEZIERTO:
									newType = Gdip.PathPointTypeBezier;
									break;
								default:
								}
								if ((type & OS.PT_CLOSEFIGURE) !is 0)
									newType |= Gdip.PathPointTypeCloseSubpath;
								types[typeIndex] = cast(byte) newType;
							}
							auto path = Gdip.GraphicsPath_new(cast(Gdip.Point*) points.ptr,
									types.ptr, count, Gdip.FillModeAlternate);
							if (path is null)
								SWT.error(SWT.ERROR_NO_HANDLES);
							...
							if (fullSelection) {
								...
								if (run.style !is null && run.style.foreground !is null) {
									...
									int argb = ((alpha & 0xFF) << 24) | (
											(fg >> 16) & 0xFF) | (fg & 0xFF00) | ((fg & 0xFF) << 16);
									auto color = Gdip.Color_new(argb);
							...
							if (partialSelection) {
								...
								Gdip.Graphics_SetClip(gdipGraphics, &gdipRect,
							...
							if ((data.style & SWT.MIRRORED) !is 0) {
								...
								Gdip.Graphics_ScaleTransform(gdipGraphics, -1,
										1, Gdip.MatrixOrderPrepend);
								Gdip.Graphics_TranslateTransform(gdipGraphics,
							...
							drawLines(gdip, gdipGraphics, x, drawY + baseline, lineUnderlinePos,
									drawY + lineHeight, lineRuns, i, brush, null, alpha);
							if (partialSelection) {
								...
								if ((data.style & SWT.MIRRORED) !is 0) {
									gstate2 = Gdip.Graphics_Save(gdipGraphics);
									Gdip.Graphics_ScaleTransform(gdipGraphics,
											-1, 1, Gdip.MatrixOrderPrepend);
									Gdip.Graphics_TranslateTransform(gdipGraphics,
											-2 * drawX - run.width, 0, Gdip.MatrixOrderPrepend);
								}
								...
								Gdip.Graphics_SetSmoothingMode(gdipGraphics, antialias);
								drawLines(gdip, gdipGraphics, x, drawY + baseline, lineUnderlinePos,
										drawY + lineHeight, lineRuns, i,
										selBrushFg, &rect, alpha);
								Gdip.Graphics_Restore(gdipGraphics, gstate);
							}
							borderClip = drawBorder(gdip, gdipGraphics, x,
									drawY, lineHeight, foregroundBrush, selBrushFg,
									fullSelection, borderClip, partialSelection ? &rect : null, alpha,
									lineRuns, i, selectionStart, selectionEnd);
							Gdip.GraphicsPath_delete(path);
							if (brush !is cast(Gdip.Brush) selBrushFg
									&& brush !is cast(Gdip.Brush) foregroundBrush)
							...
							OS.ScriptTextOut(hdc, run.psc, drawX + offset, drawRunY,
									0, null, &run.analysis, null, 0, run.glyphs,
									run.glyphCount, run.advances, run.justify, run.goffsets);
							drawLines(gdip, hdc, x, drawY + baseline, lineUnderlinePos,
									drawY + lineHeight, lineRuns, i, cast(void*) fg, null, alpha);
							if (partialSelection && fg !is selectionForeground.handle) {
								OS.SetTextColor(hdc, selectionForeground.handle);
								OS.ScriptTextOut(hdc, run.psc, drawX + offset, drawRunY,
										OS.ETO_CLIPPED, &rect, &run.analysis, null, 0, run.glyphs,
										run.glyphCount, run.advances, run.justify, run.goffsets);
								drawLines(gdip, hdc, x, drawY + baseline,
										lineUnderlinePos, drawY + lineHeight, lineRuns, i,
										cast(void*) selectionForeground.handle, &rect, alpha);
							}
							int selForeground = selectionForeground !is null ? selectionForeground.handle
								: 0;
							borderClip = drawBorder(gdip, hdc, x, drawY, lineHeight, cast(void*) foreground,
									cast(void*) selForeground, fullSelection,
									borderClip, partialSelection ? &rect : null,
									alpha, lineRuns, i, selectionStart, selectionEnd);
						}
					}
				}
				drawX += run.width;
		...
		if (gdip) {
			if (selBrush !is null)
				Gdip.SolidBrush_delete(cast(Gdip.SolidBrush) selBrush);
			if (selBrushFg !is null)
				Gdip.SolidBrush_delete(cast(Gdip.SolidBrush) selBrushFg);
			if (selPen !is null)
	...
	void drawLines(bool advance, void* graphics, int x, int lineBaseline,
			int lineUnderlinePos, int lineBottom, StyleItem[] line, int index,
			void* color, RECT* clipRect, int alpha) {
		...
		if (advance) {
			...
			if (style.underline) {
				if (style.underlineColor !is null) {
					...
					int argb = ((alpha & 0xFF) << 24) | ((fg >> 16) & 0xFF) | (
							fg & 0xFF00) | ((fg & 0xFF) << 16);
				...
				case SWT.UNDERLINE_ERROR: {
						...
						int squigglyY = Math.min(underlineY - squigglyHeight / 2,
								lineBottom - squigglyHeight - 1);
						int squigglyX = runX;
						for (int i = index; i > 0 && style.isAdherentUnderline(line[i - 1].style);
								i--) {
						...
						if (clipRect is null) {
							...
							Gdip.Graphics_SetClip(cast(Gdip.Graphics) graphics,
									&gdipRect, Gdip.CombineModeIntersect);
						}
						int[] points = computePolyline(squigglyX, squigglyY,
								runX + run.width, squigglyY + squigglyHeight);
						auto pen = Gdip.Pen_new(cast(Gdip.Brush) brush, squigglyThickness);
						Gdip.Graphics_DrawLines(cast(Gdip.Graphics) graphics, pen,
								cast(Gdip.Point*) points.ptr, cast(int) /*64bit*/ points.length / 2);
						Gdip.Pen_delete(pen);
						if (gstate !is 0)
				...
				case SWT.UNDERLINE_SINGLE:
					Gdip.Graphics_FillRectangle(cast(Gdip.Graphics) graphics,
							cast(Gdip.Brush) brush, runX, underlineY,
							run.width, run.underlineThickness);
					break;
				case SWT.UNDERLINE_DOUBLE:
					Gdip.Graphics_FillRectangle(cast(Gdip.Graphics) graphics,
							cast(Gdip.Brush) brush, runX, underlineY,
							run.width, run.underlineThickness);
					Gdip.Graphics_FillRectangle(cast(Gdip.Graphics) graphics, cast(Gdip.Brush) brush,
							runX, underlineY + run.underlineThickness * 2,
							run.width, run.underlineThickness);
					break;
				case UNDERLINE_IME_THICK:
					Gdip.Graphics_FillRectangle(cast(Gdip.Graphics) graphics, cast(Gdip.Brush) brush,
							runX - run.underlineThickness, underlineY,
							run.width, run.underlineThickness * 2);
					break;
				case UNDERLINE_IME_DOT:
				case UNDERLINE_IME_DASH: {
						auto pen = Gdip.Pen_new(cast(Gdip.Brush) brush, 1);
						int dashStyle = style.underlineStyle is UNDERLINE_IME_DOT
							? Gdip.DashStyleDot : Gdip.DashStyleDash;
						Gdip.Pen_SetDashStyle(pen, dashStyle);
						Gdip.Graphics_DrawLine(cast(Gdip.Graphics) graphics,
								pen, runX, underlineY, runX + run.width, underlineY);
						Gdip.Pen_delete(pen);
						break;
					}
				default:
			...
			if (style.strikeout) {
				...
				Gdip.Graphics_FillRectangle(cast(Gdip.Graphics) graphics,
						cast(Gdip.Brush) brush, runX, strikeoutY, run.width,
			...
			if (style.underline) {
				...
				case SWT.UNDERLINE_ERROR: {
						...
						if (clipRect !is null) {
							OS.IntersectClipRect(graphics, clipRect.left,
									clipRect.top, clipRect.right, clipRect.bottom);
						}
						else {
							OS.IntersectClipRect(graphics, runX, squigglyY,
									runX + run.width + 1, squigglyY + squigglyHeight + 1);
						}
						int[] points = computePolyline(squigglyX, squigglyY,
								runX + run.width, squigglyY + squigglyHeight);
						auto pen = OS.CreatePen(OS.PS_SOLID, squigglyThickness, colorRefUnderline);
						auto oldPen = OS.SelectObject(graphics, pen);
						OS.Polyline(graphics, cast(POINT*) points.ptr, cast(int) /*64bit*/ points.length
								/ 2);
						int length_ = cast(int) /*64bit*/ points.length;
						if (length_ >= 2 && squigglyThickness <= 1) {
							OS.SetPixel(graphics, points[length_ - 2],
									points[length_ - 1], colorRefUnderline);
						}
				...
				case SWT.UNDERLINE_SINGLE:
					...
					OS.SetRect(&rect, runX, underlineY, runX + run.width,
							underlineY + run.underlineThickness);
					if (clipRect !is null) {
				...
				case SWT.UNDERLINE_DOUBLE:
					...
					OS.FillRect(graphics, &rect, brushUnderline);
					OS.SetRect(&rect, runX, underlineY + run.underlineThickness * 2,
							runX + run.width, underlineY + run.underlineThickness * 3);
					if (clipRect !is null) {
						rect.left = Math.max(rect.left, clipRect.left);
						rect.right = Math.min(rect.right, clipRect.right);
					}
				...
				case UNDERLINE_IME_THICK:
					brushUnderline = OS.CreateSolidBrush(colorRefUnderline);
					OS.SetRect(&rect, runX, underlineY - run.underlineThickness,
							runX + run.width, underlineY + run.underlineThickness);
					if (clipRect !is null) {
						rect.left = Math.max(rect.left, clipRect.left);
						rect.right = Math.min(rect.right, clipRect.right);
					}
				...
				case UNDERLINE_IME_DOT: {
						...
						int penStyle = style.underlineStyle is UNDERLINE_IME_DASH
							? OS.PS_DASH : OS.PS_DOT;
			...
			if (style.strikeout) {
				...
				OS.SetRect(&rect, runX, strikeoutY, runX + run.width,
						strikeoutY + run.strikeoutThickness);
				if (clipRect !is null) {
					rect.left = Math.max(rect.left, clipRect.left);
					rect.right = Math.min(rect.right, clipRect.right);
				}
			...
			if (brushUnderline !is null)
				OS.DeleteObject(brushUnderline);
			if (brushStrikeout !is null && brushStrikeout !is brushUnderline)
				OS.DeleteObject(brushStrikeout);
		}
	}

	RECT* drawBorder(bool advance, void* graphics, int x, int y, int lineHeight,
			void* color, void* selectionColor, bool fullSelection, RECT* clipRect, RECT* rect, int alpha,
			StyleItem[] line, int index, UTF8index selectionStart, UTF8index selectionEnd) {
		StyleItem run = line[index];
		TextStyle style = run.style;
		if (style is null)
			return null;
		if (style.borderStyle is SWT.NONE)
			return null;
		if (rect !is null) {
			if (clipRect is null) {
				clipRect = new RECT();
		...
		if (index + 1 >= line.length || !style.isAdherentBorder(line[index + 1].style)) {
			int left = run.x;
			for (int i = index; i > 0 && style.isAdherentBorder(line[i - 1].style);
					i--) {
				left = line[i - 1].x;
			}
			if (advance) {
				...
				if (customColor !is -1) {
					int argb = ((alpha & 0xFF) << 24) | ((customColor >> 16) & 0xFF) | (
							customColor & 0xFF00) | ((customColor & 0xFF) << 16);
					auto gdiColor = Gdip.Color_new(argb);
					brush = Gdip.SolidBrush_new(gdiColor);
					Gdip.Color_delete(gdiColor);
				}
				...
				Gdip.Graphics_TranslateTransform(cast(Gdip.Graphics) graphics,
						gdipXOffset, gdipYOffset, Gdip.MatrixOrderPrepend);
				if (style.borderColor is null && clipRect !is null) {
					int gstate = Gdip.Graphics_Save(cast(Gdip.Graphics) graphics);
					if (clipRect.left is -1)
						clipRect.left = 0;
					if (clipRect.right is -1)
					...
					Gdip.Graphics_SetClip(cast(Gdip.Graphics) graphics,
							&gdipRect, Gdip.CombineModeExclude);
					Gdip.Graphics_DrawRectangle(cast(Gdip.Graphics) graphics,
							pen, x + left, y, run.x + run.width - left - 1, lineHeight - 1);
					Gdip.Graphics_Restore(cast(Gdip.Graphics) graphics, gstate);
					gstate = Gdip.Graphics_Save(cast(Gdip.Graphics) graphics);
					Gdip.Graphics_SetClip(cast(Gdip.Graphics) graphics,
							&gdipRect, Gdip.CombineModeIntersect);
					...
					Gdip.Pen_SetDashStyle(pen, lineStyle);
					Gdip.Graphics_DrawRectangle(cast(Gdip.Graphics) graphics,
							selPen, x + left, y, run.x + run.width - left - 1, lineHeight - 1);
					Gdip.Pen_delete(selPen);
					Gdip.Graphics_Restore(cast(Gdip.Graphics) graphics, gstate);
				}
				else {
				...
				auto newPen = OS.ExtCreatePen(lineStyle | OS.PS_GEOMETRIC,
						Math.max(1, lineWidth), &logBrush, 0, null);
				...
				if (style.borderColor is null && clipRect !is null && color !is selectionColor) {
					...
					if (clipRect.right is -1)
						clipRect.right = 0x7ffff;
					OS.IntersectClipRect(graphics, clipRect.left, clipRect.top,
							clipRect.right, clipRect.bottom);
					logBrush.lbColor = cast(uint) selectionColor;
					auto selPen = OS.ExtCreatePen(lineStyle | OS.PS_GEOMETRIC,
							Math.max(1, lineWidth), &logBrush, 0, null);
		...
		for (int i = 0; i < allRuns.length - 1; i++) {
			...
			if (run.UTF8start <= start && start < runEnd) {
				...
				if (run.style !is null && run.style.metrics !is null) {
					...
					cx = metrics.width * cast(int) /*64bit*/ (
							getUTF16index(start) - getUTF16index(run.UTF8start));
				}
				else if (!run.tab) {
					...
					int piX;
					int* advances = run.justify !is null ? run.justify : run.advances;
					OS.ScriptCPtoX(cast(int) /*64bit*/ iCP, false, cChars, run.glyphCount,
							run.clusters, run.visAttrs, advances, &run.analysis, &piX);
			...
			if (run.UTF8start <= end && end < runEnd) {
				...
				if (run.style !is null && run.style.metrics !is null) {
					GlyphMetrics metrics = run.style.metrics;
					cx = metrics.width * cast(int) /*64bit*/ (
							getUTF16index(end) - getUTF16index(run.UTF8start) + 1);
				}
				else if (!run.tab) {
					...
					int piX;
					int* advances = run.justify !is null ? run.justify : run.advances;
					OS.ScriptCPtoX(cast(int) /*64bit*/ iCP, true, cChars, run.glyphCount,
							run.clusters, run.visAttrs, advances, &run.analysis, &piX);
					cx = isRTL ? run.width - piX : piX;
				}
				if (run.analysis.fRTL ^ isRTL) {
...
*/
	...
	HFONT getItemFont(StyleItem item) {
		if (item.fallbackFont !is null)
			return cast(HFONT) item.fallbackFont;
 ...
 */
	public int getLevel(int i_offset) {
		...
		int length = cast(int) /*64bit*/ text.length;
		if (!(0 <= offset.internalValue && offset.internalValue <= length))
 ...
 */
	public Point getLocation(int i_offset, bool trailing) {
		...
		if (!(0 <= offset.internalValue && offset <= length))
			SWT.error(SWT.ERROR_INVALID_RANGE);
		...
		while (high - low > 1) {
			...
			if (run.UTF8start > offset) {
				...
				if (run.style !is null && run.style.metrics !is null) {
					GlyphMetrics metrics = run.style.metrics;
					width = metrics.width * cast(int) /*64bit*/ (getUTF16index(
							offset) - getUTF16index(run.UTF8start) + trailing);
				}
				else if (run.tab) {
					...
					int piX;
					int* advances = run.justify !is null ? run.justify : run.advances;
					OS.ScriptCPtoX(cast(int) /*64bit*/ runOffset, trailing, cChars, gGlyphs,
							run.clusters, run.visAttrs, advances, &run.analysis, &piX);
 ...
 */
	...
	int _getOffset(int i_offset, int movement, bool forward) {
		...
		if (!(0 <= offset.internalValue && offset <= length))
			SWT.error(SWT.ERROR_INVALID_RANGE);
		if (forward && offset is length)
			return cast(int) /*64bit*/ length.internalValue;
		if (!forward && offset.internalValue is 0)
			return 0;
		int step = forward ? 1 : -1;
		if ((movement & SWT.MOVEMENT_CHAR) !is 0)
			return cast(int) /*64bit*/ (offset + text.toUTF8shift(offset, step)).internalValue;
		length = segmentsText.endIndex();
		offset = translateOffset(offset);
		SCRIPT_LOGATTR* logAttr;
		SCRIPT_PROPERTIES* properties;
		...
		do {
			...
			if (run.UTF8start <= offset && offset < run.UTF8start + run.UTF8length) {
				if (run.lineBreak && !run.softBreak)
					return untranslateOffset(run.UTF8start);
				if (run.tab)
					return untranslateOffset(run.UTF8start);
				properties = device.scripts[run.analysis.eScript];
				...
				while (run.UTF8start <= offset && offset < run.UTF8start + run.UTF8length) {
					...
					case SWT.MOVEMENT_WORD: {
							if (properties.fNeedsWordBreaking) {
								...
								if (offset.internalValue > 0) {
									bool letterOrDigit = Compatibility.isLetterOrDigit(
											segmentsText.dcharAt(offset));
									bool previousLetterOrDigit = Compatibility.isLetterOrDigit(
											segmentsText.dcharBefore(offset));
									if (letterOrDigit !is previousLetterOrDigit || !letterOrDigit) {
										if (!Compatibility.isWhitespace(
												segmentsText.dcharAt(offset))) {
											return untranslateOffset(offset);
										}
									}
					...
					case SWT.MOVEMENT_WORD_END: {
							if (offset.internalValue > 0) {
								bool isLetterOrDigit = Compatibility.isLetterOrDigit(
										segmentsText.dcharAt(offset));
								bool previousLetterOrDigit = Compatibility.isLetterOrDigit(
										segmentsText.dcharBefore(offset));
 ...
 */
	public int getOffset(int x, int y, int[] trailing) {
		checkLayout();
		computeRuns(null);
		if (trailing !is null && trailing.length < 1)
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
		...
		while (high - low > 1) {
			...
			if (run.x > x) {
				...
				if (run.style !is null && run.style.metrics !is null) {
					GlyphMetrics metrics = run.style.metrics;
					if (metrics.width > 0) {
						UTF8index res = addUTF16shift(run.UTF8start,
								cast(UTF16shift)(xRun / metrics.width));
						if (trailing !is null) {
							trailing[0] = (xRun % metrics.width < metrics.width / 2) ? 0
								: cast(int) /*64bit*/ segmentsText.UTF8strideAt(res).internalValue;
						}
				...
				if (run.tab) {
					...
					if (trailing !is null)
						trailing[0] = x < (run.x + run.width / 2) ? 0
							: cast(int) /*64bit*/ segmentsText.UTF8strideAt(res).internalValue;
					return untranslateOffset(res);
				}
				UTF16shift cChars = getUTF16length(run); // make it wchar
				...
				if (trailing !is null)
					trailing[0] = cast(int) /*64bit*/ (addUTF16shift(res, piTrailing) - res)
						.internalValue;
				return untranslateOffset(res);
			}
		}
	...
	public int[] getRanges() {
		...
		for (int i = 0; i < stylesCount - 1; i++) {
			if (styles[i].style !is null) {
				...
				result[count++] = cast(int) /*64bit*/ getUTF8index(
						cast(UTF16index)(getUTF16index(styles[i + 1].UTF8start) - 1)).internalValue;
			}
		}
 ...
 */
	...
	void getSegmentsText(out String resUtf8, out String16 resUtf16) {
		...
		}
		{
			...
			while (charCount < length_) {
				if (segmentCount < nSegments && charCount is segments[segmentCount].internalValue) {
					int start = charCount + cast(int) /*64bit*/ (
							segmentCount * MARK_SIZE.internalValue);
					newChars[start .. start + MARK_SIZE.internalValue] = separator;
					segmentCount++;
				}
				else {
					newChars[charCount + (segmentCount * MARK_SIZE.internalValue)] = oldChars[
					charCount];
					charCount++;
				}
			}
			...
			resUtf8 = cast(String) newChars[0 .. Math.min(charCount + (segmentCount * MARK_SIZE.internalValue),
					newChars.length)];
		}
		// now for the wide chars
		{
			...
			resUtf16 = cast(String16) newWChars[0 .. Math.min(charCount + (segmentCount * WMARK_SIZE),
					newWChars.length)];
		}
		buildIndexTables();
	}
 ...
 */
	StyleItem[] itemize() {
		...
		auto hHeap = OS.GetProcessHeap();
		auto pItems = cast(SCRIPT_ITEM*) OS.HeapAlloc(hHeap,
				OS.HEAP_ZERO_MEMORY, MAX_ITEM * SCRIPT_ITEM.sizeof + 1);
		if (pItems is null)
			SWT.error(SWT.ERROR_NO_HANDLES);
		int pcItems;
		String16 wchars = segmentsWText;
		OS.ScriptItemize(wchars.ptr, cast(int) /*64bit*/ wchars.length, MAX_ITEM,
 ...
 */
	public void setAlignment(int alignment) {
		...
		if ((alignment & SWT.RIGHT) !is 0)
			alignment = SWT.RIGHT;
		if (this.alignment is alignment)
			return;
		freeRuns();
 ...
 */
	public void setOrientation(int orientation) {
		...
		if (orientation is 0)
			return;
		if ((orientation & SWT.LEFT_TO_RIGHT) !is 0)
			orientation = SWT.LEFT_TO_RIGHT;
		...
		if (this.segments !is null && i_segments !is null)
			if (this.segments.length is i_segments.length) {
				...
				for (i = 0; i < i_segments.length; i++) {
					if (this.segments[i]!is text.takeIndexArg(i_segments[i],
							"segments@setSegments"))
						break;
				}
				if (i is i_segments.length)
					return;
			}
		freeRuns();
 ...
 */
	public void setSpacing(int spacing) {
		...
		if (spacing < 0)
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
		if (this.lineSpacing is spacing)
			return;
		freeRuns();
 ...
 */
	public void setStyle(TextStyle style, int i_start, int i_end) {
		...
		while (modifyEnd < stylesCount) {
			if (styles[modifyEnd + 1].UTF8start > end)
				break;
			modifyEnd++;
		}
		...
		if (start is styles[modifyStart].UTF8start)
			modifyStart--;
		if (end is text.offsetBefore(styles[modifyEnd + 1].UTF8start))
			modifyEnd++;
 ...
 */
	public void setWidth(int width) {
		checkLayout();
		if (width < -1 || width is 0)
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
		if (this.wrapWidth is width)
			return;
		freeRuns();
		this.wrapWidth = width;
	}

	bool shape(HDC hdc, StyleItem run, String16 wchars, int[] glyphCount,
		...
		if (useCMAPcheck) {
			...
			if (OS.ScriptGetCMap(hdc, run.psc, wchars.ptr,
					cast(int) /*64bit*/ wchars.length, 0, glyphs.ptr) !is OS.S_OK) {
				if (run.psc !is null) {
		...
		auto hr = OS.ScriptShape(hdc, run.psc, wchars.ptr,
				cast(int) /*64bit*/ wchars.length, maxGlyphs, &run.analysis,
				run.glyphs, run.clusters, run.visAttrs, glyphCount.ptr);
		...
		if (hr !is OS.USP_E_SCRIPT_NOT_IN_FONT) {
			...
			for (i = 0; i < glyphs.length; i++) {
				if (glyphs[i] is fp.wgDefault)
					break;
			}
			if (i is glyphs.length)
				return true;
		}
		if (run.psc !is null) {
			OS.ScriptFreeCache(run.psc);
 ...
 */
	void shape(HDC hdc, StyleItem run) {
		int[1] buffer;
		auto wchars = segmentsWText[getUTF16index(
				run.UTF8start) .. getUTF16index(run.UTF8start + run.UTF8length)];
		...
		if (run.glyphs is null)
			SWT.error(SWT.ERROR_NO_HANDLES);
		run.clusters = cast(WORD*) OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY, maxGlyphs * 2);
		if (run.clusters is null)
			SWT.error(SWT.ERROR_NO_HANDLES);
		run.visAttrs = cast(SCRIPT_VISATTR*) OS.HeapAlloc(hHeap,
				OS.HEAP_ZERO_MEMORY, maxGlyphs * SCRIPT_VISATTR_SIZEOF);
		if (run.visAttrs is null)
			SWT.error(SWT.ERROR_NO_HANDLES);
		...
		if (!shapeSucceed) {
			...
			auto ssa = cast(SCRIPT_STRING_ANALYSIS*) OS.HeapAlloc(hHeap,
					OS.HEAP_ZERO_MEMORY, SCRIPT_STRING_ANALYSIS.sizeof);
			...
			if (OS.ScriptStringAnalyse(metaFileDc, wchars.ptr,
					cast(int) /*64bit*/ wchars.length, 0, -1, flags, 0, null,
					null, null, null, null, ssa) is OS.S_OK) {
			...
			static extern (Windows) int metaFileEnumProc(HDC hDC,
					HANDLETABLE* table, ENHMETARECORD* record, int nObj, LPARAM lpData) {
			...
			if (!shapeSucceed) {
				if (mLangFontLink2 !is null) {
					...
					LONG cchCodePages;
					/* GetStrCodePages() */
					OS.VtblCall(4, mLangFontLink2, wchars.ptr,
							cast(int) /*64bit*/ wchars.length, 0, &dwCodePages, &cchCodePages);
					/* MapFont() */
					if (OS.VtblCall(10, mLangFontLink2, hdc, dwCodePages,
							wchars[0], &hNewFont) is OS.S_OK) {
						LOGFONT logFont;
			...
			if (newFont !is run.fallbackFont)
        ...
        */
			OS.ScriptShape(hdc, run.psc, wchars.ptr, cast(int) /*64bit*/ wchars.length,
					maxGlyphs, &run.analysis, run.glyphs, run.clusters, run.visAttrs, buffer.ptr);
			run.glyphCount = buffer[0];
		}
		int[3] abc;
		run.advances = cast(int*) OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY, run.glyphCount * 4);
		if (run.advances is null)
			SWT.error(SWT.ERROR_NO_HANDLES);
		run.goffsets = cast(GOFFSET*) OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY,
				run.glyphCount * GOFFSET_SIZEOF);
		if (run.goffsets is null)
			SWT.error(SWT.ERROR_NO_HANDLES);
		OS.ScriptPlace(hdc, run.psc, run.glyphs, run.glyphCount, run.visAttrs,
				&run.analysis, run.advances, run.goffsets, cast(ABC*) abc.ptr);
	...
	UTF16shift getUTF16length(StyleItem run) {
		return cast(UTF16shift)(getUTF16index(
				run.UTF8start + run.UTF8length) - getUTF16index(run.UTF8start));
	}
 ...
 */
	...
	UTF8index translateOffset(UTF8index offset) {
		...
		if (length is 0)
			return offset;
		if (nSegments is 2) {
			if (segments[0].internalValue is 0 && segments[1].internalValue is length)
				return offset;
		}
		for (int i = 0; i < nSegments && offset.internalValue - i >= segments[i].internalValue;
				i++) {
			offset += MARK_SIZE;
		}
		return offset;
