	...
	public this(Device device, Image srcImage, int flag) {
		...
		case SWT.IMAGE_COPY: {
				...
				case SWT.BITMAP:
					...
					handle = OS.CreateCompatibleBitmap(hdcSource, rect.width,
							bm.bmBits !is null ? -rect.height : rect.height);
					if (handle is null)
				...
				case SWT.ICON:
					static if (OS.IsWinCE) {
						...
						handle = OS.CopyImage(srcImage.handle, OS.IMAGE_ICON,
		...
		case SWT.IMAGE_DISABLE: {
				...
				ImageData newData = new ImageData(rect.width, rect.height, 8,
						new PaletteData(rgbs));
				newData.alpha = data.alpha;
				newData.alphaData = data.alphaData;
				newData.maskData = data.maskData;
				newData.maskPad = data.maskPad;
				if (data.transparentPixel !is -1)
				...
				for (int y = 0; y < rect.height; y++) {
					...
					for (int x = 0; x < rect.width; x++) {
						...
						if (!((data.transparentPixel !is -1 && pixel is data.transparentPixel)
								|| (mask !is null && maskScanline[x] is 0))) {
							int red, green, blue;
							if (palette.isDirect) {
								red = pixel & redMask;
				...
				if (!palette.isDirect) {
					...
					for (int i = 0; i < rgbs.length; i++) {
						if (data.transparentPixel !is i) {
							...
							int blue = color.blue;
							int intensity = (red + red + green + green + green + green + green
									+ blue) >> 3;
					...
					for (int y = 0; y < rect.height; y++) {
						...
						for (int x = 0; x < rect.width; x++) {
							...
							if (pixel !is data.transparentPixel) {
								...
								blue = (blueShift < 0) ? blue >>> -blueShift : blue << blueShift;
								int intensity = (red + red + green + green
										+ green + green + green + blue) >> 3;
								if (newData.transparentPixel is intensity)
									intensity = 255;
 ...
 */
    ...
    */
		if (gdip && (void*).sizeof is 8 && filename.toLowerCase().endsWith(".gif"))
			gdip = false;
		if (gdip) {
					...
					if (filename.toLowerCase().endsWith(".ico")) {
                        ...
                        */
							...
							if (graphics !is null) {
								...
								rect.Width = width;
								rect.Height = height;
								status = Gdip.Graphics_DrawImage(graphics, cast(Gdip.Image) bitmap, rect, 0, 0,
										width, height, Gdip.UnitPixel, null, null, null);
								if (status !is 0) {
							...
							if (lockedBitmapData !is null) {
								Gdip.Bitmap_LockBits(bitmap, null, 0,
								...
								if (depth !is 0) {
									...
									case Gdip.PixelFormat4bppIndexed:
									case Gdip.PixelFormat8bppIndexed:
										int paletteSize = Gdip.Image_GetPaletteSize(
												cast(Gdip.Image) bitmap);
										auto hHeap = OS.GetProcessHeap();
										auto palette = cast(Gdip.ColorPalette*) OS.HeapAlloc(hHeap,
												OS.HEAP_ZERO_MEMORY, paletteSize);
										if (palette is null)
											SWT.error(SWT.ERROR_NO_HANDLES);
										Gdip.Image_GetPalette(cast(Gdip.Image) bitmap,
												palette, paletteSize);
										...
										for (int i = 0; i < colorPalette.Count; i++) {
											...
											if (((*(palette.Entries.ptr + i) >> 24) & 0xFF) is 0
													&& (
														colorPalette.Flags & Gdip
														.PaletteFlagsHasAlpha) !is 0) {
												transparentPixel = i;
											}
											rgbs[i] = new RGB(((*(palette.Entries.ptr + i) & 0xFF0000) >> 16),
													((*(palette.Entries.ptr + i) & 0xFF00) >> 8),
													((*(palette.Entries.ptr + i) & 0xFF) >> 0));
										}
										OS.HeapFree(hHeap, 0, palette);
										break;
									case Gdip.PixelFormat16bppARGB1555:
									...
									case Gdip.PixelFormat16bppARGB1555:
										...
										for (int i = 1, j = 0; i < data.length; i += 2, j++) {
											alphaData[j] = cast(byte)((data[i] & 0x80) !is 0 ? 255
													: 0);
										}
										break;
									case Gdip.PixelFormat32bppARGB:
									...
									ImageData img = new ImageData(width, height,
											depth, paletteData, scanlinePad, data);
 ...
 */
	HBITMAP createDIBFromDDB(HDC hDC, HBITMAP hBitmap, int width, int height) {
		...
		if (!isDirect) {
			...
			OS.GetPaletteEntries(device.hPalette, 0, numColors,
					cast(PALETTEENTRY*) logPalette.ptr);
			rgbs = new RGB[numColors];
			for (int i = 0; i < numColors; i++) {
				rgbs[i] = new RGB(logPalette[i] & 0xFF, logPalette[i + 1] & 0xFF,
						logPalette[i + 2] & 0xFF);
			}
		}
		...
		void* pBits;
		HBITMAP hDib = OS.CreateDIBSection(null, cast(BITMAPINFO*) bmi.ptr,
				...
				if (alpha !is -1 || alphaData !is null || transparentPixel !is -1) {
					...
					if (transparentPixel !is -1) {
						if (bm.bmBitsPixel <= 8) {
							...
							OS.GetDIBColorTable(srcHdc, transparentPixel, 1,
									cast(RGBQUAD*) color.ptr);
							blue = color[0];
							green = color[1];
							red = color[2];
						}
							...
							case 16:
								...
								byte[] blues = ImageData.ANY_TO_EIGHT[ImageData.getChannelWidth(blueMask,
										blueShift)];
								blue = blues[(transparentPixel & blueMask) >> blueShift];
								int greenMask = 0x3E0;
								int greenShift = ImageData.getChannelShift(greenMask);
								byte[] greens = ImageData.ANY_TO_EIGHT[ImageData.getChannelWidth(greenMask,
										greenShift)];
								green = greens[(transparentPixel & greenMask) >> greenShift];
								int redMask = 0x7C00;
								int redShift = ImageData.getChannelShift(redMask);
								byte[] reds = ImageData.ANY_TO_EIGHT[ImageData.getChannelWidth(redMask,
										redShift)];
					...
					if (alpha !is -1) {
						...
						for (int y = 0, dp = 0; y < imgHeight; ++y) {
							for (int x = 0; x < imgWidth; ++x) {
								if (srcData[dp] is blue
										&& srcData[dp + 1] is green && srcData[dp + 2] is red) {
					...
					auto hHeap = OS.GetProcessHeap();
					auto pixels = cast(ubyte*) OS.HeapAlloc(hHeap,
							OS.HEAP_ZERO_MEMORY, srcData.length);
					if (pixels is null)
						SWT.error(SWT.ERROR_NO_HANDLES);
					OS.MoveMemory(pixels, srcData.ptr, sizeInBytes);
					return [cast(ptrdiff_t) Gdip.Bitmap_new(imgWidth, imgHeight, dibBM.bmWidthBytes,
							Gdip.PixelFormat32bppARGB, pixels), cast(ptrdiff_t) pixels];
				}
		...
		case SWT.ICON: {
            ...
            */
				if (imgWidth > imgHeight || bm.bmBitsPixel is 32) {
					...
					BITMAP dibBM;
					OS.GetObject(memDib, BITMAP.sizeof, &dibBM);
					OS.BitBlt(memHdc, 0, 0, imgWidth, imgHeight, srcHdc, 0,
							hBitmap is iconInfo.hbmMask ? imgHeight : 0, OS.SRCCOPY);
					OS.SelectObject(memHdc, oldMemBitmap);
					OS.DeleteObject(memHdc);
					...
					img = Gdip.Bitmap_new(imgWidth, imgHeight,
							dibBM.bmWidthBytes, Gdip.PixelFormat32bppARGB, pixels);
				}
				else {
	...
	public Color getBackground() {
		...
		if (bm.bmBitsPixel <= 8) {
			static if (OS.IsWinCE) {
				...
				pBits[0] = cast(byte)((transparentPixel << (8 - bm.bmBitsPixel)) | (
						pBits[0] & ~mask));
 ...
 */
	public ImageData getImageData() {
		...
		case SWT.ICON: {
				...
				static if (OS.IsWinCE)
					SWT.error(SWT.ERROR_NOT_IMPLEMENTED);
				OS.GetDIBits(hBitmapDC, hBitmap, 0, height, null,
				...
				static if (OS.IsWinCE)
					SWT.error(SWT.ERROR_NOT_IMPLEMENTED);
				OS.GetDIBits(hBitmapDC, hBitmap, 0, height, lpvBits,
						cast(BITMAPINFO*) bmi.ptr, OS.DIB_RGB_COLORS);
				...
				if (depth <= 8) {
					...
					int srcIndex = 40;
					for (int i = 0; i < numColors; i++) {
						rgbs[i] = new RGB(bmi[srcIndex + 2] & 0xFF,
								bmi[srcIndex + 1] & 0xFF, bmi[srcIndex] & 0xFF);
						srcIndex += 4;
					}
				...
				if (info.hbmColor is null) {
					...
					static if (OS.IsWinCE)
						SWT.error(SWT.ERROR_NOT_IMPLEMENTED);
					OS.GetDIBits(hBitmapDC, hBitmap, height, height, lpvBits,
							cast(BITMAPINFO*) bmi.ptr, OS.DIB_RGB_COLORS);
					...
					static if (OS.IsWinCE)
						SWT.error(SWT.ERROR_NOT_IMPLEMENTED);
					OS.GetDIBits(hBitmapDC, info.hbmMask, 0, height, null,
							cast(BITMAPINFO*) bmi.ptr, OS.DIB_RGB_COLORS);
					OS.MoveMemory(&bmiHeader, bmi.ptr, BITMAPINFOHEADER.sizeof);
					imageSize = bmiHeader.biSizeImage;
					maskData = new byte[imageSize];
					auto lpvMaskBits = OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY, imageSize);
					if (lpvMaskBits is null)
						SWT.error(SWT.ERROR_NO_HANDLES);
					static if (OS.IsWinCE)
						SWT.error(SWT.ERROR_NOT_IMPLEMENTED);
					OS.GetDIBits(hBitmapDC, info.hbmMask, 0, height, lpvMaskBits,
							cast(BITMAPINFO*) bmi.ptr, OS.DIB_RGB_COLORS);
		...
		case SWT.BITMAP: {
            ...
            */
				...
				if (isDib) {
					...
					static if (OS.IsWinCE)
						SWT.error(SWT.ERROR_NOT_IMPLEMENTED);
					OS.GetDIBits(hBitmapDC, handle, 0, height, null,
							cast(BITMAPINFO*) bmi.ptr, OS.DIB_RGB_COLORS);
					OS.MoveMemory(&bmiHeader, bmi.ptr, BITMAPINFOHEADER.sizeof);
					imageSize = bmiHeader.biSizeImage;
				}
				...
				if (isDib) {
					...
					static if (OS.IsWinCE)
						SWT.error(SWT.ERROR_NOT_IMPLEMENTED);
					OS.GetDIBits(hBitmapDC, handle, 0, height, lpvBits,
							cast(BITMAPINFO*) bmi.ptr, OS.DIB_RGB_COLORS);
					OS.MoveMemory(data.ptr, lpvBits, imageSize);
					OS.HeapFree(hHeap, 0, lpvBits);
				}
				...
				if (depth <= 8) {
					...
					if (isDib) {
                        ...
                        */
							...
							int mask = (0xFF << (8 - bm.bmBitsPixel)) & 0x00FF;
							for (int i = 0; i < numColors; i++) {
								pBits[0] = cast(byte)((i << (8 - bm.bmBitsPixel)) | (
										pBits[0] & ~mask));
								OS.MoveMemory(bm.bmBits, pBits.ptr, 1);
							...
							OS.GetDIBColorTable(hBitmapDC, 0, numColors,
									cast(RGBQUAD*) colors.ptr);
							int colorIndex = 0;
							for (int i = 0; i < rgbs.length; i++) {
								rgbs[i] = new RGB(colors[colorIndex + 2] & 0xFF,
										colors[colorIndex + 1] & 0xFF, colors[colorIndex] & 0xFF);
								colorIndex += 4;
							}
						}
					}
 ...
 */
    ...
    */
	...
	static HGDIOBJ createDIB(int width, int height, int depth) {
		...
		void* pBits;
		return OS.CreateDIBSection(null, cast(BITMAPINFO*) bmi.ptr,
				OS.DIB_RGB_COLORS, &pBits, null, 0);
	}
 ...
 */
     ...
     */
		...
		if ((OS.IsWin95 && i.depth is 1
				&& i.getTransparencyType() !is SWT.TRANSPARENCY_MASK) || i.depth is 2) {
			ImageData img = new ImageData(i.width, i.height, 4, i.palette);
			ImageData.blit(ImageData.BLIT_SRC, i.data, i.depth, i.bytesPerLine,
					i.getByteOrder(), 0, 0, i.width, i.height, null, null,
					null, ImageData.ALPHA_OPAQUE, null, 0, 0, 0, img.data,
					img.depth, img.bytesPerLine, i.getByteOrder(), 0, 0,
					img.width, img.height, null, null, null, false, false);
    ...
    */
		if (i.palette.isDirect) {
			...
			if (newPalette !is null) {
				...
				ImageData.blit(ImageData.BLIT_SRC, i.data, i.depth,
						i.bytesPerLine, i.getByteOrder(), 0, 0, i.width, i.height,
						redMask, greenMask, blueMask, ImageData.ALPHA_OPAQUE, null, 0, 0,
						0, img.data, img.depth, img.bytesPerLine, newOrder, 0, 0,
						img.width, img.height, newPalette.redMask,
		...
		void* pBits;
		auto hDib = OS.CreateDIBSection(null, cast(BITMAPINFO*) bmi.ptr,
				OS.DIB_RGB_COLORS, &pBits, null, 0);
		if (hDib is null)
			SWT.error(SWT.ERROR_NO_HANDLES);
