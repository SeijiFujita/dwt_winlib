 ...
 */
	...
	void checkGDIP() {
		...
		static if (!OS.IsWinCE)
			oldErrorMode = OS.SetErrorMode(OS.SEM_FAILCRITICALERRORS);
		try {
			ULONG_PTR token;
 ...
 */
	...
	static extern (Windows) int EnumFontFamFunc(ENUMLOGFONT* lpelfe,
			NEWTEXTMETRIC* lpntme, int FontType, LPARAM lParam) {
 ...
 */
	public FontData[] getFontList(String faceName, bool scalable) {
         ...
         */
             ...
             */
				...
				static if (OS.IsUnicode) {
					OS.EnumFontFamiliesW(hDC, (cast(LOGFONTW*) lf)
							.lfFaceName.ptr, &EnumFontFamFunc, cast(LPARAM)&cbdata);
				}
				else {
					OS.EnumFontFamiliesA(hDC, (cast(LOGFONTA*) lf)
							.lfFaceName.ptr, &EnumFontFamFunc, cast(LPARAM)&cbdata);
				}
			}
		}
         ...
         */
			cbdata.scalable = scalable;
			OS.EnumFontFamilies(hDC, .StrToTCHARz(faceName), &EnumFontFamFunc,
					cast(LPARAM)&cbdata);
		}
		int logPixelsY = OS.GetDeviceCaps(hDC, OS.LOGPIXELSY);
		internal_dispose_GC(hDC, null);
	...
	String getLastErrorText() {
		...
		if (error is 0)
			return ""; //$NON-NLS-1$
		TCHAR* buffer = null;
		int dwFlags = OS.FORMAT_MESSAGE_ALLOCATE_BUFFER
			| OS.FORMAT_MESSAGE_FROM_SYSTEM | OS.FORMAT_MESSAGE_IGNORE_INSERTS;
		int length = OS.FormatMessage(dwFlags, null, error,
				OS.LANG_USER_DEFAULT, cast(TCHAR*)&buffer, 0, null);
 ...
 */
     ...
     */
        ...
        */
			if (numReserved is 0 && numEntries >= 20)
				numReserved = 20;
		}
 ...
 */
	...
	void printErrors() {
		...
		if (tracking) {
			synchronized (trackingLock) {
				if (objects is null || errors is null)
					return;
				int objectCount = 0;
				...
				for (int i = 0; i < objects.length; i++) {
					...
					if (object !is null) {
						objectCount++;
						if (null !is cast(Color) object)
							colors++;
						if (null !is cast(Cursor) object)
							cursors++;
						if (null !is cast(Font) object)
							fonts++;
						if (null !is cast(GC) object)
							gcs++;
						if (null !is cast(Image) object)
							images++;
						if (null !is cast(Path) object)
							paths++;
						if (null !is cast(Pattern) object)
							patterns++;
						if (null !is cast(Region) object)
							regions++;
						if (null !is cast(TextLayout) object)
							textLayouts++;
						if (null !is cast(Transform) object)
							transforms++;
					}
				}
				if (objectCount !is 0) {
					...
					if (colors !is 0)
						string ~= String_valueOf(colors) ~ " Color(s), ";
					if (cursors !is 0)
						string ~= String_valueOf(cursors) ~ " Cursor(s), ";
					if (fonts !is 0)
						string ~= String_valueOf(fonts) ~ " Font(s), ";
					if (gcs !is 0)
						string ~= String_valueOf(gcs) ~ " GC(s), ";
					if (images !is 0)
						string ~= String_valueOf(images) ~ " Image(s), ";
					if (paths !is 0)
						string ~= String_valueOf(paths) ~ " Path(s), ";
					if (patterns !is 0)
						string ~= String_valueOf(patterns) ~ " Pattern(s), ";
					if (regions !is 0)
						string ~= String_valueOf(regions) ~ " Region(s), ";
					if (textLayouts !is 0)
						string ~= String_valueOf(textLayouts) ~ " TextLayout(s), ";
					if (transforms !is 0)
						string ~= String_valueOf(transforms) ~ " Transforms(s), ";
					...
					for (int i = 0; i < errors.length; i++) {
						if (errors[i]!is null)
							ExceptionPrintStackTrace(errors[i]);
					}
				}
			}
		}
