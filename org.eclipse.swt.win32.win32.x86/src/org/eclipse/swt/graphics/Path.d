 ...
 */
	public this(Device device, Path path, float flatness) {
		...
		if (flatness !is 0)
			Gdip.GraphicsPath_Flatten(handle, null, flatness);
		if (handle is null)
			SWT.error(SWT.ERROR_NO_HANDLES);
		init_();
	}
 ...
 */
	public bool contains(float x, float y, GC gc, bool outline) {
		...
		int mode = OS.GetPolyFillMode(gc.handle) is OS.WINDING
			? Gdip.FillModeWinding : Gdip.FillModeAlternate;
		Gdip.GraphicsPath_SetFillMode(handle, mode);
		if (outline) {
			return Gdip.GraphicsPath_IsOutlineVisible(handle, x, y,
					gc.data.gdipPen, gc.data.gdipGraphics);
		}
		else {
 ...
 */
	public void cubicTo(float cx1, float cy1, float cx2, float cy2, float x, float y) {
		if (isDisposed())
			SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
		Gdip.GraphicsPath_AddBezier(handle, currentPoint.X, currentPoint.Y,
				cx1, cy1, cx2, cy2, x, y);
		Gdip.GraphicsPath_GetLastPoint(handle, currentPoint);
	}
	...
	void init_(PathData data) {
		...
		for (int i = 0, j = 0; i < types.length; i++) {
			...
			case SWT.PATH_CUBIC_TO:
				cubicTo(points[j++], points[j++],
						points[j++], points[j++], points[j++], points[j++]);
				break;
