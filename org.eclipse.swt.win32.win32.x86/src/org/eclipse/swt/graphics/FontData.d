 ...
 */
	public this(String string) {
		...
		try {
			if (Integer.parseInt(version1) !is 1)
				SWT.error(SWT.ERROR_INVALID_ARGUMENT);
		}
 ...
 */
	override public equals_t opEquals(Object object) {
		...
		if (auto fd = cast(FontData) object) {
            ...
            */
				...
				height is fd.height
				&& data.lfWidth is lf.lfWidth && data.lfEscapement is lf.lfEscapement
				&& data.lfOrientation is lf.lfOrientation && data.lfWeight is lf.lfWeight
				&& data.lfItalic is lf.lfItalic && data.lfUnderline is lf.lfUnderline
				&& data.lfStrikeOut is lf.lfStrikeOut && data.lfCharSet is lf.lfCharSet
				&& data.lfOutPrecision is lf.lfOutPrecision && data.lfClipPrecision is lf.lfClipPrecision
				&& data.lfQuality is lf.lfQuality
				&& data.lfPitchAndFamily is lf.lfPitchAndFamily && getName() == /*eq*/ fd.getName();
		}
		return false;
	}
	...
	extern (Windows) static int EnumLocalesProc(TCHAR* lpLocaleString) {
		...
		if (size <= 0 || s_this.lang != /*eq*/ .TCHARzToStr(buffer.ptr).substring(0, size - 1))
			return 1;

		/* Check the country */
		if (s_this.country !is null) {
			size = OS.GetLocaleInfo(lcid, OS.LOCALE_SISO3166CTRYNAME, buffer.ptr, length_);
			if (size <= 0 || s_this.country != /*eq*/ .TCHARzToStr(buffer.ptr)
					.substring(0, size - 1))
				return 1;
		}
 ...
 */
	public int getStyle() {
		...
		if (data.lfWeight is 700)
			style |= SWT.BOLD;
		if (data.lfItalic !is 0)
			style |= SWT.ITALIC;
		return style;
	}
 ...
 */
	override public hash_t toHash() {
		hash_t nameHash = 0;
		for (size_t i = 0; '\0' != data.lfFaceName[i] && i < data.lfFaceName.length;
				i++) {

			nameHash = nameHash * 31 + data.lfFaceName[i];
		}
		int height = cast(int)(0.5f + height);
		return data.lfCharSet ^ height ^ data.lfWidth ^ data.lfEscapement ^ data.lfOrientation
			^ data.lfWeight ^ data.lfItalic ^ data.lfUnderline ^ data.lfStrikeOut
			^ data.lfCharSet ^ data.lfOutPrecision ^ data.lfClipPrecision
			^ data.lfQuality ^ data.lfPitchAndFamily ^ nameHash;
	}
