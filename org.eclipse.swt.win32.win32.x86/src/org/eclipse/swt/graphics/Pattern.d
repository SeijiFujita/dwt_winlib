 ...
 */
	public this(Device device, Image image) {
		...
		handle = cast(Gdip.Brush) Gdip.TextureBrush_new(img, Gdip.WrapModeTile,
				0, 0, width, height);
 ...
 * @since 3.2
 */
	public this(Device device, float x1, float y1, float x2, float y2, Color color1,
		...
		if (x1 is x2 && y1 is y2) {
			...
			if (alpha1 != 0xFF || alpha2 !is 0xFF) {
				...
				int r = cast(int)(((colorRef1 & 0xFF) >> 0) * 0.5f + ((colorRef2 & 0xFF) >> 0)
						* 0.5f);
				int g = cast(int)(((colorRef1 & 0xFF00) >> 8) * 0.5f + (
						(colorRef2 & 0xFF00) >> 8) * 0.5f);
				int b = cast(int)(((colorRef1 & 0xFF0000) >> 16) * 0.5f + (
				...
				f[1] = 0.5f;
				f[2] = 1;
				Gdip.LinearGradientBrush_SetInterpolationColors(cast(Gdip.LinearGradientBrush) handle,
						c.ptr, f.ptr, 3);
