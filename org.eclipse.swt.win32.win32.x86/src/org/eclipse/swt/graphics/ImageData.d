		...
		synchronized {
			...
			for (int b = 0; b < 9; ++b) {
				...
				if (b is 0)
					continue;
				int inc = 0;
				for (int bit = 0x10000; (bit >>= b) !is 0;)
					inc |= bit;
				for (int v = 0, p = 0; v < 0x10000; v += inc)
					data[p++] = cast(byte)(v >> 8);
			}
 ...
 */
	public this(int width, int height, int depth, PaletteData palette) {
		this(width, height, depth, palette, 4, null, 0, null, null, -1, -1,
 ...
 */
	public this(int width, int height, int depth, PaletteData palette, int scanlinePad, byte[] data) {
		this(width, height, depth, palette, scanlinePad, checkData(data), 0,
				null, null, -1, -1, SWT.IMAGE_UNDEFINED, 0, 0, 0, 0);
	}
 ...
 */
	public this(InputStream stream) {
		...
		ImageData i = data[0];
		setAllFields(i.width, i.height, i.depth, i.scanlinePad, i.bytesPerLine,
				i.data, i.palette, i.transparentPixel, i.maskData, i.maskPad,
				i.alphaData, i.alpha, i.type, i.x, i.y, i.disposalMethod, i.delayTime);
	}
 ...
 * </p>
 */
	this(int width, int height, int depth, PaletteData palette, int scanlinePad,
			byte[] data, int maskPad, byte[] maskData, byte[] alphaData, int alpha,
			int transparentPixel, int type, int x, int y, int disposalMethod, int delayTime) {
		if (palette is null)
		...
		int bytesPerLine = (((width * depth + 7) / 8) + (scanlinePad - 1)) / scanlinePad
			* scanlinePad;
     ...
     */
		int minBytesPerLine = type is SWT.IMAGE_PNG ? ((((width + 7) / 8) + 3) / 4) * 4
			: bytesPerLine;
		if (data !is null && data.length < minBytesPerLine * height) {
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
		}
		setAllFields(width, height, depth, scanlinePad, bytesPerLine, data !is null ? data
				: new byte[bytesPerLine * height], palette, transparentPixel, maskData,
				maskPad, alphaData, alpha, type, x, y, disposalMethod, delayTime);
	}
 ...
 * </p>
 */
	void setAllFields(int width, int height, int depth, int scanlinePad, int bytesPerLine,
			byte[] data, PaletteData palette, int transparentPixel, byte[] maskData,
			int maskPad, byte[] alphaData, int alpha, int type, int x, int y,
 ...
 * </p>
 */
	public static ImageData internal_new(int width, int height, int depth,
			PaletteData palette, int scanlinePad, byte[] data, int maskPad, byte[] maskData,
			byte[] alphaData, int alpha, int transparentPixel, int type, int x,
			int y, int disposalMethod, int delayTime) {
		return new ImageData(width, height, depth, palette, scanlinePad, data, maskPad, maskData,
				alphaData, alpha, transparentPixel, type, x, y, disposalMethod, delayTime);
	}

	ImageData colorMaskImage(int pixel) {
		ImageData mask = new ImageData(width, height, 1, bwPalette(), 2, null,
				0, null, null, -1, -1, SWT.IMAGE_UNDEFINED, 0, 0, 0, 0);
 ...
 */
	public ImageData scaledTo(int width, int height) {
		...
		ImageData dest = new ImageData(width, height, depth, palette, scanlinePad,
				null, 0, null, null, -1, transparentPixel, type, x, y, disposalMethod, delayTime);

		/* Scale the image contents */
		if (palette.isDirect)
			blit(BLIT_SRC, this.data, this.depth, this.bytesPerLine,
					this.getByteOrder(), 0, 0, this.width, this.height, 0, 0,
					0, ALPHA_OPAQUE, null, 0, 0, 0, dest.data, dest.depth,
					dest.bytesPerLine, dest.getByteOrder(), 0, 0, dest.width,
					dest.height, 0, 0, 0, flipX, flipY);
		else
			blit(BLIT_SRC, this.data, this.depth, this.bytesPerLine,
					this.getByteOrder(), 0, 0, this.width, this.height, null,
					null, null, ALPHA_OPAQUE, null, 0, 0, 0, dest.data,
					dest.depth, dest.bytesPerLine, dest.getByteOrder(), 0, 0,
					dest.width, dest.height, null, null, null, flipX, flipY);
		...
		if (maskData !is null) {
			...
			blit(BLIT_SRC, this.maskData, 1, srcBpl, MSB_FIRST, 0, 0,
					this.width, this.height, null, null, null, ALPHA_OPAQUE,
					null, 0, 0, 0, dest.maskData, 1, destBpl, MSB_FIRST, 0, 0,
					dest.width, dest.height, null, null, null, flipX, flipY);
		}
		else if (alpha !is -1) {
			...
			blit(BLIT_SRC, this.alphaData, 8, this.width, MSB_FIRST, 0, 0,
					this.width, this.height, null, null, null, ALPHA_OPAQUE,
					null, 0, 0, 0, dest.alphaData, 8, dest.width, MSB_FIRST, 0,
					0, dest.width, dest.height, null, null, null, flipX, flipY);
		}
		return dest;
	}
 ...
 */
	static int closestMatch(int depth, byte red, byte green, byte blue, int redMask,
			int greenMask, int blueMask, byte[] reds, byte[] greens, byte[] blues) {
		if (depth > 8) {
			...
			return (((red << 24) >>> rshift) & redMask) | (
					((green << 24) >>> gshift) & greenMask) | (((blue << 24) >>> bshift) & blueMask);
		}
		int r, g, b;
	...
	static final ImageData convertMask(ImageData mask) {
		...
		if (rgbs !is null) {
			while (blackIndex < rgbs.length) {
				if (rgbs[blackIndex] is palette.colors[0])
					break;
				blackIndex++;
			}
		}
	...
	static final byte[] convertPad(byte[] data, int width, int height, int depth, int pad,
			int newPad) {
		if (pad is newPad)
			return data;
 ...
 */
	static const int BLIT_SRC = 1, // copy source directly, else applies logic operations
 ...
 */
	static const int ALPHA_OPAQUE = 255, // Fully opaque (ignores any alpha data)
 ...
 * Data types (internal)
 */
	private static const int// direct / true color formats with arbitrary masks & shifts
	TYPE_GENERIC_8 = 0, TYPE_GENERIC_16_MSB = 1, TYPE_GENERIC_16_LSB = 2, TYPE_GENERIC_24 = 3,
		TYPE_GENERIC_32_MSB = 4, TYPE_GENERIC_32_LSB = 5,// palette indexed color formats
		TYPE_INDEX_8 = 6,
		TYPE_INDEX_4 = 7, TYPE_INDEX_2 = 8, TYPE_INDEX_1_MSB = 9, TYPE_INDEX_1_LSB = 10;
 ...
 */
	static void blit(int op, byte[] srcData, int srcDepth, int srcStride,
			int srcOrder, int srcX, int srcY, int srcWidth, int srcHeight, int srcRedMask,
			int srcGreenMask, int srcBlueMask, int alphaMode, byte[] alphaData, int alphaStride,
			int alphaX, int alphaY, byte[] destData, int destDepth, int destStride,
			int destOrder, int destX, int destY, int destWidth, int destHeight,
			int destRedMask, int destGreenMask, int destBlueMask, bool flipX, bool flipY) {

		static_this();

		if ((destWidth <= 0) || (destHeight <= 0) || (alphaMode is ALPHA_TRANSPARENT))
			return;
		...
		int dpr = ((flipY) ? destY + dhm1 : destY) * destStride + ((flipX)
				? destX + dwm1 : destX) * dbpp;
		...
		int dp = dpr;
		int sp = spr;
		if ((alphaMode is 0x10000) && (stype is dtype) && (srcRedMask is destRedMask)
				&& (srcGreenMask is destGreenMask)
			...
			case 1:
				for (int dy = destHeight, sfy = sfyi; dy > 0; --dy,
						...
						sfy = (sfy & 0xffff) + sfyi, dp = dpr += dpryi) {
					for (int dx = destWidth, sfx = sfxi; dx > 0; --dx,
							dp += dprxi, sfx = (sfx & 0xffff) + sfxi) {
		...
		for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcStride,
				ap = apr += (sfy >>> 16) * alphaStride, sfy = (sfy & 0xffff) + sfyi,
				dp = dpr += dpryi) {
			for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi,
					sfx = (sfx & 0xffff) + sfxi) {
				/*** READ NEXT PIXEL ***/
				switch (stype) {
				case TYPE_GENERIC_8: {
				...
				case TYPE_GENERIC_24: {
						int data = ((((srcData[sp] & 0xff) << 8) | (srcData[sp + 1] & 0xff)) << 8) | (
								srcData[sp + 2] & 0xff);
						sp += (sfx >>> 16) * 3;
						r = srcReds[(data & srcRedMask) >>> srcRedShift] & 0xff;
						g = srcGreens[(data & srcGreenMask) >>> srcGreenShift] & 0xff;
				...
				case TYPE_GENERIC_32_MSB: {
						int data = ((((((srcData[sp] & 0xff) << 8) | (srcData[sp + 1] & 0xff)) << 8) | (
								srcData[sp + 2] & 0xff)) << 8) | (srcData[sp + 3] & 0xff);
						sp += (sfx >>> 16) * 4;
						r = srcReds[(data & srcRedMask) >>> srcRedShift] & 0xff;
						g = srcGreens[(data & srcGreenMask) >>> srcGreenShift] & 0xff;
						b = srcBlues[(data & srcBlueMask) >>> srcBlueShift] & 0xff;
				...
				case TYPE_GENERIC_32_LSB: {
						int data = ((((((srcData[sp + 3] & 0xff) << 8) | (srcData[sp + 2] & 0xff)) << 8) | (
								srcData[sp + 1] & 0xff)) << 8) | (srcData[sp] & 0xff);
						sp += (sfx >>> 16) * 4;
						r = srcReds[(data & srcRedMask) >>> srcRedShift] & 0xff;
						g = srcGreens[(data & srcGreenMask) >>> srcGreenShift] & 0xff;
						b = srcBlues[(data & srcBlueMask) >>> srcBlueShift] & 0xff;
				...
				if (alpha !is 0x10000) {
					...
					case TYPE_GENERIC_16_LSB: {
						...
						break;
					case TYPE_GENERIC_24: {
							int data = ((((destData[dp] & 0xff) << 8) | (destData[dp + 1] & 0xff)) << 8) | (
									destData[dp + 2] & 0xff);
							rq = destReds[(data & destRedMask) >>> destRedShift] & 0xff;
							gq = destGreens[(data & destGreenMask) >>> destGreenShift] & 0xff;
							bq = destBlues[(data & destBlueMask) >>> destBlueShift] & 0xff;
					...
					case TYPE_GENERIC_32_MSB: {
							int data = (
									(((((destData[dp] & 0xff) << 8) | (destData[dp + 1] & 0xff)) << 8) | (
									destData[dp + 2] & 0xff)) << 8) | (destData[dp + 3] & 0xff);
							rq = destReds[(data & destRedMask) >>> destRedShift] & 0xff;
							gq = destGreens[(data & destGreenMask) >>> destGreenShift] & 0xff;
							bq = destBlues[(data & destBlueMask) >>> destBlueShift] & 0xff;
							aq = destAlphas[(data & destAlphaMask) >>> destAlphaShift] & 0xff;
					...
					case TYPE_GENERIC_32_LSB: {
							int data = (
									(((((destData[dp + 3] & 0xff) << 8) | (destData[dp + 2] & 0xff)) << 8) | (
									destData[dp + 1] & 0xff)) << 8) | (destData[dp] & 0xff);
							rq = destReds[(data & destRedMask) >>> destRedShift] & 0xff;
							gq = destGreens[(data & destGreenMask) >>> destGreenShift] & 0xff;
							bq = destBlues[(data & destBlueMask) >>> destBlueShift] & 0xff;
							aq = destAlphas[(data & destAlphaMask) >>> destAlphaShift] & 0xff;
				...
				/*** WRITE NEXT PIXEL ***/
				int data = (r >>> destRedPreShift << destRedShift) | (g >>> destGreenPreShift << destGreenShift) | (
 ...
 */
	static void blit(int op, byte[] srcData, int srcDepth, int srcStride,
			int srcOrder, int srcX, int srcY, int srcWidth, int srcHeight,
			byte[] srcReds, byte[] srcGreens, byte[] srcBlues, int alphaMode,
			byte[] alphaData, int alphaStride, int alphaX, int alphaY, byte[] destData,
			int destDepth, int destStride, int destOrder, int destX, int destY,
			int destWidth, int destHeight, byte[] destReds, byte[] destGreens,
			byte[] destBlues, bool flipX, bool flipY) {
		...
		case 0x10000:
			...
			if ((stype is dtype) && (srcReds is destReds)
					&& (srcGreens is destGreens) && (srcBlues is destBlues)) {
				...
				if (srcDepth <= destDepth) {
					...
					for (int i = 0; i < paletteMapping.length; ++i)
						paletteMapping[i] = cast(byte)(i & mask);
				}
				break;
			}
			goto case;
				...
				case TYPE_INDEX_4:
					for (int dy = destHeight, sfy = sfyi; dy > 0; --dy,
						...
						for (int dx = destWidth, sfx = sfxi; dx > 0; --dx,
							...
							sp += (sfx >>> 16);
							if ((dp & 1) !is 0)
								destData[dp >> 1] = cast(byte)((destData[dp >> 1] & 0xf0) | v);
							else
								destData[dp >> 1] = cast(byte)((destData[dp >> 1] & 0x0f) | (v << 4));
						}
					}
					break;
				case TYPE_INDEX_2:
					for (int dy = destHeight, sfy = sfyi; dy > 0; --dy,
						...
						for (int dx = destWidth, sfx = sfxi; dx > 0; --dx,
							...
							sp += (sfx >>> 16);
							int shift = 6 - (dp & 3) * 2;
							destData[dp >> 2] = cast(byte)(
									destData[dp >> 2] & ~(0x03 << shift) | (index << shift));
						}
					}
					break;
				case TYPE_INDEX_1_MSB:
					for (int dy = destHeight, sfy = sfyi; dy > 0; --dy,
						...
						for (int dx = destWidth, sfx = sfxi; dx > 0; --dx,
							...
							sp += (sfx >>> 16);
							int shift = 7 - (dp & 7);
							destData[dp >> 3] = cast(byte)(
									destData[dp >> 3] & ~(0x01 << shift) | (index << shift));
						}
					}
					break;
				...
				for (int dy = destHeight, sfy = sfyi; dy > 0; --dy,
					...
					for (int dx = destWidth, sfx = sfxi; dx > 0; --dx,
						...
						case ALPHA_MASK_RGB: {
								...
								int i = 0;
								while (i < alphaData.length) {
									if ((r is alphaData[i]) && (g is alphaData[i + 1])
											&& (b is alphaData[i + 2]))
										break;
									i += 3;
								}
						...
						case TYPE_INDEX_4:
							if ((dp & 1) !is 0)
								destData[dp >> 1] = cast(byte)((destData[dp >> 1] & 0xf0) | index);
							else
								destData[dp >> 1] = cast(byte)(
										(destData[dp >> 1] & 0x0f) | (index << 4));
							break;
						case TYPE_INDEX_2: {
								int shift = 6 - (dp & 3) * 2;
								destData[dp >> 2] = cast(byte)(
		...
		for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcStride,
			...
			for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi,
				...
				/*** DO SPECIAL PROCESSING IF REQUIRED ***/
				int r = srcReds[index] & 0xff, g = srcGreens[index] & 0xff,
				...
				if (r !is lastr || g !is lastg || b !is lastb) {
					...
					for (int j = 0, dr, dg, db, distance, minDistance = 0x7fffffff;
							j < destPaletteSize; ++j) {
						dr = (destReds[j] & 0xff) - r;
						dg = (destGreens[j] & 0xff) - g;
						db = (destBlues[j] & 0xff) - b;
				...
				case TYPE_INDEX_4:
					if ((dp & 1) !is 0)
						destData[dp >> 1] = cast(byte)((destData[dp >> 1] & 0xf0) | lastindex);
					else
						destData[dp >> 1] = cast(byte)((destData[dp >> 1] & 0x0f) | (lastindex << 4));
					break;
				case TYPE_INDEX_2: {
						int shift = 6 - (dp & 3) * 2;
						destData[dp >> 2] = cast(byte)(
								destData[dp >> 2] & ~(0x03 << shift) | (lastindex << shift));
					}
					break;
				case TYPE_INDEX_1_MSB: {
						int shift = 7 - (dp & 7);
						destData[dp >> 3] = cast(byte)(
								destData[dp >> 3] & ~(0x01 << shift) | (lastindex << shift));
					}
					break;
				case TYPE_INDEX_1_LSB: {
						int shift = dp & 7;
 ...
 */
	static void blit(int op, byte[] srcData, int srcDepth, int srcStride,
			...
			byte[] srcReds, byte[] srcGreens, byte[] srcBlues, int alphaMode,
			byte[] alphaData, int alphaStride, int alphaX, int alphaY, byte[] destData,
			int destDepth, int destStride, int destOrder, int destX, int destY, int destWidth, int destHeight,
			int destRedMask, int destGreenMask, int destBlueMask, bool flipX, bool flipY) {
 ...
 */
	static void blit(int op, byte[] srcData, int srcDepth, int srcStride,
			...
			int alphaX, int alphaY, byte[] destData, int destDepth, int destStride,
			int destOrder, int destX, int destY, int destWidth, int destHeight,
			byte[] destReds, byte[] destGreens, byte[] destBlues, bool flipX, bool flipY) {
 ...
 */
	static ImageData createGradientBand(int width, int height, bool vertical,
		...
		if (redBits !is 0 && greenBits !is 0 && blueBits !is 0) {
			...
			if (redBits >= 8 && greenBits >= 8 && blueBits >= 8) {
				...
				buildPreciseGradientChannel(fromRGB.blue, toRGB.blue, steps,
						bandWidth, bandHeight, vertical, bitmapData, 0, bytesPerLine);
				buildPreciseGradientChannel(fromRGB.green, toRGB.green, steps,
						bandWidth, bandHeight, vertical, bitmapData, 1, bytesPerLine);
				buildPreciseGradientChannel(fromRGB.red, toRGB.red, steps,
						bandWidth, bandHeight, vertical, bitmapData, 2, bytesPerLine);
			}
			else {
				/* Dithered color */
				int steps;
				...
				int bytesPerLine = bandWidth * 4;
				bitmapData = new byte[bandHeight * bytesPerLine];
				buildDitheredGradientChannel(fromRGB.blue, toRGB.blue, steps, bandWidth,
						bandHeight, vertical, bitmapData, 0, bytesPerLine, blueBits);
				buildDitheredGradientChannel(fromRGB.green, toRGB.green, steps, bandWidth,
						bandHeight, vertical, bitmapData, 1, bytesPerLine, greenBits);
				buildDitheredGradientChannel(fromRGB.red, toRGB.red, steps, bandWidth,
						bandHeight, vertical, bitmapData, 2, bytesPerLine, redBits);
			}
		}
		else {
			/* Dithered two tone */
			...
			bitmapData = new byte[bandHeight * bytesPerLine];
			if (vertical) {
				for (int dy = 0, blend = 0, dp = 0; dy < bandHeight; ++dy,
						blend += blendi, dp += bytesPerLine) {
					for (int dx = 0; dx < bandWidth; ++dx) {
						bitmapData[dp + dx] = (blend + DITHER_MATRIX[dy & 7][dx]) < 0x1000000 ? cast(
								byte) 0 : cast(byte) 1;
					}
				}
			}
				...
				for (int dx = 0, blend = 0; dx < bandWidth; ++dx, blend += blendi) {
					for (int dy = 0, dptr = dx; dy < bandHeight; ++dy, dptr += bytesPerLine) {
						bitmapData[dptr] = (blend + DITHER_MATRIX[dy][dx & 7]) < 0x1000000 ? cast(
								byte) 0 : cast(byte) 1;
					}
				}
			}
 ...
 */
	static final void buildPreciseGradientChannel(int from, int to, int steps, int bandWidth,
			int bandHeight, bool vertical, byte[] bitmapData, int dp, int bytesPerLine) {
		int val = from << 16;
 ...
 */
	static final void buildDitheredGradientChannel(int from, int to, int steps, int bandWidth,
			int bandHeight, bool vertical, byte[] bitmapData, int dp, int bytesPerLine, int bits) {
			...
			for (int dy = 0; dy < bandHeight; ++dy, dp += bytesPerLine) {
				for (int dx = 0, dptr = dp; dx < bandWidth; ++dx, dptr += 4) {
					...
					if (temp > 0xffffff)
						...
						bitmapData[dptr] = cast(byte)((temp >>> 16) & mask);
				}
				val += inc;
			}
		}
 ...
 * @param blueBits the number of significant blue bits, 0 for palette modes
 */
	static void fillGradientRectangle(GC gc, Device device, int x, int y, int width,
			int height, bool vertical, RGB fromRGB, RGB toRGB, int redBits,
			int greenBits, int blueBits) {
		/* Create the bitmap and tile it */
		ImageData band = createGradientBand(width, height, vertical, fromRGB,
				toRGB, redBits, greenBits, blueBits);
		...
		if ((band.width is 1) || (band.height is 1)) {
			...
			if (vertical) {
				for (int dx = 0; dx < width; dx += band.width) {
					...
					gc.drawImage(image, 0, 0, blitWidth, band.height, dx + x, y,
							blitWidth, band.height);
				}
			}
			else {
				for (int dy = 0; dy < height; dy += band.height) {
					...
					gc.drawImage(image, 0, 0, band.width, blitHeight, x, dy + y,
							band.width, blitHeight);
				}
			}
		}
		image.dispose();
