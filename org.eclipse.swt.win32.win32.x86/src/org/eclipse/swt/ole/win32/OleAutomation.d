 ...
 */
	public OleFunctionDescription getFunctionDescription(int index) {
		...
		for (int i = 0; i < data.args.length; i++) {
			...
			short[1] wParamFlags;
			COM.MoveMemory(wParamFlags.ptr, (cast(void*) funcdesc.lprgelemdescParam) + i * 16 + 12,
					2);
 ...
 */
	public int[] getIDsOfNames(String[] names) {
		...
		// TODO: NULL GUID ??
		GUID id;
		HRESULT result = objIDispatch.GetIDsOfNames(&id, wcNames.ptr, count,
				COM.LOCALE_USER_DEFAULT, rgdispid.ptr);
		if (result != COM.S_OK)
			return null;
 ...
 */
	public Variant getProperty(int dispIdMember, Variant[] rgvarg, int[] rgdispidNamedArgs) {
		Variant pVarResult = new Variant();
		HRESULT result = invoke(dispIdMember, COM.DISPATCH_PROPERTYGET, rgvarg,
				rgdispidNamedArgs, pVarResult);
		return (result is OLE.S_OK) ? pVarResult : null;
	}
 ...
 */
	public Variant invoke(int dispIdMember, Variant[] rgvarg, int[] rgdispidNamedArgs) {
		Variant pVarResult = new Variant();
		HRESULT result = invoke(dispIdMember, COM.DISPATCH_METHOD, rgvarg,
				rgdispidNamedArgs, pVarResult);
		return (result is COM.S_OK) ? pVarResult : null;
	}

	private int invoke(int dispIdMember, ushort wFlags, Variant[] rgvarg,
     ...
     */
		HRESULT result = objIDispatch.Invoke(dispIdMember, &id, COM.LOCALE_USER_DEFAULT,
				wFlags, &pDispParams, pVarResultAddress, &excepInfo, &pArgErr);
		...
		if (pDispParams.rgvarg !is null) {
			for (int i = 0, length = cast(int) /*64bit*/ rgvarg.length; i < length;
					i++) {
