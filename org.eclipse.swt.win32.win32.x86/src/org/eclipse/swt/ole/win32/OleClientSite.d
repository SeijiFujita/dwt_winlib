...
public class OleClientSite : Composite {
     ...
     */
		...
		if (frame is null)
			OLE.error(__FILE__, __LINE__, SWT.ERROR_INVALID_ARGUMENT);
		frame.AddRef();
 ...
 */
	public this(Composite parent, int style, File file) {
		...
		try {
			...
			result = COM.OleCreateFromFile(appClsid, fileName, &COM.IIDIUnknown,
 ...
 */
	public this(Composite parent, int style, String progId, File file) {
		...
		try {
			...
			if (COM.IsEqualGUID(appClsid, fileClsid)) {
				...
				tempStorage = createTempStorage();
				// Create ole object with storage object
				HRESULT result = COM.OleCreateFromFile(appClsid, fileName, &COM.IIDIUnknown,
						COM.OLERENDER_DRAW, null, null, tempStorage, cast(void**)&objIUnknown);
				if (result !is COM.S_OK)
					OLE.error(__FILE__, __LINE__, OLE.ERROR_CANNOT_CREATE_OBJECT, result);
			}
				...
				if (COM.StgIsStorageFile(fileName) is COM.S_OK) {
					...
					int mode = COM.STGM_READWRITE | COM.STGM_DIRECT
						| COM.STGM_SHARE_EXCLUSIVE | COM.STGM_CREATE;
					HRESULT result = COM.StgCreateDocfile(null,
							mode | COM.STGM_DELETEONRELEASE, 0, &storage); // Increments ref count if successful
					if (result !is COM.S_OK)
						OLE.error(__FILE__, __LINE__, OLE.ERROR_CANNOT_OPEN_FILE, result);
					...
					if (wordGUID !is null && COM.IsEqualGUID(appClsid, wordGUID))
						streamName = StrToWCHARz("WordDocument"); //$NON-NLS-1$
					IStream stream;
				...
				// create ole client
				result = COM.CoCreateInstance(appClsid, null, COM.CLSCTX_INPROC_HANDLER | COM.CLSCTX_INPROC_SERVER,
						&COM.IIDIUnknown, cast(void**)&objIUnknown);
				if (result !is COM.S_OK)
					OLE.error(__FILE__, __LINE__, OLE.ERROR_CANNOT_CREATE_OBJECT, result);
	...
	protected void addObjectReferences() {
		...
		//
		HRESULT result = objIUnknown.QueryInterface(&COM.IIDIViewObject2,
				cast(void**)&objIViewObject2);
		if (result !is COM.S_OK)
	...
	protected IStorage createTempStorage() {
		...
		if (result !is COM.S_OK)
			OLE.error(__FILE__, __LINE__, OLE.ERROR_CANNOT_CREATE_FILE, result);
		return (tmpStorage);
	}
	/**
 ...
 */
	public int exec(int cmdID, int options, Variant pvaIn, Variant pvaOut) {
		...
		if (objIOleCommandTarget is null) {
			if (objIUnknown.QueryInterface(&COM.IIDIOleCommandTarget,
					cast(void**)&objIOleCommandTarget) !is COM.S_OK)
 ...
 */
	...
	int ActivateMe(IOleDocumentView pViewToActivate) {
		if (pViewToActivate is null) {
			void* ppvObject;
			if (objIUnknown.QueryInterface(&COM.IIDIOleDocument, &ppvObject) !is COM.S_OK)
				return COM.E_FAIL;
			IOleDocument objOleDocument = cast(IOleDocument) ppvObject;
			if (objOleDocument.CreateView(iOleInPlaceSite, null, 0, &objDocumentView) !is COM.S_OK)
				return COM.E_FAIL;
	...
	private int GetWindowContext(IOleInPlaceFrame* ppFrame, IOleInPlaceUIWindow* ppDoc,
 ...
 */
     ...
     */
		...
		if (objIOleObject.QueryInterface(&COM.IIDIPersistFile,
				cast(void**)&permStorage) !is COM.S_OK)
			return true;
		// Are the contents of the permanent storage different from the file?
		auto result = permStorage.IsDirty();
		permStorage.Release();
		if (result is COM.S_FALSE)
			return false;
		return true;
	}

	override public bool isFocusControl() {
		checkWidget();
	...
	private int OnInPlaceActivate() {
		...
		if (objIOleObject.QueryInterface(&COM.IIDIOleInPlaceObject,
				cast(void**)&objIOleInPlaceObject) is COM.S_OK) {
	...
	private void onResize(Event e) {
		Rectangle area = frame.getClientArea();
		setBounds(borderWidths.left, borderWidths.top, area.width - borderWidths.left - borderWidths.right,
	...
	private int OnUIActivate() {
		...
		if (objIOleInPlaceObject.GetWindow(&phwnd) is COM.S_OK) {
			OS.SetWindowPos(phwnd, cast(HWND) OS.HWND_TOP, 0, 0, 0, 0,
					OS.SWP_NOSIZE | OS.SWP_NOMOVE);
		}
		return COM.S_OK;
	}
	...
	private int OnUIDeactivate(int fUndoable) {
		...
		if (frame is null || frame.isDisposed())
			return COM.S_OK;
		state = STATE_INPLACEACTIVE;
 ...
 */
	private bool saveToStorageFile(File file) {
		...
		// get access to the persistent storage mechanism
		IPersistStorage permStorage;
		if (objIOleObject.QueryInterface(&COM.IIDIPersistStorage,
				cast(void**)&permStorage) !is COM.S_OK)
			return false;
		try {
			IStorage storage;
			LPCTSTR path = StrToWCHARz(file.getAbsolutePath());
			int mode = COM.STGM_TRANSACTED | COM.STGM_READWRITE
				| COM.STGM_SHARE_EXCLUSIVE | COM.STGM_CREATE;
 ...
 */
	private bool saveToTraditionalFile(File file) {
		...
		if (tempStorage.OpenStream(("CONTENTS"w).ptr, null,
				COM.STGM_DIRECT | COM.STGM_READ | COM.STGM_SHARE_EXCLUSIVE, 0, &stream) is COM.S_OK) //$NON-NLS-1$
			return saveFromContents(stream, file);

		// Look for Ole 1.0 object stream
		if (tempStorage.OpenStream(("\1Ole10Native"w).ptr, null,
				COM.STGM_DIRECT | COM.STGM_READ | COM.STGM_SHARE_EXCLUSIVE, 0, &stream) is COM.S_OK) //$NON-NLS-1$
	...
	private void setExtent(int width, int height) {
		...
		if (objIOleObject is null || isStatic || inUpdate)
			return;
		SIZE* currentExtent = getExtent();
		if (width is currentExtent.cx && height is currentExtent.cy)
			return;
		...
		if (objIOleObject.SetExtent(aspect, newExtent) is COM.S_OK) {
			...
			inUpdate = false;
			if (!alreadyRunning) // Close server if it wasn't already running upon entering this method.
 ...
 */
	public void showProperties(String title) {
		...
		if (objIUnknown.QueryInterface(&COM.IIDISpecifyPropertyPages,
				cast(void**)&objISPP) !is COM.S_OK)
			return;
		...
		result = COM.OleCreatePropertyFrame(frame.handle, 10, 10, chTitle, 1,
				&objIUnknown, caGUID.cElems, caGUID.pElems, COM.LOCALE_USER_DEFAULT, 0, null);
	...
	private bool updateStorage() {
		...
		if (objIUnknown.QueryInterface(&COM.IIDIPersistStorage,
				cast(void**)&iPersistStorage) !is COM.S_OK)
			return false;
...
extern (Windows):
	...
	HRESULT GetContainer(IOleContainer* ppContainer) {
		return parent.GetContainer(ppContainer);
	}
...
extern (Windows):
	...
	HRESULT ActivateMe(IOleDocumentView pViewToActivate) {
		return parent.ActivateMe(pViewToActivate);
	}
}
...
extern (Windows):
	...
	HRESULT CanInPlaceActivate() {
		return parent.CanInPlaceActivate();
	}

	HRESULT OnInPlaceActivate() {
		return parent.OnInPlaceActivate();
	}

	HRESULT OnUIActivate() {
		return parent.OnUIActivate();
	}

	HRESULT GetWindowContext(IOleInPlaceFrame* ppFrame, IOleInPlaceUIWindow* ppDoc,
			LPRECT lprcPosRect, LPRECT lprcClipRect, LPOLEINPLACEFRAMEINFO lpFrameInfo) {
	...
	HRESULT OnUIDeactivate(BOOL fUndoable) {
		return parent.OnUIDeactivate(fUndoable);
	}

	HRESULT OnInPlaceDeactivate() {
		return parent.OnInPlaceDeactivate();
	}
	...
	HRESULT OnPosRectChange(LPCRECT lprcPosRect) {
		return parent.OnPosRectChange(lprcPosRect);
	}
}
