...
public class OleControlSite : OleClientSite {
	...
	alias AddRef = OleClientSite.AddRef;
	...
	public this(Composite parent, int style, String progId) {
		...
		try {
			...
			if (licinfo is null) {
				...
				HRESULT result = COM.OleCreate(appClsid, &COM.IIDIUnknown,
						COM.OLERENDER_DRAW, null, null, tempStorage, cast(void**)&objIUnknown);
				...
				try {
					...
					HRESULT result = COM.CoGetClassObject(appClsid,
							COM.CLSCTX_INPROC_HANDLER | COM.CLSCTX_INPROC_SERVER,
							null, &COM.IIDIClassFactory2, cast(void**)&classFactory);
					if (result !is COM.S_OK) {
						OLE.error(__FILE__, __LINE__,
								OLE.ERROR_CANNOT_ACCESS_CLASSFACTORY, result);
					}
					// Create Com Object
					result = classFactory.CreateInstanceLic(null, null,
							&COM.IIDIUnknown, licinfo, cast(void**)&objIUnknown);
					classFactory.Release();
					if (result !is COM.S_OK)
						OLE.error(__FILE__, __LINE__,
								OLE.ERROR_CANNOT_CREATE_LICENSED_OBJECT, result);
				}
				finally {
				...
				if (objIUnknown.QueryInterface(&COM.IIDIPersistStorage,
						cast(void**)&persist) is COM.S_OK) {
					tempStorage = createTempStorage();
			...
			if (COM.OleRun(objIUnknown) is OLE.S_OK)
				state = STATE_RUNNING;
 ...
 */
	public void addEventListener(int eventID, OleListener listener) {
		if (listener is null)
			OLE.error(__FILE__, __LINE__, SWT.ERROR_NULL_ARGUMENT);
		...
		if (unknown.QueryInterface(&COM.IIDIProvideClassInfo, cast(void**)&pci) is COM.S_OK) {
			...
			if (result is COM.S_OK && classInfo !is null) {
				...
				if (result is COM.S_OK && typeAttribute !is null) {
					int implMask = COM.IMPLTYPEFLAG_FDEFAULT
						| COM.IMPLTYPEFLAG_FSOURCE | COM.IMPLTYPEFLAG_FRESTRICTED;
 ...
 */
	public void addEventListener(OleAutomation automation, int eventID, OleListener listener) {
		if (listener is null || automation is null)
			OLE.error(__FILE__, __LINE__, SWT.ERROR_NULL_ARGUMENT);
 ...
 * </ul>
 */
	public void addEventListener(OleAutomation automation, String eventSinkId,
			int eventID, OleListener listener) {
		if (listener is null || automation is null || eventSinkId is null)
			OLE.error(__FILE__, __LINE__, SWT.ERROR_NULL_ARGUMENT);
		...
		GUID* guid = new GUID();
		if (COM.IIDFromString(buffer.ptr, guid) !is COM.S_OK)
			return;
	...
	void addEventListener(IUnknown iunknown, GUID* guid, int eventID, OleListener listener) {
		if (listener is null || iunknown is null || guid is null)
			OLE.error(__FILE__, __LINE__, SWT.ERROR_NULL_ARGUMENT);
	...
	override protected void addObjectReferences() {
		...
		if (objIUnknown.QueryInterface(&COM.IIDIOleControl,
				cast(void**)&objIOleControl) is COM.S_OK) {
 ...
 */
	...
	override protected void createCOMInterfaces() {
	...
	override protected void disposeCOMInterfaces() {
	...
	override public Color getBackground() {
		...
		if (objIUnknown !is null) {
			...
			if (varBackColor !is null) {
				COLORREF colorRef;
				if (COM.OleTranslateColor(varBackColor.getInt(), getDisplay()
						.hPalette, &colorRef) is COM.S_OK)
	...
	override public Font getFont() {
		...
		if (objIUnknown !is null) {
			...
			if (varDispFont !is null) {
				...
				if (lfFaceName !is null && lfHeight !is null && lfItalic !is null && lfBold !is null) {
	...
	override public Color getForeground() {
		...
		if (objIUnknown !is null) {
			...
			if (varForeColor !is null) {
				COLORREF colorRef;
				if (COM.OleTranslateColor(varForeColor.getInt(), getDisplay()
						.hPalette, &colorRef) is COM.S_OK)
					return Color.win32_new(getDisplay(), colorRef);
			}
		}
	...
	protected BSTR getLicenseInfo(GUID* clsid) {
		IClassFactory2 classFactory;
		if (COM.CoGetClassObject(clsid, COM.CLSCTX_INPROC_HANDLER | COM.CLSCTX_INPROC_SERVER,
				null, &COM.IIDIClassFactory2, cast(void**)&classFactory) !is COM.S_OK) {
			return null;
		}
		LICINFO licinfo;
 ...
 */
	...
	override protected HRESULT GetWindow(HWND* phwnd) {
	...
	private HRESULT Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD dwFlags,
		...
		if (pVarResult is null || dwFlags !is COM.DISPATCH_PROPERTYGET) {
			if (pExcepInfo !is null)
				COM.MoveMemory(pExcepInfo, &nullv, 4);
			if (pArgErr !is null)
				COM.MoveMemory(pArgErr, &nullv, 4);
		...
		case COM.DISPID_AMBIENT_SHOWHATCHING:
			if (pVarResult !is null)
				COM.MoveMemory(pVarResult, &nullv, 4);
			if (pExcepInfo !is null)
				COM.MoveMemory(pExcepInfo, &nullv, 4);
			if (pArgErr !is null)
				COM.MoveMemory(pArgErr, &nullv, 4);
			...
			if (lpgui1.hwndCaret !is null) {
				...
				if (lpgui2.hwndCaret is null && lpgui1.hwndCaret is OS.GetFocus()) {
					if (SWT_RESTORECARET is 0) {
						SWT_RESTORECARET = OS.RegisterWindowMessage(StrToTCHARz(0,
								"SWT_RESTORECARET"));
					}
					/*
	...
	override protected int Release() {
	...
	override protected void releaseObjectInterfaces() {
 ...
 * </ul>
 */
	public void removeEventListener(OleAutomation automation, GUID* guid,
			int eventID, OleListener listener) {
		checkWidget();
		if (automation is null || listener is null || guid is null)
			SWT.error(__FILE__, __LINE__, SWT.ERROR_NULL_ARGUMENT);
 ...
 */
	public void removeEventListener(OleAutomation automation, int eventID, OleListener listener) {
		checkWidget();
		if (automation is null || listener is null)
			SWT.error(__FILE__, __LINE__, SWT.ERROR_NULL_ARGUMENT);
	...
	void removeEventListener(IUnknown iunknown, GUID* guid, int eventID, OleListener listener) {
		if (listener is null || guid is null)
			SWT.error(__FILE__, __LINE__, SWT.ERROR_NULL_ARGUMENT);
		for (int i = 0; i < oleEventSink.length; i++) {
			if (COM.IsEqualGUID(oleEventSinkGUID[i], guid)) {
				if (iunknown is oleEventSinkIUnknown[i]) {
					...
					if (!oleEventSink[i].hasListeners()) {
						...
						if (oldLength is 1) {
							...
							System.arraycopy(oleEventSink, i + 1,
									newOleEventSink, i, oldLength - i - 1);
							oleEventSink = newOleEventSink;

							GUID*[] newOleEventSinkGUID = new GUID*[oldLength - 1];
							SimpleType!(GUID*).arraycopy(oleEventSinkGUID, 0,
									newOleEventSinkGUID, 0, i);
							SimpleType!(GUID*).arraycopy(oleEventSinkGUID,
									i + 1, newOleEventSinkGUID, i, oldLength - i - 1);
							oleEventSinkGUID = newOleEventSinkGUID;

							IUnknown[] newOleEventSinkIUnknown = new IUnknown[oldLength - 1];
							SimpleType!(IUnknown).arraycopy(oleEventSinkIUnknown,
									0, newOleEventSinkIUnknown, 0, i);
							SimpleType!(IUnknown).arraycopy(oleEventSinkIUnknown,
									i + 1, newOleEventSinkIUnknown, i, oldLength - i - 1);
	...
	override public void setFont(Font font) {
		...
		if (objIUnknown !is null) {
			...
			if (varDispFont !is null) {
				...
				iDispFont.setProperty(COM.DISPID_FONT_ITALIC,
						new Variant(fdata[0].getStyle() & SWT.ITALIC));
				//iDispFont.setProperty(COM.DISPID_FONT_CHARSET, new Variant(fdata[0].getCharset));
				iDispFont.setProperty(COM.DISPID_FONT_BOLD,
						new Variant((fdata[0].getStyle() & SWT.BOLD)));
				iDispFont.dispose();
			}
		}
		...
		for (int i = 0; i < sitePropertyIds.length; i++) {
			if (sitePropertyIds[i] is dispId) {
				...
				if (value !is null) {
					...
					System.arraycopy(sitePropertyIds, i + 1,
							newSitePropertyIds, i, oldLength - i - 1);
					System.arraycopy(sitePropertyValues, 0, newSitePropertyValues, 0, i);
					System.arraycopy(sitePropertyValues, i + 1,
							newSitePropertyValues, i, oldLength - i - 1);
...
extern (Windows):
	...
	HRESULT GetIDsOfNames(REFCIID riid, LPCOLESTR* rgszNames, UINT cNames,
	...
	HRESULT Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags,
		...
		return parent.Invoke(dispIdMember, riid, lcid, wFlags, pDispParams,
...
extern (Windows):
	...
	HRESULT OnControlInfoChanged() {
		return parent.OnControlInfoChanged();
	}
	...
	HRESULT TransformCoords(POINTL* pPtlHimetric, //Address of POINTL structure
	...
	HRESULT OnFocus(BOOL fGotFocus //Indicates whether the control gained focus
	) {
		return COM.S_OK;
	}
