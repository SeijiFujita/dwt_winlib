 ...
 */
	...
	private static void initCheckFocus(Display display_) {
		if (display_.getData(CHECK_FOCUS) !is null)
			return;
		...
		auto timer = new class(display_) Runnable {
			...
			public void run() {
				...
				if (lastFocus[0] is null || lastFocus[0].isDisposed()
						|| !lastFocus[0].isFocusControl()) {
					...
					if (lastFocus[0]!is currentFocus) {
						...
						if ((null !is cast(OleClientSite) lastFocus[0])
								&& !lastFocus[0].isDisposed()) {
							lastFocus[0].notifyListeners(SWT.FocusOut, event);
						}
						if ((null !is cast(OleClientSite) currentFocus)
								&& !currentFocus.isDisposed()) {
	...
	private static void initMsgHook(Display display) {
		if (display.getData(HHOOK) !is null)
			return;
		...
		display.disposeExec(new class(hHook_) Runnable {
			void* hHook; this(void* hHook) {
				this.hHook = hHook;}

				public void run() {
					if (hHook !is null)
						OS.UnhookWindowsHookEx(hHook);//if (callback !is null) callback.dispose();
				}
			}
);
		}
				...
				if (display !is null) {
					...
					if (widget !is null && (null !is cast(OleClientSite) widget)) {
						...
						if (site.handle is hwnd) {
							...
							int mask = OS.GUI_INMENUMODE | OS.GUI_INMOVESIZE
								| OS.GUI_POPUPMENUMODE | OS.GUI_SYSTEMMENUMODE;
							if (!rc || (lpgui.flags & mask) is 0) {
								...
								if (frame.getData(CONSUME_KEY) !is null)
									consumed = false;
								frame.setData(CONSUME_KEY, null);
							}
							...
							case OS.WM_SYSKEYDOWN: {
									static if (!OS.IsWinCE) {
                                        ...
                                        */
												int mapKey = OS.MapVirtualKey(cast(int) /*64bit*/ msg.wParam,
														2);
												if (mapKey !is 0) {
													accentKey = (mapKey & (OS.IsWinNT
															? 0x80000000 : 0x8000)) !is 0;
													if (!accentKey) {
														for (int i = 0; i < ACCENTS.length;
																i++) {
															int value = OS.VkKeyScan(ACCENTS[i]);
															if (value !is -1
																	&& (value & 0xFF) is msg.wParam) {
																int state = value >> 8;
																if ((OS.GetKeyState(OS.VK_SHIFT) < 0) is(
																		(state & 0x1) !is 0)
																		&& (OS.GetKeyState(OS.VK_CONTROL) < 0) is(
 ...
 */
		...
		private HRESULT InsertMenus(HMENU hmenuShared, LPOLEMENUGROUPWIDTHS lpMenuWidths) {
			...
			lpmii.cbSize = OS.MENUITEMINFO_sizeof;
			lpmii.fMask = OS.MIIM_STATE | OS.MIIM_ID | OS.MIIM_TYPE | OS.MIIM_SUBMENU | OS
				.MIIM_DATA;
		...
		private HRESULT QueryInterface(REFCIID riid, void** ppvObject) {
			...
			if (COM.IsEqualGUID(riid, &COM.IIDIUnknown)
					|| COM.IsEqualGUID(riid, &COM.IIDIOleInPlaceFrame)) {
 ...
 */
		...
		private HRESULT SetBorderSpace(LPCBORDERWIDTHS pborderwidths) {
			...
			if (pborderwidths is null || currentdoc is null)
				return COM.S_OK;
 ...
 */
		...
		HRESULT SetMenu(HMENU hmenuShared, HOLEMENU holemenu, HWND hwndActiveObject) {
			...
			Menu menubar = getShell().getMenuBar();
			if (menubar is null || menubar.isDisposed()) {
				return COM.OleSetMenuDescriptor(null, getShell().handle,
						hwndActiveObject, iOleInPlaceFrame, inPlaceActiveObject);
			}
			...
			if (hmenuShared is null)
				return COM.E_FAIL;

			OS.SetMenu(handle, hmenuShared);
			OS.DrawMenuBar(handle);

			return COM.OleSetMenuDescriptor(holemenu, handle, hwndActiveObject,
					iOleInPlaceFrame, inPlaceActiveObject);
		}
 ...
 */
	...
	extern (Windows):
		...
		HRESULT GetBorder(LPRECT lprectBorder) {
			return parent.GetBorder(lprectBorder);
		}
		...
		HRESULT SetBorderSpace(LPCBORDERWIDTHS pborderwidths) {
			return parent.SetBorderSpace(pborderwidths);
		}
