...
final class OleEventSink {
	...
	void connect() {
		...
		if (objIUnknown.QueryInterface(&COM.IIDIConnectionPointContainer,
				cast(void**)&cpc) is COM.S_OK) {
			IConnectionPoint cp;
	...
	private HRESULT Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags,
			DISPPARAMS* pDispParams, VARIANT* pVarResult, EXCEPINFO* pExcepInfo, UINT* puArgErr) {
		if (eventTable is null || !eventTable.hooks(dispIdMember))
			return COM.S_OK;
...
*/
	...
	private HRESULT QueryInterface(REFCIID riid, void** ppvObject) {
		...
		if (COM.IsEqualGUID(riid, &COM.IIDIUnknown) || COM.IsEqualGUID(riid,
				&COM.IIDIDispatch) || COM.IsEqualGUID(riid, eventGuid)) {
